...
'   ===============================================================================
	...
	Function envLog(strLog)
		...
		Dim DIR_ERRINFO, strPath, physicalPath, strFileName

	'	physicalPath 				= Request.ServerVariables("APPL_PHYSICAL_PATH")
	'	DIR_ERRINFO					= sys_sprintf("{0}syslog/sysInfo/", Array(physicalPath) )		
		DIR_ERRINFO					= "C:/WEBLOGFILE/icheon/syslog/500ErrorLog/dev_svr/"
		Call sys_CheckDirByDate(DIR_ERRINFO)
		strPath = sys_GetFilePathWithDate(DIR_ERRINFO, "SysLog")
...
<%
   ...
   Function TraceErrorInfo(req, err_obj)
      ...
      strLog = sys_sprintf("{0} {1} {2} {3} {4} {5}", Array(user_id, curUrl, req_type, os, browser, clientIP ))
		Call envLog(strLog)
      ...
      LOGDBOpen()
         Dim strSql
            strSql = strSql & " Insert Into TBL_LOG (DB_TYPE ,USER_ID, USER_IP, USER_OS, BROWSER ,ERR_URL ,ERR_CONTENT ,ERR_FILE ,ERR_LINE) "
	         strSql = strSql & sys_sprintf("    Values (2, '{0}', '{1}', '{2}', '{3}', '{4}', '{5}', '{6}', '{7}') ", Array(user_id, clientIP, os, browser, curUrl, desc, err_obj.File, err_obj.Line))
            Call ExecuteUpdate(strSql, LOGDB)
      LOGDBClose()
	End Function
