...
<style>
	...
	.E2>.drow-con__table td{min-width:160px;
	min-height:40px;}
	.E2>.drow-con__table td:last-child{min-width:40px;}
</style>
...
<div id="app" class="l contestInfo">
  ...
  <!-- #include file = "../include/gnbapp.asp" -->
    ...
    <!-- S: main -->
  <div class="l_content m_scroll [ _content _scroll ]" style="overflow:visible;">
		<!-- 여기에 내용 넣어주세요 -->
    <div class="drow">
      ...
      <div class="drow-con">
        <div class="drow-con__search clear">
          <div class="drow-con__search__box-selc">
					...
					<select id="CDA"  class="drow-con__search__box-selc__selc"
					 onchange="
					 mx.getMachList('<%=tidx%>',$(this).val());
					 $('#sw_gametable').attr('class' , $(this).val());$('#sw_gametable').html('');
					 ">
					<%
          ...
          <div class="drow-con__search__box-selc" id="s_cdbc">
					<input type="hidden" id="showtype" value="tbl">
					<select id="cdbc" class="drow-con__search__box-selc__selc" onchange="mx.getMachTable('<%=tidx%>',$(this).val())">
						<option value=""  selected>::부또는 그룹 선택해 주십시오.::</option>
					<%
							If IsArray(fr) Then
								For ari = LBound(fr, 2) To UBound(fr, 2)
		...
		<%'##########################%>
        <span id="sw_gametable" class="D2"></span>
		<%'##########################%>
