    ...
    <div class="match-order">
      ...
      <div class="match-order-con">
        <div class="match-order-con__search clear" action="" method="post">
          <div class="match-order-con__search__box-selc">
            <label for="selcMatchDate" class="hide">경기 날짜 선택</label>
						...
						<select id="selcMatchDate" onchange="px.goSubmit({'TIDX':<%=tidx%>,'DD':$(this).val()},'gameorder2.asp?tidx=<%=tidx%>');">
						<%
        ...
        <nav class="match-order-con__nav clear">
          <!-- s_on = 링크 강조 표시 경영만 시간표시-->
          <a href="javascript:px.goSubmit({'TIDX':<%=tidx%>,'LIDX':'<%=l_idx%>','LNO':'<%=l_levelno%>','JOONO':1,'TABNO':1,'DD':$('#selcMatchDate').val()},'gameorder.asp');" id="linkMorningTime" class="match-order-con__nav__link">
            오전 <%If IsArray(arrR) Then%><%=start_am%><%end if%> ~
          </a>
          <a href="javascript:px.goSubmit({'TIDX':<%=tidx%>,'LIDX':'<%=l_idx%>','LNO':'<%=l_levelno%>','JOONO':1,'TABNO':1,'DD':$('#selcMatchDate').val()},'gameorder2.asp');" id="linkAfternoonTime" class="match-order-con__nav__link  s_on">
            오후 <%If IsArray(arrR) Then%><%=start_pm%><%end if%> ~
          </a>
        </nav>
								...
								<%
								  '오전
									For ari = LBound(arrR, 2) To UBound(arrR, 2)
											...
											'오후잖아....
											l_gubun = arrR(21, ari) 'pm 과 각 2번을 불러오면된다
											l_gameno = arrR(18, ari)
											l_joono = arrR(19, ari)
                      ...
                      For ari = LBound(arrRs, 2) To UBound(arrRs, 2)
                        ...
                        if l_ITgubun = "I" then
                          ...
                          itgubunstr = "단체"
												...
												if ((l_tryoutgamestarttime = "13:00" or l_tryoutgamestarttime = "12:00") and l_tryoutgamedate = start_gamedate) or _
												((l_finalgamestarttime = "13:00" or l_finalgamestarttime = "12:00") and l_finalgamedate = start_gamedate)  then 
                        if l_CDA = "E2" then
                        %>
												<tr>
													<td><%=i%></td>
          ...
          %>
                    ...
                    <%
                      ...
                      For ari = LBound(arrRs, 2) To UBound(arrRs, 2)
                        ...
                        '오전경기만
												if ((l_tryoutgamestarttime = "13:00" or l_tryoutgamestarttime = "12:00") and l_tryoutgamedate = dd) or _
												((l_finalgamestarttime = "13:00" or l_finalgamestarttime = "12:00") and l_finalgamedate = dd)  then 
                        if l_CDA = "F2"  then
                        %>
												<tr>
												<td><%=i%><%'=l_gameno%></td>
