...
<%
 'Controller ################################################################################################
  ...
  'request 처리##############
	...
	SQL = "select sum(cast(gameno as int)) from  tblRGameLevel Where delyn = 'N' and gametitleidx =  "&tidx&"  and CDA='F2' "
	Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
	If CDbl(rs(0)) = 0  then
	SQL = "UPDATE A  SET A.gameno = A.RowNum FROM ( Select gameno,rank() over(Order By cdc,sexno,Rgamelevelidx) As rowNum From tblRGameLevel Where delyn = 'N' and gametitleidx =  "&tidx&"  and CDA='F2' and CDC <> '31'  ) as A " '수구는 빼야함
	...
	'수구빼고 31
	SQL = "select "&fld&" from tblRGameLevel  as a where  delyn = 'N' and gametitleidx =  " & tidx  & "  and CDA='F2'  and CDC <> '31' and (  grouplevelidx is null or grouplevelidx = RGameLevelidx )   " & sexstr & cdcstr & sortstr  '소팅 게임날짜 게임순서, cdc , sexno , cdb
	Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
...
<!-- 															<option value="13" <%If  F2 = "13" then%>selected<%End if%>>프리 콤비네이션</option> -->
	  ...
	  &nbsp;&nbsp; &nbsp;&nbsp; &nbsp;&nbsp; &nbsp;&nbsp;<a href="javascript:mx.refereeWindow(<%=tidx%>,'F2')" class="btn btn-warning">대회심판배정</a>
	  </div>
			...
			<div class="box-header with-border" style="text-align:right;margin-bottom:-17px;">
						...
						<div style="width:645px;float:right;">&nbsp;</div>
		...
		<%
				...
				lastno = UBound(arrR, 2)
					...
					'rsdata
							...
							makegame =  arrR(26,ari) '추첨완료여부 (1)
							 ...
							 l_finaldate = arrR(33,ari)
							 l_finalgamestarttime = isnulldefault(arrR(34, ari),"") '오전 오후 시간으로 구분

							l_gameno_temp = arrR(35,ari)
						'redata
				...
				<%If makegame = "1"  Then '설정후 대진표 추첨%>
							  ...
							  <div class="box-tools pull-right">
								  <%'통합된거, 검색된거라면 그리지 않는다.%>
									<input id="gameno_<%=l_idx%>" value="<%=l_gameno_temp%>" class="form-control" style="width:100px;float:left;" 
									  onkeyup="this.value=this.value.replace(/[^0-9]/g,'');mx.tempOrder('F2',<%=tidx%>,<%=l_idx%>,$(this).val(), $(this).prop('defaultValue'))">


									<!-- 순서대로보여줄꺼임 -->
								  <input id="player_<%=l_idx%>" value="<%=l_gameno%>" class="form-control" style="width:100px;float:left;" <%If l_grouplevelidx <> "" then%>disabled<%End if%> onkeyup="this.value=this.value.replace(/[^0-9]/g,'');mx.changeOrder('F2',<%=tidx%>,<%=l_idx%>,$(this).val(), $(this).prop('defaultValue'))">
									<%
									...
									<%If l_grouplevelidx <> "" then%>disabled<%End if%>>
											...
											<%End if%>
											<option value="5" <%If CDbl(l_RoundCnt) = 5 then%>selected<%End if%> >피겨4+프리1</option>
											<option value="6" <%If CDbl(l_RoundCnt) = 6 then%>selected<%End if%> >테크5+프리1</option>
											<option value="1" <%If CDbl(l_RoundCnt) = 1 then%>selected<%End if%> >프리1</option>
									</select>
									...
									<%If l_grouplevelidx <> "" then%>disabled<%End if%>>
											...
											<%End if%>
											<%For i = 12 To 15%>
											<option value="<%=i%>"  <%If CDbl(l_judgeCnt) = i then%>selected<%End if%>><%=i%>심</option>
											<%next%>
									</select>

									

									<%If l_CDC = "05" Or l_CDC = "07" Or l_CDC = "11" Then '프리 게임이라면%>

										<input type="text" id="gamedate_<%=l_idx%>" class="form-control"  style="width:100px;float:left;text-align:right" disabled >
										<input id="ap_<%=l_idx%>" class="form-control" style="width:80px;float:left;" disabled>

									<%else%>

										<input type="text" id="gamedate_<%=l_idx%>" placeholder="대회날짜" value="<%=l_tryoutgamedate%>" class="form-control"  style="width:100px;float:left;text-align:right" onchange="mx.setGamedate2(<%=tidx%>, <%=l_idx%>, <%=l_GbIDX%>, $(this).val())"  <%If l_grouplevelidx <> "" then%>disabled<%End if%> >

										<select id="ap_<%=l_idx%>" class="form-control" style="width:80px;float:left;" onchange="mx.setAMPM(<%=tidx%>, <%=l_idx%>, <%=l_GbIDX%>, $(this).val())"  <%If l_grouplevelidx <> "" then%>disabled<%End if%> >
									...
									<input type="text" id="gamedatenext_<%=l_idx%>" placeholder="대회날짜" value="<%=l_finaldate%>" class="form-control"  style="width:100px;float:left;text-align:right" onchange="mx.setGamedateNext(<%=tidx%>, <%=l_idx%>, <%=l_GbIDX%>, $(this).val())"  <%If l_grouplevelidx <> "" then%>disabled<%End if%> >

									<select id="apnext_<%=l_idx%>" class="form-control" style="width:80px;float:left;" onchange="mx.setAMPMNext(<%=tidx%>, <%=l_idx%>, <%=l_GbIDX%>, $(this).val())"  <%If l_grouplevelidx <> "" then%>disabled<%End if%> >
											<%If l_finalgamestarttime = "" then%>
											<option value="" >선택</option><!-- 난의도 등의 설정 가져오기 -->
											<%End if%>
											<option value="10:00"  <%If Cstr(l_finalgamestarttime) = "10:00" then%>selected<%End if%>>오전</option>
											<option value="13:00" <%If Cstr(l_finalgamestarttime) = "13:00" then%>selected<%End if%>>오후</option>
									</select>
								  ...
								  <button class = "btn btn-<%If l_chkjidx = "0" Or l_chkjidx ="" then%>default<%else%>warning<%End if%>" type="button" style="margin-right:60px;" onclick="mx.setreferee(<%=tidx%>,<%=l_idx%>,<%=l_GbIDX%>, 'chkpass')" id ="btn4_<%=l_idx%>">심판배정</button>
					...
					<%Else '팀은 솔로에 의해 컨크롤 된다.'#######################################%>
						  <div class="box box-primary collapsed-box" style="margin-bottom:1px;">
							<div class="box-header with-border" >
							  <h3 class="box-title"> &nbsp;&nbsp;<%=l_CDCNM%> / <%=l_CDBNM%></h3>
							  ...
							  <div class="box-tools pull-right">
								...
								<input id="gameno_<%=l_idx%>" value="<%=l_gameno_temp%>" class="form-control" style="width:100px;float:left;" 
								  ...
								  <input id="player_<%=l_idx%>" value="<%=l_gameno%>" class="form-control" style="width:100px;float:left;" <%If l_grouplevelidx <> "" then%>disabled<%End if%>  								  onkeyup="this.value=this.value.replace(/[^0-9]/g,'');mx.changeOrder('F2',<%=tidx%>,<%=l_idx%>,$(this).val(), $(this).prop('defaultValue'))">
									...
									<input type="text" id="gamedate_<%=l_idx%>" placeholder="대회날짜" value="<%=l_tryoutgamedate%>" class="form-control"  style="width:100px;float:left;text-align:right"  disabled>

									<select id="ap_<%=l_idx%>" class="form-control" style="width:80px;float:left;" disabled>
											<%If l_tryoutgamestarttime = "" then%>
											<option value="" selected></option><!-- 난의도 등의 설정 가져오기 -->
											<%End if%>
											<option value="10:00"  <%If Cstr(l_tryoutgamestarttime) = "10:00" then%>selected<%End if%>>오전</option>
											<option value="13:00" <%If Cstr(l_tryoutgamestarttime) = "13:00" then%>selected<%End if%>>오후</option>
									</select>
							...
							<div class="box-header with-border" >
							  ...
							  <div class="box-tools pull-right">
								  <input id="player_<%=l_idx%>" value="<%=l_gameno%>" class="form-control" style="width:100px;float:left;" <%If l_grouplevelidx <> "" then%>disabled<%End if%>  								  onkeyup="this.value=this.value.replace(/[^0-9]/g,'');mx.changeOrder('F2',<%=tidx%>,<%=l_idx%>,$(this).val(), $(this).prop('defaultValue'))">
									...
									<input type="text" id="gamedatenext_<%=l_idx%>"  class="form-control"  style="width:100px;float:left;text-align:right"  disabled>

									<select id="apnext_<%=l_idx%>" class="form-control" style="width:80px;float:left;" disabled>
									</select>
