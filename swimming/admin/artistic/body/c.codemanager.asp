...
<%
 'Controller ################################################################################################
	...
	'search
	If chkBlank(F2) Then
		strWhere = " DelYN = 'N'  and CDA = 'F2' " '다이빙
	Else
		strWhere = " DelYN = 'N' and CDA = 'F2' and "&F1&" like '%"& F2 &"%' "
	End if
							...
							<div class="col-md-6" style="width:10%;padding-left:20px;padding-right:0px;text-align:left;">
								  <div class="form-group">
										<select id="F1" class="form-control">
											<option value="title" <%If F1 = "title" then%>selected<%End if%>>종목</option>
											<option value="code1" <%If F1 = "code1" then%>selected<%End if%>>난이도번호</option>
										</select>
								  </div>
							</div>
							...
							<div class="col-md-6" style="width:40%;padding-left:20px;padding-right:0px;text-align:left;">
								  <div class="form-group">
									...
									<div class="input-group date">
										 <input type="text" id="F2" placeholder="검색어를 입력해주세요" value="<%=F2%>" class="form-control" onkeydown="if(event.keyCode == 13){px.goSubmit({'F1':$('#F1').val(),'F2':$('#F2').val()},'codemanager.asp')}">
										<div class="input-group-addon" onmousedown="px.goSubmit({'F1':$('#F1').val(),'F2':$('#F2').val()},'codemanager.asp')">
