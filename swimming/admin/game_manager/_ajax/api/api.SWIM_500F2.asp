...
'#############################################
	...
	'아티스틱
		...
		Select Case CDC 
		Case "01","02","03" '피겨
		fld = fld & " case when b.gameround = 1 then '자유종목(1)' when b.gameround = 2 then '자유종목(2)' when b.gameround = 3 then '자유종목(3)' when b.gameround = 4 then '자유종목(4)' when b.gameround = 5 then '프리 루틴' end as r_roundnonm "
		Case "04","06","12" '테크니컬
		fld = fld & " case when b.gameround < 6 then '테크니컬' when b.gameround = 6 then '프리 루틴-'  end as r_roundnonm "
		Case Else
		fld = fld & " '프리 루틴'  as r_roundnonm "		
		End Select 
