...
'Response.end
	...
	SQL = SQL & " and (cast(playerIDX as varchar) + gameresult) in ( select (cast(playerIDX as varchar) + max(gameresult)) from tblRecord where  delyn = 'N' and  CDC ='"&cdc&"' and gameResult  > 0 and gameResult < 'a'  and sex = 1 " & tidxwhere & " group by playerIDX  ) 	order by gameresult asc "
	Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
...
'Response.end
	...
	Response.AddHeader "Content-disposition","attachment;filename="&boonm & "("&Replace(Date(),"-","")&")_남" & ".xls"
	'엑셀출력################
