...
<%
 'Controller ################################################################################################
  ...
  '결제와 연동해서 보여주자.
  SQL = "select  a.idx,a.gametitleidx,a.team,a.teamNm,a.fileUrl from sd_schoolConfirm  as a inner join tblSwwimingOrderTable as b "
  SQL = SQL & " on a.gubun = "&F1&" and a.team = b.team and a.gametitleidx = b.gametitleidx and b.oOrderState = '01'  and b.del_yn = 'N'  and a.leaderidx = b.leaderidx    where a.GameTitleIDX = " & tidx '결제된 것만 보여주면된다.
            ...
            <div class="box-header" style="text-align:right;padding-right:20px;">
						...
						<div class="row">
							...
							<div class="col-md-6" style="width:49%;padding-right:20px;padding-right:0px;text-align:right;">
								  <div class="form-group">
										  <a href="javascript:px.goSubmit({'F1':1,'IDX':<%=tidx%>,'TITLE':'<%=title%>'},'schoolcheck.asp')" class="btn btn-<%If F1 = "1" then%>danger<%else%>default<%End if%>"></i>학교장확인서</a>
										  <a href="javascript:px.goSubmit({'F1':2,'IDX':<%=tidx%>,'TITLE':'<%=title%>'},'schoolcheck.asp')" class="btn btn-<%If F1 = "2" then%>danger<%else%>default<%End if%>"></i>학교폭력처분이력부존재서약서</a>
