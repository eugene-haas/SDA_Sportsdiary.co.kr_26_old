...
<%
 'Controller ################################################################################################
	...
	If F1 <> "" And F2 <> "" Then
		strWhere =  " SiteCode = '"&SiteCode&"' and " & F1 & " like '%" & F2 & "%'  and  isnull(Kind_code,'') not in ('E2', 'F2')  "  'NOT IN 절 내의 NULL 값  null은 unKnown 이므로 비교하지 않고 내보낸다. isnull 체크해야한다고.
	else
		strWhere = " SiteCode = '"&SiteCode&"' and  isnull(Kind_code,'') not in ('E2', 'F2')  "
	End if
