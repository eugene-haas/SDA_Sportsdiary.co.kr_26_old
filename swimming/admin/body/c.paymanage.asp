...
'	결제되었지만 참가신청에 누락된 경우 찾기 샘플
'	21.07.02 황은영 요청으로 생성한 쿼리

'$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
';with paytbl as (
'select a.teamnm, a.playeridx,a.team,a.tidx,a.cdb,a.username
'from tblSwwimingOrderTable as pay inner join 
'tblGameRequest_imsi as a on pay.leaderidx = a.leaderidx and  pay.GameTitleIDX = a.tidx  and a.delyn = 'N '
'where a.tidx = 149 and a.delyn = 'N' and pay.OorderState = '01' and pay.del_yn = 'N' )
'
'select a.* , b.RequestIDX from paytbl as a left join tblGameRequest as b on a.tidx = b.GameTitleIDX and a.playeridx = b.p1_playeridx  and b.itgubun = 'I'  and b.DelYN = 'N' and b.GameTitleIDX = 149  order by b.RequestIDX
'
'select rr.username,r.cdbnm,r.p1_teamnm from tblGameRequest as r left join tblGameRequest_r as rr on r.RequestIDX = rr.requestIDX and rr.DelYN = 'N' where r.DelYN = 'N' and r.GameTitleIDX = 149  and r.cdcnm = '수구'
'$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
...
S1 = oJSONoutput.get("S1")
S2 = oJSONoutput.get("S2")


	'search
	If chkBlank(S2) = false Then
			strWhere = " a.Del_YN = 'N' and a.gubun='1' and b.gametitleidx = '"&F2&"' and "&S1&" like '%"&S2&"%' "
	Else
...
<div class="box-header" style="text-align:right;padding-right:20px;">
		...
		<div class="col-md-6" style="width:10%;padding-left:20px;padding-right:0px;text-align:left;">
				<div class="form-group">
					<select id="S1" class="form-control">
						<option value="TeamNm">소속명</option>
						<option value="leadername">이름</option>
					</select>
				</div>
		</div>
		...
		<div class="col-md-6" style="width:40%;padding-left:20px;padding-right:0px;text-align:left;">
				...
				<div class="input-group date">
					<input type="text" id="S2" placeholder="검색어를 입력해주세요" value="" class="form-control" onkeydown="if(event.keyCode == 13){px.goSubmit({'F1':$('#F1').val(),'F2':$('#F2').val(),'S1':$('#S1').val(),'S2':$('#S2').val()},'paymanage.asp')}">
					<div class="input-group-addon" onmousedown="px.goSubmit({'F1':$('#F1').val(),'F2':$('#F2').val(),'S1':$('#S1').val(),'S2':$('#S2').val()},'paymanage.asp')">
