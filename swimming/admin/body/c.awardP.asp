 ...
 'Controller ################################################################################################
	...
	'등록된 최소년도
	SQL = "Select min(GameE) from sd_gameTitle where delYN = 'N' "
	...
	rs.close
  	...
  	'search
	If chkBlank(F2) Then
		strWhere = " DelYN = 'N'  and GameE >= '"&year(date)&"-01-01' "
		findWhere = " DelYN = 'N'  and GameE >= '"&year(date)&"-01-01' "
	...
	Else
		...
		If IsArray(F1) Then
			fieldarr = array("GameE","gametitleidx","gbidx")
			F2_0 = F2(0)
			F2_1 = F2(1)
			F2_2 = F2(2)
			...
			findWhere = " DelYN = 'N'  and GameE >= '"&F2_0&"-01-01' "

			nowGameE = F2_0
		Else
			strWhere = " DelYN = 'N' and "&F1&" = '"& F2 &"' "

			If LCase(F1) = "GameE" Then
				nowGameE = F2
			End if
		End if
	End if
	...
	'년도별 대회명검색
	fieldstr =  "GameTitleIDX,GameTitleName,GameS,GameE,year(GameE) as gameyear ,GameArea,kgame, titlecode  "
