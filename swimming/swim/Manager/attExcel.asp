...
levelno = request("levelno")
	...
	If levelno <> "" And  isnumeric(levelno) = True then
	...
	gboo = rs(1)
	Else
	SQL = "select a.gametitlename ,b.teamgbnm from sd_tennistitle as a inner join tblrgamelevel as b on a.gametitleidx = b.gametitleidx where a.gametitleidx = "  & tidx 
	Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
	...
	'field2 = "P2_ID,P2_UserName as '선수2', CASE when P2_TeamNm2 ='' then P2_TeamNm else (P2_TeamNm +','+ P2_TeamNm2) end as '클럽'     ," & phone2 & "," & rankq1 & "," & rankq2 
	...
	Else
		If levelno = "" then
			SQL = "select "&field1&field2&"  from  tblGameRequest as a where GameTitleIDX = "&tidx&"  and DelYN = 'N' order by RequestIDX asc"
		Else
			SQL = "select "&field1&field2&"  from  tblGameRequest as a where GameTitleIDX = "&tidx&"  and p1_teamNm = '"&levelno&"'  and DelYN = 'N' order by RequestIDX asc"
		End if
	End if
	Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
