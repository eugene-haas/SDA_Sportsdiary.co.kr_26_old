...
<%
   Dim strPos, strQPos, playType, strGroupGameGb, strPlayType, strGInfo, strGTitle, strGKind, aryGInfo
	Dim manualSet, strGameNum, strQGameNum

   strPos = Request.Form("pos")
   strQPos = Request.Form("q_pos")
	strGameNum = Request.Form("game_num")
   strQGameNum = Request.Form("q_game_num")
...
''   strLog = sprintf("strPos = {0}<br><br>", Array(strPos))
''   Response.Write strLog
''
''   strLog = sprintf("strQPos = {0}<br><br>", Array(strQPos))
''   Response.Write strLog
''
''   strLog = sprintf("strGameNum = {0}<br><br>", Array(strGameNum))
''   Response.Write strLog
''
''   strLog = sprintf("strQGameNum = {0}<br><br>", Array(strQGameNum))
''   Response.Write strLog
''
''   strLog = sprintf("strGroupGameGb = {0}<br><br>", Array(strGroupGameGb))
''   Response.Write strLog
''
''   strLog = sprintf("strPlayType = {0}<br><br>", Array(strPlayType))
''   Response.Write strLog
''
''
''   strLog = sprintf("strGInfo = {0}<br><br>", Array(strGInfo))
''   Response.Write strLog
''
''   response.end
%>
...
<style>
	...
	.lotteryMatch{ position:absolute; width:100%; height:44px; margin:auto; box-sizing:border-box;
		border-color:#f2f2f2; border-style:solid;
		display:flex;
		opacity:1;
	}
  .tour_single .lotteryMatch>.lotteryMatch__game_num,
  .tour_double .lotteryMatch>.lotteryMatch__game_num{height:22px;}
	...
	.lotteryMatch_first{ top:50%;transform:translateY(-50%);
  ...
  .lotteryMatch_second .lotteryMatch__playerWrap{ top:50%;
	...
	.lotteryMatch__seedWrap1{
		...
		min-width:30px;
		height:100%; font-size:13px; letter-spacing:-0.05em; line-height:26px;overflow:hidden;
	}
	.lotteryMatch__seedWrap{
		display:flex;align-items:center;justify-content:center;
		min-width:30px;
		height:100%; font-size:13px; letter-spacing:-0.05em; line-height:26px;
		background-color:#dbe1e8;overflow:hidden;
	}
  ...
  .lotteryMatch_first>.lotteryMatch__game_num{position:absolute;top:50%;transform:translateY(-52%);z-index:99;display:flex;justify-content:center;align-items:center;width:40px;height:110%;border:1px solid #ccc;color:#000;font-size:10px;font-weight:500;box-sizing:border-box;}
  .round_1 .lotteryMatch_first>.lotteryMatch__game_num{position:relative;top:0;transform:none;width:30px;border-radius:0 4px 4px 0;border-left:0;height:200%;}
  .round_1 .lotteryMatch_first{bottom:50%;top:auto;transform:none;}
...
<script>
   ...
   var gAryQPos = null;                 // 예선조 참가자 배치 Position Array
	var gAryGameNum = null;              // Tonament Game Num
   var gAryQGameNum = null;             // 예선조 Game Num

   var gStrPos = "<%=strPos%>";
   var gStrQPos = "<%=strQPos%>";
	var gStrGameNum = "<%=strGameNum%>";					// gaem num
	var gStrQGameNum = "<%=strQGameNum%>";					// q game num
    ...
    $(document).ready(function () {
      if(gStrPos != "") {
         gAryPos =  utx.Get2DAryFromStr(gStrPos, "|", ",");
			gAryGameNum =  gStrGameNum.split(',');
         createTournament(gAryPos, gAryGameNum);
      }

      if(gStrQPos != "") {
         gAryQPos =  utx.Get2DAryFromStr(gStrQPos, "|", ",");
			gAryQGameNum =  gStrQGameNum.split(',');

         createQTournament(gAryQPos, gAryQGameNum);
      }
    });
</script>
...
/* ==================================================================================
   ...
   ================================================================================== */
   function createQTournament(rAryPos, rAryGameNum)
   {
      ...
      var aryData, nOrder = 0, IsFinal = 0, cntUser = 0, bPageBreak = false;
      var nQPerPage = 6, nScale = 1, nBlockHeight = 48, round_depth = 2;
      var strQInfo;
      ...
      strQInfo = GetStrGInfoQ(gStrGTitle, gStrGKind, cntUser);
      ctx.appendHtmlToDiv("div_QTournamentInfo", strQInfo);

      for(i = 0; i< nQGroup; i++)
      {
         bPageBreak = false;
         if( (i > 0 && (i+1) % nQPerPage == 0) || i+1 == nQGroup) bPageBreak = true;

         id_divQInfo = utx.strPrintf("{0}_info{1}", id_base, i+1);
         id_divQ = utx.strPrintf("{0}{1}", id_base, i+1);
         createDivForTonamentEx(id_parent, id_divQ, id_divQInfo, bPageBreak);
         strQInfo = GetStrGInfoQSub(i+1);
         ctx.appendHtmlToDiv(id_divQInfo, strQInfo);
         aryData[i] = new Array();

			for(j = 0; j<round_depth; j++)
			{
				aryData[i][j] = new Array();
			}
      }
      ...
      if(len == nQGroupSize)
      {
         if(rAryPos[1][2] ==  E_POS_BYE && rAryPos[2][2] ==  E_POS_BYE)
         {
            ...
            {
               obj = fillUserInfoSetNum(0, rAryPos[0], rAryPos[3], 1, IsDblPlay);
            }
            else if(gGroupGameGB == "B0030002")    // 단체전
            {
               obj = fillTeamUserInfoSetNum(0, rAryPos[0], rAryPos[3], 1, IsDblPlay);
            }
            aryData[nOrder][0].push(obj)
         }
      }
      ...
      if(!IsFinal)
      {
         for(i = 0; i< len; i+=2)
         {
            strLog = utx.sprintf("Q Insert Idx = {0}, pos = {1}", ref_param.Idx, rAryGameNum[ref_param.Idx]);
            console.log(strLog);
            if(i && i%nQGroupSize == 0) nOrder++;
            if(gGroupGameGB == "B0030001")         // 개인전
            {
               obj = fillUserInfoSetNum(i, rAryPos[i], rAryPos[i+1], rAryGameNum[ref_param.Idx++], IsDblPlay);
            }
            else if(gGroupGameGB == "B0030002")    // 단체전
            {
               obj = fillTeamUserInfoSetNum(i, rAryPos[i], rAryPos[i+1], rAryGameNum[ref_param.Idx++], IsDblPlay);
            }
            aryData[nOrder][0].push(obj)
            }
         }
		...
		var empty_round = roundQ / 2;
		pos_info = roundQ / 2;
		for(i = 0; i < nQGroup; i++)
      {
			pos_info = MakeSetNumEmptyData(aryData[i][1], empty_round, pos_info, rAryGameNum, ref_param);
      }
      ...
      for(i = 0; i< nQGroup; i++)
      {
         ...
         {
            nBlockHeight = 48;
				nFirstBoarderGap = 210, nBoardWidth = 40;
				tournamentQ = makeSetNumTournamentDouble(id_divQ, nScale, nBlockHeight,nBoardWidth, nFirstBoarderGap);
         }
         else
         {
				nBlockHeight = 48;
				...
				tournamentQ = makeSetNumTournamentSingle(id_divQ, nScale, nBlockHeight, nBoardWidth, nFirstBoarderGap);
         }
         // drawSetNumTournament(tournamentQ, roundQ, roundQ, roundQ, aryData[i]);
			drawSetNumTournament(tournamentQ, roundQ, round_depth, aryData[i]);
...
/* ==================================================================================
   ...
   ================================================================================== */
   function createTournament(rAryPos, rAryGameNum)
   {
      var aryData, i = 0, len = 0, obj, IsDblPlay = 1, round = 0, round_kind = 0, cur_round = 0;
      var f_tournament, strInfo, cntUser, nScale = 1, nBlockHeight = 48, round_depth = 0;
      aryData = new Array();
      ...
      {
			pos_info = MakeSetNumEmptyData(aryData[i], empty_round, pos_info, rAryGameNum, ref_param);
      ...
      if(IsDblPlay == 1)
      {
         ...
         f_tournament = makeSetNumTournamentDouble("div_FTournament", nScale, nBlockHeight,nBoardWidth, nFirstBoarderGap);
            }
            else
            {
         nBlockHeight = 48;
         ...
         f_tournament = makeSetNumTournamentSingle("div_FTournament", nScale, nBlockHeight, nBoardWidth, nFirstBoarderGap);
            }
      // drawTournament2(f_tournament, round, round, round, aryData);
		drawSetNumTournament(f_tournament, round, round_depth, aryData);
         }


   /* ==================================================================================
         예선전 Title String을 얻는다.
      ================================================================================== */
   function GetStrGInfoQ(strGTitle, strGKind, nQGroupCnt)
   {
      var strInfoTitle, strInfoKind, strInfoQGroup;
      var strInfo = utx.sprintf("   \
        <h3>{0}</h3>    \
        <p><strong>[{1} 예선] </strong> <span>참가선수: {2}팀</span></p>    ", strGTitle, strGKind, nQGroupCnt);
      return strInfo;
            }
...
/* ==================================================================================
   ...
   function drawSetNumTournament(objT, round, round_kind, rAryData)
   {
	...
	/* ==================================================================================
      빈 토너먼트 생성
   	================================================================================== */
	function MakeSetNumEmptyData(rAryEmpty, empty_round, pos_info, rAryGameNum, ref_param)
   {
      var i = 0;

      for(i=0; i<empty_round; i += 2)
      {
         strLog = utx.sprintf("MakeSetNumEmptyData Idx = {0}, pos = {1}", ref_param.Idx, rAryGameNum[ref_param.Idx]);
            console.log(strLog);
         ...
         rAryEmpty.push({
            ...
            l_player:'', l_team:'',
            r_player:'', r_team:'',
				gameNum: rAryGameNum[ref_param.Idx++],
         });
      }
...
/* ==================================================================================
   ...
   ================================================================================== */
   function fillTeamUserInfoSetNum(Idx, l_info, r_info, gameNum, IsDblPlay)
   {
      var obj = {};
...
/* ==================================================================================
      tournament에 사용할 data object을 만든다.
   ================================================================================== */
   function fillUserInfoSetNum(Idx, l_info, r_info, gameNum, IsDblPlay)
   {
      var obj = {};
...
function makeSetNumTournamentDouble(id_div, nScale, nBlockHeight, nBoardWidth, nFirstBoardWidth)
{
  ...
  tournament.setOption({
   ...
   tournament.boardInner = function(data){
      	return boardInner_setNumDouble(data);
   }
  return tournament;
}
  ...
  sindou=0;
   ...
   if(data){
      ...
      r_firstMan   = data.r_firstMan;
		...
		game_num		= (data.gameNum == 0) ? '' : data.gameNum;
	...
	if(round_1) {
      ...
      '<p id = '+l_id+' class="lotteryMatch lotteryMatch_first [ _match ] '+ l_sel + ' ' + l_fill + '" data-match-no="'+matchNo+'" data-player-no="'+ (matchNo * 2) +'">',
         ...
         '<span class="lotteryMatch__playerWrap">',
            ...
            '</span>',
         '</span>',
        '<span class="lotteryMatch__game_num">'+game_num+'</span>',
      '</p>',
      '<p id = '+r_id+' class="lotteryMatch lotteryMatch_second [ _match ] '+ r_sel + ' ' + r_fill + '" data-match-no="'+matchNo+'" data-player-no="'+ (matchNo * 2 + 1) +'">',
         ...
         '<span class="lotteryMatch__playerWrap">',
            ...
            '</span>',
         '</span>',
        '<span class="lotteryMatch__game_num"></span>',
      '</p>'
   ].join('');
	}
	else {
