...
'               strLog = strPrintf("entry_type = {0}, l_score = {1}, r_score = {2}, rAryLeague(10,infoIdx) = {3}, rAryLeague(12,infoIdx) = {4}, rAryLeague(11,infoIdx) = {5}, rAryLeague(13,infoIdx) = {6}<br>", _ 
'                           Array(entry_type, l_score, r_score, rAryLeague(10,infoIdx), rAryLeague(12,infoIdx), rAryLeague(11,infoIdx), rAryLeague(13,infoIdx)))
'                  response.write strLog
               ...
               If(infoIdx <> -1) Then ' 정보를 찾았다. 셋팅을 하자.
                  ...
                  if(entry_type = "A") Then ' ama이면  score를 L_SetJumsu	,R_SetJumsu로 사용    
                     ...
                     If(l_score = "0") Then l_score = "" End If 
                     If(r_score = "0") Then r_score = "" End If 
...
<head>
  ...
  <style>
   ...
   <style>
		.main.tournamentTree{margin:2mm 0 5mm 0mm;padding:0 3mm;box-sizing:border-box;overflow:hidden;}

		/* .tt__item.s_roundOf_4 .tt__block .tt__board{border:1px solid #666 !important; background-color:#fff;} */

		.main .tt__inner>.tt__item .tt__board{display:flex;border:1px solid #666;background-color:#fff;}
		.tt__seedWrap{display:none;flex-direction:column;justify-content:center;align-items:center;width:40px;height:100%;text-align:left;text-indent:2px;font-size:9pt;letter-spacing:-0.05em;}
		.tt__seedWrap>span{display:flex;align-items:center;}
		.tt__seedWrap>span:first-child{width:100%;height:50%;line-height:1.1;justify-content:flex-start;}
		.tt__seedWrap>span:last-child{width:100%;height:50%;line-height:1.1;justify-content:flex-end;}
		.s_round_1 .tt__seedWrap{display:flex;}

		.tt__board_playerWrap{display:flex;flex-direction:column;justify-content:center;width:100%;min-width:0;height:100%;padding:0 2px 0 4px;box-sizing:border-box;}
		.tt__board_playerInner{width:100%;display:inline-block;white-space:nowrap;text-overflow:ellipsis;overflow:hidden;}
		.tt__board_player{display:inline;font-size:9.7pt;font-weight:700;font-weight:700;padding-right:2px;}
		.tt__board_belong{display:inline;font-size:9pt;}

		.tt__board_score{flex-shrink:0;flex-grow:0;width:30px;border-left:1px solid #666;display:flex;align-items:center;justify-content:center;font-size:12pt;}

		.s_roundOf_2 .tt__seedWrap,
		.s_roundOf_4 .tt__seedWrap{font-size:8pt;}
		.s_roundOf_2 .tt__board_player,
		.s_roundOf_4 .tt__board_player{font-size:7pt;}
		.s_roundOf_2 .tt__board_belong,
		.s_roundOf_4 .tt__board_belong{font-size:6pt;}

		.s_roundOf_2 .tt__board_playerInner,
		.s_roundOf_4 .tt__board_playerInner{width:100%;display:flex;height:50%;white-space:nowrap;text-overflow:ellipsis;overflow:hidden;}
...
<script>
	...
	function makeTournament(id_div)
	{
		...
		if(makeTournament.fg){
			makeTournament.fg = false;

			tournament.setStyle({ selector: '' });
			tournament.setStyle({
				selector: '.s_roundOf_4',
				style:{
					blockMatchWidth: 320,
					blockBranchWidth: 15,
					blockHeight: 32,
	...
	function drawTournament(objT, round, round_kind, rAryData)
	{
		...
		$('.tournament__inner').css({'display':'block'})
...
%>
 ...
 '        USQL2 = "tblGameTourneyChartLeague_Searched '" & Arr_GameLevelDtlIDX(0,i) & "'"
         ...
         strLog = strPrintf("Player Sql  = {0}", Array(USQL))
...
'         response.End
   ...
   '         response.write "개인전일때 리그  = " & CSQL & "<br>"
            ...
            strLog = strPrintf("개인전일때 리그  = {0}", Array(CSQL))
            ' Call TraceLog(SPORTS_LOG1, strLog)
...
'            Call printInfo(aryLeague)
            ...
            '' ' Call TraceLogInfo(aryLeague, " aryLeague " )
            '' ' Call TraceLogInfo(Arr_Player, " Arr_Player " )
...
'            Call printInfo(aryInfo)
         ...
         Else
            ...
            strLog = strPrintf("단체전일때 리그  = {0}", Array(CSQL))
            ' Call TraceLog(SPORTS_LOG1, strLog)
  ...
  <div class="wrapper">
		...
		<div class="header">
			...
			<div class="moreInfo">
				...
				<span class="attend"> 
               <span>참가선수: <%=UBOUND(Arr_Player,2) + 1%><%If GroupGameGb = "B0030001" Then%>명<%Else%>팀<%End If%></span>
               <% If IsTeamGame = 1 Then %> &nbsp&nbsp&nbsp<span>※팀 게임은 점수가 표시되지 않습니다.</span> <%End If%>
            </span>
			</div>
		</div>
...
<%
   ...
   strLog = sprintf("Prev ama Program Time = {0}", Array(endTimer-sTartTimer))
   Call TraceLog(SPORTS_LOG1, strLog)
%>