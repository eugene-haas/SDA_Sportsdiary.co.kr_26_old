...
'   =============================================================================== 
	Function uxCheckWinner(rAryHistory, rAryRank, sPos, IsTeamGame)
      ...
      Dim l_teamDtl, r_teamDtl, game_result, rKey
      ...
      strLog = strPrintf("uxCheckWinner l_pos = {0}, r_pos = {1}, IsTeamGame = {2}<br>", Array(l_pos, r_pos, IsTeamGame))
      ' response.write strLog 

      If( IsTeamGame = 1 ) Then 
         l_team1        = rAryRank(8, l_pos)             
         l_teamDtl      = rAryRank(10, l_pos)
         r_team1        = rAryRank(8, r_pos)
         r_teamDtl      = rAryRank(10, r_pos)
      Else 
         l_player1      = rAryRank(4, l_pos)             
         l_player2      = rAryRank(5, l_pos)
         l_team1        = rAryRank(8, l_pos)
         l_team2        = rAryRank(9, l_pos)
         r_player1      = rAryRank(4, r_pos)             
         r_player2      = rAryRank(5, r_pos)
         r_team1        = rAryRank(8, r_pos)
         r_team2        = rAryRank(9, r_pos)
      End If 

      is_reverse = 0
      For cidx = 0 to cub    
         If( IsTeamGame = 1 ) Then         
            infoIdx = uxFindTeamGameInfo2(rAryHistory, l_team1,l_teamDtl, r_team1,r_teamDtl, is_reverse )            
         Else
            infoIdx = uxFindPersonGameInfo2(rAryHistory, l_player1, l_team1, l_player2, l_team2, r_player1, r_team1, r_player2, r_team2, is_reverse )
         End If
         If(infoIdx <> -1) Then
            ...
            strLog = strPrintf("infoIdx = {0}, game_result = {1}, cntWin = {2}<br>", Array(infoIdx, game_result, cntWin))
            ' response.write strLog 
         End If 
      Next
   ...
   Function uxFindPersonGameInfo(rAry, l_player1, l_team1, l_player2, l_team2, r_player1, r_team1, r_player2, r_team2, is_reverse)
      ...
      For ai = 0 To ul 
         If(InStr(rAry(0, ai), "|")) Then 
            ...
            If(aryLP(0) = CStr(l_player1) And aryLP(1) = CStr(l_player2) ) And (aryLT(0) = l_team1 And aryLT(1) = l_team2) And _
              (aryRP(0) = CStr(r_player1) And aryRP(1) = CStr(r_player2) ) And (aryRT(0) = r_team1 And aryRT(1) = r_team2) Then 
              retIdx = ai
              Exit For 
            End If 
         Else 
            If(rAry(1, ai) = CStr(l_player1) Or rAry(1, ai) = CStr(l_player2) ) And (rAry(3, ai) = l_team1 Or rAry(3, ai) = l_team2) And _
              (rAry(6, ai) = CStr(r_player1) Or rAry(6, ai) = CStr(r_player2) ) And (rAry(8, ai) = r_team1 Or rAry(8, ai) = r_team2) Then 
              retIdx = ai
              Exit For 
            End If 
         End If          
...
'      7-15 R_TeamNames, R_Teams, R_TeamDtl,  L_SetJumsu	,L_MatchJumsu,	R_SetJumsu,R_MatchJumsu, L_Result, EnterType 
'   =============================================================================== 
   Function uxFindPersonGameInfo2(rAry, l_player1, l_team1, l_player2, l_team2, r_player1, r_team1, r_player2, r_team2, is_reverse)
      Dim ai, ul, aryLP, aryRP, aryLT, aryRT, retIdx
      ...
      if(is_reverse = 0) then 
         strLog = strPrintf("uxFindPersonGameInfo2 -- l_player1 = {0}, l_player2 = {0}, l_team1 = {0}, l_team2 = {0}<br>", Array(l_player1, l_player2, l_team1, l_team2))
         ' response.write strLog 
         strLog = strPrintf("uxFindPersonGameInfo2 -- r_player1 = {0}, r_player2 = {0}, r_team1 = {0}, r_team2 = {0}<br>", Array(r_player1, r_player2, r_team1, r_team2))
         ' response.write strLog 
      End if 
      ...
      For ai = 0 To ul 
         If(InStr(rAry(0, ai), "|")) Then 
            ...
            if(is_reverse = 0) then 
               strLog = strPrintf("aryLP1 = {0}, aryLP2 = {1}, aryLT1 = {2}, aryLT2 = {3}<br>", Array(aryLP(0), aryLP(1), aryLT(0), aryLT(1)))
               ' response.write strLog 
               strLog = strPrintf("aryRP1 = {0}, aryRP2 = {1}, aryRT1 = {2}, aryRT2 = {3}<br>", Array(aryRP(0), aryRP(1), aryRT(0), aryRT(1)))
               ' response.write strLog 
            end if 
            ...
            If(aryLP(0) = CStr(l_player1) And aryLP(1) = CStr(l_player2) ) And (aryLT(0) = l_team1 And aryLT(1) = l_team2) And _
              ...
              strLog = strPrintf("******************uxFindPersonGameInfo2 Find idx = {0}-- <br>", Array(retIdx))
               ' response.write strLog 
              Exit For 
            End If 
         Else 
            If(rAry(1, ai) = CStr(l_player1) Or rAry(1, ai) = CStr(l_player2) ) And (rAry(3, ai) = l_team1 Or rAry(3, ai) = l_team2) And _
              (rAry(6, ai) = CStr(r_player1) Or rAry(6, ai) = CStr(r_player2) ) And (rAry(8, ai) = r_team1 And rAry(8, ai) = r_team2) Then 
              retIdx = ai
...
'      7-15 R_TeamNames, R_Teams, R_TeamDtl,  L_SetJumsu	,L_MatchJumsu,	R_SetJumsu,R_MatchJumsu, L_Result, EnterType 
'   =============================================================================== 
   Function uxFindTeamGameInfo2(rAry, l_team1,l_teamDtl, r_team1,r_teamDtl, is_reverse)
      Dim ai, ul, l_team, l_dtl, r_team, r_dtl 
      ...
      if(is_reverse = 0) then 
         strLog = strPrintf("uxFindTeamGameInfo2 -- l_team = {0}, l_dtl = {1}, r_team = {2}, r_dtl = {3}<br>", Array(l_team1,l_teamDtl, r_team1,r_teamDtl))
         ' response.write strLog 
      end if 
      ...
      For ai = 0 To ul 
         ...
         if(is_reverse = 0) then 
            strLog = strPrintf("l_team = {0}, l_dtl = {1}, r_team = {2}, r_dtl = {3}<br>", Array(l_team, l_dtl, r_team, r_dtl))
            ' response.write strLog 
         end if 
         ...
         If(l_team = l_team1 And l_dtl = l_teamDtl) And (r_team = r_team1 And r_dtl = r_teamDtl) Then 
            ...
            strLog = strPrintf("******************uxFindTeamGameInfo2 Find idx = {0}-- <br>", Array(retIdx))
            ' response.write strLog 
