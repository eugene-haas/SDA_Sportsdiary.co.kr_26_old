  ...
  <% if (IPHONEYN() = "0" and iLinkType = "2") then %>
    ...
    <% else %>
      ...
      <div style="background-color: #<%=LRs("BColor")%>"> <a href="<%=MALLURL%>tube.asp?p=<%=Server.URLEncode(malljsondata)%>" class="banner_area" target="_blank" onclick=""> <img src="<%=global_filepathUrl_ADIMG %><%=LRs("ImgFileNm") %>" alt="" /> </a> </div>
      <% elseif iSDMallYN = "Y" and iLIMemberIDXG = "" then %>
      <div style="background-color: #<%=LRs("BColor")%>"> <a href="<%=MALLURL%>tube.asp?p=<%=Server.URLEncode(malljsondata)%>" class="banner_area" target="_blank" onclick=""> <img src="<%=global_filepathUrl_ADIMG %><%=LRs("ImgFileNm") %>" alt="" /> </a> </div>
      <% else %>
      <div style="background-color: #<%=LRs("BColor")%>"> <a href="<%=iLink %>" class="banner_area" target="_blank" onclick=""> <img src="<%=global_filepathUrl_ADIMG %><%=LRs("ImgFileNm") %>" alt="" /> </a> </div>
      <% end if %>
    <% end if %>
  <% else %>
    <% if iLink = "" then %>
    ...
    <% else %>
      <% if iSDMallYN = "Y" and iLIMemberIDXG <> "" then %>
      <div style="background-color: #<%=LRs("BColor")%>"> <a href="javascript:;" onclick="alert('sportsdiary://urlblank=<%=MALLURL%>tube.asp?p=<%=Server.URLEncode(malljsondata)%>');" class="banner_area"> <img src="<%=global_filepathUrl_ADIMG %><%=LRs("ImgFileNm") %>" alt="" /> </a> </div>
      <% elseif iSDMallYN = "Y" and iLIMemberIDXG = "" then %>
      <div style="background-color: #<%=LRs("BColor")%>"> <a href="javascript:;" onclick="alert('sportsdiary://urlblank=<%=MALLURL%>tube.asp?p=<%=Server.URLEncode(malljsondata)%>');" class="banner_area"> <img src="<%=global_filepathUrl_ADIMG %><%=LRs("ImgFileNm") %>" alt="" /> </a> </div>
      <% else %>
      <div style="background-color: #<%=LRs("BColor")%>"> <a href="javascript:;" onclick="javascript:fn_mclicklink('<%=iLinkType %>','<%=iLink %>');" class="banner_area"> <img src="<%=global_filepathUrl_ADIMG %><%=LRs("ImgFileNm") %>" alt="" /> </a> </div>
      <% end if %>
    <% end if %>
  <% end if %>
  <%
