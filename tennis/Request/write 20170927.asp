...
<script>
      ...
      success: function(retDATA) {
        ...
        if(retDATA){
          ...
          if(strcut[0]=="TRUE"){
            ...
            //대회참가 신청기간이 만료되면 수정 및 삭제권한 없음처리
            if(strcut[5]=="off"){
              $('#btn_Del').hide();
              $('#btn_Mod').hide();
            }
  ...
  function CHK_OnSubmit(valType){
    ...
    if(!$('#UserPass').val()){ msg += "비밀번호\n";  num = 1; }
      ...
      if($('#UserPass').val().length<4 || $('#UserPass').val().length>12){    msg += "패스워드는 4자~12자입니다.";  num = 1;}   
    }
    ...
    //성별체크
    $("input[name='P1_GenderIDX']").each(function (i) {
      if(!$("input[name='P1_GenderIDX']").eq(i).val()){ msg += "참가자 성별 선택["+(i+1)+"]\n"; num = 1; }
    });
    

    //생년월일
    $("input[name='P1_Birthday']").each(function (i) {
      if(!$("input[name='P1_Birthday']").eq(i).val()){ 
      msg += "참가자 생년월일["+(i+1)+"]\n"; num = 1; 
    }
    else{
      if($("input[name='P1_Birthday']").eq(i).val().length < 8){
        msg += "참가자 생년월일["+(i+1)+"] 8자리 입력\n"; num = 1;
      }
      else{
        data = $("input[name='P1_Birthday']").eq(i).val();
        ...
        if(dt.getDate() != d) { msg += "참가자 생년월일["+(i+1)+"] 일이 유효하지 않습니다.\n"; num = 1; } 
        else if(dt.getMonth()+1 != m) { msg += "참가자 생년월일["+(i+1)+"] 월이 유효하지 않습니다.\n"; num = 1; } 
        else if(dt.getFullYear() != y) { msg += "참가자 생년월일["+(i+1)+"] 년도가 유효하지 않습니다.\n"; num = 1; } 
        else { }  
        }
      }
    });
    ...
    //성별체크
    $("input[name='P2_GenderIDX']").each(function (i) {
      if(!$("input[name='P2_GenderIDX']").eq(i).val()){ msg += "파트너 성별 선택["+(i+1)+"]\n"; num = 1; }
    });
    

  //생년월일
    $("input[name='P2_Birthday']").each(function (i) {
      if(!$("input[name='P2_Birthday']").eq(i).val()){ 
      msg += "파트너 생년월일["+(i+1)+"]\n"; num = 1; 
      }
      else{
      if($("input[name='P2_Birthday']").eq(i).val().length < 8){
        msg += "파트너 생년월일["+(i+1)+"] 8자리 입력\n"; num = 1;
      }
      else{
        data = $("input[name='P2_Birthday']").eq(i).val();
      ...
      d = parseInt(data.substr(6, 2), 10); 
        ...
        if(dt.getDate() != d) { msg += "파트너 생년월일["+(i+1)+"] 일이 유효하지 않습니다.\n"; num = 1; } 
        else if(dt.getMonth()+1 != m) { msg += "파트너 생년월일["+(i+1)+"] 월이 유효하지 않습니다.\n"; num = 1; } 
        else if(dt.getFullYear() != y) { msg += "파트너 생년월일["+(i+1)+"] 년도가 유효하지 않습니다.\n"; num = 1; } 
        else { }  
      }
      }
    });
      ...
      <form name="s_frm" method="post">
            ...
            IF Not(CRs.Eof Or CRs.Bof) Then 
              ...
              End IF  
                ...
                <li id="div_Entry<%=cnt%>" class="entry el_2">
                  ...
                  <input type="hidden" id="P1_GenderIDX" class="p1_gender_idx" name="P1_GenderIDX" value="<%=CRs("P1_SEX")%>" />
                  <input type="hidden" id="P2_GenderIDX" class="p2_gender_idx" name="P2_GenderIDX" value="<%=CRs("P2_SEX")%>" />
                </li>
                <%
              CRs.Movenext
            Loop
        ...
        %>
                <li class="entry el_2">
                  ...
                  <!-- E: party -->
                  <input type="hidden" id="P1_GenderIDX" class="p1_gender_idx" name="P1_GenderIDX" />
                  <input type="hidden" id="P2_GenderIDX" class="p2_gender_idx" name="P2_GenderIDX" />
                </li>
                <%
        END SELECT  
          %>
            ...
            <li class="guide_txt club_guide">
              ...
              <p>※생년월일은 스포츠다이어리 앱 서비스 활용을 위해 필수로 입력해 주셔야 합니다. </p>
            </li>
            <!-- E: 소속 입력 안내 -->
            <!-- S: 생년월일 입력 안내 -->
