...
If Grouptype = "sd040001" Then 
	...
	If Medal_Type = "" Then
		'마스터의idx 로 조회
		LSQL = "SELECT RGameLevelIDX,SchIDX,SportsGb,TeamGb,Sex,GroupGameGb,GameTitleIDX,Level,PlayerIDX, Team, TeamDtl FROM tblRPlayerMaster Where RPlayerMasterIDX='"&MasterIDX&"'"
		...
		If Not(LRs.Eof Or LRs.Bof) Then 
			...
			If Not(LRs.Eof Or LRs.Bof) Then 
				...
				DelSQL = DelSQL&" AND Team='"&LRs("Team")&"'"
				DelSQL = DelSQL&" AND TeamDtl='"&LRs("TeamDtl")&"'"
	...
	Else
		'해당선수 입력 필드 셀렉트
		LSQL = "SELECT GameTitleIDX,RGameLevelIDX,SchIDX,SportsGb,TeamGb,Sex,Level,PlayerIDX,GroupGameGb,Team,TeamDtl FROM Sportsdiary.dbo.tblRPlayerMaster Where RPlayerMasterIDX='"&MasterIDX&"'"
		Set LRs = Dbcon.Execute(LSQL)		
		...
		If Not(LRs.Eof Or LRs.Bof) Then 
  		...
  		Dbcon.Execute(DelSQL)
			...
			InSQL = InSQL&"(RGameLevelIDX,GameTitleIDX, SportsGb, TeamGb, Level,PlayerIDX, GroupGameGb,MedelGubun, Sex, TitleResult, GameRanking,MemoLog, Team, TeamDtl)"
			InSQL = InSQL&" VALUES "
			InSQL = InSQL&"("
			...
			InSQL = InSQL&",'"&Left(Medal_Type,8)&"'"
			InSQL = InSQL&",'"&LRs("Sex")&"'"
			InSQL = InSQL&",'"&Left(Medal_Type,8)&"'"
			InSQL = InSQL&",'"&Right(Medal_Type,1)&"'"
			InSQL = InSQL&",'수동메달'"
			InSQL = InSQL&",'"&LRs("Team")&"'"
			InSQL = InSQL&",'"&LRs("TeamDtl")&"'"
			InSQL = InSQL&")"   
			Dbcon.Execute(InSQL)
			Response.Write "Ins"
			Response.End			
...
'단체전
	If Medal_Type = "" Then 
		'메달타입이 없는 경우 메달 테이블 데이터 조회 후 삭제 처리
		LSQL = "SELECT RGameLevelIDX,SchIDX,SportsGb,TeamGb,Sex,SchoolName,GroupGameGb,GameTitleIDX,Team,TeamDtl FROM Sportsdiary.dbo.tblRGameGroupSchoolMaster Where RGameGroupSchoolMasterIDX='"&MasterIDX&"'"
		Set LRs = Dbcon.Execute(LSQL)
	...
	Else
		'해당학교 입력 필드 셀렉트
		LSQL = "SELECT GameTitleIDX,RGameLevelIDX,SchIDX,SportsGb,TeamGb,Sex,SchoolName,GroupGameGb,Team,TeamDtl FROM Sportsdiary.dbo.tblRGameGroupSchoolMaster Where RGameGroupSchoolMasterIDX='"&MasterIDX&"'"
		Set LRs = Dbcon.Execute(LSQL)		
		...
		If Not(LRs.Eof Or LRs.Bof) Then 
  		...
  		Dbcon.Execute(DelSQL)
			...
			'505	72	20	judo	sd011003	lv003008	441	sd040001	sd033001	18573	Man	sd034003	3	N	2016-11-28 14:15:19.467
			InSQL = " INSERT INTO SportsDiary.dbo.tblGameScore "
			InSQL = InSQL&"(RGameLevelIDX,GameTitleIDX, SportsGb, TeamGb, PlayerIDX, GroupGameGb,MedelGubun, Sex, TitleResult, GameRanking,MemoLog, Team, TeamDtl)"
			InSQL = InSQL&" VALUES "
			InSQL = InSQL&"("
			InSQL = InSQL&"'"&LRs("RGameLevelIDX")&"'"
			InSQL = InSQL&",'"&LRs("GameTitleIDX")&"'"
