...
%>
	...
	<tr>
		...
		<%
			If MatchCntRs("Cnt") > 0 Then 
		%>
		<td><a href="javascript:matchnum_view('<%=LRs("GameTitleIDX")%>','<%=LRs("TeamGb")%>','<%=LRs("Level")%>','<%=LRs("GroupGameGb")%>','<%=LRs("TotRound")%>','<%=MatchCntRs("Cnt")%>','<%=LRs("GameType")%>');" class="btn-list">대진표(번호)보기</a></td>
		<%
			Else 
		%>
		<td><a href="javascript:alert('대진표 추첨전인 체급입니다.');" class="btn-list">추첨전</a></td>
