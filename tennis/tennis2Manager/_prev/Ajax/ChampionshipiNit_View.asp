...
%>
	...
	<tr>		
		...
		<td style="cursor:pointer;" onclick="some_init('<%=encode(LRs("RGameResultIDX"),0)%>','<%=LRs("GroupGameGbName")%>','<%=LRs("Gender")%>','<%=LRs("Level")%>','<%=LRs("Player")%>');">기술초기화</td>		
		<td style="cursor:pointer;" onclick="all_init('<%=encode(LRs("RGameResultIDX"),0)%>','<%=LRs("GroupGameGbName")%>','<%=LRs("Gender")%>','<%=LRs("Level")%>','<%=LRs("Player")%>');">체급초기화</td>		
