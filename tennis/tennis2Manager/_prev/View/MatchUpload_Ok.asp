...
<%
  ...
  Level              = fInject(Request("Level"))
	...
	%>
	<script>
	location.href="MatchUpload.asp?sportsGb=<%=SportsGb%>&GameTitleIDX=<%=GameTitleIDX%>&StadiumNumber=<%=StadiumNumber%>&GameDay=<%=GameDay%>&Search_GroupGameGb=<%=Search_GroupGameGb%>&Search_TeamGb=<%=Search_TeamGb%>&Level=<%=Level%>"
	</script>