...
<%	
	...
	LSQL = LSQL&" ,isnull(PR.NowRound,0) AS NowRound"
	...
	LSQL = LSQL&"   AND (PR.GroupGameGb = '"&SportsCode&"040001' OR (PR.GroupGameGb = '"&SportsCode&"040002' AND (ISNULL(PR.LPlayerIDX,'') = '' AND ISNULL(PR.RPlayerIDX,'') = '' ))) "
	LSQL = LSQL&" AND (ISNULL(PR.RResult,'') <> '"&SportsCode&"019006' AND ISNULL(PR.LResult,'') <> '"&SportsCode&"019006' AND ISNULL(PR.RResult,'') <> '"&SportsCode&"019021' AND ISNULL(PR.LResult,'') <> '"&SportsCode&"019021' AND ISNULL(PR.RResult,'') <> '"&SportsCode&"019022' AND ISNULL(PR.LResult,'') <> '"&SportsCode&"019022' AND ISNULL(PR.RResult,'') <> '"&SportsCode&"019012' AND ISNULL(PR.LResult,'') <> '"&SportsCode&"019012')"
	...
	If Request.Cookies("SportsGb") = "judo" Then 
		LSQL = LSQL&" order by StadiumNumber, PR.TurnNum ASC"
	Else
		LSQL = LSQL&" order by PR.Level,CONVERT(int, PR.GameNum)"
	End If 
	'Response.Write LSQL
	'Response.End
		...
		<tr>
			...
			<%
				...
				If LPlayerNm = "" And LTeamNm = "" Then 
					...
					CSQL = "select count(distinct(NowRound)) AS Cnt FROM tblPlayerResult WHERE "
					'CSQL = CSQL&" DelYN='N' AND "
					CSQL = CSQL&" GameTitleIDX='"&GametitleIDX&"' AND RGameLevelidx='"&LRs("RGameLevelIDX")&"'"
					...
					If LRs("GroupGameGb") = SportsCode&"040001" Then 
						...
						LPlayerSQL = LPlayerSQL&" AND (RGame1R = '"&LRs("GameNum")&"' or RGame2R = '"&LRs("GameNum")&"' or RGame3R = '"&LRs("GameNum")&"' or RGame4R = '"&LRs("GameNum")&"' or RGame5R = '"&LRs("GameNum")&"' or RGame6R = '"&LRs("GameNum")&"' or RGame7R = '"&LRs("GameNum")&"' ) "
					Else
						...
						LPlayerSQL = LPlayerSQL&" AND (RGame1R = '"&LRs("GroupGameNum")&"' or RGame2R = '"&LRs("GroupGameNum")&"' or RGame3R = '"&LRs("GroupGameNum")&"' or RGame4R = '"&LRs("GroupGameNum")&"' or RGame5R = '"&LRs("GroupGameNum")&"' or RGame6R = '"&LRs("GroupGameNum")&"' or RGame7R = '"&LRs("GroupGameNum")&"' ) "
					End If 
				...
				If RPlayerNm = "" And RTeamNm  = "" Then 
					...
					If LRs("GroupGameGb") = SportsCode&"040001" Then  
						...
						RPlayerSQL = RPlayerSQL&" AND (RGame1R = '"&LRs("GameNum")&"' or RGame2R = '"&LRs("GameNum")&"' or RGame3R = '"&LRs("GameNum")&"' or RGame4R = '"&LRs("GameNum")&"' or RGame5R = '"&LRs("GameNum")&"' or RGame6R = '"&LRs("GameNum")&"' or RGame7R = '"&LRs("GameNum")&"' ) "
					Else
						...
						RPlayerSQL = RPlayerSQL&" AND (RGame1R = '"&LRs("GroupGameNum")&"' or RGame2R = '"&LRs("GroupGameNum")&"' or RGame3R = '"&LRs("GroupGameNum")&"' or RGame4R = '"&LRs("GroupGameNum")&"' or RGame5R = '"&LRs("GroupGameNum")&"' or RGame6R = '"&LRs("GroupGameNum")&"' or RGame7R = '"&LRs("GroupGameNum")&"' ) "
					End If 
					...
					If Not (RPRS.Eof Or RPRS.Bof) Then 
						...
						RTeamNm = RPRS("GameNum")&"번승자"
					End If 
				End If 
			%>
			<%If LRs("GroupGameGb") = SportsCode&"040001" Then  %>
			<td><%=LPlayerNm%> VS <%=RPlayerNm%></td>
			<%else%>
			<td><%=LTeamNm%> VS <%=RTeamNm%></td>
			<%End If %>
			<%
			...
			%>
<!--			
			<td><input type="text" name="TurnNum<%=i%>" id="TurnNum<%=i%>" value="<%=TurnNum%>" onKeyUp="input_turn('<%=LRs("PlayerResultIDX")%>',this.value)" ></td>
			...
			<select name="sel_mat<%=i%>" id="sel_mat<%=i%>" onChange="input_mat('<%=LRs("PlayerResultIDX")%>',this.value)">
				...
				<%
					For k=1 To MatCnt
						...
						End If 
				%>
				<option value="<%=k%>" <%If CStr(k) = CStr(LRs("StadiumNumber")) Then%>selected<%End If%>><%=MatNum%>매트</option>
				<%
					Next					
				%>
			</select>
