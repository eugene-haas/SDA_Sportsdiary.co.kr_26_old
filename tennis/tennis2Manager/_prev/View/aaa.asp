...
function chk_save(){
	...
	if(confirm("해당 대진표를 저장하시겠습니까?")){
		var totcnt = "<%=TotRound%>"
		if(totcnt=="4"){
			sf.action = "aaa4_ok.asp"		
		}else if(totcnt=="8"){
			sf.action = "aaa8_ok.asp"		
		}else if(totcnt=="16"){
			sf.action = "aaa16_ok.asp"		
		}else if(totcnt=="32"){
			sf.action = "aaa32_ok.asp"		
		}else if(totcnt=="64"){
			sf.action = "aaa64_ok.asp"		
		}else if(totcnt=="128"){
			sf.action = "aaa128_ok.asp"		
		}
		sf.submit();
	}
}
...
<body>
    ...
    <div class="tourney-main hidden-main">
      ...
      <div class="tourney clearfix">
				<h3 class="stit"><span>대진표 입력하기</span><input type="button" value="추첨" onclick="random_start();"></h3>
        <!-- S: left-side -->
            ...
            <% 
              ...
              <div class="player-info" tabindex="0">
								<div class="player-input-wrap">
									...
									<span class="player-name-input"><input type="text" name="Player<%=k%>" id="Player<%=k%>" readonly onclick="input_data(this,'hidden_data<%=k%>')" /></span>
