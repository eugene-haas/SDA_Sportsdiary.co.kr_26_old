...
function update_frm(seq){		
	...
	if(confirm("동영상 URL을 저장하시겠습니까?")){
		var strAjaxUrl = "/Manager/Ajax/Championshipoperate_Update.asp";
...
//주심변경
function Change_CheifMain(playerresultidx,cheifidx){
	//alert(playerresultidx)
	//alert(cheifidx)
	var strAjaxUrl = "/Manager/Ajax/CheifMain_Update.asp";		
		$.ajax({
			url: strAjaxUrl,
			type: 'POST',
			dataType: 'html',			
			data: { 
				playerresultidx    : playerresultidx,
				cheifidx           : cheifidx 
			},		
			success: function(retDATA) {				
				if(retDATA){					
...
//부심1변경
function Change_CheifSubNM1(playerresultidx,cheifidx){
//	alert(playerresultidx)
//	alert(cheifidx)
	var strAjaxUrl = "/Manager/Ajax/CheifSub1_Update.asp";		
		$.ajax({
			url: strAjaxUrl,
			type: 'POST',
			dataType: 'html',			
...
//부심2변경
function Change_CheifSubNM2(playerresultidx,cheifidx){
	//alert(playerresultidx)
	//alert(cheifidx)
	var strAjaxUrl = "/Manager/Ajax/CheifSub2_Update.asp";		
		$.ajax({
			url: strAjaxUrl,
			type: 'POST',
			dataType: 'html',			
...
function view_frm(tp){
	...
	var strAjaxUrl="/Manager/ajax/ChampionshipOperate_View.asp";
	$.ajax({
			url: strAjaxUrl,
			type: 'POST',
			dataType: 'html',
...
function ExcelDown(str){
	...
	if(str == "A"){
		ExcelUrl = "ChampionshipOperate_A_Excel.asp?";
	}
	else{
		ExcelUrl = "ChampionshipOperate_B_Excel.asp?";
	}
	
	//ExcelUrl = "../ajax/ChampionshipOperate_View.asp?";
	...
	<section>
		<div id="content">
			...
			<div class="btn-right-list">
				<a href="javascript:assign_Cheif()" class="btn" id="btnCheif" accesskey="s">심판배정</a>
			</div>
			<%End If%>
			<div class="btn-right-list">
				<a href="javascript:view_frm('F');" class="btn" id="btnview" accesskey="s">검색(S)</a>
				<a href="javascript:ExcelDown('A')" class="btn" id="btnCheif" accesskey="s">Excel(개인별)</a>
				<a href="javascript:ExcelDown('B')" class="btn" id="btnCheif" accesskey="s">Excel(종별)</a>
			</div>
			</form>
			<!-- E : sch 검색조건 선택 및 입력 -->
			<!-- S : 리스트형 20개씩 노출 -->
