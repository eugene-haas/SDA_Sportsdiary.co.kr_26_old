...
<%	
	...
	LSQL = LSQL&" AND (ISNULL(PR.RResult,'') <> 'wr052002' AND ISNULL(PR.LResult,'') <> 'wr052002' AND ISNULL(PR.RResult,'') <> 'wr052013' AND ISNULL(PR.LResult,'') <> 'wr052013')"
	...
	If Request.Cookies("SportsGb") = "judo" Then 
		LSQL = LSQL&" order by StadiumNumber, PR.TurnNum, PR.Level, CONVERT(int, PR.GameNum) "
	Else
			...
			<select name="sel_mat<%=i%>" id="sel_mat<%=i%>" onChange="input_mat('<%=LRs("PlayerResultIDX")%>',this.value)">
				...
				<%
					...
					For k=1 To 6
						...
						End If 
				%>
				<option value="<%=k%>" <%If CStr(MatNum) = CStr(LRs("StadiumNumber")) Then%>selected<%End If%>><%=MatNum%>매트</option>
				<%
					Next					
				%>
			</select>
