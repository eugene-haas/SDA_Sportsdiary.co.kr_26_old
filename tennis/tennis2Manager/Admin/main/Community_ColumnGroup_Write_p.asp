...
<%
  ...
  MSeq = decode(iMSeq,0)													' ColumnistIDX 칼럼개설만
  ...
  Set iFile = Upload.Form("iFile")                ' 실첨부파일,배열
	...
	'response.Write "LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=rFileCnt="&rFileCnt&"<br>"
  'response.End
  ...
  If Err.Number = 0 Then
    ...
    LCnt = 0
		...
		N_Year = ""
		Temp = ""
		ColumnistIDX = MSeq ' ColumnistIDX 로 사용
		TeamGb = ""
		FileExt = ""
		cMSeq = ""

    LSQL = "EXEC Community_tblColumnistG_M '" & iType & "','" & ColumnistIDX & "','" & iDivision & "','" & Name & "','" & Subject & "','" & iContents & "','" & iLink & "','" & SubType & "','" & NoticeYN & "','" & N_Year & "','" & Temp & "','" & ID & "','" & cMSeq & "','" & TeamGb & "','" & FileExt & "','','','','',''"
    ...
    For i = 1 To CInt(rFileCnt) step 1
      ...
      If Upload.Form("iFile")(i) <> "" Then
        ...
        LSQL = "EXEC Community_tblColumnistG_Pds_M '" & iType & "','" & irMSeq & "','" & iFileName & "','" & ID & "','" & iDivision & "','','','',''"
    ...
    response.Write "<script type='text/javascript'>alert('글 등록이 잘 돼었습니다.');location.href='./Community_ColumnGroup_List.asp';</script>"
    response.End
  ...
  Else
    ...
    response.Write "<script type='text/javascript'>alert('글 등록에 오류가 있습니다.');location.href='./Community_ColumnGroup_List.asp';</script>"
    response.End
