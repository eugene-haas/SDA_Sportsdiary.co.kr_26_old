...
' response.Write "CIDX="&CIDX&"<br>"
  ...
  IF CIDX <> "" Then
	...
	CSQL = CSQL & "     ,A.ViewYN ViewYN " 
    CSQL = CSQL & " FROM [SD_tennis].[dbo].[tblSvcFaq] Q "
    CSQL = CSQL & "   left join [SD_tennis].[dbo].[tblSvcFaq] A on Q.FaqIDX = A.ReFaqIDX"
...
<script>
  function chk_Submit(valType){
    ...
    if(valType == "LIST"){  // 목록페이지
      ...
      var strAjaxUrl = "./Ajax/Board_ok_faq.asp";
      ...
      $.ajax({
        ...
        success: function(retDATA) {
          ...
          if(retDATA){
            ...
            if (strcut[0]=="TRUE") {      
              switch (strcut[1]) { 
                case "66" : alert('새 글을 등록하였습니다.'); break;
                case "99" : alert('글을 수정하였습니다.'); break;
                case "100" : alert('글을 삭제하였습니다.'); break;   
                default:
              }
              ...
              switch (strcut[1]) { 
                case "66" : alert('새 글을 등록하지 못하였습니다.'); break;
                case "99" : alert('글을 수정하지 못하였습니다.'); break;
                case "100" : alert('글을 삭제하지 못하였습니다.'); break;   
 ...
 <br />
        ...
        <table class="Community_wtite_box">
          ...
          <tbody>
			...
			<tr>
              <th>출력구분</th>
              <td><input type="radio" name="ViewYN" id="ViewYN" value="Y" <%IF ViewYN = "Y" Then response.write "checked" End IF%> />출력
				  <input type="radio" name="ViewYN" id="ViewYN" value="N" <%IF ViewYN = "N" Then response.write "checked" End IF%> />출력안함
              </td>
            </tr>  
          </tbody>
        </table>
