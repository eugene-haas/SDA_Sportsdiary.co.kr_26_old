...
<%
  ...
  Upload.Start global_filepath_temp
	...
	iTKnsid = fInject(Upload.Form("iTKnsid"))       ' 테니스코리아 뉴스ID
	diTKnsid = decode(iTKnsid,0)
  ...
  If Err.Number = 0 Then
    ...
    LSQL = "EXEC Community_Board_M '" & iType & "','" & ColumnistIDX & "','" & iDivision & "','" & Name & "','" & Subject & "','" & iContents & "','" & iLink & "','" & SubType & "','" & NoticeYN & "','" & N_Year & "','" & Temp & "','" & ID & "','" & MSeq & "','" & TeamGb & "','" & FileExt & "','" & Source & "','" & diTKnsid & "','','',''"
    ...
    response.Write "<script type='text/javascript'>alert('글 등록이 잘 돼었습니다.');location.href='./Community_News_List.asp';</script>"
    response.End
  ...
  Else
    ...
    response.Write "<script type='text/javascript'>alert('글 등록에 오류가 있습니다.');location.href='./Community_News_List.asp';</script>"
    response.End
