...
<%
  ...
  If Err.Number = 0 Then
    ...
    LSQL = "EXEC News_Board_Mod_STR '" & iType & "','" & iDivision & "','" & Name & "','" & Subject & "','" & iContents & "','" & iLink & "','" & SubType & "','" & NoticeYN & "','','','" & ID & "','" & MSeq & "'"
    ...
    For i = 1 To CInt(rFileCnt) step 1
      ...
      If Upload.Form("iFile")(i) <> "" Then
        ...
        LSQL = "EXEC News_Board_Pds_Mod_STR '" & iType & "','" & iMSeq & "','" & iFileName & "','" & ID & "'"
	      'response.Write "LSQL="&LSQL&"<br>"
        'response.End
    ...
    response.Write "<script type='text/javascript'>alert('글 등록이 잘 돼었습니다.');location.href='./News_Notice_List.asp';</script>"
    response.End
  ...
  Else
    ...
    response.Write "<script type='text/javascript'>alert('글 등록에 오류가 있습니다.');location.href='./News_Notice_List.asp';</script>"
    response.End
