...
<script type="text/javascript">
    function chk_frm(val) {
        ...
        if (val == "FND") {
            sf.action = "./GMS_Stadium_Sketch.asp";
            sf.submit();
        } 
    }
    ...
    function WriteLink(i2) {
    	...
    	post_to_url('./GMS_Stadium_Sketch_Write.asp', { 'i2': i2, 'iType': '1' });

    }
	function PagingLink(i2){
		post_to_url('./GMS_Stadium_Sketch.asp', { 'i2': i2, 'Search_GameTitleIDX': '<%=Search_GameTitleIDX%>', 'Search_TeamGbIDX': '<%=Search_TeamGbIDX%>' });
	}
  function chk_Submit(valType, valIDX, chkPage){
    ...
    if(valType == "VIEW"){
      $("#idx").val(valIDX);   
      $('form[name=search_frm]').attr('action',"./GMS_Stadium_sketch_view.asp");     
