...
<li>
	...
	<!-- as-is -->
	<!-- <a href="javascript:;" onclick="javascript:ReadListLink('<%=encode(LRs("MSeq"),0) %>','<%=NowPage %>');"> -->

	<!-- test -->
	<a onclick="javascript:fn_ReadView('<%=encode(LRs("MSeq"),0) %>');" class="_view">

	<!-- !@# -->
		<%
		if LRs("Link") <> "" and LRs("Division") = "2" and LRs("TKnsid") = "" then
			ImgSum = global_filepathImgUrl&"\"&LRs("Link")
		%>
		<div class="img">
			<div class="mv_box">
				<img src='<%=ImgSum %>' alt='' class='photo' />
			</div>
		</div>
		<%
		elseif LRs("Link") <> "" and LRs("Division") = "3" then
			ImgSum1 = golbal_youtubeImg&LRs("Link")&"/hqdefault.jpg"
		%>
		<div class="img">
			<div class="mv_box">
				<img src='<%=ImgSum1 %>' alt='' class="photo" />
				<img src='http://img.sportsdiary.co.kr/sdapp/main/icon_mv.png' alt='' class='mv_icon' />
			</div>
		</div>
		<%
		elseif LRs("Link") <> "" and LRs("Division") = "2" and LRs("TKnsid") <> "" then
			ImgSum2 = LRs("Link")
		%>
		<div class="img">
			<div class="mv_box">
				<img src='<%=ImgSum2 %>' alt='' class="photo" />
			</div>
		</div>
		<%
		else
      ...
      <p class="bt_con">
        ...
        <span class="companny_name"><%=LRs("Source") %></span>
      </p>
    </div>
