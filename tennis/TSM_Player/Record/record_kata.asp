...
<head>
  ...
  function view_keyword() {
   ...
   var ipagenum = 1  //현재페이지 
    ...
    function view_list(username,PlayerIDX) {
    ...
    function fn_list2(arg) {
      ...
      $.ajax({
        ...
        success: function (retDATA, Search_NewTeamGbName2) {
          ...
          if (error != "") {
            alert("Record_List222, 오류발생! - 시스템관리자에게 문의하십시오!");
            return;
          }
        }
      });
...
<div class="l">
    ...
    <div class="l_recordTab__wrap">
      <div class="l_recordTab__tab">
        <div class="l_recordList__wrap">
         ...
         <input type="hidden" id="PlayerIDX" /> 
          <!-- 리스트 Ajax 받아오는 부분 --> 
				  <ul id="gametitlelist">
    ...
    .m_searchPopup__cont{position:relative;height:calc(100vh - 2.5rem);overflow:hidden;}
      ...
      .m_searchPopup__panelWrap.s_filtering{transform:translateX(0%);} */
        ...
        .m_searchPopup__panelTop{padding-top:0.35rem;}
          ...
          .m_searchPopup__listname .icon__search_add{position: absolute;right: 2rem;top: -0.15rem;display: inline-block;width: 0.7rem;pointer-events: none;}
  </style>
  ...
  <script>
    ...
    // 부서 선택 시
    let div_change_click=$("input[name='search_div_kata']").next();
    div_change_click.off().on({
      click:function(){
        search_layer.close();
