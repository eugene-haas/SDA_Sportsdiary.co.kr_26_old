...
mx.ReceivePacket = function( reqcmd, data, sender ){// data는 response string
  ...
  if( jsondata !='' && jsondata != null){
	...
	case 10101 : alert("예선전 종료후 본선대진표 공개됩니다"); return; break;
    }
  }
  ...
  case mx.CMD_SETDPOINT:
  this.OnsetPoint( reqcmd, jsondata, htmldata, sender );    break;
...
//console.log(packet.drd_No);
			...
			switch (packet.drd_No) {
              ...
              case 16:
                // elWidth = '710';
                $('.tourney.tourn').css({'width':'710px'});
                break;
              case 32:
                // elWidth = '910';
                $('.tourney.tourn').css({'width':'910px'});
                break;
              case 64:
                // elWidth = '1010';
                $('.tourney.tourn').css({'width':'1010px'});
                break;
              case 128:
                // elWidth = '1110';
                $('.tourney.tourn').css({'width':'1110px'});
                $('.tourney.tourn').css({'height':'1410px'});
                break;
              default:
                // elWidth = '1210';
                $('.tourney.tourn').css({'width':'1210px'});
                $('.tourney.tourn').css({'height':'2410px'});
                break;
            }

            // $('.tourney.tourn').css({'transform': 'scale(' + (($('body').width()/elWidth).toFixed(2) - 0.01)+') translateX(-50%)'})
          }
        }
        else if ( cmd == mx.CMD_GAMESEARCH_Home || cmd == mx.CMD_GAMESEARCH_Result_Home) {
            if (packet) {
                ...
                switch (packet.drd_No) {
                    ...
                    case 8:
                    case 16:
                        $(".preli").css("height", "600px");
                        break;
                    case 128:
                        $("#scoregametable").css("height", "1900px");
                        $(".preli").css("height", "1900px");
                        $(".sub.sub-main.tourney.h-fix").css("height", "1900px");
                        break;
                    default:
                        if (packet.drd_No > 128) {
                            console.log(packet.drd_No);
                            console.log("128강 이상 설정일경우 처리 해야됨 추후");
                            $("#scoregametable").css("height", "3300px");
                            $(".preli").css("height", "3300px");
                            $(".sub.sub-main.tourney.h-fix").css("height", "3300px");
                        } else {
    ...
    mx.OnLoadCode = function(cmd, packet, html, sender){
      ...
      if (score.smenu == null || score.smenu == ""){
        ...
        if (myArr.length > 0){
            ...
            {
			  if(myArr[i].sitegubun == "S"){
				  if (myArr[i].selectteamgb == myArr[i].TeamGb.substr(0,5)){
            ...
            if (localStorage.getItem('live') == "ok"){
				score.gameSearch("1");
...
//########################################################
	...
	mx.closeModal = function(mid){
		var $viewport = $('head meta[name="viewport"]');
		$viewport.attr('content', 'width=device-width, minimum-scale=1, initial-scale=1, maximum-scale=3');
        $('#'+mid).modal('toggle');
	};
