  ...
  var SelPubCode = function(id,ppubcode,selpubcode)
  {
    ...
    var $d=$.Deferred();
    var obj = {};
	obj.CMD = mx.CMD_PCODE;
    obj.SportsGb = localStorage.getItem("SportsGb");
    obj.PPubCode = ppubcode;

	//url: '/ajax/tennis_os/management/DirPubCodeList.ashx',		
    //var jsonData = JSON.stringify(obj);
	var jsonData = "REQ=" + encodeURIComponent( JSON.stringify( obj  ) );

        $.ajax({
			url: '/pub/ajax/reqTennis.asp',
            type: 'post',
            data: jsonData,
  ...
  var SelTeamCode = function(id,selTeamGb)
  {
    ...
    var jsonData = JSON.stringify(obj);
        ...
        $.ajax({
            url: '/ajax/tennis_os/management/DirTeam.ashx',
            type: 'post',
            data: jsonData,
            success: function(data){fsuccess(data)},error:ferror
				});
...
/* 안쓸꺼 같음 일딴 막아두자 지우겠지
  var SelTeamCode_NowGame = function(id,selTeamGb)
  {
    ...
    var jsonData = JSON.stringify(obj);
        ...
        $.ajax({
            url: '/ajax/tennis_os/management/DirTeamNowGame.ashx',
            type: 'post',
            data: jsonData,
            success: function(data){fsuccess(data)},error:ferror
				});
...
//        console.log(data);
  ...
  //날짜리턴 2016-01-01 -> 2016년01월01일
    ...
    var getUrlParameter = function(name){
		...
		//url: '/Ajax/tennis_os/Management/LevelInfo_sum.ashx',
		//지도자버전에서 검색 체급목록 공통 SSG 20160802
		var drLevelList_sum = function(id,TeamGb,sellevelcode)
		{
			...
			function fsuccess(data){
				...
				if (myArr.length > 0)
				{
					if(localStorage.getItem("GroupGameGb") == "tn001001" || localStorage.getItem("GroupGameGb") == "tn001002"){
						if ( sellevelcode == 'lineinsert' ){ //추가할 내용
							$(id).append("<option value='0' selected>전체</option>"); //전체 검색 value 0
						}

						for (var i = 0; i < myArr.length; i++)
						{
							if (myArr[i].TeamGb == sellevelcode || sellevelcode == 'lineinsert'){
								if ( sellevelcode = 'lineinsert' ){ //추가할 내용
									$(id).append("<option value='" + myArr[i].TeamGb +"'>" +  myArr[i].TeamGbNm + "</option>");
								}
								else{
									$(id).append("<option value='" + myArr[i].TeamGb +"' selected>" +  myArr[i].TeamGbNm + "</option>");
								}
							}
							else{
								$(id).append("<option value='" + myArr[i].TeamGb +"'>" +  myArr[i].TeamGbNm +"</option>");
						...
						$(id).append("<option data-sex='"+myArr[0].Sex+"' data-level='' value='WoMan' >종목선택</option>");
					}
				}
				else{
					if(localStorage.getItem("GroupGameGb") == "" ){		
						$(id).append("<option data-sex='' data-level='' value='' selected>::종목선택::</option>");						
					}
					else{
		...
		var drLevelList_sum_NowGame = function(id,TeamGb,sellevelcode)
		{
			...
			$.ajax({
					url: '/Ajax/tennis_os/Management/LevelInfo_sum_NowGame.ashx',
					type: 'post',
					data: jsonData,
					success: function(data){fsuccess(data)},error:ferror
			});
