...
mx.SetPlayers = function(packet){
	...
	//세팅된 코트 선수 정보
	if( packet == null){
		c1 =	$('#courtidx_1').text(); //(1,3) 짝  //코트에 인덱스
	...
	for(var i = 0 ; i < score.cplayers.length;i++){
		if( score.cplayers[i].cidx == serveidx ){ //선수는 (위 1,2  아래 3, 4)
			...
			if (score.TIEBREAK == true){
				...
				if( mx.pos(i).split("_")[0] == "right"){ //서브자 오른쪽 위치  아래부터 시작(오른쪽 이니까 위부터??
...
//타이브레이크 중이라면 서브가 2개임 마다 변경되므로 그냥 매 점수 마다 호출 해주자.
//###########################
mx.recivePosition = function(){
	...
	if (serveidx == undefined){ //두번째 서브 지정단계라면
		serveidx = score.jsondata.SERVE;
	}

	var orgleftright = 'right';
	var orgtopbottom;			//원래 서브 위아래
	var orgpos;

	var svleftright = 'right';
	var svtopbottom;			//현재 서브 위아래
	
	//////////////////////////////////

	for(var i = 0 ; i < temp.length;i++){
		if( temp[i].cidx == serveidx ){				  //선수는 (위 1,2  아래 3, 4)
			svleftright = mx.pos(i).split("_")[0];	  //서브팀 현재좌우
			svtopbottom = mx.pos(i).split("_")[1]; //현재서브 위아래 정보
		}
	}

	for(var i = 0 ; i < rcarr.length;i++){
		if( rcarr[i].cidx == serveidx ){				
			orgleftright = rcarr[i].cteam;					 //서브 위치 최초 좌우
			orgtopbottom = mx.pos(i).split("_")[1];  //최초 위아래 정보
		}
	}

	//============================
	if (orgleftright == 'left'){ //원래 리시브 오른쪽
		p1 = rcarr[1]; //위선수
		p2 = rcarr[3]; //아래선수

		if( svleftright == 'left'){ //현재리시브 오른쪽
			temp = [temp[0], p1, temp[2], p2];
		}
		else{//서브오른쪽,  바뀜
			temp = [p2, temp[1], p1, temp[3]];
		}
	}
	else{ //원래 서브오른쪽 리시브 왼쪽
		p1 = rcarr[0]; //위선수
		p2 = rcarr[2]; //아래선수

		if( svleftright == 'left'){ //바뀜 (리시브가 오른쪽)
			temp = [temp[0], p1, temp[2],p2];
		}
		else{//현재 리시브 왼쪽
			temp = [p2, temp[1], p1, temp[3]];
...
//		
//		
//		//서브가 원래 위치가 top 이라면 리시브는 그대로,  bottom 이라면 리시브 변경
//		if( orgtopbottom ==  svtopbottom){ 
//			if (svleftright == 'left'){ //서브가 왼쪽이면 그대로
//				temp = [temp[0], p1, temp[2], p2];	
//			}
//			else{ //오른쪽이면 바꿔
//				temp = [p2, temp[1], p1, temp[3]];	
...
//		}
//		else{//리시브 위아래 변경
//			if (svleftright == 'left'){ //바꾸고
//				temp = [temp[0], p2, temp[2], p1];
//			}
//			else{ //안바꾸고
//				temp = [p1, temp[1], p2, temp[3]];	
//			}		
//		}
//
//
