	...
	mx.findServePlayer = function(gameno){
		...
		for(var i = 0 ; i < score.fplayers.length;i++){
				...
				if (score.TIEBREAK == true){ //타이브레이크라면
					...
					//if ($("#left_pt").text() != "0" || $("#right_pt").text() != "0" ){
//					if (score.jsondata.POINTNO > 0)	{
//						gameno = Number(score.jsondata.POINTNO);
//					}
	...
	if ( Number(packet.LSCORE) == -1)	{ //첫서브실패
		...
		if( score.TIEBREAK == true ){ //게임점수
			...
			if ( scoreposition == 'left'){ //눌린위치(스코어 얻은쪽)
				if( (Number(l_score) + 1) >=7 ){//먼저 7점 득점
					$("#left_pt").html( "0" );
					$("#right_pt").html( "0" );
				...
				if( (Number(r_score) + 1) >=7 ){//먼저 7점 득점
					$("#left_pt").html( "0" );
					$("#right_pt").html( "0" );
		...
		if( score.cplayers[i].cidx == serveidx ){ //선수는 (위 1,2  아래 3, 4)
			...
			if (score.TIEBREAK == true){
				...
				if( mx.pos(i).split("_")[0] == "left"){ //왼쪽 위치
					...
					if(  Number(pointno) > 0 && Number(pointno) % 2 == 1 ){ //홀수 점수
						if (mx.pos(i) == 'left_bottom'){ //위얌
							temp = [score.cplayers[2],score.cplayers[1],score.cplayers[0],score.cplayers[3]];	
						}
						else{
				...
				if( mx.pos(i).split("_")[0] == "right"){ //서브자 오른쪽 위치  아래부터 시작
					...
					if(  Number(pointno) > 0  &&  Number(pointno) % 2 == 1){ //홀 
...
mx.recivePosition = function(){
	...
	var temp = [];
	var firstrc = 'left', nowrc = 'left';			//지금리시브받는 팀 처음위칠, 현재위치

	
	// ** cteam?  이건 상단 위츠를 저장해놓음. 최초 설정 위치가 아님 ㅡㅡ+  색구분, 좌우 스코어 구분에 사용
	//그럼 처음 왼쪽 오른쪽 구분은 어떻게 해야할까 (fplayers 의 인덱스로  구분해야한다. 1,3 왼쪽)
	...
	for(var i = 0 ; i < fplayers.length;i++){
		if( fplayers[i].cidx == serveidx ){				
			if (mx.pos(i).split("_")[0] == 'left'){
				firstrc = 'right';
			}
		}
	}
	...
	if (firstrc == 'left'){									//처음서브 왼쪽 (처음 설정에서 위아래 바뀌어서 저장되어있다)
		if( nowrc == 'left'){
			//fplayers[0] index 랑 players[0] 인덱스랑 비교
			if( fplayers[0].cidx == players[0].cidx ){
				temp = [players[2],players[1],players[0],players[3]]; //원래 위에랑 값이랑 비교해서 같다면 바꿈 (처음하고 위아래 바꿈)
			}
			else{
				temp = [players[0],players[1],players[2],players[3]];
			}
		}
		else{ //현재리시브는 오르쪽
			//fplayers[0] index 랑 players[1] 인덱스랑 비교 다르면 다꿈 //최초 왼쪽이랑 지금 오른쪽이랑 비교
			if( fplayers[0].cidx == players[1].cidx ){
				temp = [players[0],players[1],players[2],players[3]];
			}
			else{
				temp = [players[0],players[3],players[2],players[1]]; //원래 위에랑 값이랑 비교해서 다르다면 바꿈	(처음하고 위아래 같음)
			}
		}
	}
	else{ //오른쪽
		if( nowrc == 'left'){ //현재리시브 왼쪽
			if( fplayers[1].cidx == players[0].cidx ){
				temp = [players[2],players[1],players[0],players[3]];//원래 위에 값이랑 비교해서 같다면 바꿈 (처음하고 위아래 바꿈)
			}
			else{
				temp = [players[0],players[1],players[2],players[3]];
			}
			...
			if( fplayers[1].cidx == players[1].cidx ){
				temp = [players[0],players[1],players[2],players[3]];//원래 위에랑 값이랑 비교해서 다르다면 바꿈	(처음하고 위아래 같음)
			}
			else{
...
//리시브 위치를 찾아서 조정값 리턴
//mx.receiveSearch = function(rcidx,player){
//	for(var i = 0 ; i < player.length;i++){
//		if ( Number(player[i].cidx) == Number(rcidx) ){ //리시브 위랑비교
//			if (score.TIEBREAK == true){
//				if( mx.pos(i) == 'left_top' ){ //위에 있다면 
//					return [player[2],player[1],player[0],player[3]];
//				}
//				if( mx.pos(i) == 'right_top' ){ //위있다면 
//					return [player[0],player[3],player[2],player[1]];
//				}
//			}
//			else{
//				if( mx.pos(i) == 'left_bottom' ){ //아래있다면 
//					return [player[2],player[1],player[0],player[3]];
//				}
//				if( mx.pos(i) == 'right_bottom' ){ //아래있다면 
//					return [player[0],player[3],player[2],player[1]];
//				}
//			}
//		}
...
//	var orgsvrc = 'right';
//	var temp = score.cplayers;
//	var rcarr = score.fplayers;
//
//	var serveinfo = mx.findServePlayer(); //서브선수 배열
//	var serveidx = serveinfo[0];
//
//	for(var i = 0 ; i < temp.length;i++){
//		if( temp[i].cidx == serveidx ){				//선수는 (위 1,2  아래 3, 4)
//			//svrc = temp[i].cteam;					//서브팀 원래위치
//			orgsvrc = mx.pos(i).split("_")[0];		//서브팀 현재위치
//		}
//	}
//
//	if (orgsvrc == 'left'){ //서브팀원래위치가 (원래위치가 오른쪽에 있는애들로 비교) 
//		temp = mx.receiveSearch(temp[1].cidx,temp); //right_top index
//	}
//	else{ //서브원래 위치가 오른쪽 
//		temp = mx.receiveSearch(temp[0].cidx,temp); //left_top index
//	}
//
//	//html 변경
//	mx.reDrowCourt(temp);
...
mx.changeCourt = function(drowtype, gameno, pointno){ //pointno 포인트 득점 카운트 (gameend  = 1 > pointno = 0
	...
	case 2: var temp = [score.cplayers[1],score.cplayers[0],score.cplayers[3],score.cplayers[2]];	break;
	case 3:
	break;
	}
	...
	//if( score.TIEBREAK == false  ||  (score.TIEBREAK == true &&  Number(score.jsondata.POINTNO) == 0 )  ){
		//( 타이브레이크시 포인트점수로 구하자 현재순서를)
		if (score.TIEBREAK == true ){
			//mx.servePosition(score.jsondata.POINTNO);
			//if( Number(score.jsondata.POINTNO) > 0  && Number(score.jsondata.POINTNO) % 2 == 1 ){//2번째마다 위치 변경
				mx.playingServePosition(score.jsondata.POINTNO); //위치변경 recod psortno
			//}
		}
		else{
	...
	if (Number(gameno) == 2  && Number(score.jsondata.POINTNO) == 0 )	{
	}
	else{
		mx.recivePosition(); //리시브위치조정
	}
