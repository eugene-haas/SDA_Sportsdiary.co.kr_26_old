...
<script src="/pub/js/tennis_Request.js?ver=18" type="text/javascript"></script>
<script type="text/javascript">
    ...
    function chek_form_data(val) {
        ...
        /*참가자 정보 2  파트너*/
        if (!frm_in.p2name.value) {
            //alert("파트너 이름이 없습니다."); $("#p2name").focus(); return; 
        }
        else {
    ...
    function savedata(val) {
		...
		obja.p1boono = $("#p1_boono").val();
		obja.p2boono = $("#p2_boono").val();
        ...
        if ($("#attphone").val() == "") {
            ...
            obja.attphone = $("#attphone").val(); // + frm_in.attphone2.value + frm_in.attphone3.value;
        }
...
<div class="l_apply">
		...
		%>
			<%if Cdbl(MemberCnt)<=0 then %>
					...
					<div class="list_section_1">
						<ul> 
							<li>
								<div class="text_box">
									<p class="red_font"><i class="fa fa-exclamation-circle" aria-hidden="true"></i><span>  참가자 이름을 검색하고 해당 클럽의 이름을 선택해주세요.(참가자 핸드폰번호 뒷자리 확인) </span></p>
									<p class=""><i class="fa fa-exclamation-circle" aria-hidden="true"></i><span>  만약 조회된 이름이 없으면 '신규등록'을 클릭하여 이름입력을 완성해주세요.  </span></p>
									<p class="red_font"><i class="fa fa-exclamation-circle" aria-hidden="true"></i><span>  조회된 이름은 있으나 새로운 클럽으로 출전을 희망하는 선수는  '선수정보 변경요청' 버튼을 클릭 후 수정사항을 반드시 적어주세요.</span></p> 
									<p class=""><i class="fa fa-exclamation-circle" aria-hidden="true"></i><span>입력하신 참가자(파트너 포함) 휴대폰 번호를 통해 참가선수 본인 확인 절차가 진행됩니다. </span></p>
								</div>
							</li>
						</ul>
					</div>
			...
			<%else%>
				...
				<div class="comp-guide ">
					...
					<dl>
						<dt>참가비 금액</dt>
						<dd><%=acctotal%>원</dd> <%If payok="ok" then%><span style="color:red;">입금완료</span><%End if%>
					</dl>
				</div>
			<%End if%>
	<%else%>
	...
	<%End If%>
            ...
            <%
            SQL = " select top 1 GameTitleName,convert(date,GameS)GameS,convert(date,GameE)GameE,titleGrade from sd_TennisTitle  where DelYN='N' and GameTitleIDX= '"&GameTitleIDX&"' "
            Set Rs = Dbcon.Execute(SQL)
            If Not Rs.Eof Then 
    ...
    <!-- s: 참가자 정보 & 파트너 정보 -->
    <div class="deposit_info">
        <span class="big_title">[<%=gamegrade%>] <%=GameTitleName %></span>
    </div>
    <div class="participant_info">
        <dl>
            <dd>
                        ...
                        <li>
                            ...
                            <span class="r_con">
                                    ...
                                    SQL = SQL &"  group by  b.GameType, b.TeamGb,b.TeamGbNm,c.Level,c.LevelNm ,GameDay,GameTime,EntryCntGame,b.cfg  "
									 %> 
                                <select class="ipt select_1" id="levelno" name="levelno" onChange="mx_player.chkBoo()">
                                    ...
                                    <%
                                        ...
                                        Do Until Rs.Eof  
                                            if ChekMode = 0 and ridx ="" then 
                                                if Rs("Ch_i") ="Y" Or Request.ServerVariables("REMOTE_ADDR") = "118.33.86.240" then
													i=i+1
                                                %>
        ...
        <dl id="ContentsList">
            ...
            <% if ridx <>"" and  ChekMode = 1 then  
                   p1subq = " (select openrnkboo from tblplayer where playerIDx = a.P1_PlayerIDX ) as p1boo "
                   p2subq = " (select openrnkboo from tblplayer where playerIDx = a.P2_PlayerIDX ) as p2boo "
					SQL=""
                    SQL = SQL&"  select "&p1subq&","&p2subq&",RequestIDX,SportsGb,GameTitleIDX,Level,EnterType,UserPass,UserName,isnull(UserPhone,'')UserPhone,txtMemo,PaymentDt,PaymentNm "
                    ...
                    <ul> 
                        <li>
                            ...
                            <span class="r_con">
                                ...
                                <%
                                'ChekMode가 1이고, 참가자정보가 있는 경우 수정모드로 보고 첫번째 선수 수정을 막음
                                '위변조 가능성이 있지만 프론트엔드에서만 막음.
                                '20220922 개나리부/신인부만 선수1 수정 불가능하도록 수정
                                if  ChekMode = 1 and P1_PlayerIDX <> "" then
                                %>
                                <input type="text" class="ipt input_1" id="p1name" name="p1name" readonly="readonly" disabled value="<%=P1_UserName %>" />
                                <% else %>
                                <input type="text" class="ipt input_1" id="p1name" name="p1name" autocomplete="off" placeholder=":: 이름을 입력하세요 ::" onkeyup="fnkeyup(this);"  value="<%=P1_UserName %>" />
                                <ul class="ui-autocomplete ui-front ui-menu ui-widget ui-widget-content ui-corner-all" id="ui-user" ></ul>
                                <% end if  %>
                            </span> 
                        </li>

						<li id="p1openboornk" <%If  Left(levelno,5) <> "20105"  then%>style="display:none;"<%End if%>>
                            <span class="l_name">랭킹적용</span>
                            <span class="r_con">
                                <select class="ipt select_1" id="p1_boono" name="p1_boono">
                                <option value="20105" <%If p1boo = "오픈부" then%>selected<%End if%>>오픈부</option>
                                <option value="20103" <%If p1boo = "베테랑부" then%>selected<%End if%>>베테랑부</option>
                                <option value="20104" <%If p1boo = "신인부" then%>selected<%End if%>>신인부</option>
								</select>
							</span>
							</span>
						</li>
                        ...
                        <li>
                            ...
                            <span class="r_con">
                                ...
                                <input type="text" class="ipt input_1" id="p1team1txt" name="p1team1txt"  placeholder=":: 기본소속명을 입력(1개 클럽) ::" 
                                 ...
                                 onkeyup="fnkeyup(this);" autocomplete="off" value="<%=P1_TeamNm %>" disabled/>
                        ...
                        <li>
                            ...
                            <span class="r_con">
                                ...
                                <input type="text" class="ipt input_1" id="p1team2txt" name="p1team2txt"  placeholder=":: 기타소속명을 입력(1개 클럽) ::" onkeyup="fnkeyup(this);"  autocomplete="off" value="<%=P1_TeamNm2 %>" disabled/> 
                            </span>
                        </li>
                        <li>
                    ...
                    <ul>
                        <li>
                            ...
                            <span class="r_con">
                                ...
                                <input type="text" class="ipt input_1 ipt-200" id="p2name" name="p2name" autocomplete="off" placeholder=":: 이름을 입력하세요 ::"  onkeyup="fnkeyup(this);"    value="<%=P2_UserName %>"/>
                                <input type ="checkbox" id="ptn_undefined" class="ml-10" name="ptn_undefined" value="1" onclick="mx_player.ptnUndefined()"><label for="ptn_undefined" class="no-using">미정</label>
                            </span>
                        </li> 

						<li id="p2openboornk" <%If  Left(levelno,5) <> "20105"  then%>style="display:none;"<%End if%>>
                            <span class="l_name">랭킹적용</span>
                            <span class="r_con">
                                <select class="ipt select_1" id="p2_boono" name="p2_boono">
                                <option value="20105" <%If p2boo = "오픈부" then%>selected<%End if%>>오픈부</option>
                                <option value="20103" <%If p2boo = "베테랑부" then%>selected<%End if%>>베테랑부</option>
                                <option value="20104" <%If p2boo = "신인부" then%>selected<%End if%>>신인부</option>
								</select>
							</span>
							</span>
						</li>
						...
						<li>
                            ...
                            <span class="r_con">
                                ...
                                <input type="text" class="ipt input_1" id="p2team1txt" name="p2team1txt"  placeholder=":: 기본소속명을 입력(1개 클럽) ::" onkeyup="fnkeyup(this);"  autocomplete="off"  value="<%=P2_TeamNm %>" disabled/>
                            </span>
                        </li>
                        <li>
                            <span class="l_name">기타클럽</span>
                            <span class="r_con">
                                ...
                                <input type="text" class="ipt input_1" id="p2team2txt" name="p2team2txt"  placeholder=":: 기타소속명을 입력(1개 클럽) ::" onkeyup="fnkeyup(this);"  autocomplete="off"  value="<%=P2_TeamNm2 %>" disabled/> 
                            </span>
                        </li> 
                        <li>
                            <span class="l_name">휴대폰번호</span>
    ...
    <div class="btn_list" style="text-align:right;text-align:right;">
		...
		<a href="http://m.ikata.org/bbs/board.php?bo_table=apply_modi" class="red_btn" target="_blank">선수정보 수정 요청</a>
    ...
    <div class="s_btn_list">
        ...
        <a  <% if Ch_d ="Y" then  %> href="javascript:chk_frm('1');" <% else  %> href="javascript:alert('참가신청 취소는 마감 되었습니다.');"<% end if  %> class="yello_btn" >신청삭제</a>
        <a <% if Ch_u ="Y" then  %> href="javascript:chk_frm('2');" <% else  %> href="javascript:alert('참가신청 수정이 마감 되었습니다.');" <% end if  %>class="green_btn">수정하기</a>
    </div>
    <%end if  %>
    <!-- s: 버튼리스트 -->
    <!-- s: 대회 참가신청 절차 -->
                    ...
                    <ul>
                        ...
                        <li>    
                            ...
                            <span class="r_con">
                                ...
                                <span class="divn">-</span>
                                <input type="password" class="ipt col_1 phone_line" id="attphone2" name="attphone2" max="9999" maxlength="4" oninput="maxLengthCheck(this);" value="<%=UserPhone2 %>"onfocus="this.select()" autocomplete="off">
                                <span class="divn">-</span>
                                <input type="number" class="ipt col_1 phone_line" id="attphone3" name="attphone3" max="9999" maxlength="4" oninput="maxLengthCheck(this);" value="<%=UserPhone3 %>"onfocus="this.select()" autocomplete="off">
                            </span>
                        </li>
...
<%End if%>
                    ...
                    <div class="etx_con"> 
                            <ul>
                                ...
                                <span class="r_con">
                                    <input type="text" id="attpwd" name="attpwd" maxlength="20" class="ipt input_1" value="<%=UserPass %>" autocomplete="off"  onkeyup="this.value=this.value.replace(/[^0-9]/g,'');fnkeyup(this);" >
