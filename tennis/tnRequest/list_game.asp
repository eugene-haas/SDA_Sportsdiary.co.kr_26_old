...
<script src="/pub/js/tennis_Request.js?ver=3" type="text/javascript"></script>
...
<div class="l_apply_list">
	...
	<div class="search_box">
		<ul>
			<li class="li_search_box"> 
				...
				<% YEARSQL = " select GameYear    From sd_TennisTitle   where  isnull(GameYear,'')<>'' and ViewYN='Y' group by GameYear  ORDER BY GameYear DESC "
	...
	<div class="competition_list">
		<dl>
            ...
            YEARSQL = " select convert(date ,a.GameS)GameS " & _ 
            ...
            if Months="" then 
            YEARSQL=YEARSQL& "  and a.GameYear='"&Years &"' "
            else
            YEARSQL=YEARSQL& "  and (left(convert(date ,a.GameS),7)<='"&Years&"'+'-'+'"&Months&"' and left(convert(date ,a.GameE),7)>='"&Years&"'+'-'+'"&Months&"' ) " 
            end if 
            YEARSQL=YEARSQL& " order by GameS"

            if  cdbl(month(date))>=10 then 
                YEARSQL=YEARSQL& " desc"
            end if 
