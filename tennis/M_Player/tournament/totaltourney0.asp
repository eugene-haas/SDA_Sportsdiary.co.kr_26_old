...
<html lang="ko">
	...
	<style type="text/css">
		...
		.highlighted { background-color: yellow; padding:0px; }
		.matched { font-weight:700; padding:0px; }
	</style>
	  ...
	  // 화면 검색 로직 : S
		...
		DocFinder.prototype = {
			...
			_mapping : function($area, word, classname){
					...
					for( node=node.firstChild; node; node=node.nextSibling ){
						if( node.nodeType==3 && node.textContent.trim() ){
					        ...
					        node.parentNode.insertBefore(temp.firstChild, node);
								...
								// node.nodeValue = text.replace(regExp, '<span class="'+ classname +'">' + word + '</span>');
								// node.parentElement.innerHTML = node.parentElement.innerHTML.replace(regExp, '<span class="'+ classname +'">' + word + '</span>');
							};
						}
						else{
							mapping(node)
				...
				// 슬래쉬 막는 법 필요
				// var regExp = new RegExp(RegExp.escape(word).replace(/&/g, '&amp;'), "ig");
				// var html = $area.html();
				// var mappedHtml = html.replace(regExp, '<span class="'+ classname +'">' + word + '</span>');
				// $area.html(mappedHtml);
		...
		$(function(){
			var finder = new DocFinder($('#playernm'), $('#scoregametable'), 'matched', 'highlighted');
			$('#playernm').on('keydown', function(evt){
				if(evt.keyCode == 13){
      ...
      <ul class="clearfix">
		...
		<div class="top-search-box">
		  ...
		  <input type="text" id="playernm"  style="width:50%;" placeholder="검색어를 입력해 주세요." onkeydown="if(event.keyCode == 13){mx.docfindstr('playernm','scoregametable div div table tr td span','#scoregametable' , mx.findtype);}">
			<!-- <input type="text" id="playernm"  style="width:50%;" placeholder="검색어를 입력해 주세요."> -->
		</div>
