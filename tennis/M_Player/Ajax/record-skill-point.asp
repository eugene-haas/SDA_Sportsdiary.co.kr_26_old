...
%>
<h2>
		<span><%=iYearSDate %>년</span> / <span><%=isub_iteamgbNm %></span> / <span><%=isub_ilevelnameNm %></span> / <span><%=isub_iskilltypeNm %></span>
</h2>
<!-- S: match-list -->
 ...
 %>
        <tr>
              ...
              <% end if %>
				  ...
				  <td class="belong">
					  <a href="javascript:;" onclick="javascript:mySkillChart('<%=encode(LRs("PlayerIDX"),0) %>','<%=LRs("UserName") %>','<%=LRs("Teamnm") %>','<%=LRs("PhotoPath") %>')" data-toggle="modal" data-target=".modal-record-rank">
							<p><%=LRs("UserName") %></p>
							<span><%=LRs("TeamNm") %></span>
					  </a>
				  </td>
				  <td><%=LRs("ResultCnt") %></td>
				  <td><%=LRs("TotalResultCnt") %></td>
