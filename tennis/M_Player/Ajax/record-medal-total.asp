...
<%
 ...
 %>        
        <tr>
			    <td><%=LRs("iRank") %></td>
			    <td class="belong">
				    <a href="javascript:;" onclick="javascript:iMainTotal('<%=iSportsGb %>','<%=SDate %>','<%=EDate %>','','<%=encode(LRs("PlayerIDX"),0) %>','1^2^3^6','<%=iteamgb %>');" data-toggle="modal" data-target=".modal-player-profile">
							<p><%=LRs("UserName") %></p>
							<span><%=LRs("TeamNm") %></span>
					  </a>
			    </td>
			    <td><%=LRs("RankingPoint") %></td>
		    </tr>
<%
      LRs.MoveNext
    Loop
