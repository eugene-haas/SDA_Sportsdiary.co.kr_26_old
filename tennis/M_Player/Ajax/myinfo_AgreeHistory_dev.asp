...
<%
	...
	Else
		...
		CSQL =  " 		SELECT CASE WHEN A.PlayerReln IS NULL OR A.PlayerReln = '' THEN '' ELSE A.PlayerReln END PlayerReln "
		...
		CSQL = CSQL & " 	,A.Team " 
		CSQL = CSQL & " 	,CASE A.LeaderType WHEN 2 THEN '감독' WHEN 3 THEN '코치' ELSE '' END LeaderTypeNm"
		CSQL = CSQL & " FROM (  "
		...
		SELECT CASE PlayerReln
			CASE "K", "R", ""	: CSQL = CSQL & "AND (PlayerReln = 'A' OR PlayerReln = 'B' OR PlayerReln = 'Z') "	'선수의 경우 부모정보 조회
			CASE "A", "B", "Z"	: CSQL = CSQL & "AND (PlayerReln = '' OR PlayerReln IS NULL OR PlayerReln = 'K') "	'부모의 경우 선수/비등록선수 조회
		END SELECT		
