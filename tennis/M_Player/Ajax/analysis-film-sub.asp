...
<%
	...
	iLoginPlayerIDX = fInject(Request("iLoginPlayerIDX"))

	iPlayerIDX = decode(iPlayerIDX,0)
	iLoginPlayerIDX = decode(iLoginPlayerIDX,0)
 ...
 '2017-06-26 추가
  ...
  LSQL = "EXEC Analysis_Film_Search '" & iType & "','" & iSportsGb & "','" & iPlayerIDX & "','" & SDate & "','" & EDate & "','" & GameTitleIDX & "','" & iLoginPlayerIDX & "'"
  ...
  Dim LRsCnt3, iPlayerResultIdx3, iGameTitleIDX3, iLeftUser3, iRightUser3, iLeftTeamNm3, iRightTeamNm3, iResult3, iResultWin3, iTopRoundNm3, iFavYN3, iLevelNm3, iMediaLink3
  LRsCnt3 = 0
  ...
  Dim LRsCnt4, iPlayerResultIdx4, iGameTitleIDX4, iLeftUser4, iRightUser4, iLeftTeamNm4, iRightTeamNm4, iResult4, iResultWin4, iTopRoundNm4, iFavYN4, iMediaLink4
  LRsCnt4 = 0
...
<script type="text/javascript">
  ...
  if (TCnt3 > 0) {
    ...
    var iFavYN3arr = iFavYN3.split("^");
    var iLevelNm3arr = iLevelNm3.split("^");
    var iMediaLink3arr = iMediaLink3.split("^");
    ...
    for (var i = 1; i < TCnt3 + 1; i++) {
      ...
      iHtmlPC = iHtmlPC + '  <dt class="sw-chara">';

      if (iMediaLink3arr[i] == "") {

        if (iFavYN3arr[i] == "Y") {
          iHtmlPC = iHtmlPC + '    <span onclick="javascript:alert(&#39;해당경기는 동영상이 없습니다.&#39;);" class="icon-on-favorite df">★</span>';
        }
        else {
          iHtmlPC = iHtmlPC + '    <span onclick="javascript:alert(&#39;해당경기는 동영상이 없습니다.&#39;);" class="icon-off-favorite df">★</span>';
        }
        ...
        if (iFavYN3arr[i] == "Y") {
          iHtmlPC = iHtmlPC + '    <span onclick="javascript:iFavLink(&#39;' + iPlayerResultIdx3arr[i] + '&#39;);" class="icon-on-favorite">★</span>';
        }
        else {
          iHtmlPC = iHtmlPC + '    <span onclick="javascript:iFavLink(&#39;' + iPlayerResultIdx3arr[i] + '&#39;);" class="icon-off-favorite">★</span>';
        }
  ...
  if (TCnt4 > 0) {
    ...
    var iFavYN4 = "<%=iFavYN4%>";
    var iMediaLink4 = "<%=iMediaLink4%>";

    //alert("TCnt4:" + TCnt4 + "\r\n" + "iLeftTeamNm4:" + iLeftTeamNm4 + "\r\n" + "iRightTeamNm4:" + iRightTeamNm4 + "\r\n");
    ...
    var iFavYN4arr = iFavYN4.split("^");
    var iMediaLink4arr = iMediaLink4.split("^");
    ...
    for (var i = 1; i < TCnt4 + 1; i++) {
      ...
      iHtmlTC = iHtmlTC + '  <dt class="sw-chara">';

      if (iMediaLink4arr[i] == "") {

        if (iFavYN4arr[i] == "Y") {
          iHtmlTC = iHtmlTC + '    <span onclick="javascript:alert(&#39;해당경기는 동영상이 없습니다.&#39;);" class="icon-on-favorite df">★</span>';
        }
        else {
          iHtmlTC = iHtmlTC + '    <span onclick="javascript:alert(&#39;해당경기는 동영상이 없습니다.&#39;);" class="icon-off-favorite df">★</span>';
        }
        ...
        if (iFavYN4arr[i] == "Y") {
          iHtmlTC = iHtmlTC + '    <span onclick="javascript:iFavLink(&#39;' + iPlayerResultIdx4arr[i] + '&#39;);" class="icon-on-favorite">★</span>';
        }
        else {
          iHtmlTC = iHtmlTC + '    <span onclick="javascript:iFavLink(&#39;' + iPlayerResultIdx4arr[i] + '&#39;);" class="icon-off-favorite">★</span>';
        }
