...
<%
  ...
  IF Not (LRs.Eof Or LRs.Bof) Then
    Do Until LRs.Eof
      LCnt = LCnt + 1
      %>
      <li class="m_list__item">
        <a href="javascript:;" class="m_list__link" onclick="javascript:MainMediaViewLink('<%=encode(LRs("MSeq"),0) %>','<%=NowPage %>');">
					<span>· <%=LRs("Subject")%></span>
					<% if LRs("NewYN") = "Y" then %>
					<span class="m_list__badgeNewBg"></span><span class="m_list__badgeNew">N</span>
					<% end if %>
        </a>
      </li>
      <%
