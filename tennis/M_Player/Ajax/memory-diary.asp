...
<%
  ...
  Dim LRsCnt1, iAdtMyDiay, iAdtMyDiayCklYn, iWriteDate, iWriteDatea, iTrRerdIDX, iTrRerdDate, enciTrRerdIDX, enciType
  LRsCnt1 = 0

  iType = "21"
  iSportsGb = "judo"
  enciType = encode(21,0)
  ...
  If Not (LRs.Eof Or LRs.Bof) Then
        ...
        iAdtMyDiay = ReplaceTagReText(LRs("AdtMyDiay"))
        iAdtMyDiayCklYn = LRs("AdtMyDiayCklYn")
        ...
        if iAdtMyDiayCklYn <> "" then
          if iAdtMyDiayCklYn = "Y" then
            iAdtMyDiayCklYn = "on"
          else
            iAdtMyDiayCklYn = "off"
          end if
...
<div class="panel panel-default">
  <div class="panel-heading" role="tab">
   ...
   <p class="panel-ic-q sw-chara"><span onclick="javascript:iFavLink('<%=enciType %>','<%=enciTrRerdIDX %>','');" class="img-icon icon-<%=iAdtMyDiayCklYn %>-favorite"><!-- <img src="../images/memory/favorite-star-<%=iAdtMyDiayCklYn %>@3x.png" alt="" /> -->★</span></p>
    <a data-toggle="collapse" data-parent="#sub_table" href=".collapse<%=LRsCnt1 %>" aria-expanded="true" aria-controls="collapse<%=LRsCnt1 %>">
  	  <p class="panel-txt-q">
        [<%=iWriteDate %>] <%=iAdtMyDiay %>
  	  </p>
  ...
  <div class="collapse<%=LRsCnt1 %> panel-collapse collapse" role="tabpanel">
   <div class="panel-body">
    ...
    <div class="memory-txt container">
     <p class="user-cont">
       <%=replace(replace(iAdtMyDiay, chr(10), "<br>"),Chr(32), "&nbsp;")%>
    </p>
    <!--<p class="reply">
  	  고맙다^^ <br>
