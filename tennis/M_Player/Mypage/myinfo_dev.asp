...
<script type="text/javascript">
  //회원정보 수정항목 체크
    ...
    if(!$('#Job').val()){
  	...
  	<%
  	'등록선수(R)/비등록선수(K)
  	ElseIF PlayerReln = "K" OR PlayerReln = "R" Then
  	%>
    //체급체크
    if(!$('#PlayerLevel').val()){
    ...
    */
    ...
    var strAjaxUrl    = "../Ajax/join_mod_dev_OK.asp";
			...
			success: function(retDATA) {
				if(retDATA){
					if (retDATA=="TRUE") {
						...
						$('form[name=s_frm]').attr('action',"./myinfo_dev.asp");
						$('form[name=s_frm]').submit();


						//location.href="../Mypage/myinfo_dev.asp"
					}
					else{
  ...
  function inputExc($this){
	...
	function CHK_AgreeHistory(){
		...
		var strAjaxUrl = "../Ajax/myinfo_AgreeHistory_dev.asp";
		var PlayerIDX = $('#PlayerIDX').val();
		var PlayerReln = $('#PlayerReln').val();
		var Team = $('#Team').val();
  ...
  function chk_athletenum(){
    ...
    var strAjaxUrl = "../Ajax/Check_AtleteNum_dev.asp";
  ...
  function CHK_UP_PLAYERNUMBER(){
    ...
    if(AthleteYN=='N') {
      ...
      if(confirm("등록선수로 정보업데이트를 진행하시겠습니까?")){
		  var strAjaxUrl = "../Ajax/myinfo_dev_AtleteNumUpdate.asp";
		  var MemberIDX  = $('#MemberIDX').val();
		  var PlayerIDX  = $('#PlayerIDX').val();
		  var Team = $('#Team').val();
		  var SportsType = $('#SportsType').val();
...
<div class="sub mypage">
  <form name="s_frm" method="post">
    ...
    %>
        ...
        <%
    	IF PlayerReln = "K" OR PlayerReln = "R" Then   '등록선수(R)/비등록선수(K) 가입회원
      	%>
        <li class="hang-deco">
          <p>운동시작년도</p>
          <p class="over-txt">
  ...
  <div class="parent-data-wrap panel-title clearfix"> <a href=".fold" class="tit-parent-data" data-toggle="collapse" aria-expanded="false" aria-controls=".fold" tabindex="1">
    ...
    <div class="collapse fold" aria-expanded="false">
      <div class="parent-data well">
        ...
        <span>(스포츠 다이어리팀 담당 : 070-7493-8111)</span> </div>
    </div>
  </div>
  <%
  	CASE ELSE
