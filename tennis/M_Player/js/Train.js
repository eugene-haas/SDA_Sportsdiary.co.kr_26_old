$(document).ready(function () {
        ...
        if (p_TEAMTRAI == "Y") {
            ...
            if (p_Injury == "") {
                for (var i = 0; i < InjuryArea.length; i++) {
                    var injury_Code = InjuryArea[i].value;
                    var tbody_Id = "tbody_" + injury_Code;

                    if (InjuryArea[i].checked) {
                        $("#" + tbody_Id).removeClass("on").attr("display", "none").hide();
                        InjuryArea[i].checked = false;
                    }
                }
            } else {
                for (var i = 0; i < InjuryArea.length; i++) {
                    ...
                    for (var t = 0; t < s_Injury.length; t++) {
                        if (s_Injury[t] != "") {
                            if (injury_Code == s_Injury[t]) {
                                checkSeq++;
                            }
                        }
                    }
                    ...
                    if (checkSeq > 0) {
                        $("#" + tbody_Id).addClass("on").attr("display", "table-cell").show();
                        InjuryArea[i].checked = true;
                    } else {
                        $("#" + tbody_Id).removeClass("on").attr("display", "none").hide();
                        InjuryArea[i].checked = false;
                    }
    ...
    $(".btn.btn-save").on("click", function (e) {
        //history.replaceState({ page: 1, name: '부상팝업' }, '', location.href); 
        history.back();
        var p_Injury = $("#p_Injury").val();
        var InjuryArea = document.getElementsByName("InjuryArea");
        var countInjury = 0;
        ...
        // console.log(tblSvcTrRerdJury);
        $("#p_Injury").val(tblSvcTrRerdJury)
        $("#injury-chk").modal("hide");
    });
    $('#injury-chk').on('hidden.bs.modal', function (e) {
    ...
    /*부상부위 취소*/
    $("#injury-close").on("click", function (e) {
        history.back();
    });
...
function train_idx() {
    ...
    var strAjaxUrl = "../select/train_idx.asp";
    var TrRerdDate = $('#datetimepicker1').val();
...
function chage_search(id_value) {
    ...
    var strAjaxUrl = "../select/train_Search.asp";
    var TrRerdDate = $('#datetimepicker1').val();
    var id = id_value;
