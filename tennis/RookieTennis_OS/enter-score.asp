...
<!--
  ...
  score.enterScore = function(inputvalue, no){
      ...
      if (Number(score.jsondata.CMODE) == 1)  { //저장완료
		  ...
		  //서브이 모두 오프인지 체크 19년 9월 2일 수정 by baek
		  var chkserve = false;
		  for(var i= 1; i < 5;i++){
				if ($("#serve_" + i).attr('src') == "images/tournerment/public/tennis_ball_on@3x.png")
				{
					chkserve = true;
				}
		  }
		  ...
		  if ( Number(no) == 0  && ( (Number(score.jsondata.GAMENO) == 1 &&  chkserve == false)  || (Number(score.jsondata.GAMENO) == 2 &&  chkserve == false)   ) ){
			score.splashmsg('서브를 넣을  선수를 지정해 주십시오.',1500);
			return;
		  }
		  else{
  ...
  score.init = function(){
    ...
    if (score.jsondata.PRERESULT == 'ING' && Number(score.jsondata.GAMENO)+ (Number(score.jsondata.STARTSC)*2) == Number(score.jsondata.TIESC)*2 + 1) { //타이브레이크 상태 6: 6 > 13경기시작 
      ...
      //기록관 창 호출
      if (score.SERVE == 0) { //stateno 체크 0 (sd_tennisResult (1번 서브가 지정안되면 무조건 호출 코트지정과같이 사용하므로 변경 )
  ...
  score.gameEnd = function(){
	...
	//위에 내용은 모르겠고 ㅠㅠ 1세트 점수 안에다 넣기나 하자 ㅡㅡ
	$('#rt_left_score option:eq('+$('#l_set1').text()+')').attr("selected","selected");
	$('#rt_right_score option:eq('+$('#r_set1').text()+')').attr("selected","selected");
