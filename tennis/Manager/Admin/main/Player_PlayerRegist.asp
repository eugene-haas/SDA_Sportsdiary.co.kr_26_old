...
<%
    ...
    ReqWeightVal =  fInject(Request("i6"))
    iTP_Type = "T00002" ' 선수등록현황
...
<script type="text/javascript">
  ...
  function PagingLink(i1) {
     post_to_url('./Player_PlayerRegist.asp', { 'i1':i1 , 'i2':"<%=ReqLocalVal%>",'i3': "<%=ReqDivisionVal%>",'i4': "<%=ReqOptionVal%>",'i5': "<%=ReqTeamNameVal%>",'i6':"<%=ReqWeightVal%>"});
  }
  ...
  function fn_selSearch()
  {
    ...
    var selectOptionWeight = $("#selectOptionWeight option:selected").val();
    var txtSearchVal =  $("#txtSearch").val();

    post_to_url('./Player_PlayerRegist.asp', { 'i1': 1 , 'i2': selectLocalVal,'i3': selectDivisionVal,'i4': selectOptionSex,'i5': txtSearchVal,'i6':selectOptionWeight});
  }

  function ViewPagingLink(i1, i2){
		post_to_url('./Player_PlayerRegist_write.asp', { 'i1':i1 , 'i2':"<%=ReqLocalVal%>",'i3': "<%=ReqDivisionVal%>",'i4': "<%=ReqOptionVal%>",'i5': "<%=ReqTeamNameVal%>",'i6': i2, 'i7': 2});
	}

  function WriteLink(i1) {
     post_to_url('./Player_PlayerRegist_write.asp', { 'i1':i1 , 'i2':"<%=ReqLocalVal%>",'i3': "<%=ReqDivisionVal%>",'i4': "<%=ReqOptionVal%>",'i5': "<%=ReqTeamNameVal%>", 'i7': 1});
  }
      ...
      <div class="search_box">
          ...
          <select id="selectOptionWeight" class="title_select">
            <option value="">체급</option>
				...
				<input type="text" id="txtSearch" name="txtSearch" placeholder="선수 및 팀명을 입력해주세요."  class="title_input in_2"/>
