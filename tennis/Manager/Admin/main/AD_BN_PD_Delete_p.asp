...
<%
  ...
  iFType	= "1"
  LSQL = "EXEC AD_tblADProductLocate_R '" & iFType & "','" & NowPage & "','" & MSeq & "','" & iLoginID & "'"
  ...
  Else
    ...
    LSQL = "EXEC AD_tblADProductLocate_D '" & iType & "','" & MSeq & "','" & SportsGb & "','" & iLoginID & "'"
	  'response.Write "LSQL="&LSQL&"<br>"
    'response.End
    Set LRs = DBCon6.Execute(LSQL)
    If Not (LRs.Eof Or LRs.Bof) Then
    ...
    LRs.close
		...
		if Result = "NO" then
			...
			response.Write "<script type='text/javascript'>alert('광고 연결 관리 삭제에 실패 했습니다.\n관리자에게 연락해 주세요.');location.href='./AD_BN_PD_List.asp';</script>"
			response.End
		
		else

			response.Write "<script type='text/javascript'>alert('광고 연결 관리 삭제가 잘 됐습니다.');location.href='./AD_BN_PD_List.asp';</script>"
			response.End
