...
<script type="text/javascript">
  ...
  function Del_Link(i1, i2) {
    if (confirm("해당 글을 삭제 하시겠습니까?") == true) {
    	post_to_url('./Community_Column_Delete_p.asp', { 'i1': i1, 'i2': i2 });
    }
    else {
    }
  }
  ...
  function OK_Link() {
    ...
    if (theForm.selSearch5.value == "") {
    	alert('<%=global_Column_Val %>');
    	return theForm.selSearch5.focus();
    }
...
<section>
  <div id="content">
    ...
    <div class="contents">
      ...
      <!-- E: 네비게이션 -->
      <form id="form1" name="form1" action="./Community_Column_Write_p.asp" method="post" ENCTYPE="multipart/form-data">
        <table cellspacing="0" cellpadding="0" class="Community_wtite_box">
                ...
                <!-- ColumnistIDX -->
								...
								<select id="selSearch5" name="selSearch5" class="title_select">
								  ...
								  <%
								    ...
								    LSQL = "EXEC Community_tblColumnist_S '" & iType2 & "','" & iLoginID & "','','','','',''"
										'response.Write "LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL="&LSQL&"<br>"
								    'response.End
								    ...
								    If Not (LRs.Eof Or LRs.Bof) Then
								      Do Until LRs.Eof
								          LCnt2 = LCnt2 + 1
													iColumnistIDX2 =  LRs("ColumnistIDX")
													ieColumnistIDX2 = encode(iColumnistIDX2,0)
													iSubject2 =  LRs("Subject")
									%>
									<option value="<%=ieColumnistIDX2 %>"><%=iSubject2 %></option>
									<%
									      LRs.MoveNext
									    Loop
									%>
          ...
          <!--<input type="hidden" id="selSearch2" name="selSearch2" value="N" />-->
            ...
            <th>공지 유/무</th>
              ...
              <span class="existence">※공지는 최상위 <%=global_MainCLCnt %>개만 상단에 노출됩니다.</span>
						</td>
					</tr>

					<!-- 승인유무 -->
					<% if ColumnApprovalYN = "Y" then %>
          <!--<input type="hidden" id="selSearch3" name="selSearch3" value="N" />-->
					<tr>
            <th>승인 유/무</th>
						<td colspan="2">
							<span class="right_con">
...
<section>
  ...
  <script type="text/javascript">
    ...
    var iColumnYN = "<%=ColumnYN%>";
    var ieSearchCol2 = "<%=eColumnistIDX%>";

    if (iType == 2) {
      $("#selSearch1").val(iSubType);
      $("#selSearch2").val(iNoticeYN);
      $("#selSearch3").val(iColumnYN);
      $("#selSearch5").val(ieSearchCol2);
    }
