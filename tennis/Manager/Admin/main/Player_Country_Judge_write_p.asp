...
<%
  ...
  ELSE
   ...
   End If
   IF iType = 1 THEN
     response.Write "<script type='text/javascript'>alert('등록이 완료되었습니다.');location.href='./Player_Country_Judge.asp';</script>" 
   ...
   END IF
   IF iType = 2 THEN
     response.Write "<script type='text/javascript'>alert('변경이 완료되었습니다.');location.href='./Player_Country_Judge.asp';</script>" 
     response.End
   END IF
 End IF
 JudoKorea_DBClose()
