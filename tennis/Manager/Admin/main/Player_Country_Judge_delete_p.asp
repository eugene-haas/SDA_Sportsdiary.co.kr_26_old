...
<%
  ...
  If LoginIDYN = "N" Then
    JudoKorea_DBClose()
    response.Write "<script type='text/javascript'>alert('잘못된 접근 입니다.');location.href='./Player_Country_Judge.asp';</script>"
  ...
  ELSE
    ...
    LRs.close
    JudoKorea_DBClose()
    response.Write "<script type='text/javascript'>location.href='./Player_Country_Judge.asp';</script>"
    response.End
    'response.Write "<script type='text/javascript'>alert('삭제가 잘 됐습니다.');location.href='./Player_Inter_Judge.asp';</script>"
    'response.Write "<script type='text/javascript'>alert('글 등록이 잘 됐습니다.');</script>"
  End IF
%>
