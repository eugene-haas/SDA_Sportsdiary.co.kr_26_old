...
<%
  ...
  iDivision = "3"                   ' 1 : 협회소개 2 : 유도소개(유도용어) 3 : 유도소개(기술용어)
...
%>
  ...
  <script type="text/javascript">
    ...
    function WriteLink(i2) {
    	...
    	post_to_url('./Info_SkillWord_Write.asp', { 'i2': i2, 'iType': '1' });
    ...
    function ReadLink(i1, i2) {
      ...
      //alert(i1 + " , " + i2);
    	post_to_url('./Info_SkillWord_Write.asp', { 'i1': i1, 'i2': i2, 'iType': '2' });
    }

    function PagingLink(i2) {

      //location.href = 'cmList.asp?i2=' + i2;
    	post_to_url('./Info_SkillWord_List.asp', { 'i2': i2, 'iSearchCol': selSearchValue, 'iSearchText': txtSearchValue });
    }
    ...
    function fn_selSearch() {
      ...
      post_to_url('./Info_SkillWord_List.asp', { 'i2': 1, 'iSearchCol': selSearchValue, 'iSearchText': txtSearchValue });
    }
    ...
    <table class="table-list">
      ...
      <tbody id="gametitlelist">
        ...
        <tr style="cursor:pointer"  onclick="javascript:ReadLink('<%=encode(LRs("MSeq"),0) %>','<%=NowPage %>');">
          <td><%=LRs("Num") %></td>
          <td><%=LRs("Subject") %><% if LRs("NationalTermName") <> "" then %>(<%=LRs("NationalTermName") %>)<% end if %></td>
          <td><%=LRs("InsDateCv") %></td>
        </tr>
        <%
			        LRs.MoveNext
