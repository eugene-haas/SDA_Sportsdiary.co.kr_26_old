...
<script type="text/javascript">
  ...
  var iMSeq = Number("<%=iMSeq%>");
	...
	$( document ).ready(function() {
      ...
      $("#iFaxNumber ").val(autoHypenPhone("<%=iReceipt_Fax%>"))
			$("#selPublishingType").val("<%=iPublishingType%>")	
      $("#selectStatusOption").val("<%=iStatus%>");
      $("#selSubmittingOrg").val("<%=iSubmittingOrg%>")

      var receiveType="<%=iReceiveType%>";
			var levelType = "<%=ilevelType%>";
if(levelType =="소단증") {
				$("#littleLevel").prop("checked",true);
				levelChange(document.getElementById("littleLevel"))	
			}
			else if(levelType=="대단증") {
				$("#bigLevel").prop("checked",true);
				levelChange(document.getElementById("bigLevel"))	
			}
			else if(levelType =="소단증/대단증"){
				$("#mixLevel").prop("checked",true);
				levelChange(document.getElementById("mixLevel"))	
			}
			else
			{
				$("#littleLevel").prop("checked",true);
			...
			if(receiveType == "방문") {
				...
				$("#selectVisitDate").val("<%=iReceipt_VisitDate%>");
				radioBtn = document.getElementById("visit")
				feeChange(radioBtn);
			}
			else if(receiveType =="우편") {
				$("#post").prop("checked",true);
					radioBtn = document.getElementById("post")
				feeChange(radioBtn);
			}
			else if(receiveType == "팩스") {
					radioBtn = document.getElementById("fax")
  ...
  function Del_Link(i1, i2) 
  {
    if (confirm("해당 글을 삭제 하시겠습니까?") == true) {
    	post_to_url('./License_delete_p.asp', { 'i1': i1, 'i2': i2 });
    }
    else {
    }
  }
  ...
  function CancelLink(i2) {
  	post_to_url('./License_List.asp', { 'i2': i2 });
  }
...
function OK_Link() {
  if(iType == 1){
      ...
      else if($("#fax").prop("checked"))
      {
        input2 = $("<input>")
                  .attr("type", "hidden")
                  .attr("name", "iReceiveType").val("fax");
      }

      var input3="";

      if($("#littleLevel").prop("checked"))
      {
        input3 = $("<input>")
                  .attr("type", "hidden")
                  .attr("name", "iLevelType").val("littleLevel");

      }
      else if($("#bigLevel").prop("checked"))
      {
        input3 = $("<input>")
                  .attr("type", "hidden")
                  .attr("name", "iLevelType").val("bigLevel");
      }
      else if($("#mixLevel").prop("checked"))
      {
        input3 = $("<input>")
                  .attr("type", "hidden")
                  .attr("name", "iLevelType").val("mixLevel");
      }
      ...
      var elClickedObj = $("#form1").append($(input1)).append($(input2)).append($(input3)).append($(input4))
...
function feeChange(Optionfee)
{
  	var OptionType = Optionfee.id;
		...
		var IsMix = $("#mixLevel").prop("checked");
		
		if(IsMix){
			if(OptionType == "post")
			{
				//우편 수령 시 방문이랑 , 팩스는 안보여야함
				price = 4000 * 2;
				$("#span_Price").text(numberWithCommas(price) + "원");
...
function levelChange(Optionlevel)
{
		var OptionType = Optionlevel.id;
		
		if(OptionType == "littleLevel"){
			document.getElementById("spanFax").style.display = "none";
			document.getElementById("trFax").style.display = "none";
			radioBtn = document.getElementById("post")
		}
		else if(OptionType == "bigLevel"){
			document.getElementById("spanFax").style.display = "";
			document.getElementById("trFax").style.display = "";
			radioBtn = document.getElementById("fax")
		}
		else if(OptionType == "mixLevel"){
			document.getElementById("spanFax").style.display = "none";
			document.getElementById("trFax").style.display = "none";
			radioBtn = document.getElementById("post")
		}
		...
		$.ajax({
			url: '../Ajax/Online/uploadImg.asp',
				...
				success: function(result){
						...
						if($("#hidFileCnt").val() > 0) {
							$("#imgPreview").attr("src","../FileDown/" + $("#hidFileName").val())
						}
						else{
							$("#imgPreview").attr("src","../FileDown/front/dist/imgs/online/no_images.png").val()
						}
				}
		});
	}
...
<section>
	<div id="content">
      ...
      <!-- E: 네비게이션 -->
      <form id="form1" name="form1" action="./License_write_p.asp" method="post" >
          ...
          <tr>
            <th>연락처</th>
                  ...
                  IF iType = 1 Then 
                %>
                <input type="text" class="in_4" id="iUserPhone" name="iUserPhone" maxlength="13" onKeyUp="this.value = autoHypenPhone(this.value); if($('#UserPhone').val().length==13){$('#postcode').focus();}" />
                <%
                  ELSE
                %>
          ...
          <tr>
            <th>생년월일</th>
						<td colspan="2">
							<input type="text" name="selectBirthDay" id="selectBirthDay"  class="in_1 date_ipt" value="<%=iBirthDay%>"/>
						</td>
					</tr>
          ...
          <tr>
            <th>재발급 단증</th>
						<td colspan="2">
							<span class="right_con">
						    	<input type="radio" name="abstinence" id="littleLevel" class="radio_in" onchange="levelChange(this)" checked/>
									<label for="littleLevel">소단증</label>
									<input type="radio" name="abstinence" id="bigLevel" class="radio_in" onchange="levelChange(this)"/>
									<label for="bigLevel">대단증</label>
									<input type="radio" name="abstinence" id="mixLevel" class="radio_in" onchange="levelChange(this)"/>
									<label for="mixLevel">소단증/대단증</label>
							</span>
						</td>
					</tr>
          ...
          <tr>
            <th>제출처</th>
						<td colspan="2">
							<span class="right_con">
						      <select  name="selSubmittingOrg" id="selSubmittingOrg" class="in_2 cert_use">
            ...
            <th>수령방법</th>
						<td colspan="2">
							<span class="right_con">
                <span id="spanFax" style="display:none">
                  <input type="radio" name="receive" id="fax" name="fax" onchange="feeChange(this)" class="radio_in" />
                  <label for="fax">팩스수령</label>
                </span>
          ...
          <tr name="trFax" id="trFax">
            <th>팩스번호</th>
						<td colspan="2">
							<span class="right_con">
                <input type="text" class="in_4"  name="iFaxNumber" id="iFaxNumber" value="<%=iReceipt_Fax%>" maxlength="13" onKeyUp="this.value = autoHypenPhone(this.value); if($('#iFaxNumber').val().length==13){$('#postcode').focus();}" /> 
							</span>
						</td>
					</tr>
          ...
          <tr  id="trVisit" name="trVisit" style="display:none">
            <th>방문일자</th>
						<td colspan="2">
							<span class="right_con">
									<input type="text" id="selectVisitDate" name="selectVisitDate" class="in_1 date_ipt">
									<!--<span class="date_icon" ></span>-->
									<label for="selectVisitDate" class="date_icon"></label>
							</span>
						</td>
					</tr>
        ...
        <tr>
            <th>증명사진</th>
						<td colspan="2">
								<span class="r_con">
								<form id="FILE_FORM" method="post" enctype="multipart/form-data" action="../Ajax/Online/uploadImg.asp">
										<input type="file" class="file" id="FILE_TAG" name="FILE_TAG">
										<input type="button" value="미리보기" href="javascript:;" onClick="uploadFile()">
								</form>
								<p class="red_font">※사진 사이즈 110x150 (픽셀)pixel / 이미지 파일(jpg/gif)만 등록가능</p>
						</td>
        </tr>
        <tr>
            <th>미리보기</th>
						<td colspan="2">
            <%
                IF ( iType = 2 AND iLink <> "") THEN
            %>
						<img name="imgPreview" id="imgPreview"
											 src="../FileDown/<%=iLink%>" alt=""
											 width="82" 
											 height="94" />
						</td>
            <%
              ELSEIF  ( iType = 2 AND iLink = "") THEN
            %>
            <img name="imgPreview" id="imgPreview"
											 src="../front/dist/imgs/online/no_images.png" alt=""
											 width="82" 
											 height="94" />
        ...
        <div id="divResult">
          ...
          <input type="hidden" name="iLink" id="iLink" value="<%=iLink%>"/>
          <%
          End If
          %>
