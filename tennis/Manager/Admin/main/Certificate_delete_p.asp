...
<%
  ...
  If LoginIDYN = "N" Then
    JudoKorea_DBClose()
    response.Write "<script type='text/javascript'>alert('잘못된 접근 입니다.');location.href='./Certificate_list.asp';</script>"
  ...
  ELSE
    ...
    LCnt1 = 0
    iType = 1 ' 1:Delete
    LSQL = "EXEC Admin_OnlineService_Board_Delete_STR '" & iType & "','" & MSeq & "'"
    'response.Write "LSQL="&LSQL&"<br>"
    'response.End
    Set LRs = DBCon4.Execute(LSQL)
    If Not (LRs.Eof Or LRs.Bof) Then
    ...
    LRs.close
    JudoKorea_DBClose()
    response.Write "<script type='text/javascript'>alert('정상적으로 삭제되었습니다.');location.href='./Certificate_list.asp';</script>"
    response.End
