...
<%
  ...
  SubType = fInject(Upload.Form("selSearch1"))  
	...
	iLink = fInject(Upload.Form("iLink"))  ' 동영상링크주소 
  ...
  If Err.Number = 0 Then
    ...
    LSQL = "EXEC Community_Board_M '" & iType & "','" & ColumnistIDX & "','" & iDivision & "','" & Name & "','" & Subject & "','" & iContents & "','" & iLink & "','" & SubType & "','" & NoticeYN & "','" & N_Year & "','" & Temp & "','" & ID & "','" & MSeq & "','" & TeamGb & "','" & FileExt & "','" & Source & "','','','',''"
    ...
    '    LCnt1 = 0
		'
    '    LSQL = "EXEC Community_Board_Pds_M '" & iType & "','" & irMSeq & "','" & iFileName & "','" & ID & "','" & iDivision & "','','','',''"
    ...
    response.Write "<script type='text/javascript'>alert('글 등록이 잘 돼었습니다.');location.href='./Community_MotionNews_List.asp';</script>"
    response.End
  ...
  Else
    ...
    response.Write "<script type='text/javascript'>alert('글 등록에 오류가 있습니다.');location.href='./Community_MotionNews_List.asp';</script>"
    response.End
