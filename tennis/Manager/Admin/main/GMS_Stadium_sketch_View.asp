...
<script>
	function chk_Submit(valType){
		...
		if(valType == "LIST"){	//공지사항 목록페이지
			$('form[name=s_frm]').attr('action',"./GMS_Stadium_Sketch.asp");		
			$('form[name=s_frm]').submit(); 
		}
		else if(valType == "MOD"){	//공지사항 수정
			$('form[name=s_frm]').attr('action',"./GMS_Stadium_Sketch_Write.asp");		
			$('form[name=s_frm]').submit(); 
		}
		else{	//공지사항 수정/삭제		
  ...
  function Sketch_Result(sk_gubun, idx)
  {
		if (confirm("정말 삭제하시겠습니까??") == true){    //확인
			var strAjaxUrl="GMS_Stadium_Sketch_Result.asp?sk_gubun="+sk_gubun+"&idx="+idx;
						...
						if(retDATA)
							{
								...
								{	
									alert('삭제완료');
									document.s_frm.action="GMS_Stadium_Sketch.asp"
