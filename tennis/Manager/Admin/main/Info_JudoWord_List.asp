...
<%
  ...
  iDivision = "2"                   ' 1 : 협회소개 2 : 유도소개(유도용어) 3 : 유도소개(기술용어)
  ...
  LSQL = "EXEC Infomation_Board_Search_STR '" & NowPage & "','" & PagePerData & "','" & BlockPage & "','" & iType & "','" & iDivision & "','" & iTermMidKey & "','" & iSearchCol & "','" & iSearchText & "','" & iLoginID & "'"
...
%>
  ...
  <script type="text/javascript">
    ...
    function WriteLink(i2) {
    	...
    	post_to_url('./Info_JudoWord_Write.asp', { 'i2': i2, 'iType': '1' });
    ...
    function ReadLink(i1, i2) {
      ...
      //alert(i1 + " , " + i2);
    	post_to_url('./Info_JudoWord_Write.asp', { 'i1': i1, 'i2': i2, 'iType': '2' });
    }

    function PagingLink(i2) {

      //location.href = 'cmList.asp?i2=' + i2;
    	post_to_url('./Info_JudoWord_List.asp', { 'i2': i2, 'iSearchCol': selSearchValue, 'iSearchText': txtSearchValue });
    }
    ...
    function fn_selSearch() {
      ...
      post_to_url('./Info_JudoWord_List.asp', { 'i2': 1, 'iSearchCol': selSearchValue, 'iSearchText': txtSearchValue });
    }
   ...
   <section>
    ...
    <div class="search_top community">
			<div class="search_box">
				<select id="selSearch" name="selSearch" class="title_select">
					<option value="N">국제용어</option>
					<option value="P">발음</option>
					<option value="S">국내용어</option>
				</select>
    ...
    <table class="table-list">
      ...
      <thead>
        <tr>
          <th scope="col">순번</th>         
          <th scope="col">국제용어</th>
					<th scope="col">발음</th>
					<th scope="col">국내용어</th>
          <th scope="col">작성일</th>
        </tr>
      </thead>
      <tbody id="gametitlelist">
        ...
        <tr style="cursor:pointer" onclick="javascript:ReadLink('<%=encode(LRs("MSeq"),0) %>','<%=NowPage %>');">
          <td><%=LRs("Num") %></td>
					<td><%=LRs("NationalTermName") %></td>
					<td><%=LRs("Pronunciation") %></td>
          <td><%=LRs("Subject") %></td>
          <td><%=LRs("InsDateCv") %></td>
        </tr>
        <%
