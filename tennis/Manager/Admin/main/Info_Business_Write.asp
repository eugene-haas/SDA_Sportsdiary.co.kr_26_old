...
<%
  ...
  If iType = "2" Then
    ...
    If FileCnt <> "0" Then
      ...
      LSQL = "EXEC Infomation_Board_Pds_Read_STR '" & MSeq & "'"
	    'response.Write "LSQL="&LSQL&"<br>"
      'response.End
...
<script type="text/javascript">
  ...
  function Del_Link(i1, i2) {
    if (confirm("해당 글을 삭제 하시겠습니까?") == true) {
    	post_to_url('./Info_Business_Delete_p.asp', { 'i1': i1, 'i2': i2 });
    }
    else {
    }
  }
  ...
  function CancelLink(i2) {
  	post_to_url('./Info_Business_List.asp', { 'i2': i2 });
  }
  ...
  function OK_Link() {
    ...
    if (theForm.iSubject.value == "") {
      ...
      //else if (iContents == "" || iContents == null || iContents == '&nbsp;' || iContents == '<p>&nbsp;</p>') {
    else if (iContents == "") {
...
<section>
	<div id="content">
      ...
      <!-- E: 네비게이션 -->
      <form id="form1" name="form1" action="./Info_Business_Write_p.asp" method="post" ENCTYPE="multipart/form-data">
				<table cellspacing="0" cellpadding="0" class="Community_wtite_box">
					<tr>
            <th>성명</th>
						<td>
							<span class="left_name">
								<span class="right_con"><%=Name %></span>
							</span>
						</td>
					</tr>
