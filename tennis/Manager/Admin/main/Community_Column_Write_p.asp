...
<%
  ...
  Subject = fInject(Upload.Form("iSubject"))      ' 제목
	...
	Source = fInject(Upload.Form("iSource"))				' 출처
  ...
  SubType = fInject(Upload.Form("selSearch1"))  
	...
	ColumnYN = fInject(Upload.Form("selSearch3"))

	If Len(ColumnYN) = 0 Then
    ColumnYN = "N"
  End If

	eColumnistIDX = fInject(Upload.Form("selSearch5"))
	ColumnistIDX = decode(eColumnistIDX,0)
  ...
  If Err.Number = 0 Then
    ...
    LSQL = "EXEC Community_Board_M '" & iType & "','" & ColumnistIDX & "','" & iDivision & "','" & Name & "','" & Subject & "','" & iContents & "','" & iLink & "','" & SubType & "','" & NoticeYN & "','" & N_Year & "','" & Temp & "','" & ID & "','" & MSeq & "','" & TeamGb & "','" & FileExt & "','" & Source & "','','','','" & ColumnYN & "'"
    ...
    response.Write "<script type='text/javascript'>alert('글 등록이 잘 돼었습니다.');location.href='./Community_Column_List.asp';</script>"
    response.End
  ...
  Else
    ...
    response.Write "<script type='text/javascript'>alert('글 등록에 오류가 있습니다.');location.href='./Community_Column_List.asp';</script>"
    response.End
