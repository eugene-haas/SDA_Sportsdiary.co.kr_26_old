...
<%
 'Controller ################################################################################################
  ...
  '계좌 사용량
  SQL = "Select Count(*) from TB_RVAS_MAST where CUST_CD IS NOT NULL"
			...
			<!-- s: 테이블 리스트 -->
				<div class="table_list contest">
					<table cellspacing="0" cellpadding="0">
						<tr>
						<%If CDbl(ADGRADE) > 500 then%>
							<th>번호</th><th>대회명</th><th>기간</th><th>노출</th><th>계좌정리</th><th>상세</th><th>요강</th><th>랭킹반영</th>
						<%Else%>
...
<%If CDbl(ADGRADE) <= 500 then%>
	<tr class="gametitle"  style="cursor:pointer" id="titlelist_25" onmousedown="mx.golevel(25,'<%=title%>')"  style="background:#BAD6FA;">
		...
		<td   style="color:#A43A1D;"><%=Date()%>~<%=Date()%></td>
	</tr>
<%End if%>
<%End if%>
<!-- 테스트용  -->
