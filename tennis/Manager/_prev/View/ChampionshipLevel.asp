...
function input_frm(){
	...
	//체급선택
	if(f.GroupGameGb.value == "<%=SportsCode%>040001" && f.Level.value==""){
		alert("체급을 선택해 주세요.");
		f.Level.focus();
		return false;
	}
	//단체전일경우 참가자명수 체크
	if(f.GroupGameGb.value == "<%=SportsCode%>040002"){
		if(f.EntryCnt.value == "0" || f.EntryCnt.value == ""){
			alert("단체전 참가인원을 입력해 주세요.");
			return false;
		}
	}
...
function update_frm(){
	...
	if(confirm("체급등록변경을 진행하시겠습니까?")){
		var strAjaxUrl = "/Manager/Ajax/ChampionshipLevel_Update.asp";
		var GameTitle     = f.GameTitle.value;
		var SportsGb      = f.SportsGb.value;
		var GroupGameGb   = f.GroupGameGb.value;
		var TeamGb        = f.TeamGb.value;
...
//게임타입 변경
function Change_GameType(obj){
	//대회IDX,경기형태코드

	
	if(confirm("대전방식을 변경하시겠습니까?")){
		var res = obj.split(",")
		var RGameLevelIDX = res[0];
		var GameType = res[1];
		var strAjaxUrl="/Manager/ajax/ChampionshipLevel_GameType_Update.asp";
		...
		$.ajax({
			...
			success: function(retDATA) {
				...
				//parent.fBottom.popupClose("btnsave","btnsave","");
				if(error!=""){
					alert ("대전방식 변경중 에러발생! - 시스템관리자에게 문의하십시오!");     
					return;
				}
...
//정보입력 S
function input_data(gametitleidx,level,seq){	
	//alert(gametitleidx);
	//alert(level);
	//alert(seq);
	var strAjaxUrl="/Manager/Ajax/ChampionshipLevel_DataView.asp";
	$.ajax({
			url: strAjaxUrl,
			type: 'POST',
			dataType: 'html',
			...
			success: function(retDATA) {
				...
				if(retDATA){						
					...
					var f = document.frm;
					f.GameTitle.value         = strcut[0];
					f.SportsGb.value          = strcut[1];
					f.GroupGameGb.value       = strcut[2];
					f.TeamGb.value            = strcut[3];
					f.VersusGb.value          = strcut[5];
					f.GameYear.value          = strcut[6];
					f.GameMonth.value         = strcut[7];
					f.GameDay.value           = strcut[8];
					f.GameTime.value          = strcut[9];
					make_box_level("sel_Level","Level",strcut[10],"",strcut[3])					
					f.EntryCnt.value          = strcut[11];
...
function del_frm () {		
	...
	if(confirm("대회체급정보를 삭제하시겠습니까?")){

				
		//parent.fBottom.popupOpen("btndel","btndel","삭제중입니다!");
		var strAjaxUrl = "/Manager/Ajax/ChampionshipLevel_Del.asp";
		var retDATA="";
...
//종목셀렉트박스
make_box("sel_SportsGb","SportsGb","<%=Request.Cookies("SportsGb")%>","SportsGb")
//개인전단체전셀렉트박스
...
function chk_level(){
	if(document.getElementById("TeamGb").value!=""){
		make_box_level("sel_Level","Level","","Level_Check",document.getElementById("TeamGb").value)
	}
}
//대전방식셀렉트박스
...
function view_player(obj){
	location.href="ChampionshipRegist.asp?RGameLevelIDX="+obj;
}



//대진표보기
function match_view(gametitleidx,teamgb,level,groupgamegb,totround,nowcnt,gametype){
	var f = document.match_frm;
	f.GameTitleIDX.value = gametitleidx;
	f.TeamGb.value = teamgb;
	f.Level.value = level;
	f.GroupGameGb.value = groupgamegb;
	f.TotRound.value = totround;
	f.NowCnt.value = nowcnt;
	f.GameType.value = gametype;
	if (gametype=="<%=SportsCode%>043001"){
	  //리그
		f.action = "/Manager/Ajax/MatchLeague_View.asp"
	}else if(gametype=="<%=SportsCode%>043002"){
  	//토너먼트
		if(totround=="2"){
			f.action = "/Manager/Ajax/Match2_View.asp"		
		}else if(totround=="4"){
			f.action = "/Manager/Ajax/Match4_View.asp"		
		}else if(totround=="8"){
			f.action = "/Manager/Ajax/Match8_View.asp"				
		}else if(totround=="16"){
			f.action = "/Manager/Ajax/Match16_View.asp"				
		}else if(totround=="32"){
			f.action = "/Manager/Ajax/Match32_View.asp"				
		}else if(totround=="64"){
			f.action = "/Manager/Ajax/Match64_View.asp"				
		}else if(totround=="128"){
			f.action = "/Manager/Ajax/Match128_View.asp"				
		}
	}
	f.target = "_blank"
	f.submit()
...
//대진표보기
function matchnum_view(gametitleidx,teamgb,level,groupgamegb,totround,nowcnt,gametype){
	var f = document.match_frm;
	f.GameTitleIDX.value = gametitleidx;
	f.TeamGb.value = teamgb;
	f.Level.value = level;
	...
	if (gametype=="<%=SportsCode%>043001"){
	  //리그
		f.action = "/Manager/Ajax/MatchLeague_View_bak.asp"
	}else if(gametype=="<%=SportsCode%>043002"){
  	//토너먼트
		if(totround=="2"){
			f.action = "/Manager/Ajax/Match2_View_bak.asp"		
		}else if(totround=="4"){
			f.action = "/Manager/Ajax/Match4_View_bak.asp"		
		}else if(totround=="8"){
			f.action = "/Manager/Ajax/Match8_View_bak.asp"				
		}else if(totround=="16"){
			f.action = "/Manager/Ajax/Match16_View_bak.asp"				
		}else if(totround=="32"){
			f.action = "/Manager/Ajax/Match32_View_bak.asp"				
		}else if(totround=="64"){
			f.action = "/Manager/Ajax/Match64_View_bak.asp"				
		}else if(totround=="128"){
			f.action = "/Manager/Ajax/Match128_View_bak.asp"				
		}
	}
	f.target = "_blank"
	f.submit()
							...
							<table class="navi-tp-table">
								...
								<tbody>
									<tr>
										...
										<th scope="row">단체전참가인원</th>
										<td><input type="text" name="EntryCnt" id="EntryCnt" class="input-small"></td>
									</tr>
									<tr>
										<th scope="row">구분</th>
				...
				<table class="table-list">
					...
					<thead>
						<tr>
							...
							<th scope="col">대회명</th>
							<th scope="col">대진표</th>
							<th scope="col">대진표(번호)</th>
							<th scope="col">구분</th>
							<th scope="col">소속</th>
							<!--<th scope="col">성별</th>-->
							<th scope="col">체급</th>
							<th scope="col">경기</th>
							<th scope="col">참가자(소속)</th>
...
<form name="match_frm" id="match_frm" method="post">
<input type="hidden" name="GameTitleIDX" id="GameTitleIDX" value="">
<input type="hidden" name="TeamGb" id="TeamGb" value="">
<input type="hidden" name="Level" id="Level" value="">
<input type="hidden" name="GroupGameGb" id="GroupGameGb" value="">
<input type="hidden" name="TotRound" id="TotRound" value="">
<input type="hidden" name="NowCnt" id="NowCnt" value="0">
<input type="hidden" name="GameType" id="GameType" value="">
</form>