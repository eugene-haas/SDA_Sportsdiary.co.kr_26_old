...
'   GameDay = NowYear & "-" & NowMonth & "-" & NowDay
  ...
  '대회정보
   ...
   'SQL = "SELECT Top 10 B.PlayerResultIDX, B.RGameLevelidx, ISNULL(SportsDiary.dbo.FN_PlayerName(B.LPlayerIDX),'미확정') AS LUserName, 'ISNULL(SportsDiary.dbo.FN_PlayerName(B.RPlayerIDX),'미확정') AS RUserName, "
   ...
   'SQL = SQL&" B.LTeam, CASE WHEN LTeamDtl = '0' THEN '' ELSE LTeamDtl END AS LTeamDtlNM, B.RTeam, CASE WHEN B.RTeamDtl = '0' THEN '' ELSE B.RTeamDtl END AS RTeamDtlNM, 'B.LResult, B.RResult, B.GameStatus AS GameStatus, "
   ...
   'SQL = SQL&" AND B.TurnNum<>''"
   ''SQL = SQL&" AND B.NowRound >2 "
   ''SQL = SQL&" AND B.Sex = 'Man' "
    ...
    <table class="table-list">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td><%=Rs("LSchoolName") & Rs("LTeamDtlNM")%></td>
					
          <td><%If Rs("GroupGameGb") = "sd040001" Then %><%=Rs("LUserName")%><%End If%></td>
          <td>VS</td>          
           <td><%If Rs("GroupGameGb") = "sd040001" Then %><%=Rs("RUserName")%><%End If%></td>
					 
          <td><%=Rs("RSchoolName") & Rs("RTeamDtlNM")%></td>
					<!--
          <td><%=Rs("RSidoNm")%></td>
					-->
          <td>
    ...
    <div class="footer_ad">
      ...
      <div class="right_txt">
        ...
        <p class="txt txt_2">경기장 내 스포츠다이어리 직원을 찾아주세요.</p>
				-->
        <span class="caution" style="width:140px">지도자용 팀매니저</span>
        <p class="txt txt_2">스포츠다이어리 홈페이지에서 전용테블릿을 신청하세요!</p>
      </div>
    </div>
    <!-- E: footer ad -->
    </div>
...
function chk_frm(){
  var f = document.frm;
//  f.target = "fPage";
  f.action = "RoundBoard_newall.asp";
  f.submit();
}
</script>
