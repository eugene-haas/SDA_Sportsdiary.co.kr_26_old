...
'//시드추첨,71|sd040001|31002|31002005|32|24|8|2,18,26,10,6,22,28,12|16|6|7082,592,|2
		...
		'If NoMatchCnt > 0 Then 
			USQL = "SELECT NoMatchNum,MatchNum FROM Sportsdiary.dbo.tblNoMatchInfo WHERE SportsGb='judo' AND TotRound='"&TotRound&"' AND NoMatchCnt='"&NoMatchCnt&"'"
...
'								Response.Write RRs("PlayerIDX")&","&RRs("UserName")&","&RRs("TeamNM")&array_aa(i)&"<br>"
	...
	If Lottery_Type = "시드추첨" Then 		
		...
		For i=0 To Ubound(Array_SeedIDX)
			...
			PSQL = "SELECT PlayerIDX,UserName,Team,Sportsdiary.dbo.FN_TeamNm('judo',TeamGb,Team) AS TeamNM "
			...
			PSQL = PSQL&" AND GroupGameGb='sd040001'"
