...
'   GameDay = NowYear & "-" & NowMonth & "-" & NowDay
  ...
  End If 
   ...
   SQL = "SELECT B.PlayerResultIDX, B.RGameLevelidx, ISNULL(SportsDiary.dbo.FN_PlayerName(B.LPlayerIDX),'미확정') AS LUserName, ISNULL(SportsDiary.dbo.FN_PlayerName(B.RPlayerIDX),'미확정') AS RUserName, "
      ...
      <tbody>
        ...
        <tr>
          ...
          <td><%=Rs("RSchoolName")%></td>
					-->
          <td><%=Rs("RSidoNm")%></td>
          <td><%If Rs("GameStatus") = "sd050001" Then %><span class="playing">진행중<%=k%></span><%Else%><%=k%><%End If%></td>
        </tr>
        <%        
				k = k + 1
              Rs.MoveNext
