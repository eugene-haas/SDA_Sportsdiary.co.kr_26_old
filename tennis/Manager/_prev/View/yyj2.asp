...
<%
  ...
  Lottery_Type = fInject(Request("Lottery_Type"))
	...
	If Lottery_Type = "" Then 
		Lottery_Type =  "1라운드제외"
	End If 


	
  If GameType = SportsCode&"040001" Then 
  '======================================================================================================
  '개인전일경우 =========================================================================================
  '======================================================================================================
    If GameTitleIDX <> "" And GameType <> "" And TeamGb <>  "" And Level <> ""  Then 
      ...
      CntSQL = CntSQL&" AND PM.SportsGb = '"&Request.Cookies("SportsGb")&"'"
...
'			Response.End
      ...
      Set PRs   = Nothing 
			...
			'해당체급의 부전승 갯수 구하기
				...
				If UnearnedCnt > 0 Then 
					'USQL = "SELECT NoMatchNum FROM Sportsdiary.dbo.tblNoMatchInfo WHERE SportsGb='judo'"
					USQL = "SELECT NoMatchNum FROM Sportsdiary.dbo.tblNoMatchInfo WHERE SportsGb='"&Request.Cookies("SportsGb")&"'"
					...
					USQL = USQL&" AND TotRound='"&TotRound&"' AND NoMatchCnt='"&NoMatchCnt&"'"
  ...
  ElseIf GameType = SportsCode&"040002" Then 
  '======================================================================================================
  '단체전일경우 =========================================================================================
  '======================================================================================================
      '해당체급의 참여자 명단 카운트
...
If TotRound > 0 And PlayerCnt > 0 Then 
  '한쪽에 뿌려져야할 경기수를 TotRound / 2
	...
	Unearned_Number = NoMatchNum

  'Response.Write NoMatchNum&"<br>"		
  'Response.Write Unearned_Number&"<br>"
	'Response.Write UnearnedCnt
	'Response.End
End If 
...
make_box("sel_GameTitle","GameTitleIDX","<%=GameTitleIDX%>","GameTitleMatch")
...
function chk_search(){
  ...
  if(strLeagueType == "<%=SportsCode%>043001"){
		sf.target="_self"
    sf.action="TournamentInfo_league.asp";
  }
  else if(strLeagueType == "<%=SportsCode%>043002"){
		sf.target="_self"
		sf.action="yyj2.asp"; 		
  }
	else{
...
<script>
//대진표 입력관련 스크립트
function chk_player(obj,obj2,no,team, teamdtl){
  //참가선수수
  ...
  document.getElementById("sel_teamname").value = team;
  document.getElementById("sel_playeridx").value = obj2;
  document.getElementById("sel_teamdtl").value = teamdtl;
	
}

//대진표리스트->입력데이터 이동
function input_data(obj,obj2,team, strteamdtl){
	//alert(team);
  var spidx    = document.getElementById("sel_playeridx");
  var spname   = document.getElementById("sel_playername");
  var stname   = document.getElementById("sel_teamname");
  var stnamedtl   = document.getElementById("sel_teamdtl");


  var obj2idx  = document.getElementById(obj2);
  var team  = document.getElementById(team);
  var teamdtl  = document.getElementById(strteamdtl);
  //시드히든테이블
	/*
  ...
  if(spidx.value!=""){
    ...
    team.value = stname.value;
    obj2idx.value = spidx.value;
		teamdtl.value = stnamedtl.value;
...
function random_player(){
    ...
    var Lottery_Type  = document.getElementById("Lottery_Type").value;
    ...
    var TotRound      = document.getElementById("TotRound").value;
    var PlayerCnt     = document.getElementById("PlayerCnt").value;
    var NoMatchCnt    = document.getElementById("NoMatchCnt").value;
    var NoMatchNum    = document.getElementById("NoMatchNum").value;
    var ChkTot        = document.getElementById("ChkTot").value;
    var Cnt           = document.getElementById("Cnt").value;
    var seed           = document.getElementById("seed").value;
    var seedcnt           = document.getElementById("seedcnt").value;
//		document.write (Lottery_Type+","+GametitleIDX+"|"+GameType+"|"+TeamGb+"|"+Level+"|"+TotRound+"|"+PlayerCnt+"|"+NoMatchCnt+"|"+NoMatchNum+"|"+ChkTot+"|"+Cnt+"|"+seed+"|"+seedcnt)

    if(GametitleIDX==""){
      return;
    }
    if(GameType=="<%=SportsCode%>040001" && Level==""){
      return;
    }
		//alert(GameType);
    if(GameType=="<%=SportsCode%>040001"){
			if(Lottery_Type=="1라운드제외"){
      var strAjaxUrl = "/Manager/Ajax/random_proceduer2.asp";
			}else{
				var strAjaxUrl = "/Manager/view/yyj_ok.asp";			
			}
    }else{
          ...
          //input box의 수대로 for문
					...
					if(GameType=="<%=SportsCode%>040001"){
						if(Lottery_Type=="1라운드제외"){
							...
							j = 0
							x = 0
							Arr_retDATA = retDATA.split("*")
							array_Data1 = Arr_retDATA[0].split("|")
							array_Data2 = Arr_retDATA[1].split("|")
							//document.write (retDATA)
							for(k=1;k<=totcnt;k++){
								 modResult = k % 2
								if(modResult==0 && document.getElementById("Player"+k).disabled == false){
									arr_Input = array_Data1[j].split(",")
									document.getElementById("Player"+k).value = arr_Input[0];
									document.getElementById("hidden_data"+k).value = arr_Input[1];
									document.getElementById("Team"+k).value = arr_Input[2];
									j = j + 1
								}else if(modResult==1 && document.getElementById("Player"+(k+1)).disabled == false){
									arr_Input = array_Data1[j].split(",")
									document.getElementById("Player"+k).value = arr_Input[0];
									document.getElementById("hidden_data"+k).value = arr_Input[1];
									document.getElementById("Team"+k).value = arr_Input[2];
									j = j + 1						
								}else if(modResult==1 && document.getElementById("Player"+(k+1)).disabled == true){
									arr_Input = array_Data2[x].split(",")
									document.getElementById("Player"+k).value = arr_Input[0];
									document.getElementById("hidden_data"+k).value = arr_Input[1];
									document.getElementById("Team"+k).value = arr_Input[2];
									x = x + 1
							...
							for(k=0;k<PlayerCnt;k++ ){
								arr_Input = array_Data[k].split("|")
								
								document.getElementById("Player"+arr_Input[4]).value = arr_Input[1]							
								document.getElementById("hidden_data"+arr_Input[4]).value = arr_Input[0];
								document.getElementById("Team"+arr_Input[4]).value = arr_Input[3];
							}
						}
						...
						for(k=1;k<=totcnt;k++){
							if(document.getElementById("Player"+k).disabled == false){
								...
								document.getElementById("Player"+k).value = arr_Input[0];
								document.getElementById("hidden_data"+k).value = arr_Input[1];
								document.getElementById("hidden_data_TeamDtl"+k).value = arr_Input[2];
								j = j + 1
							}
						}
					}
...
function chk_save(){
  ...
  if(sf.GameType[0].checked){
    if(confirm("해당 대진표를 저장하시겠습니까?")){
      ...
      if(totcnt=="2"){
        sf.action = "/Manager/Ajax/Tournament2_ok.asp"    
      }else if(totcnt=="4"){
        sf.action = "/Manager/Ajax/Tournament4_ok.asp"    
      }else if(totcnt=="8"){
        sf.action = "/Manager/Ajax/Tournament8_ok.asp"    
      }else if(totcnt=="16"){
        sf.action = "/Manager/Ajax/Tournament16_ok.asp"   
      }else if(totcnt=="32"){
        sf.action = "/Manager/Ajax/Tournament32_ok.asp"   
      }else if(totcnt=="64"){
        sf.action = "/Manager/Ajax/Tournament64_ok.asp"   
      }else if(totcnt=="128"){
        sf.action = "/Manager/Ajax/Tournament128_ok.asp"    
      }
			sf.target = "iSQL"
      sf.submit();
    }
    ...
    if(confirm("해당 대진표를 저장하시겠습니까?")){
      ...
      if(totcnt=="2"){
        sf.action = "/Manager/Ajax/SchoolTournament2_ok.asp"    
      }else if(totcnt=="4"){
				
        sf.action = "/Manager/Ajax/SchoolTournament4_ok.asp"    
      }else if(totcnt=="8"){
        sf.action = "/Manager/Ajax/SchoolTournament8_ok.asp"    
      }else if(totcnt=="16"){
        sf.action = "/Manager/Ajax/SchoolTournament16_ok.asp"   
      }else if(totcnt=="32"){
        sf.action = "/Manager/Ajax/SchoolTournament32_ok.asp"   
      }else if(totcnt=="64"){
        sf.action = "/Manager/Ajax/SchoolTournament64_ok.asp"   
      }else if(totcnt=="128"){
        sf.action = "/Manager/Ajax/SchoolTournament128_ok.asp"    
      }
			sf.target = "iSQL"
      sf.submit();
    } 
...
//소속구분셀렉트박스
var GameTitleIDX = document.getElementById("GameTitleIDX").value;
make_box2("sel_TeamGb","TeamGb","<%=TeamGb%>","TeamGbMatch",GameTitleIDX)
}


//부전박스재생성
function chk_Unearned(obj){
	var sf = document.search_frm;
	sf.target="_self"
	sf.action="yyj2.asp"; 		
  sf.submit();
...
function chk_seed(objidx,playeridx){	
	if(document.getElementById("seed"+objidx).innerHTML.substring(0,2)=="시드"){
		document.getElementById("seed").value = document.getElementById("seed").value.replace(playeridx+",","")
		document.getElementById("seed"+objidx).innerHTML="";
		document.getElementById("seedcnt").value = Number(document.getElementById("seedcnt").value) - 1
	}else{
		document.getElementById("seed").value = document.getElementById("seed").value+playeridx+","
		document.getElementById("seedcnt").value = Number(document.getElementById("seedcnt").value) + 1
		document.getElementById("seed"+objidx).innerHTML="시드"+document.getElementById("seedcnt").value;
	}	
}
...
<body>
    ...
    <div class="header container-fluid">
      <div class="row">
        ...
        <div class="pull-right">
          <span class="sd-logo"><img src="../tablet/images/tournerment/header/logo@3x.png" alt="스포츠 다이어리" height="34"></span>
    ...
    <form name="search_frm" method="post">
		...
		<input type="hidden" name="seedcnt" id="seedcnt" value="0">
		<select name="Lottery_Type" id="Lottery_Type" onChange="chk_Unearned(this.value)">
			<option value="무작위추첨" <%If Lottery_Type = "무작위추첨" Then %>selected<%End If%>>무작위추첨</option>
			<option value="1라운드제외" <%If Lottery_Type = "1라운드제외" Then %>selected<%End If%>>1라운드제외</option>
			<option value="중고연맹1라운드제외" <%If Lottery_Type = "중고연맹1라운드제외" Then %>selected<%End If%>>중고연맹1라운드제외</option>
			<!--<option value="동일학교라운드분배">동일학교라운드분배</option>-->
			<option value="시드추첨" <%If Lottery_Type = "시드추첨" Then %>selected<%End If%>>시드추첨</option>
		</select>
		<input type="hidden" name="TotRound" id="TotRound" value="<%=TotRound%>">
		<input type="hidden" name="PlayerCnt" id="PlayerCnt" value="<%=PlayerCnt%>">
		<input type="hidden" name="NoMatchCnt" id="NoMatchCnt" value="<%=NoMatchCnt%>">
		<input type="hidden" name="NoMatchNum" id="NoMatchNum" value="<%=NoMatchNum%>">
		<input type="hidden" name="ChkTot" id="ChkTot" value="<%=ChkTot%>">
		<input type="hidden" name="Cnt" id="Cnt" value="<%=Cnt%>">
		<div class="main main-input container-fluid">
      <h2 class="stage-title row" id="sel_GameTitle" >
      ...
      <div class="input-select ent-sel row">
        ...
        <div class="enter-type tab-menu">
          <ul class="clearfix">
            ...
            <li class="game-type">
              <label ><span><img src="../tablet/images/tournerment/tourney/icon-private@3x.png" alt width="18" height="18"></span><span class="type-text">개인전</span><input type="radio" name="GameType" <%If GameType = SportsCode&"040001" Then %>checked<%End If%> value="<%=SportsCode%>040001"  onclick="chk_gamegb();"></label>
            </li>
            <li class="game-type">
              <label ><span><img src="../tablet/images/tournerment/tourney/icon-group@3x.png" alt width="16" height="21"></span><span class="type-text">단체전</span><input type="radio" name="GameType" <%If GameType = SportsCode&"040002" Then %>checked<%End If%> value="<%=SportsCode%>040002" onClick="chk_gamegb();"></label>
            </li>
            ...
            <li class="type-sel" id="sel_Level">
              <%
                If GameTitleIDX <> "" And GameType <> "" And TeamGb <>  "" And Level <> "" Then         
										If GameType = SportsCode&"040001" Then
											LSQL = "SELECT Level,Sportsdiary.dbo.FN_LevelNm('" & Request.Cookies("SportsGb") & "',TeamGb,Level) AS LevelNm FROM Sportsdiary.dbo.tblRGameLevel "
            ...
            <li class="btn-list">
              <button type="button" id="search2" class="btn btn-warning btn-search" onClick="chk_save();">대진저장</button>
            </li>
    ...
    <input type="hidden" name="sel_teamname" id="sel_teamname">
    <input type="hidden" name="sel_teamdtl" id="sel_teamdtl">
        ...
        <div class="left-side clearfix">
              ...
              <div class="player-info" tabindex="0">
                <div class="player-input-wrap">
                  ...
                  <%
                    ...
                    Next
                  %>
                  <span class="player-name-input"><input type="text" name="Player<%=i%>" id="Player<%=i%>" <%=css_disabled%> readonly onClick="input_data(this,'hidden_data<%=i%>','Team<%=i%>','hidden_data_TeamDtl<%=i%>')" /></span>
                  <span class="player-name-input"><input type="text" name="Team<%=i%>" id="Team<%=i%>" <%=css_disabled%> readonly onClick="input_data(this,'hidden_data<%=i%>','Team<%=i%>','hidden_data_TeamDtl<%=i%>')" /></span>									
                  <input type="hidden" name="hidden_data<%=i%>" id="hidden_data<%=i%>" <%=css_disabled%>>                  
									<input type="hidden" name="hidden_data_TeamDtl<%=i%>" id="hidden_data_TeamDtl<%=i%>" <%=css_disabled%>> 
                </div>
              </div>
              <%
                css_diabled = ""
          ...
          <%
            ...
            If GameType = SportsCode&"040001" Then 
              If RGameLevelIDX <> "" Then 
                LSQL = "select PlayerIDX,UserName,SportsDiary.dbo.FN_TeamCodeReturnTeamNm('"&Request.Cookies("SportsGb")&"',Team) AS TeamNm,  "
								...
								LSQL = LSQL & " AND DelYN='N' "
								LSQL = LSQL & " Order By TeamNm" 'TeamNm" 'UserName
                'Response.Write LSQL
                'Response.End
                ...
                If Not(LRs.Eof Or LRs.Bof) Then             
                  k = 1
          %>
          <div class="match-input-result stickyz">
            <div class="inner">
              <ul class="top">
                <li>시드</li>
                <li>선수이름</li>
              ...
              %>
                <dl id="No<%=k%>" class="clearfix">
                  <dt id="seed<%=k%>" onClick="chk_seed('<%=k%>','<%=LRs("PlayerIDX")%>')"></dt>
                  <dd onClick="chk_player('<%=LRs("UserName")%>','<%=LRs("PlayerIDX")%>','<%=k%>','<%=LRs("TeamNm")%>','<%=LRs("TeamDtl")%>')"><%=LRs("UserName")%></dd>
                  <dd onClick="chk_player('<%=LRs("UserName")%>','<%=LRs("PlayerIDX")%>','<%=k%>','<%=LRs("TeamNm")%>','<%=LRs("TeamDtl")%>')"><%=LRs("TeamNm") & LRs("TeamDtlNM")%></dd>
                </dl>
              <%
                      k = k + 1
											pcnt = pcnt + 1
          ...
          <%
            ...
            ElseIf GameType = SportsCode&"040002" Then 
              ...
              '--------------------------------------------------------------------------------------------------------------------------------------------------------
              If RGameLevelIDX <> "" Then 
                  LSQL = "SELECT Team,SchoolName,SportsDiary.dbo.FN_SchoolName(SchIDX) AS SchName,"
									LSQL = LSQL & " CASE WHEN ISNULL(TeamDtl,'0') = '0' THEN '' ELSE '(' + TeamDtl + ')' END AS TeamDtlNM,"
									LSQL = LSQL & " TeamDtl"
									LSQL = LSQL&" FROM SportsDiary.dbo.tblRGameGroupSchoolMaster"
                  LSQL = LSQL&" WHERE Gametitleidx='"&GameTitleIDX&"' "
									LSQL = LSQL&" AND RgameLevelIDX='"&RGameLevelIDX&"' "
									LSQL = LSQL&" AND DelYN='N' Order By SchoolName"
                  'Response.Write LSQL
              ...
              <div class="inner">
                ...
                %>
                  <dl id="No<%=k%>" class="">
                    ...
                    <dd onClick="chk_player('<%=LRs("SchoolName")%>','<%=LRs("Team")%>','<%=k%>','<%=LRs("SchoolName")%>','<%=LRs("TeamDtl")%>')"><%=LRs("SchoolName")%></dd>
										<dd ><%=LRs("TeamDtlNM")%></dd>
                  </dl>
                <%
                        k = k + 1
              ...
              <div class="player-info" tabindex="0">
                <div class="player-input-wrap">
                  ...
                  <%                    
                    ...
                    Next                  
                  %>
                  <span class="player-name-input"><input type="text" name="Player<%=k%>" id="Player<%=k%>" <%=css_diabled%> readonly onClick="input_data(this,'hidden_data<%=k%>','Team<%=k%>','hidden_data_TeamDtl<%=k%>')" /></span>
									<span class="player-name-input"><input type="text" name="Team<%=k%>" id="Team<%=k%>"  <%=css_diabled%> readonly onClick="input_data(this,'hidden_data<%=k%>','Team<%=k%>','hidden_data_TeamDtl<%=k%>')" /></span>
                  <input type="hidden" name="hidden_data<%=k%>" id="hidden_data<%=k%>">
                  <input type="hidden" name="hidden_data_TeamDtl<%=k%>" id="hidden_data_TeamDtl<%=k%>">
...
<iframe name="iSQL" id="iSQL" width="0" height="0" border="0"></iframe>
<%
If TeamGb <> "" Then 
%>
