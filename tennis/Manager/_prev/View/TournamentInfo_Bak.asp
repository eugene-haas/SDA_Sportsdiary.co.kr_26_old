          ...
          <!-- E: left-side -->
					...
					<%
						...
						If GameType = "sd040001" Then 
							If RGameLevelIDX <> "" Then 
								LSQL = "select PlayerIDX,UserName,SportsDiary.dbo.FN_SchoolName(SchIDX) AS SchName from tblrplayermaster  where gametitleidx='"&GameTitleIDX&"' and RgameLevelIDX='"&RGameLevelIDX&"' AND DelYN='N' Order By UserName"
								Set LRs = Dbcon.Execute(LSQL)
					...
					<%
						...
						ElseIf GameType = "sd040002" Then 
							...
							'--------------------------------------------------------------------------------------------------------------------------------------------------------
							If RGameLevelIDX <> "" Then 
									LSQL = "select SchIDX,SchoolName,SportsDiary.dbo.FN_SchoolName(SchIDX) AS SchName from SportsDiary.dbo.tblRGameGroupSchoolMaster"
						...
						<div class="match-input-result sticky">
							<div class="inner">
								...
								%>
									<dl id="No<%=k%>" class="">
										<dt></dt>
										<dd onclick="chk_player('<%=LRs("SchoolName")%>','<%=LRs("SchIDX")%>','<%=k%>')"><%=LRs("SchoolName")%></dd>
									</dl>
								<%
												k = k + 1
												LRs.MoveNext
