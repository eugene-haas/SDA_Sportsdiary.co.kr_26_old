...
<%Else%>		
<%	Do until LRs.EOF	%>		
   <%=LRs("GameGb")%>|><%=LRs("SportsGb")%>|><%=LRs("GameYear")%>|><%=LRs("GameTitleName")%>|><%=LRs("Sido")%>|><%=LRs("SidoDtl")%>|><%=LRs("GameS_Year")%>|><%=LRs("GameS_Month")%>|><%=LRs("GameS_Day")%>|><%=LRs("GameE_Year")%>|><%=LRs("GameE_Month")%>|><%=LRs("GameE_Day")%>|><%=LRs("GameArea")%>|><%=LRs("GameRcv_SYear")%>|><%=LRs("GameRcv_SMonth")%>|><%=LRs("GameRcv_SDay")%>|><%=LRs("GameRcv_EYear")%>|><%=LRs("GameRcv_EMonth")%>|><%=LRs("GameRcv_EDay")%>|><%=LRs("EnterType")%>|><%=LRs("HostCode")%>|><%=LRs("ViewYN")%>|><%=LRs("MatchYN")%>|><%=LRs("ViewState")%>
