...
<%
  ...
  LSQL = LSQL&" Top "&ViewCnt
	...
	LSQL = LSQL&" ,TeamGb ,EnterType"
	LSQL = LSQL&" ,Sportsdiary.dbo.FN_LevelReturnSex(SportsGb,Level) AS Sex "
	LSQL = LSQL&" ,Level "
	LSQL = LSQL&" ,SportsDiary.dbo.FN_LevelNm('"&Request.Cookies("SportsGb")&"',TeamGb,Level) AS LevelNm "
	LSQL = LSQL&" ,GameType"
...
%>
	<%
		...
		Do Until LRs.Eof 
			...
			If LRs("GroupGameGb") = SportsCode&"040001" Then 
				PlayerCntSQL = "SELECT Count(RPlayerMasterIDX) AS Cnt FROM SportsDiary.dbo.tblRPlayerMaster WHERE SportsGb='"&Request.Cookies("SportsGb")&"' AND Level='"&LRs("Level")&"' AND GroupGameGb ='"&SportsCode&"040001' AND GameTitleIDX='"&LRs("GameTitleIDX")&"' AND DelYN='N'"
				
				Set PlayerCntRs = Dbcon.Execute(PlayerCntSQL)
				'Response.Write PlayerCntSQL
				'Response.End

				MatchCntSQL = "SELECT Count(RPlayerIDX) AS Cnt FROM SportsDiary.dbo.tblRPlayer WHERE SportsGb='"&Request.Cookies("SportsGb")&"' AND Level='"&LRs("Level")&"' AND GroupGameGb ='"&SportsCode&"040001' AND GameTitleIDX='"&LRs("GameTitleIDX")&"' AND DelYN='N'"
			...
			Else
				'단체전 참가자 학교
				PlayerCntSQL = "SELECT Count(RGameGroupSchoolMasterIDX) AS Cnt FROM SportsDiary.dbo.tblRGameGroupSchoolMaster WHERE SportsGb = '"&Request.Cookies("SportsGb")&"'   AND GroupGameGb ='"&SportsCode&"040002' AND GameTitleIDX='"&LRs("GameTitleIDX")&"' AND TeamGb ='"&LRs("TeamGb")&"' AND DelYN='N'"
				'Response.Write LRs("TeamGb")&"<br>"
				'Response.Write PlayerCntSQL
				'Response.End
				Set PlayerCntRs = Dbcon.Execute(PlayerCntSQL)

				MatchCntSQL = "SELECT Count(RGameGroupSchoolIDX) AS Cnt FROM SportsDiary.dbo.tblRGameGroupSchool WHERE SportsGb = '"&Request.Cookies("SportsGb")&"'   AND GameTitleIDX='"&LRs("GameTitleIDX")&"' AND TeamGb ='"&LRs("TeamGb")&"' AND DelYN='N'"
				'Response.Write MatchCntSQL
				'Response.End
				Set MatchCntRs = Dbcon.Execute(MatchCntSQL)
	...
	<tr>
        ...
        <td><a href="javascript:tourney_view('<%=LRs("GameTitleIDX")%>','<%=LRs("GameTitleName")%>','<%=LRs("TeamGb")%>','<%=LRs("Level")%>','<%=LRs("GroupGameGb")%>','<%=LRs("Sex")%>','<%=LRs("EnterType")%>');" class="btn-list">대진결과보기</a></td>
		<%
			Else 
		%>
		...
		<td style="cursor:pointer;" onclick="input_data('<%=LRs("GameTitleIDX")%>','<%=LRs("Level")%>','<%=LRs("RGameLevelIDX")%>');">
			<%If LRs("GameType") = "" Then %>
				경기
			<%
				Else 
			%>
		...
		<td>
			<select onChange="Change_GameType(this.value);">
				<option value="<%=LRs("RGameLevelIDX")%>,<%=SportsCode%>043002" <%If LRs("GameType") =SportsCode&"043002" Then%>selected<%End If%>>토너먼트</option>
				<option value="<%=LRs("RGameLevelIDX")%>,<%=SportsCode%>043001" <%If LRs("GameType") =SportsCode&"043001" Then%>selected<%End If%>>리그</option>
				<option value="<%=LRs("RGameLevelIDX")%>,<%=SportsCode%>043003" <%If LRs("GameType") =SportsCode&"043003" Then%>selected<%End If%>>리그>토너먼트</option>
			</select>
		</td>
		<%
			If LRs("GroupGameGb") = SportsCode&"040001" Then 
