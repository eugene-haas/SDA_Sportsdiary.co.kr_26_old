...
var tennis_global_filepathUrl_script = "/FileDown/";
var tennis_global_filepathImgUrl_script = "/FileImg/";
var judo_global_filepathUrl_script = "/FileDown/";
var judo_global_filepathImgUrl_script = "/FileImg/";
...
function chk_logout_Admin() {
	if (confirm("로그아웃 하시겠습니까?")) {
		...
		var strAjaxUrl = "/Admin/main/Ajax/AdminLogout.asp";
		...
		$.ajax({
			...
			success: function (retDATA) {
				if (retDATA) {
					if (retDATA == "TRUE") {
						alert("정상적으로 로그아웃 되었습니다.\n로그인 후 서비스를 이용하세요");
						$(location).attr('href', 'http://sdmainadmin.sportsdiary.co.kr/admin/main/admin_Login.asp');   //a href	
						return;
					}
					else {
						alert('로그아웃중에 오류가 발생하였습니다.');
