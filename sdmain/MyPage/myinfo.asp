...
<div id="app" class="l m_bg_edf0f4 myinfo" v-cloak>
  ...
  <div class="l_content m_scroll [ _content _scroll ]">
    <form name="s_frm" method="post" @submit.prevent="chk_frm">
		  <fieldset>
      ...
      <div class="m_bottomBtn">
        ...
        <button type="submit" class="m_bottomBtn__btn s_modify">수정완료</button>
      </div>
    </form>
	</div>

  <!--S: 다음 주소찾기 API-->
  <div id="wrap" style="display: none; border:1px solid #000; width:100%; height:100%; margin:48px 0; position: absolute; top:0; z-index:1000;"> <img src="//t1.daumcdn.net/localimg/localimages/07/postcode/320/close.png" id="btnFoldWrap" style=" width:19px;cursor:pointer;position:absolute;right:0px;top:-1px;z-index:1" @click="foldDaumPostCode" alt="접기 버튼"> </div>
</div>
    ...
    methods:{
      ...
      myInfoData:function(url){
        axios.get(url).then(response=>{
          if(response.data.jlist.length==1){
