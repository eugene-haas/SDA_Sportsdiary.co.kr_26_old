...
sda.ajaxurl = "/api/req/reqAdmin.asp";
sda.ajaxtype = "POST";
sda.dataType = "text";
...
sda.Send = function (sender, packet) {
  ...
  $.ajax({
    url: sda.ajaxurl, type: sda.ajaxtype, data: strdata, dataType: sda.dataType,
...
sda.SendEx = function (sender, packet, urlAjax) {
  ...
  var strdata = "REQ=" + encodeURIComponent(JSON.stringify(packet));
  $.ajax({
    url: urlAjax, type: sda.ajaxtype, data: strdata, dataType: sda.dataType,
    success: function (returnData) {
      //console.log(returnData);
      sda.Receive(packet.CMD, returnData, sender)
    }
