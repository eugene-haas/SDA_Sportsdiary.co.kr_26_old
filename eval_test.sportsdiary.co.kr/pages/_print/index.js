   ...
   methods: {
      ...
      initEvalList: function() {
         ...
         axios.post(path, params).then(function(res) {
            ...
            this.eval_list = cm_fn.copyObjEx(res.data.list_info, []).reverse();
      ...
      /* -----------------------------------------------------------------------------------
         ...
         ---------------------------------------------------------------------------------- */
      reqTblData: function() {
         const path = "/api/ajax/mgr_viewer/result/result.asp";
         const params = {
      ...
      reqScoreData: function() {
         const path = "/api/ajax/mgr_viewer/result/association_eval_point.asp";
         const params = {
            eval_table_idx: this.sel_eval,
      ...
      openModal_review: function(){
         const path = "/api/ajax/mgr_viewer/result/association_eval_desc.asp";
         const params = {
            eval_table_idx: this.sel_eval,
            association_idx: this.sel_group,
         };
         axios.post(path, params).then(function(res) {
            ...
            this.modal_review.sel_cate = this.getSelCateList[0].eval_cate_cd;
      ...
      getReview: function(item_type_idx){
         return this.getReviewMap[item_type_idx]
            && this.getReviewMap[item_type_idx].eval_desc;
      },
   },
   computed: {
      /* -----------------------------------------------------------------------------------
      ...
      getReviewMap: function(){
         ...
         for (let i = 0; i < review_len; i++) {
            ...
            review_map[review_info.item_type_idx] = review_info;
         }
         return review_map;
      },
      /* -----------------------------------------------------------------------------------
      ...
      /* -----------------------------------------------------------------------------------
            선택된 카테고리의 정보 반환
         ---------------------------------------------------------------------------------- */
      getSelCateInfoOfReviewModal: function(){
         return this.getSelCateList.find(function(obj){
