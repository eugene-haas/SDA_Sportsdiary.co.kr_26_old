...
<%
' http://eval.sportsdiary.co.kr/api/ajax/mgr_input/reg_data/reg_desc.asp
%>
...
<%
   ...
   '=================================================================================
	'  평가 위원 - 평가 의견 등록  - Valid Check 
	'=================================================================================  
	Function getSqlCheckValid(eval_table_idx, association_idx, eval_member_idx, eval_descs, item_idxs)
      Dim strSql, err_no
		
		If(eval_table_idx = "") Or (association_idx = "") Or (eval_member_idx = "") Or (eval_descs = "") Or (item_idxs = "") Then err_no = 1 End If 
		
		If( err_no <> 1 ) Then 	        
         ' 평가 위원 - 평가 항목 등록  "
         strSql = strSql & " Declare @valid_id int, @vaild_association int, @valid_items int , @valid_data_cnt int  "
         strSql = strSql & " Declare @cnt_desc int , @cnt_idx int   "
         ...
         strSql = strSql & " ;with cte_desc As (	 "
         strSql = strSql & sprintf("    Select value As eval_desc From string_split('{0}', '|') " ,Array(eval_descs))
         strSql = strSql & " ) "
         ...
         strSql = strSql & " Select @cnt_desc = (Select Count(eval_desc) From cte_desc),  "
         ...
         ' 클라이언트로 부터 받아온 점수를 select문으로 바꾼다.  "
         strSql = strSql & " ;with cte_desc As (	 "
         strSql = strSql & "    Select Row_Number() Over(Order By(Select 1)) As Idx, eval_desc From ( "
         strSql = strSql & sprintf("       Select value As eval_desc From string_split('{0}', '|') " ,Array(eval_descs))
         strSql = strSql & "    ) As C "
         strSql = strSql & " ) "
         ...
         ' 클라이언트로 부터 받아온 item_type_idx를 select문으로 바꾼다.  "
         strSql = strSql & " , cte_client_idx As ( "
         strSql = strSql & "    Select Row_Number() Over(Order By(Select 1)) As Idx, EvalItemTypeIDX From ( "
         strSql = strSql & sprintf("       Select Cast(value As int) As EvalItemTypeIDX From string_split('{0}', ',') " ,Array(item_idxs))
         strSql = strSql & "    ) As C "
         strSql = strSql & " ) "
         ...
         strSql = strSql & "    Select I.Idx, I.EvalItemTypeIDX,  eval_desc "
         strSql = strSql & "       From cte_desc As P  "
         ...
         ' 정상 등록  "
         strSql = strSql & " If(@valid_id = 1 And @vaild_association = 1 And @valid_items = 1 And @valid_data_cnt = 1)  	  "
   ...
   '=================================================================================
	'  평가 위원 - 최고관리자 이상 - 평가 의견 등록  - Valid Check 
	'=================================================================================  
	Function getSqlCheckValidMgr(eval_table_idx, association_idx, eval_member_idx, eval_descs, item_idxs)
      Dim strSql, err_no
   ...
   '  평가 의견 등록 - Insert
   '=================================================================================  
   Function getSqlInsertDesc(eval_table_idx, association_idx, eval_descs, item_idxs, eval_member_idx)
      Dim strSql, err_no
      
      If(eval_table_idx = "") Or (association_idx = "") Or (eval_descs = "") Or (item_idxs = "") Or (eval_member_idx = "") Then err_no = 1 End If 
      
      If( err_no <> 1 ) Then 	
         '/* ---------------------------------------------------------------------------------------
         '   입력된 평가 의견을 추가한다 .
         '   --------------------------------------------------------------------------------------- */         
         ' 클라이언트로 부터 받아온 평가의견을 select문으로 바꾼다.  "
         strSql = strSql & " ;with cte_desc As (	 "
         strSql = strSql & "    Select Row_Number() Over(Order By(Select 1)) As Idx, eval_desc From ( "
         strSql = strSql & sprintf("       Select value As eval_desc From string_split('{0}', '|') " ,Array(eval_descs))
         strSql = strSql & "    ) As C "
         ...
         ' 클라이언트로 받아온 item_type_idx과 point를 merge한다.  "
         strSql = strSql & " , cte_client_item As ( "
         strSql = strSql & "    Select I.Idx, I.item_type_idx,  eval_desc "
         strSql = strSql & "       From cte_desc As P  "
         strSql = strSql & "       Inner Join cte_client_idx As I On I.Idx = P.Idx  "
         strSql = strSql & " )  "

         ' 이미 입력된 의견을 제외한 평가 항목을 구한다. 
         strSql = strSql & " , cte_diff_item As ( "
         strSql = strSql & "    Select *  "
         strSql = strSql & "       From cte_client_item "
         strSql = strSql & "       Where item_type_idx Not In (Select EvalItemTypeIDX From tblEvalDesc  "
         strSql = strSql & sprintf("          Where DelKey = 0 And EvalTableIDX = {0} And AssociationIDX = {1} and AdminMemberIDX = {2} ) ", Array(eval_table_idx, association_idx, eval_member_idx)) 'by baek
         strSql = strSql & " ) "

         strSql = strSql & " Insert Into tblEvalDesc(EvalTableIDX, AssociationIDX, EvalGroupCD, EvalItemIDX, EvalItemTypeIDX, EvalTypeCD, EvalDesc ,AdminMemberIDX) " 'by baek

         strSql = strSql & sprintf(" Select T.EvalTableIDX, A.AssociationIDX, A.EvalGroupCD, T.EvalItemIDX, T.EvalItemTypeIDX, T.EvalTypeCD, dbo.FN_ENC_VAL(I.eval_desc) ,{0}  ",Array(eval_member_idx)) 'by baek
		...
		getSqlInsertDesc = strSql 
	End Function

   '=================================================================================
	'  평가 의견 등록 - Update
	'=================================================================================  
	Function getSqlModDesc(eval_table_idx, association_idx, eval_descs, item_idxs, eval_member_idx)
      Dim strSql, err_no
		
		If(eval_table_idx = "") Or (association_idx = "") Or (eval_descs = "") Or (item_idxs = "") Then err_no = 1 End If 
		
		If( err_no <> 1 ) Then 	
         '/* ---------------------------------------------------------------------------------------
         '      입력된 평가 의견을 수정한다 .
         '   --------------------------------------------------------------------------------------- */         
         ' 클라이언트로 부터 받아온 의견를 select문으로 바꾼다.  "
         strSql = strSql & " ;with cte_desc As (	 "
         strSql = strSql & "    Select Row_Number() Over(Order By(Select 1)) As Idx, eval_desc From ( "
         strSql = strSql & sprintf("       Select value As eval_desc From string_split('{0}', '|') " ,Array(eval_descs))
         strSql = strSql & "    ) As C "
         ...
         ' 이미 입력된 의견과 같은 평가 항목을 구해서 Update한다. 
         strSql = strSql & " , cte_same_item As ( "
         strSql = strSql & "    Select *  "
         strSql = strSql & "       From cte_client_item "
         strSql = strSql & "       Where item_type_idx In (Select EvalItemTypeIDX From tblEvalDesc  "
         ...
         strSql = strSql & " Update tblEvalDesc Set EvalDesc = dbo.FN_ENC_VAL(I.eval_desc), ModDate = GetDate() "
         strSql = strSql & "    From tblEvalDesc As D  "
         strSql = strSql & "    Inner Join cte_same_item AS I On I.item_type_idx = D.EvalItemTypeIDX "
         strSql = strSql & sprintf("    Where DelKey = 0 And EvalTableIDX = {0} And AssociationIDX = {1}  and AdminMemberIDX = {2} ", Array(eval_table_idx, association_idx, eval_member_idx))
		End If  
...
ENC_DBOpen()
   ...
   Dim aryKey, strKeys
   Dim eval_table_idx, association_idx, eval_member_idx, eval_descs, item_idxs, is_mgr, rsp_err
   ...
   eval_descs      		   = InjectionChk(JsonObj.get("eval_descs"))
   ...
   If( err_no <> 1 ) Then 	
      ...
      eval_descs = htmlEncode(eval_descs)
      ...
      If(is_mgr = "1") Then 
         '  ----------------------------------------------------
         '  최고관리자 이상 - 평가 의견 등록  - Valid Check  
         strSql = getSqlCheckValidMgr(eval_table_idx, association_idx, eval_member_idx, eval_descs, item_idxs)
         RS_DATA = ExecuteReturn(strSql, ENC_DB)
      ...
      Else 
         '  ----------------------------------------------------
         '  평가 의견 등록  - Valid Check   
         strSql = getSqlCheckValid(eval_table_idx, association_idx, eval_member_idx, eval_descs, item_idxs)
         RS_DATA = ExecuteReturn(strSql, ENC_DB)
      ...
      If(rsp_err = "0") Then 
         '  ----------------------------------------------------
         '  평가 의견 등록 - Insert         
         strSql = getSqlInsertDesc(eval_table_idx, association_idx, eval_descs, item_idxs,eval_member_idx)  'by baek
         Call ExecuteUpdate(strSql, ENC_DB)

         '  ----------------------------------------------------
         '  평가 의견 등록 - Update
         strSql = getSqlModDesc(eval_table_idx, association_idx, eval_descs, item_idxs,eval_member_idx) 'by baek
         Call ExecuteUpdate(strSql, ENC_DB)
