...
<% 	
	...
	'=================================================================================
	'  Bytes data를 utf8 string data로 변환한다. 
	'================================================================================= 
	...
	'=================================================================================
	'  Get 방식 데이터일 경우 request를 parsing 하여 json object string으로 변환한다 .
	'================================================================================= 
	Function GetJsonFromGetParam(strParam)
		...
		If(strParam <> "") Then
			...
			For Idx = 0 To ub
				...
				If(Idx = 0) Then
					strData = sprintf(" ""{0}"":""{1}"" ", Array(aryData(0), aryData(1) ))
				Else
					strData = sprintf("{0}, ""{1}"":""{2}"" ", Array(strData, aryData(0), aryData(1) ))
				End If
			Next
...
'   ===============================================================================
	Function utxJSonStringifyByLimit(jObj, nLimit)
		...
		for each key in jObj.keys()
			...
			If(nLen < nLimit) Then 
				If(n = 0) Then 
					strJson = sprintf(" ""{0}"":""{1}"" ", Array(key, val))
				Else 
					strJson = sprintf(" {0}, ""{1}"":""{2}"" ", Array(strJson, key, val))
				End If 
				n = n + 1
			End If 
		next
...
'   ===============================================================================
  ...
  '=================================================================================
	...
	Function utxAddQuotationInWord(src)	
		...
		strRet = sprintf( "'{0}'", Array(ary(0)) )
		For i=1 To UBound(ary)
        	strRet = sprintf( "{0},'{1}'", Array(strRet, ary(i)) )
    	Next
    ...
    Function utxStrAddStr(src1, src2, strAppend)	
		...
		Else
			If (src1 = "") Then 			
				strRet = src2			
			Else
                strRet = sprintf(strAppend,  Array(src1, src2) ) 
			End If
		End If
