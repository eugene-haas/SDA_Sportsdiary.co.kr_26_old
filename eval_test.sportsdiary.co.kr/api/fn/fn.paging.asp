<%
	...
	' 수영
	Sub userPagingScript2 (ByVal total_page, ByVal block_size, ByVal page, ByVal function_name)
		Dim max_page, min_page, i
	...
	'루키
    Sub userPagingT (ByVal total_page, ByVal block_size, ByVal page, ByVal function_name, ByVal packet)
		Dim max_page, min_page, i
		...
		For i = min_page To max_page
			If Int(i) = Int(page)  Then
				response.write "<li class=""active""><a href=""javascript:;"">" & i & "</a></li>" & vbcrlf
			Else
				response.write "<li><a href=""javascript:"" onclick='" & function_name & "(" &packet&"," & i & ")' > " & i & " </a></li>" & vbcrlf
			End if
		Next

		If max_page < total_page Then
			response.write "<li class=""next""><a href=""javascript:"" onclick='" & function_name & "(" &packet&", " & max_page + 1 & ")'>&gt;</a></li>" & vbcrlf
		Else
			response.write "<li class=""next""><a href=""javascript:alert('마지막 페이지 입니다.');"">&gt;</a></li>" & vbcrlf
		End If
		response.write "</ul>" & vbcrlf
	...
	'수영사용
	Sub userPagingT2 (ByVal total_page, ByVal block_size, ByVal page, ByVal function_name, ByVal packet)
	Dim max_page, min_page, i
	...
	response.write "<div class='container-fluid text-center'>" & vbcrlf
	response.write "<ul class='pagination'>" & vbcrlf
...
Sub userPagingRH (ByVal total_page, ByVal block_size, ByVal page, ByVal function_name, ByVal packet)
	Dim max_page, min_page, i
	...
	If min_page > block_size Then
		response.write "<button tabindex=""-1"" class="""" type=""button"" name=""btnPrev"" onclick='" & function_name & "("&packet&", "& min_page-1 &")'><strong class=""hide"">이전 리스트</strong></button>"
	Else
		response.write "<button tabindex=""-1"" class="""" type=""button"" name=""btnPrev"" onclick=""alert('첫 페이지 입니다.')""><strong class=""hide"">이전 리스트</strong></button>"
	End if

	response.write "<ul>"
	For i = min_page To max_page
		If Int(i) = Int(page)  Then
			response.write "<li class=""tbl-nav__list s_on""><button type=""button"" name=""button"" data-list=""1"" onclick=""javascript:;"">"&i&"</button></li>"
		Else
			response.write "<li class=""tbl-nav__list""><button type=""button"" name=""button"" data-list=""1"" onclick='" & function_name & "(" &packet&"," & i & ")' >"&i&"</button></li>"
		End if
	Next
	response.write "</ul>"

	If max_page < total_page Then
		response.write "<button tabindex=""-1"" class="""" type=""button"" name=""btnNext"" onclick='" & function_name & "(" &packet&", " & max_page + 1 & ")'><strong class=""hide"">다음 리스트</strong></button>"
	Else
		response.write "<button tabindex=""-1"" class="""" type=""button"" name=""btnNext"" onclick=""alert('마지막 페이지 입니다.')""><strong class=""hide"">다음 리스트</strong></button>"
	End If
