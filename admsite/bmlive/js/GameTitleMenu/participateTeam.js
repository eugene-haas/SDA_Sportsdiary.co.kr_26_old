...
function inputGameParticipateTeam_frm(NowPage) {
  ...
  var tTeamSido = $( "#hiddenSido" ).val(); 
  var tTeamGugun = $( "#hiddenGugun" ).val(); 
  ...
  packet.tTeamSido = tTeamSido;
  packet.tTeamGugun = tTeamGugun;
    ...
    source : function( request, response ) {
      $.ajax(
                ...
                response(
                    $.map(data, function(item) {
                        return {
                            ...
                            label: item.teamNm + "(" + item.teamCode + ")/[" + item.PTeamGb + "]/[" + item.teamtype + "]",
                            value: item.teamNm,
                            team: item.team,
                            teamCode: item.teamCode,
                            teamSido: item.teamSido,
                            teamGugun: item.teamGugun
                        }
                    })
                );
            }
        ...
        select: function( event, ui ) {
          ...
          obj.teamSido = ui.item.teamSido
          obj.teamGugun = ui.item.teamGugun
          $("#tdTeam").text(obj.value)
          $("#hiddenTeamName").val(obj.value)
          $("#hiddenTeam").val(obj.team)
          $("#hiddenSido").val(obj.teamSido)
          $("#hiddenGugun").val(obj.teamGugun)
        },
