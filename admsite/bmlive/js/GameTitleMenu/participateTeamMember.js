...
function inputGameParticipateTeamMember_frm(NowPage) {
  ...
  var tTeamdtl = $( "#hiddenTeamdtl" ).val(); 
  var tSido = $( "#hiddenSido" ).val(); 
  var tGugun = $( "#hiddenGugun" ).val();   

  if(tMemberIdx == ""){
    alert("선수명을 검색하여, 선택해야 등록 가능합니다.");
    return;
  }
    ...
    source : function( request, response ) {
      ...
      $.ajax(
            ...
            success: function(data) {
                ...
                response(
                    $.map(data, function(item) {
                        return {
                          label: item.RegYear + "년 " + item.data + "("+item.teamNm + ") / " + item.tEnterType + "[" + item.SidoNM + " " + item.GuGunNM + "]",
                          value: item.data,
                          uidx:item.uidx,
                          team : item.team,
                          teamNm : item.teamNm,
        ...
        select: function( event, ui ) {
          ...
          $("#hiddenSido").val(obj.sido)
          $("#hiddenGugun").val(obj.gugun)          
        },
