...
function OnReceiveAjax(CMD, dataType, htmldata, jsondata) {
  switch(CMD) {
    ...
    case CMD_INSERTGAMELEVELDTL :
    {
      ...
      console.log("결과:" + jsondata.tGameLevelIdx);
      //return;
      href_gameLevel2 (jsondata, jsondata.beforeNowPage)
    }break;
...
function inputGameLevelDtl_frm(packet){
  ...
  var tGameIdx = $( "#selGameIdx" ).val();                           // Game Title Idx
  var tPlayLevelType = $( "#selPlayLevelType" ).val();               // 예선, 본선, 예선+본선, 결선
  var tGameType = $( "#selGameType" ).val();                         // 리그, 토너먼트 
  //var tSelStadium = $( "#selStadiums" ).val(); 
  var tTotalRound = $( "#selTotalRound" ).val();                     // 2강, 4강, ... 512강 
  var tEntryCnt = $( "#txtEntryCnt" ).val(); 
  //var tGameDay = $( "#selGameDay" ).val(); 
  //var tGameTime = $( "#selGameTime" ).val(); 
  var tViewYN = $( "#selViewYN" ).val();                             // 노출, 미노출
  var tGameLevelIdx = $( "#selGameLevelIdx" ).val();                 // GameLevelIdx
  var tLevelJooNum = $( "#selLevelJooNum" ).val();                   // 선택된 조 ( 미선택, 예선 1조, 2조, ... )
  var tJooDivision = $( "#txtJooDivision" ).val();                   // 조생성 갯수 
  var tFullGameYN = $("#selFullGameYN").val();                       // 풀게임 여부 ( 미선택, 풀게임)

  var tJooRank = $("#txtJooRank").val();                             // 본선진출 txt value
  ...
  if(tJooRank == ""){
    alert("예선에서 본선으로 진출하는 팀 갯수를 입력하세요.(해당 팀갯수로, 본선 토너먼트 강수가 결정됩니다.)");
    return;
  }

  if(parseInt(tJooDivision) > 50){
    alert("예선조 갯수는 50개 초과로 입력하실 수 없습니다. ");
    return;
  }
...
function chg_fullgame(tobj){
  if(tobj == "Y"){
    $("#txtJooDivision").val("1")
    $("#txtJooRank").val("1")
  }
  else{
    $("#txtJooDivision").val("0")
    $("#txtJooRank").val("")
  }
}
...
function updateGameLevelDtl_frm(packet){
  ...
  var tLevelJooNum_SubNM= $("#txtLevelJooNum_SubNM").val();
 ...
 // packet.tGameDay = tGameDay
  ...
  packet.tFullGameYN = tFullGameYN;
  packet.tJooRank = tJooRank;
  packet.tLevelJooNum_SubNM = tLevelJooNum_SubNM;
  //packet.tLevelJoo = tLevelJoo;
  //packet.tLevelJooNum = tLevelJooNum;
  //console.log(packet)
  SendPacket(Url, packet);
...
function LotteryEmpty_frm(tGameIdx, tGameLevelDtlIDX, tGameLevelIdx, strnum, tGroupGameGb){

  var Url = "";

  var bool_con = confirm("기존대진표를 삭제하고, 빈대진표를 생성합니다. 동의하시면 확인버튼을 눌러주세요.");
  ...
  //단체전일때
  if(tGroupGameGb == "B0030002"){
    Url = "/Ajax/GameTitleMenu/LotteryTeamEmpty_frm.asp";
  }
  else{
    Url = "/Ajax/GameTitleMenu/LotteryEmpty_frm.asp";
  }

  var tEmptyCnt = $( "#EmptyCnt_" +  strnum).val(); 

  var packet = {};

  packet.CMD = CMD_LOTTETYEMPTY;
  packet.tGameIdx = tGameIdx;
  packet.tGameLevelDtlIDX = tGameLevelDtlIDX;
  packet.tGameLevelIdx = tGameLevelIdx;
  

  packet.tEmptyCnt = tEmptyCnt;
...
href_gameLevel2 = function(packet, NowPage){
  post_to_url('./levelDtl.asp', { 'tIdx': packet.tIdx,'tGameLevelIdx': packet.tGameLevelIdx,'tPGameLevelIdx':packet.tPGameLevelIdx,'pType':packet.pType,'iSearchText':packet.iSearchText ,'iSearchCol' :packet.iSearchCol,'i2': NowPage,'NowPage':NowPage, 'beforeNowPage':packet.beforeNowPage});
};
...
href_LotteryPublic = function (tIdx,tGameLevelDtlIdx, tGameLevelidx) {
  post_to_url('./Lottery_PublicTest.asp', { 'tIdx': tIdx, 'tGameLevelDtlIdx': tGameLevelDtlIdx, 'tGameLevelidx': tGameLevelidx});
};
