...
function updateGameLevel_frm(NowPage) {
  ...
  var schRallyPoint = $( "#schRallyPoint" ).val();
  var schDeuceYN = $( "#schDeuceYN" ).val();
  var schMaxPoint = $( "#schMaxPoint" ).val();

  var MaxUnit1 = $( "#MaxUnit1" ).val();
  var MaxUnit2 = $( "#MaxUnit2" ).val();
  var MaxUnit3 = $( "#MaxUnit3" ).val();
  var AttendLimitPerPlayer = $( "#AttendLimitPerPlayer" ).val();
  ...
  //console.log(packet)
  packet.schRallyPoint = schRallyPoint;
  packet.schDeuceYN = schDeuceYN;
  packet.schMaxPoint = schMaxPoint;

  packet.MaxUnit1 = MaxUnit1;
  packet.MaxUnit2 = MaxUnit2;
  packet.MaxUnit3 = MaxUnit3;
  packet.AttendLimitPerPlayer = AttendLimitPerPlayer;
...
function OnGroupGameGbChanged(GroupGameGb){
  if(GroupGameGb =="B4E57B7A4F9D60AE9C71424182BA33FE") {
    ...
    document.getElementById("GroupGameGb_sub1").style.visibility = "hidden";
    document.getElementById("GroupGameGb_sub2").style.visibility = "hidden";
    document.getElementById("GroupGameGb_sub3").style.visibility = "visible";
  }else{
    ...
    OnGroupGameGbChanged_type(document.getElementById("selPlayType").value);
  }
};

function OnGroupGameGbChanged_type(GameType){
  if(GameType =="704C5971F9D17ABC8687A215715ABCE6") {
    document.getElementById("GroupGameGb_sub1").style.visibility = "visible";
    document.getElementById("GroupGameGb_sub2").style.visibility = "hidden";
    document.getElementById("GroupGameGb_sub3").style.visibility = "hidden";
  }else{
    document.getElementById("GroupGameGb_sub1").style.visibility = "hidden";
    document.getElementById("GroupGameGb_sub2").style.visibility = "visible";
    document.getElementById("GroupGameGb_sub3").style.visibility = "hidden";
  }
};
...
function selectLevel(sexValue, playTypeValue){
  selObject.Sex= sexValue;
  selObject.PlayType = playTypeValue;
  searchLevelList();
};

function selectLevelJoo(obj){
  selObject.LevelJoo = obj.value;
  searchLevelList();
};


function searchLevelList()
{
  Url = "/Ajax/GameTitleMenu/Level_LevelSearch.asp"
  var selNowPage = $( "#selNowPage" ).val();
  var selGameTitleIdx = $( "#selGameTitleIdx" ).val();
  var packet = {};
  packet.CMD = CMD_SEARCHLEVEL;
  packet.tGameTitleIDX = selGameTitleIdx;
  packet.tSex = selObject.Sex;
  packet.tPlayType = selObject.PlayType;
  packet.tLevelJoo = selObject.LevelJoo;
  packet.NowPage = selNowPage;
...
href_LevelDtl = function(packet){
  post_to_url('./artest_levelDtl.asp', { 'tIdx': packet.tIdx,'tGameLevelIdx': packet.tGameLevelIdx,'pType':"level", 'beforeNowPage':packet.NowPage});
};


href_LevelDtl = function(GametitleIdx, GameLeveIdx, NowPage) {
  post_to_url('./artest_levelDtl.asp', { 'tIdx': GametitleIdx,'tGameLevelIdx': GameLeveIdx,'pType':"level", 'beforeNowPage':NowPage});
};

href_LevelSum = function(GametitleIdx){
  post_to_url('./levelRequestSumParticipate.asp', { 'tIdx': GametitleIdx});
};


href_LevelMove = function(GametitleIdx, GameLeveldtlIDX){
  post_to_url('./levelRequestMoveParticipate.asp', { 'tIdx': GametitleIdx, 'tGameLevelIDX': GameLeveldtlIDX});
};
...
href_Participate = function(GametitleIdx, GameLeveIdx, NowPage) {
  post_to_url('./participate.asp', { 'tIdx': GametitleIdx,'tGameLevelIdx': GameLeveIdx, 'pType':"level", 'beforeNowPage':NowPage});
};
...
href_TeamGameOrder = function(tIdx,tGameLevelIdx){
  post_to_url('./teamGameOrder.asp', { 'tIdx': tIdx,'tGameLevelIdx': tGameLevelIdx});
};
