...
CMD_GAMERESET = 23;
CMD_GAMETEAMRESET = 24;

CMD_CHANGEPERSONSTADIUM = 25;

CMD_PRINTUPDATEYN = 26;
////////////명령어////////////
...
function OnReceiveAjax(CMD, dataType, htmldata, jsondata) {
  ...
  switch(CMD) {
    ...
    { 
      ...
      if("F9A43D4DE4191C125B08095CC465CD4B" == jsondata.GroupGameGb || jsondata.GroupGameGb == "B0030001"){
        OnSearchClick();
        return;
      }
    ...
    { 
      ...
      if(jsondata.GroupGameGb == "B0030001" || jsondata.GroupGameGb == "F9A43D4DE4191C125B08095CC465CD4B"){
    ...
    { 
      if(jsondata.before_CMD == CMD_EDITRESULT){
        ...
        //개인전
        if(jsondata.GroupGameGb == "F9A43D4DE4191C125B08095CC465CD4B" || jsondata.GroupGameGb == "B0030001"){
    ...
    case CMD_GAMETEAMRESET:
    if(dataType == "json")
    { 
      alert("단체전 결과 리셋이 완료 되었습니다.");
      $(".group_modal").modal("hide");
      OnSearchClick();
    }     
    break;      
    ...
    if(dataType == "json")
    { 
      alert("개인전 결과 리셋이 완료 되었습니다.");
      $(".play_detail_modal").modal("hide");
      OnSearchClick();
    }     
    break;  
    
    case CMD_PRINTUPDATEYN:
    ...
    { 
      ...
      if(jsondata.tPrintYN == "Y")
      {
        $("#PrintBtn" + jsondata.tStrNum).html("출력완료");
        $("#PrintBtn" + jsondata.tStrNum).attr("class","");
      }
      else{
        if(jsondata.strPlayLevelType == "B0100001")
        {
          $("#PrintBtn" + jsondata.tStrNum).html("예선출력");
          $("#PrintBtn" + jsondata.tStrNum).attr("class",jsondata.tPrintBtnColor);
        }
        else if(jsondata.strPlayLevelType == "B0100002")
        {
          $("#PrintBtn" + jsondata.tStrNum).html("본선출력");
          $("#PrintBtn" + jsondata.tStrNum).attr("class",jsondata.tPrintBtnColor);
        }
      }
    }     
...
function OnAnthResultDtlBWFChecked(str, resultbwfdtl)
{ 

  for (var i = 1; i <= $("a[name=AnthResultDtlBWF]").length; i++) {

    if (i == parseInt(str)) {

        console.log("선택");
        if ($("#DP_AnthResultDtlBWF_" + str).attr("class") == "white_btn") {
            $("#DP_AnthResultDtlBWF_" + str).attr("class", "red_btn");
            AnthResultChk.AnthResultDtlBWF = resultbwfdtl;


        }
        else {
            $("#DP_AnthResultDtlBWF_" + str).attr("class", "white_btn");
            AnthResultChk.AnthResultDtlBWF = "";

        }

    }
    else{
      $("#DP_AnthResultDtlBWF_" + i).attr("class", "white_btn");
    }
...
function onEditResultClick(gameleveldtlidx, teamgamenum, gamenum, gameround){
  ...
  packet.AnthResultDtlBWF = AnthResultChk.AnthResultDtlBWF;

  SendPacket(Url, packet);     
  
}

function OnGameResultReset(gameleveldtlidx, teamgamenum, gamenum){

  var con_bool = confirm("해당 경기결과(승패)를 리셋합니다. 동의하시면 확인버튼을 눌러주세요.");
  ...
  Url ="/Ajax/GameTitleMenu/GameReset.asp";

  packet.CMD = CMD_GAMERESET;
  packet.GameLevelDtlIDX = gameleveldtlidx;
  packet.TeamGameNum = teamgamenum;
  packet.GameNum = gamenum;  
...
function OnGameResultReset_Team(gameleveldtlidx, teamgamenum, gamenum){

  var con_bool = confirm("해당 단체전 경기결과(승패)를 리셋합니다. 동의하시면 확인버튼을 눌러주세요.");
  ...
  Url ="/Ajax/GameTitleMenu/GameReset_Team.asp";

  packet.CMD = CMD_GAMETEAMRESET;
  packet.GameLevelDtlIDX = gameleveldtlidx;
  packet.TeamGameNum = teamgamenum;
  packet.GameNum = gamenum;  
...
function OnSearchClick(liveSearch = false){
    ...
    Url = "/Ajax/GameTitleMenu/GameOrder_Operate.asp"
...
function OnGroupResultDtlBtnExcelClick_elite(gametitleidx, gameleveldtlidx, teamgamenum, gamenum, setnum, tourneygroupidx){
  GameLevelDtlIDX = gameleveldtlidx;
  TeamGameNum = teamgamenum;
  GameNum = gamenum;
  SetNum = setnum;
  ...
  /* 
		2019 배드민턴국가대표선발전
		제36회 한.일국가대항 배드민턴경기대회
		2019년 한.일 우수청소년배드민턴대회     
		
		심판입력 프로그램을 새로 고치면서 Misconduct 저장 방식이 바뀌어서
		스코어 시트를 2가지 종류로 제공한다. 
		
		위 대회만 예전버전 파일(scorepage_excel_inter2.asp)
		나머지대회는 최신파일(scorePage_Excel_interpage.asp)
	*/

  if(gametitleidx == "1176" || gametitleidx == "1294" || gametitleidx == "1328"){
    Url = "/Main/GameTitleMenu/scorePage_Excel_inter2.asp?" 
    + "GameLevelDtlIDX=" + gameleveldtlidx
    + "&TeamGameNum=" + teamgamenum
    + "&GameNum=" + gamenum
    + "&IsPrint=1";
    ...
    Url = "/Main/GameTitleMenu/scorePage_Excel_interpage.asp?" 
    + "GameLevelDtlIDX=" + gameleveldtlidx
    + "&TeamGameNum=" + teamgamenum
    + "&GameNum=" + gamenum
    + "&IsPrint=1";    
...
function OnGroupResultPrintClick(gameleveldtlidx, teamgamenum, gamenum, isprint, groupgamegb, tempnum){
  ...
  Url = "/Main/GameTitleMenu/scorePageGroup_empty_Excel_New.asp?" 
  + "GameLevelDtlIDX=" + gameleveldtlidx
  + "&TeamGameNum=" + teamgamenum
  + "&GameNum=" + gamenum
  + "&GroupGameGb=" + groupgamegb
...
function PrintOneGame(gametitleidx, gameleveldtlidx, teamgamenum, gamenum, stadiumNum, tempNum, listnum, entertype, groupgamegb){

  if(entertype == "E"){

    if(groupgamegb =="B0030001" || groupgamegb == "F9A43D4DE4191C125B08095CC465CD4B"){
      /* 
        2019 배드민턴국가대표선발전
        제36회 한.일국가대항 배드민턴경기대회
		  2019년 한.일 우수청소년배드민턴대회     
      ...
      */
      ...
      /*
      Url = "/Main/GameTitleMenu/scorepage_excel_inter_Team.asp?" 
      + "GameLevelDtlIDX=" + gameleveldtlidx
      + "&TeamGameNum=" + teamgamenum
      + "&GameNum=" + gamenum
      + "&StadiumNum=" + stadiumNum
      ...
      + "&IsPrint=0";      
      */
      Url = "/Main/GameTitleMenu/scorepagegroup_elite.asp?"
      + "GameLevelDtlIDX=" + gameleveldtlidx
      + "&TeamGameNum=" + teamgamenum
      + "&GameNum=" + gamenum
      + "&StadiumNum=" + stadiumNum
    ...
    Url = "/Main/GameTitleMenu/GamePaper.asp?" 
    + "GameLevelDtlIDX=" + gameleveldtlidx
    + "&TeamGameNum=" + teamgamenum
    + "&GameNum=" + gamenum
    + "&StadiumNum=" + stadiumNum
...
function excel_GameOrder_list(){
  ...
  Url = "/Main/GameTitleMenu/GameOrder_List_Excel.asp?" 
...
function print_EliteOrder_list(){
  ...
  Url = "/Main/GameTitleMenu/scorePage_Excel.asp?" + "GameTitleIDX=" + GameTitleIDX;
...
//경기진행현황 리스트
function print_GameOrder_Select(){
  ...
  $("input[name=chk_Operate]:checked").each(function() {

    str_reqnum = $(this).data('id'); 
    $("#PrintBtn" + str_reqnum).html("출력완료");
    $("#PrintBtn" + str_reqnum).attr("class","");  
  ...
  if(str_reqidx == "") {
    alert('출력하실 경기 체크박스를 선택하시기 바랍니다.');
    return;
  }

  $("#GameOperateIDX").val(str_reqidx);

  Url = "/Main/GameTitleMenu/gamePaper_a5.asp?" 
  + "GameTitleIDX=" + GameTitleIDX
  + "&GameDay=" + GameDay
  + "&StadiumIDX=" + StadiumIDX
  + "&StadiumNumber=" + StadiumNumber
...
function chg_TeamPersonStadiumNum(gameleveldtlidx,teamgamenum, gamenum, tobj){
  var packet = {};

  GameLevelDtlIDX = gameleveldtlidx;
  TeamGameNum = teamgamenum;
  GameNum = gamenum;
  PersonStadiumNum = $(tobj).val();


  Url = "/Ajax/GameTitleMenu/ChangePersonStadium.asp"

  packet.CMD = CMD_CHANGEPERSONSTADIUM;
  packet.tGameLevelDtlIDX = GameLevelDtlIDX;
  packet.tTeamGameNum = TeamGameNum;
  packet.tGameNum = GameNum;
  packet.tPersonStadiumNum = PersonStadiumNum;
...
function OnPrintChanged(gameleveldtlidx,teamgamenum, gamenum, strnum, printbtncolor){
  ...
  GameNum = gamenum;
  StrNum = strnum;
  PrintBtnColor = printbtncolor;

  if( $('input:checkbox[id="Print_Update_' + strnum + '"]').is(":checked") == true){
    PrintYN = "Y";
  }
  else{
    PrintYN = "N";
  ...
  Url = "/Ajax/GameTitleMenu/PrintUpdateYN.asp"

  packet.CMD = CMD_PRINTUPDATEYN;
  
  packet.tGameLevelDtlIDX = GameLevelDtlIDX;
  packet.tTeamGameNum = TeamGameNum;
  packet.tGameNum = GameNum;
  packet.tStrNum = StrNum;
  packet.tPrintYN = PrintYN;
  packet.tPrintBtnColor = PrintBtnColor;
...
var liveObj = null;
var liveObjCnt = 0;
var liveInterval = 60000;
function SearchLive()
{
  var isLiveChecked = $('input:checkbox[id="checkSearchLive"]').is(":checked");

  if(isLiveChecked) {
    liveObj = setInterval("LiveFunc()", liveInterval);
    
  }
  else{
    clearInterval(liveObj);
  }
}
