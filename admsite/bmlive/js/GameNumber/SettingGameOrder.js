...
function OnReceiveAjax(CMD, dataType, htmldata, jsondata) {
  ...
  switch(CMD) {
      ...
      case CMD_UPDATEGAMETOURNEY :{
        ...
        OnGameScheduleSearch(jsondata.int_scroll);
...
function OnGameScheduleSearch(int_scroll) {
...
function BoardControl(){
  ...
  var popUrl = "../GameTitleMenu/GameStadiumBoard.asp"; //팝업창에 출력될 페이지 URL
  ...
  post_to_url_popup('JoResult', './' + "../GameTitleMenu/GameStadiumBoard.asp", urltext);  
}
        ...
        if (mx.GameLevelDtlIDX != "" && 
              mx.TeamGameNum != "" && 
                mx.GameNum != "" && 
                  mx.GroupGameGb != "" )
        {
          console.log("기존 위치 gameLevelDtlIdx : " + mx.GameLevelDtlIDX + " gameGameNum : " + mx.GameNum + "gameTeamGameNum :" +  mx.TeamGameNum + ", gameGroupGb : " + mx.GroupGameGb )
          ...
          if ((mx.GameLevelDtlIDX != gameLevelDtlIdx) || (mx.GameNum !=  gameGameNum) || (mx.TeamGameNum != gameTeamGameNum) || (mx.GroupGameGb != gameGroupGb))
          {
            if(confirm("코트 :" + gameCourt + ", 순서 " + gameOrder))
            {
              ...
              //암호화해야하는 URL
              var int_scroll = $("#DP_OperateOrder").scrollTop();

              console.log(int_scroll);
