...
<script type="text/javascript" src="GameURL.js?ver=8.00"></script>
...
<script>
  href_Move = function(hrefMove){
    ...
    GroupGameGbValue = $("#selGroupGameGb").val();
  ...
  $(function(){
    $('.play_detail_modal').on('hide.bs.modal', function (e) {
      $('.play_detail_modified').modal('hide');
    })

    $('.play_detail_modal').on('click', '._play_detail_modified_trigger', function(e){
      $('.play_detail_modified').modal('show');
    })
  })
...
function PrintListSchedule()
{
  ...
  selGroupGameGb = $("#selGroupGameGb").val();
  selPlayLevelType = $("#selPlayLevelType").val();
  selStadiumNumber = $("#selStadiumNumber").val();
  ...
  if(GameTitleIdx!="" && selStadiumIDX != "" && selGameDay != "" ) {
  // if (mx.GameTitleIdx != "" && selStadiumIDX != "" && selGameDay != "" ) {
	Url = "/Main/GameNumber/Print_SettingGameOrderList_Go.asp?"
    + "GameTitleIdx=" + GameTitleIdx  + "&StadiumIdx=" + selStadiumIDX + "&GameDay=" + selGameDay
	+ "&txtSearchName=" + txtSearchName + "&selGroupGameGb=" + selGroupGameGb
	+"&selPlayLevelType="+selPlayLevelType+"&selStadiumNumber="+selStadiumNumber;
    location.href = Url;
  }
  else {
...
function PrintStadiumSchedule()
{

  var GameTitleIdx = $("#selGameTitleIdx").val();

  if(GameTitleIdx == ""){
    alert("출력 하실 대회를 선택하세요.");
    return;
  }

  Url = "/Main/GameNumber/Print_SettingStadiumList.asp?"
  + "GameTitleIDX=" + GameTitleIdx +"&StadiumIdx=&GameDay=";
...
<!-- S: content -->
<div class="Game_operation" id="DP_BODY">
  <h2 class="t_title">경기동영상 URL</h2>
  <!-- s: 대회선택 -->
  ...
  <%
      ...
      IF (Admin_Authority <> "O") Then
        ...
        LSQL = " SELECT GameTitleIDX, GameTitleName, EnterType"
        ...
        IF cdbl(tblGameTitleCnt) = 0 Then
          LSQL = " SELECT Top 1 GameTitleIDX, GameTitleName, EnterType"
          LSQL = LSQL & " FROM tblGameTitle "
      ...
      <select id="selGameTitleIdx" name="selGameTitleIdx"  onchange="OnGameTitleChanged(this.value)">
        <option value="">::대회 선택::</option>
          <%
              ...
              LSQL = " SELECT a.GameTitleIDX, a.GameTitleName, a.EnterType"
  ...
  <div class="search_box" id="DP_SelBox" >
    ...
    <option value="">경기유형 선택</option>
      ...
      LSQL = LSQL & " FROM dbo.tblTourney A "
      LSQL = LSQL & " INNER join dbo.tblGameLevel C ON A.GameLevelIdx = C.GameLevelidx AND C.DelYN ='N' "
      LSQL = LSQL & " WHERE A.DelYN = 'N' "
      LSQL = LSQL & " AND A.GameTitleIDX = '" & reqGameTitleIdx & "'"
      IF reqGameDay <> "" Then
        LSQL = LSQL & " AND A.GameDay = '" & reqGameDay & "'"
    ...
    <select id="selPlayLevelType" onChange='OnSearchChanged()'>
      <option value="">경기구분 선택</option>
        ...
        LSQL = LSQL & " INNER JOIN dbo.tblGameTitle B ON B.GameTitleIDX = A.GameTitleIDX "
        LSQL = LSQL & " INNER join dbo.tblGameLevelDtl C ON A.GameLevelDtlidx = C.GameLevelDtlidx AND C.DelYN ='N'"
        LSQL = LSQL & " INNER join dbo.tblGameLevel E ON A.GameLevelIdx = E.GameLevelidx AND E.DelYN ='N'  "
        LSQL = LSQL & " WHERE A.DelYN = 'N' "
        LSQL = LSQL & " AND A.GameTitleIDX = '" & reqGameTitleIdx & "'"
    ...
    <select id="selIngType">
      <option value="">진행상태 선택</option>
      <option value="GameEmpty">대기</option>
      <option value="GameIng">경기중</option>
      <option value="GameEnd">경기종료</option>
    </select>

    <select id="selGameURLYN">
      <option value="">URL입력여부 선택</option>
      <option value="N">미입력</option>
      <option value="Y">입력완료</option>
    </select>    

    <input id="txtSearchName" type="text" placeholder="팀명 및 선수명을 검색하세요" value="<%=reqSearchName%>" style="width:300px;position:relative;top:7px;">
    <a href="javascript:OnSearchClick();" class="gray_btn" style="position:relative;top:7px;">검색</a>
  </div>
  <!-- e: 검색 -->
  <!-- s: 태아불 상단 버튼 -->
  <div class="top_btn_list">
 <!-- 190115 SEH 이석종 매니저 요청 [경기 진행 결과 출력]  버튼 삭제 후 새로 추가한 [리스트 출력] 을 [경기진행결과출력]으로 명칭 변경 -->
  <!--  <a href="javascript:excel_GameOrder_list();">
	  ...
	  <span>  <font color="#FF0000">총 경기 :</font> <span id="DP_TotalCnt">-</span>경기 /<font color="#FF0000"> 잔여 경기 : </font><span id="DP_RestCnt">-</span>경기</span>
  ...
  <div>
    <form name="optfrm">
      <input type="hidden" id="GameOperateIDX" name="GameOperateIDX" value="">
     
    </form>
    <table class="match-order" style="width:95%;">
      <thead>
        <tr>
          <th rowspan="2" style="width:50px;"><input type="checkbox" name="all_reqidx" id="all_reqidx" onclick="checkall_list();">선택</th>
          <th rowspan="2" style="width:50px;">코트</th>
          <th rowspan="2" style="width:50px;">운영<br/>코트</th>
          <th rowspan="2" style="width:50px;">순서</th>
          <th rowspan="2" style="width:50px;">시간</th>
          <th rowspan="2" style="width:70px;">단체전오더</th>
          <!-- 190102 SS  출력 단체전 오더등록 컬러을 승패결과 쪽으로 이동 요청 건 -->

  		    <th rowspan="2">종목</th>
          <!--<th>경기 타입</th>-->

          <th rowspan="2" style="width:180px;">편성표(경기번호)</th>
          <th colspan="2">팀1</th>
          <th rowspan="2" style="width:55px;">승/패</th>
          <th colspan="2">팀2</th>
          <th rowspan="2" style="width:55px;">승/패</th>
          <th rowspan="2" style="width:105px;">그외판정</th>
          <th rowspan="2" style="width:405px;">경기동영상 URL</th>
        </tr>
        <tr>
          <th>소속</th>
          <th>선수</th>
      ...
      <tbody id="tbGameOrder" name="tbGameOrder">
        <tr>
  ...
  <script>
    ...
    function popupListOpen(addrs, gametitleidx, groupgamegb, teamgb, playtypesex, level){
      ...
      tGameTitleIdx = gametitleidx;
      GroupGameGbValue = groupgamegb;
      PlayTypeSexValue = playtypesex;
      TeamGbValue = teamgb;
      LevelValue = level;
    ...
    function popupTourneyOpen(addrs, gametitleidx, groupgamegb, teamgb, playtypesex, level, gamelevelidx){
      ...
      post_to_url_popup('JoResult1', './' + addrs, { 'GameTitle': tGameTitleIdx,'GroupGameGb': GroupGameGbValue,'TeamGb': TeamGbValue,'Level': LevelValue, 'PlayType': PlayTypeSexValue, 'GameLevelIDX' : GameLevelIDX });
...
<!-- S: group-order -->
<!-- #include file = "group.order.asp" -->
<!-- E: group-order -->

<!-- S: live-score -->
<!-- #include file = "live.score.asp" -->
<!-- E: live-score -->

<!-- S: etc-judge -->
<!-- #include file = "etc.judge.asp" -->
<!-- E: etc-judge -->

<!-- S: winner-sign -->
<!-- include file = "winner.sign.asp" -->
<!-- E: winner-sign -->

<!-- S: play_detail_modified -->
<!-- #include file = "play_detail_modified.asp" -->
<!-- E: play_detail_modified -->

  <!-- S: winner-sign -->
  <div class="modal fade group_modal">
    <!-- S: modal-dialog -->
    <div class="modal-dialog">
      <!-- S: modal-content -->
      <div class="modal-content" id="DP_GroupResult_Popup">
        <!-- S: modal-body -->
          ...
          <!-- s: 그외판정결과 선택 -->
					<div class="select_results">
						<div class="l_con">
							...
							<span class="belong_name">서울B</span>
						</div>
						<div class="c_con">
							<span>그 외 판정결과 선택</span>
						</div>
						<div class="r_con">
							<span class="belong_name">제주</span>
							<span class="belong_title">소속</span>
						</div>
					</div>
					<!-- e: 그외판정결과 선택 -->
					...
					<div class="table_list_2">
						<table cellspacing="0" cellpadding="0">
							...
							<tr>
								...
								<td>
									<img src="data:image/png;base64," width="79" height="49" alt="">
								</td>
								<td>
									<span>혼합복식</span>
								</td>
								...
								<td>
									<a href="#" class="modified_btn">수정</a>
								</td>
							</tr>
							<tr>
								<td>
