...
function view_frm(tp){
	...
	//정렬관련필드
	var order_Sports      = sf.order_Sports.value;
	var order_TeamGb      = sf.order_TeamGb.value;
	var order_Area        = sf.order_Area.value; 
	var order_SchoolName  = sf.order_SchoolName.value;
	var order_Sex         = sf.order_Sex.value;
	var order_PlayerName  = sf.order_PlayerName.value;
	var order_PlayerIDX   = sf.order_PlayerIDX.value;
	...
	$.ajax({
			...
			data: {
				...
				order_Sports      : order_Sports ,
				order_TeamGb      : order_TeamGb ,
				order_Area        : order_Area ,
				order_SchoolName  : order_SchoolName ,
				order_Sex         : order_Sex ,
				order_PlayerName  : order_PlayerName ,
				order_PlayerIDX   : order_PlayerIDX
...
function input_view(){
	if(document.getElementById("input_area").offsetHeight>0){
		...
		document.getElementById("input_button_type").className = "fa fa-sort-desc";
	}else{
		document.getElementById("input_area").style.display="block";
		document.getElementById("input_button_type").className = "fa fa-minus";
	}						
}
/*개인프로필열고 닫기E*/
...
/*아이디 중복확인 팝업창S*/
function pop_UserID(){
	window.open('/Manager/Popup/UserID_Search.asp','_blank','width=600 height=600');
}
/*아이디 중복확인 팝업창E*/
...
function chk_orderby(obj){
	var sf = document.search_frm;
	var input_value = eval("document.search_frm."+obj);
	if(document.getElementById(obj).innerHTML=="▼"){
		document.getElementById(obj).innerHTML = "▲";
		input_value.value ="ASC";
	}else{
		document.getElementById(obj).innerHTML = "▼";
		input_value.value = "DESC";		
	}
	view_frm('F');
}
</script>
	...
	<section>
		<div id="content">
			...
			<div class="top-navi">
				<div class="top-navi-inner">
					...
					<div class="top-navi-tp">
						...
						<a href="#" class="btn-top-sdr" title="닫기" onclick="input_view();"><i id="input_button_type" class="fa fa-minus" aria-hidden="true"></i></a>
					</div>
			...
			<div class="sch-result">
				전체 (<strong id="totcnt">0</strong>)건 / 현재(<strong id="nowcnt">0</strong>) 
				<a href="#" class="btn-more-result" id="btnnext" onClick="javascript:view_frm('N');">
					<span>검색결과 더보기</span> <i class="fa fa-plus" aria-hidden="true"></i>
				</a>
			</div>
			<div class="table-list-wrap">
				...
				<table class="table-list">
					...
					<thead>
						<tr>
							<th scope="row">번호</th>
							<th scope="row"><a href="javascript:chk_orderby('order_Sports');">경기종목 <span id="order_Sports" >▼</span></a></th>
							<th scope="row"><a href="javascript:chk_orderby('order_TeamGb')">소속구분 <span id="order_TeamGb">▼</span></a></th>
							<th scope="row"><a href="javascript:chk_orderby('order_Area')">지역 <span id="order_Area">▼</span></a></th>
							<th scope="row"><a href="javascript:chk_orderby('order_SchoolName')">소속명 <span id="order_SchoolName">▼</span></a></th>
							<th scope="row"><a href="javascript:chk_orderby('order_Sex')">성별 <span id="order_Sex">▼</span></a></th>
							<th scope="row"><a href="javascript:chk_orderby('order_PlayerName')">이름 <span id="order_PlayerName">▼</span></a></th>
							<th scope="row"><a href="javascript:chk_orderby('order_PlayerIDX')">선수코드 <span id="order_PlayerIDX">▼<span></a></th>
							<th scope="row">프로필 </th>
						</tr>
					</thead>				
					...
					<!-- 정렬을 위한 히든필드S -->
					<input type="hidden" name="order_Sports" value="DESC">
					<input type="hidden" name="order_TeamGb" value="DESC">
					<input type="hidden" name="order_Area" value="DESC">
					<input type="hidden" name="order_SchoolName" value="DESC">
					<input type="hidden" name="order_Sex" value="DESC">
					<input type="hidden" name="order_PlayerName" value="DESC">
					<input type="hidden" name="order_PlayerIDX" value="DESC">
					<!-- 정렬을 위한 히든필드E -->
					</form>
					<!--리스트가 보여지는 부분-->
					<tbody id="list">
