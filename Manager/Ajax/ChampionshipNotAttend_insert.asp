...
<%
	...
	If trim(tp)="" or trim(Search_GroupGameGb)="" or trim(LPlayerIDX) = "" or trim(RPlayerIDX) = "" or trim(GameTitleIDX) = "" or trim(GroupGameGb) = ""  or trim(Sex) = "" or trim(Level) = "" or trim(RGameLevelidx) = "" or trim(TeamGb) = ""  or trim(Game1R) = ""   or trim(LSchIDX) = ""  or trim(RSchIDX) = "" then 
		Response.End
	End If 
	
	'기존에 입력된 값 존재여부 체크
	if trim(Search_GroupGameGb) = "1" then '개인전
		LSQL = "        SELECT COUNT(RGameResultIDX) CNT"	
	...
	if trim(Search_GroupGameGb) = "1" then '개인전
		...
		INSSQL = " INSERT INTO Sportsdiary.dbo.tblRGameResult"
    ...
    INSSQL = INSSQL & "   ,'" & LPlayerIDX & "'"
    INSSQL = INSSQL & "   ,'" & RPlayerIDX & "'"
    INSSQL = INSSQL & "   ,CASE WHEN '" & TP & "' = 'A' THEN 'sd019021' WHEN '" & TP & "' = 'B' THEN '' WHEN '" & TP & "' = 'C' THEN 'sd019006' END"  '" & LPlayerResult & "'"
    INSSQL = INSSQL & "   ,'" & LSchIDX & "'"
    INSSQL = INSSQL & "   ,'" & RSchIDX & "'"
    ...
    INSSQL = INSSQL & "   ,CASE WHEN '" & TP & "' = 'A' THEN 'sd019021' WHEN '" & TP & "' = 'B' THEN 'sd019006' WHEN '" & TP & "' = 'C' THEN '' END"  '" & LPlayerResult & "'"
