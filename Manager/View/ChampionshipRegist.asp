...
<%
	...
	InfoSQL = InfoSQL&" ,Sportsdiary.dbo.FN_LevelNm('judo',TeamGb,Level) AS LevelName"
	InfoSQL = InfoSQL&" ,SportsDiary.dbo.FN_LevelNm('judo',TeamGb,Level) AS LevelNm "
...
function input_frm(){		
	var f = document.frm;
	if(f.GroupGameGb.value == "sd040001"){
	//개인전
		if(f.Team.value==""){
			alert("소속을 선택해 주세요");
			return false;
...
//출전선수등록
function view_player(obj,obj2){
	location.href="ChampionshipRegist_School.asp?RGameLevelIDX="+obj+"&Team="+obj2;
}
</script>
<script type="text/javascript">
//메달변경
...
<body onload="view_frm('F');">
	...
	<section>
		<div id="content">
			<div class="loaction">
				<strong>대회정보관리</strong> &gt; <%=GameTitleName%> &gt; <%=GroupGameGbName%> &gt; <%If GroupGameGb = "sd040001" Then %>출전선수관리<%Else%>출전소속관리<%End If%>
			</div>
			<!-- S : top-navi -->
			<div class="top-navi">
