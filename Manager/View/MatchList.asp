...
function leadingZeros(n, digits) {
  ...
  return zero + n;
	...
	<section>
		<div id="content">
		<form method="post" name="search_frm" id="search_frm" action="MatchList.asp"> 
			...
			<tbody>
				...
				<tr>
					...
					<%
						'If GroupGameGb ="sd040001" Then 
					%>
					<td class="left"><input type="button" value="복사" onclick="copy_path('<%=Rs("StadiumNumber")%>경기장<%=Rs("LSchoolName")%> VS <%=Rs("RSchoolName")%> <%=Rs("TeamGBNM")%> <%=Rs("LevelNM")%>/<%=Rs("PlayerResultIDX")%>');"></td>
					<%
						'Else 
					%>
					<!--
