...
<%
	...
	IF CIDX <> "" Then
		...
		LSQL = LSQL & "   	,CASE WHEN A.Birthday<>'' THEN SUBSTRING(A.Birthday, 1, 4)+'.'+SUBSTRING(A.Birthday, 5, 2)+'.'+SUBSTRING(A.Birthday, 7, 2) END UserBirth"
		LSQL = LSQL & "   	,A.Sex"
	...
	IF Photo = "" Then Photo = "../images/profile@3x.png"
  
%>
<script language='javascript'>
  var locationStr = "NationalTeam_Manage";
	...
	*/
	...
	function chk_Submit(valType){
		...
		if(valType=='DEL'){
			...
			$(location).attr('href', './NationalTeam_Manage.asp');
		}
		else{
			history.back();   
		}
	...
	function on_Submit(valType){
		...
		var strAjaxUrl = '../Ajax/NationalTeam_Write.asp';
			...
			success: function(retDATA) {
				...
				if(retDATA){
					...
					if (strcut[0] == 'TRUE') {
						...
						$('form[name=s_frm]').attr('action','./NationalTeam_Manage.asp');
	...
	function INPUT_PLAYERINFO(valMemberIDX, valUserName, valUserEnName, valSex, valBirthday, valPhoto){
...
<div id="content" class="referee_write"> 
  ...
  <div class="page_title clearfix">
    <h2>대표팀 관리</h2>
    <a href="./NationalTeam_Manage.asp" class="btn btn-back"><span class="ic_deco"><i class="fas fa-angle-left"></i></span>뒤로가기</a> 
  ...
  <form name="s_frm" method="post">
    ...
    <table class="left-head view-table">
      ...
      <tr>
        <th rowspan="7" valign="top"><img src="<%=Photo%>" id="img_Member" width="102" alt=""></th>
        <th>등록년도</th>
      ...
      <tr>
        <th>영문이름</th>
        <td><input type="text" name="UserEnName" id="UserEnName" value="<%=UserEnName%>" readonly /></td>
      </tr>
      <tr>
        <th>성별</th>
        <td><input type="text" name="Sex" id="Sex" value="<%=Sex%>" readonly />
			<!--
			<%
			   SELECT CASE Sex
			   	CASE "Man" 		: response.write "남" 
			   	CASE "WoMan" 	: response.write "여"
			   END SELECT
			 %>--></td>
      </tr>
      <tr>
        <th>생년월일</th>
        <td><input type="text" name="Birthday" id="Birthday" value="<%=Birthday%>" readonly /></td>
      </tr>            

    </table>
	
    <%IF CIDX<>"" Then%>
    <div class="btn-list-center"> <a href="javascript:chk_Submit('MOD');" class="btn btn-confirm">수정</a> <a href="javascript:chk_Submit('DEL');" class="btn btn-red">삭제</a> <a href="javascript:chk_Submit('CANCEL');" class="btn btn-cancel">취소</a> <a href="javascript:chk_Submit('LIST');" class="btn btn-gray">목록</a> </div>
    <%Else%>
    <div class="btn-list-center"> <a href="javascript:chk_Submit('SAVE');" class="btn btn-confirm">등록</a> <a href="javascript:chk_Submit('LIST');" class="btn btn-gray">목록</a> </div>
    <%End IF%>
