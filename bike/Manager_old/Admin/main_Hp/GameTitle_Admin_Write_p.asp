...
<%
  ...
  Set iFile = Upload.Form("iFile")                ' 실첨부파일,배열
	...
	chkiRoleDetail2 	= fInject(Upload.Form("chkiRoleDetail2"))
	chkiRoleDetail2arr 	= split(chkiRoleDetail2,",")

	For i=LBound(chkiRoleDetail2arr) to UBound(chkiRoleDetail2arr)

		if i = 0 then
			iRole = Trim(crypt.DecryptStringENC(chkiRoleDetail2arr(i)))
		else
			iRole = iRole + "^" + Trim(crypt.DecryptStringENC(chkiRoleDetail2arr(i)))
		end if
  ...
  If Err.Number = 0 Then
    ...
    LCnt = 0
		...
		GameTitleIDX = MSeq
		AdminMemberIDXarr = iRole
		AGTAuthority = "F"
		UseYN = "Y"

    LSQL = "EXEC AdminGameTitle_M '" & iType & "','" & GameTitleIDX & "','" & AdminMemberIDXarr & "','" & AGTAuthority & "','" & UseYN & "','" & ID & "','','','','',''"
    ...
    response.Write "<script type='text/javascript'>alert('글 등록이 잘 되었습니다.');location.href='./GameTitle_Admin_List.asp';</script>"
    response.End
  ...
  Else
    ...
    response.Write "<script type='text/javascript'>alert('글 등록에 오류가 있습니다.');location.href='./GameTitle_Admin_List.asp';</script>"
    response.End
