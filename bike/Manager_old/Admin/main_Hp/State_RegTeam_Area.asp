  ...
  <script language="javascript">
	...
	*/
	...
	function chk_Submit(valType){
		...
		var strAjaxUrl = '../Ajax/State_RegTeam_Area.asp';
		var fnd_Year = $('#fnd_Year').val();
		var fnd_AreaGb = $('#fnd_AreaGb').val();

		if(valType=='PRINT') {
			$('form[name=s_frm]').attr('action','./State_RegTeam_Area_Excel.asp');
	...
	$(document).ready(function(){
		...
		//Make SelectBox OPTION LIST: 시/도
		make_box('sel_fnd_AreaGb','fnd_AreaGb','','AreaGb');	
  ...
  <div id="content" class="association_list reg_team"> 
    ...
    <form name="s_frm" method="post">
      <div class="search_box reg-team"> <span id="sel_fnd_Year">
        ...
        </span> <span id="sel_fnd_AreaGb">
        <select name="fnd_AreaGb" id="fnd_AreaGb" class="title_select">
          <option value="">시/도</option>
        </select>
        </span> <span><a href="javascript:chk_Submit('PRINT');" class="btn btn-search">엑셀저장</a></span> </div>
