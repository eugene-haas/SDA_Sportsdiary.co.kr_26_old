<!--#include virtual="/include/db_open.inc"-->
<!--#include virtual="/back/dogo/DoHBack.asp"-->
<%
	menu					= 3
	submenu1			= 13
	...
	division					= Request.form("division")						' 발급요청자 1-개인 2-단체
	pl1_name					= Request.form("pl1_name")						' 성명
	pl1_no1						= Request.form("pl1_no1")						' 생년월일(yymmdd)
	tm_kname					= Request.form("tm_kname")						' 단체명
	tm_id						= Request.form("tm_id")							' 단체아이디
	use							= Request.form("use")							' 용도
	present						= Request.form("present")						' 제출처
	sort						= Request.form("sort")							' 증명서 종류 1-경기실적 2-지도실적
	If sort=1 then
		proof_name			= "경기"
	Else
		proof_name			= "지도"
	End if
	witness						= Request.form("witness")						' 기록 대조·확인자
	witness2					= Request.form("witness2")						' 사무처장(default:박종훈)
	start_year					= Request.form("start_year")					' 기간(yyyy)
	end_year					= Request.form("end_year")						' 기간(yyyy)
	nono						= Request.form("nono")							' 발급번호
	print_count					= Request.form("print_count")					' 발급매수
	add							= Request.form("add")							' 주소
	position					= Request.form("position")						' 소속
...
<body topmargin=50 leftmargin=20 width=100% height=100%>
<table border=0 cellpadding=0 cellspacing=0 class=menu_sub width=100%>
	<tr>
		<td colspan=2 align=center><!--#include virtual="/back/topmenu.asp"--></td>
	</tr>
	<tr valign=top>
		<td width=15% valign=top><!--#include virtual="/back/registration/submenu.asp"--></td>
		<td>
			<table border=0 cellpadding=0 cellspacing=10>
				<tr>
					...
					IF Cint(division)=1 then
						...
						SQLString = "SELECT * FROM people1_d WHERE pl1_name='"&pl1_name&"' AND pl1_no1="&pl1_no1&"  and pl2_serial is not null order by  pl1_serial  desc "
						rs.Open SQLString, ADODBString,3
						...
						Else
							...
							rs1.Open SQLString, ADODBString, 0
					...
					%>
						<table border=0 cellpadding=0 cellspacing=0 class=plane height=100%>
							<tr height=100%>
								<td height=100%>
													...
													<tr height=100%>
														<td colspan=4 class=b_blr height=100% valign=top>
															<table border=0 cellpadding=5 cellspacing=0 width=100% class=pr_font valign=top>
																...
																rs1.Open SQLString,ADODBString,3
																IF rs1.EOF AND rs1.BOF then
																%>
																<tr><td></td>	</tr>
																<tr>
																		...
																		<table border=0 cellpadding=0 cellspacing=0 width=100% class=pr_font>
																			...
																			<%
																				...
																				Do While Not rs1.EOF
																					...
																					rs2.Open SQLString, ADODBString,3
																					...
																					if Cint(tm_ok)=1 then
																						...
																						rs5.Open SQLString, ADODBString,3
						...
						rs.Open SQLString,ADODBString,0
						If rs.EOF AND rs.BOF then
						Else
							tm_add1		= rs("tm_add1")
							tm_add2		= rs("tm_add2")
								...
								<td>
									<table border=0 cellpadding=0 cellspacing=0>
										...
										<tr height=50 valign=center>
											<td align=center class=pr_font>
												<b>사단법인 &nbsp;&nbsp;<font style="font-family:바탕; font-size:17pt; color:#000000; line-height:20pt;">대 한 배 드 민 턴 협 회 장</font></b><br>
												<!--<b>회&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;장 &nbsp;&nbsp;<font style="font-family:바탕; font-size:17pt; color:#000000; line-height:20pt;">강&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;영 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;중</font></b>-->
											</td>
										</tr>
									</table>
				...
				<tr height=40>
					<td align=center>
						<a href=# OnClick="window.open('proof_print.asp?division=<%=division%>&pl1_name=<%=pl1_name%>&pl1_no1=<%=pl1_no1%>&tm_kname=<%=tm_kname%>&tm_id=<%=tm_id%>&use=<%=use%>&present=<%=present%>&sort=<%=sort%>&witness=<%=witness%>&witness2=<%=witness2%>&start_year=<%=start_year%>&end_year=<%=end_year%>&nono=<%=nono%>&print_count=<%=print_count%>&add=<%=add%>&position=<%=position%>','proof_print','toolbar=no,menubar=yes,scrollbars=yes,resizable=yes,width=750,height=600');"><input type=image src=/images/b_print.gif></a>
					</td>
				</tr>
			</table>
		</td>
	...
	<tr valign=bottom>
		<td colspan=2 align=center><!--#include virtual="/back/gamemaker/bot.asp"--></td>
	</tr>
</table>
</body>
</html>