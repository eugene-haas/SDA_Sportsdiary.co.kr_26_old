...
<%
    '===================================================================================================================================
    'PAGE : /Main_HP/cert_req_popup.asp
    'DATE : 2018년 04월 18일
    'DESC : [관리자] 증명서 발급 팝업
    '===================================================================================================================================
    ...
    Dim CertificateIDX
    Dim certNumber, certCount, certAttach, certWitness, certDirector
    Dim UserID, UserName, UserBirth, UserPhone, UserFax, TypeCertificate, TypeUse, TypeRecive, SubmitOrg, txtTitleNm1, DateDuringS1, DateDuringE1
    Dim txtTitleNm2, DateDuringS2, DateDuringE2, txtTitleNm3, DateDuringS3, DateDuringE3, DateVisit, ZipCode, UserAddr
    Dim UserAddrDtl, MemberZipCode, Address, AddressDtl, TypeCertificatePubName, TypeUsePubName
    Dim gRow, CSQL, i, ry, intNum, Buff, Cnt

	xUpPath = global_filepathImgUrl

    CertificateIDX = crypt.DecryptStringENC(fInject(Request.Form("idx")))
    If CertificateIDX = "" Or IsNumeric(Trim(CertificateIDX)) = false Then CertificateIDX = 0 Else CertificateIDX = Cdbl(CertificateIDX)

    If CertificateIDX = 0 Then
        Call DBClose()

        Response.Write "<script type='text/javascript'>alert('잘못된 접근 입니다.');self.close();location.replace('/');</script>"
        Response.End
    End If

	CSQL = "SELECT CertNumber, CertCount, CertAttach, CertWitness, CertDirector"
	CSQL = CSQL & "	from [KoreaBadminton].[dbo].[tblOnlineCertificate] WITH(NOLOCK)"
	CSQL = CSQL & " WHERE CertificateIDX = ?"
	...
	With objCmd
        ...
        .Parameters.Append .CreateParameter("@CertificateIDX", adBigInt, adParamInput, 20, CertificateIDX)
	...
	If gRow.eof Or gRow.bof Then
		...
		CertDirector		= ""
	Else
		CertNumber			= Trim(gRow(0))
		CertCount			= CDbl(gRow(1))
		CertAttach			= Trim(gRow(2))
		CertWitness			= Trim(gRow(3))
		CertDirector		= Trim(gRow(4))
	End If
    Set gRow = Nothing


	CSQL = "SELECT TOP 1 CertFile, CertWFile, CertDFile from [KoreaBadminton].[dbo].[tblOnlineCertificateInfo] WITH(NOLOCK) ORDER BY CertInfoIDX DESC"
    ...
    CSQL = "SELECT"
    CSQL = CSQL & " OCF.UserID, OCF.UserName, CONVERT(CHAR(10), CONVERT(DATE, OCF.UserBirth), 102) UserBirth,"
    CSQL = CSQL & " OCF.UserPhone, OCF.UserFax, OCF.TypeCertificate, OCF.TypeUse, OCF.TypeRecive, OCF.SubmitOrg, OCF.txtTitleNm1, OCF.DateDuringS1, OCF.DateDuringE1,"
    CSQL = CSQL & " OCF.txtTitleNm2, OCF.DateDuringS2, OCF.DateDuringE2, OCF.txtTitleNm3, OCF.DateDuringS3, OCF.DateDuringE3, OCF.DateVisit, OCF.ZipCode, OCF.UserAddr,"
    CSQL = CSQL & " OCF.UserAddrDtl, TMO.ZipCode, TMO.Address, TMO.AddressDtl, TPC.PubName, TPC2.PubName"
    CSQL = CSQL & " from [KoreaBadminton].[dbo].[tblOnlineCertificate] OCF WITH(NOLOCK)"
    CSQL = CSQL & " INNER JOIN [KoreaBadminton].[dbo].[tblMembershipOnline] TMO WITH(NOLOCK) ON OCF.UserID = TMO.UserID AND TMO.DelYN = 'N'"
    CSQL = CSQL & " INNER JOIN [koreabadminton].[dbo].[tblPubcode] TPC WITH(NOLOCK) ON OCF.TypeCertificate = TPC.PubCode"
    CSQL = CSQL & " INNER JOIN [koreabadminton].[dbo].[tblPubcode] TPC2 WITH(NOLOCK) ON OCF.TypeUse = TPC2.PubCode"
    CSQL = CSQL & " WHERE OCF.CertificateIDX = ? AND OCF.DelYN = 'N'"
	...
	Else
		UserID                  = Trim(gRow(0))
        UserName                = Trim(gRow(1))
        UserBirth               = Trim(gRow(2))
        UserPhone               = Trim(gRow(3))
        UserFax                 = Trim(gRow(4))
        TypeCertificate         = Trim(gRow(5))
        TypeUse                 = Trim(gRow(6))
        TypeRecive              = Trim(gRow(7))
        SubmitOrg               = ReHtmlSpecialChars(Trim(gRow(8)))
        txtTitleNm1             = ReHtmlSpecialChars(Trim(gRow(9)))
        DateDuringS1            = ReHtmlSpecialChars(Trim(gRow(10)))
        DateDuringE1            = ReHtmlSpecialChars(Trim(gRow(11)))
        txtTitleNm2             = ReHtmlSpecialChars(Trim(gRow(12)))
        DateDuringS2            = ReHtmlSpecialChars(Trim(gRow(13)))
        DateDuringE2            = ReHtmlSpecialChars(Trim(gRow(14)))
        txtTitleNm3             = ReHtmlSpecialChars(Trim(gRow(15)))
        DateDuringS3            = ReHtmlSpecialChars(Trim(gRow(16)))
        DateDuringE3            = ReHtmlSpecialChars(Trim(gRow(17)))
        DateVisit               = ReHtmlSpecialChars(Trim(gRow(18)))
        ZipCode                 = ReHtmlSpecialChars(Trim(gRow(19)))
        UserAddr                = ReHtmlSpecialChars(Trim(gRow(20)))
        UserAddrDtl             = ReHtmlSpecialChars(Trim(gRow(21)))
        MemberZipCode           = ReHtmlSpecialChars(Trim(gRow(22)))
        Address                 = ReHtmlSpecialChars(Trim(gRow(23)))
        AddressDtl              = ReHtmlSpecialChars(Trim(gRow(24)))
        TypeCertificatePubName  = Trim(gRow(25))
        TypeUsePubName          = Trim(gRow(26))

        'If TypeRecive = "POST" Then
        '    CertZipCode = ZipCode
        '    CertAddr    = UserAddr
        '    CERTAddrDtl = UserAddrDtl
        'ELSE 
            CertZipCode = MemberZipCode
            CertAddr    = Address
            CERTAddrDtl = AddressDtl
        'END IF 
	End If
    Set gRow = Nothing
    Call DBClose()
    ...
    pl1_name				= UserName						' 발급요청자
    pl1_no1					= Replace(UserBirth, ".", "")	' 생년월일(yymmdd)
	division				= 1								' 발급요청자 1-개인 2-단체
	'단체 '단체의 기본 정보 가져오기 부터 시작됨.
	'tm_kname				= Request.form("tm_kname")		' 단체명
	'tm_id					= Request.form("tm_id")			' 단체 아이디
	use						= TypeUsePubName				' 용도
	present					= SubmitOrg						' 제출처

	Select Case TypeCertificate
		Case "CER01001", "CER01003", "CER01005"
			sort			= 1
		Case "CER01002", "CER01004"
			sort			= 2
	End Select
	'sort					= 1								' 증명서 종류 1-경기실적 2-지도실적

	witness					= certWitness					' 기록 대조·확인자
	witness2				= certDirector					' 사무국장
	start_year				= ""							' 기간(yyyy)
	end_year				= ""							' 기간(yyyy)
	nono					= certNumber					' 발급번호
	print_count				= certCount						' 발급매수
	add						= CertZipCode &"&nbsp;&nbsp;"& CertAddr &"&nbsp;"& CERTAddrDtl
	position				= certAttach					' 소속
...
<head>
    ...
    <style type="text/css">
        ...
        .height1   { font-family:"굴림",verdana; font-size:1px; color:ffffff;text-decoration: none;  line-height:0pt; }
				.pr_font .sign_box{position:relative;}
				.pr_font .sign_box .sign_img{position:absolute;right:-22px;top:-19px;width:50px;}
    </style>
...
<body topmargin=0 leftmargin=15 width=100% height=100%>
...
<div style="text-align:center;">
	...
	IF rs.EOF AND rs.BOF then
		...
		SET rs = Nothing
		Call DBClose2()
		Response.Write "<script type='text/javascript'>alert('죄송합니다. 해당 데이타가 없습니다.\n다시한번 확인해 주십시요.');self.close();location.replace('/');</script>"
        Response.End
	Else
		...
		Else
			...
			if Cint(tm_year)=>Cint(tm_year_before) AND tm_id_before<>tm_id then
				...
				IF Cdbl(pl2_serial)=rs1("pl2_serial_head") OR Cdbl(pl2_serial)=rs1("pl2_serial_head2")  then
					pl2_position				= "감독"
	...
	%>
							<table border=0 cellpadding=0 cellspacing=0 class=plane width="100%" height=100%>
								<tr height=100%>
									<td height=100% align="center">
										<table border=0 cellpadding=0 cellspacing=0>
											<tr height=40>
												<td align=center valign=bottom><font style="font-family:바탕; font-size:20pt; color:#000000; line-height:30pt;"><b><%=TypeCertificatePubName%> 증명서</b></font></td>
											</tr>
											<tr height=100%>
												<td height=100%>
													 <table border=0 cellpadding=4 cellspacing=0 width=680 class=pr_font  valign=top >
														<tr>
	...
	'End if
	if Len(DateDuringS1) > 9 then
	SQLString = SQLString & " AND ('"& DateDuringS1 &" 00:00:00' <=ev.ev_stime)"
	end if 
	'실적검색 마감기간이 입력되었을 경우
	'If Len(end_year)>2 then
	'SQLString = SQLString & " AND (year(ev.ev_stime)<="&Cint(end_year)&")"
	'End If
	if Len(DateDuringE1) > 9 then
	SQLString = SQLString & " AND (ev.ev_stime<='"& DateDuringE1 &" 23:59:59')"
	end if 
	...
	'Response.Write "국내대회 ===== "& SQLString &"<br /><br /><br /><br /><br /><br /><br />"
	...
	<%
		...
		Do While Not rs1.EOF
			...
			elseif instr(rs1("ev_kname"),"-일반부")>0 then
				ev_kname				= replace(rs1("ev_kname"),"- 일반부","")
			elseif instr(rs1("ev_kname"),"(초.중)")>0 then
	...
	<%
		if len(ev_kname)>25 then    '20151207 대회명 2줄 됨에 따른 카운트수 늘리기
	...
	<H6></H6>
													 <table border=0 cellpadding=0 cellspacing=0 width=680 class=pr_font >
														<tr>
	...
	%>
																	<tr><td><% record_row=record_row+3%></td></tr>
	<%
	End if
	%>
	...
	'Response.Write "국제대회 ===== "& SQLString &"<br /><br /><br /><br /><br /><br /><br />"
	...
	%>
																	...
																	<tr height=10><td><% record_row=record_row+4%></td></tr>
	<%
	else
		record_row=0
		print_page=1
	...
	%>
																...
																<table border=0 cellpadding=0 cellspacing=0 width=100% class=pr_font>
																	<tr height=50>
																		...
																		<td width=30%>
																			...
																			<span class="sign_box">
																			<%=witness%>&nbsp;&nbsp;&nbsp;(인)
																			<img src="<%=xUpPath&CertWFile%>" alt="" class="sign_img">
																			</span>
																			<%END IF %>
																		</td>
																		<td width=1>|</td>
																		...
																		<td width=30%>
																			...
																			<span class="sign_box">
																			<%=witness2%>&nbsp;&nbsp;&nbsp;(인)
																			<img src="<%=xUpPath&CertDFile%>" alt="" class="sign_img">
																			</span>
																			<%END IF %>
																		</td>
																	</tr>
											...
											<tr height=60 valign=bottom>
												<td align=center class=pr_font>
													<span class="sign_box"><b><font style="font-family:바탕; font-size:20pt; color:#000000; line-height:25pt;">대 한 배 드 민 턴 협 회 장</font></b><img src="<%=xUpPath&CertFile%>" alt="" class="sign_img"></span><br>
												</td>
											</tr>
										</table>
									</td>
	...
	Set rs=Nothing
	%>
							<table border=0 cellpadding=0 cellspacing=0 class=plane>
								<tr>
									<td>
										<table border=0 cellpadding=0 cellspacing=0>
											<tr height=60>
												<td align=center><font style="font-family:바탕; font-size:20pt; color:#000000; line-height:30pt;"><b><%=TypeCertificatePubName%> 증명서</b></font></td>
											</tr>
											<tr>
												<td>
													 <table border=1 cellpadding=0 cellspacing=0 width=680 class=pr_font bordercolor="FFFFFF" bordercolorlight="CDCDCC" bordercolordark="FFFFFF">
														<tr>
															<td width=15% align=center>종&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;목</td>
															<td width=45%>배드민턴</td>
															<td width=15% align=center>발급번호</td>
															<td width=25%><%=nono%></td>
														</tr>
														<tr>
															<td align=center>발급일자</td>
															<td><%=year(now)%>년 <%=month(now)%>월 <%=day(now)%>일</td>
															<td align=center>발급부수</td>
															<td><%=print_count%> 매</td>
														</tr>
														<tr>
															<td align=center>단&nbsp;체&nbsp;명</td>
															<td colspan=3><%=tm_kname2%></td>
														</tr>
														<tr>
															<td align=center>주&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;소</td>
															<td colspan=3><%=tm_add%></td>
														</tr>
														<tr>
															<td align=center>용&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;도</td>
															<td><%=use%>&nbsp;</td>
															<td align=center>제&nbsp;&nbsp;출&nbsp;&nbsp;처</td>
															<td><%=present%>&nbsp;</td>
														</tr>
														<tr>
															<td colspan=4>
																<table border=0 cellpadding=5 cellspacing=0 width=100% class=pr_font>
																	<tr>
																		<td align=center><u><%=TypeCertificatePubName%></u></td>
																	</tr>
																	<tr>
																		<td>
																			<table border=0 cellpadding=0 cellspacing=0 width=100% class=pr_font>
	...
	<div id="printDiv" style="text-align:center;">
		<div class="btn-list-center" style="height:60px;">
			<a href="javascript:;" class="btn btn-confirm" onclick="fn_certPrint()">인쇄하기</a>
		</div>
	</div>
</body>
</html>
