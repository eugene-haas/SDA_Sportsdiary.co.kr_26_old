...
<script language="javascript">
  	var locationStr = "LeaderInfo_List";
	...
	function chk_Submit(valType, valIDX, chkPage){
		...
		var fnd_LeaderType = $('#fnd_LeaderType').val();
		...
		switch(valType){
			case 'VIEW'	: //수정/VIEW
				...
				$('form[name=s_frm]').attr('action','./LeaderInfo_Mod.asp');
				$('form[name=s_frm]').submit(); 
				break;
			
			case 'WRITE' :	//등록
				$('form[name=s_frm]').attr('action','./LeaderInfo_Write.asp');
				$('form[name=s_frm]').submit(); 
				break;
	...
	$(document).ready(function(){
		...
		make_box('sel_fnd_Year', 'fnd_Year', '<%IF fnd_Year <> "" Then response.write fnd_Year Else response.write Year(Date()) End IF%>', 'RegYear_Leader');
      ...
      <div class="search_top community">
        <div class="search_box"><span class="tit">등록년도</span>
            ...
            <!--<a href="#" class="btn btn-close" data-dismiss="modal">X</a>-->
           	<select name="fnd_LeaderType" id="fnd_LeaderType" class="title_select">
				<option value="" selected>지도자 구분</option>
				<option value="2" <%IF fnd_LeaderType = "2" Then response.write "selected" End IF%>>감독</option>
	  			<option value="3" <%IF fnd_LeaderType = "3" Then response.write "selected" End IF%>>코치</option>
			</select>
