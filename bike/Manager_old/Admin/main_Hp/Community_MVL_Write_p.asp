...
<%
  ...
  If Err.Number = 0 Then
    ...
    LSQL = "EXEC Community_Board_M '" & iType & "','" & ColumnistIDX & "','" & iDivision & "','" & Name & "','" & Subject & "','" & iContents & "','" & iLink & "','" & SubType & "','" & NoticeYN & "','" & N_Year & "','" & Temp & "','" & ID & "','" & MSeq & "','" & TeamGb & "','" & FileExt & "','','','','',''"
    ...
    response.Write "<script type='text/javascript'>alert('글 등록이 잘 되었습니다.');location.href='./Community_MVL_List.asp';</script>"
    response.End
  ...
  Else
    ...
    response.Write "<script type='text/javascript'>alert('글 등록에 오류가 있습니다.');location.href='./Community_MVL_List.asp';</script>"
    response.End
