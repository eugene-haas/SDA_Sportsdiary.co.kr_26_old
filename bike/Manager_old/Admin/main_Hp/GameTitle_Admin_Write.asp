...
<%
  ...
  If iType = "2" Then
    ...
    'DBClose()
		...
		LSQL = "EXEC AdminGameTitle_R '" & iType & "','" & iSubType1 & "','" & NowPage & "','" & GameTitleIDX & "','" & AdminGameTitleIDX & "','" & iLoginID & "','','','','',''"
		'response.Write "LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL="&LSQL&"<br>"
		'response.End
		...
		If Not (LRs.Eof Or LRs.Bof) Then
			Do Until LRs.Eof
			  LCnt1_1 = LCnt1_1 + 1
				iRoleDetail1_1e = iRoleDetail1_1e&"^"&crypt.EncryptStringENC(LRs("AdminMemberIDX"))&""
...
<script type="text/javascript">
   ...
   * left-menu 체크
   */
	var locationStr = "GameTitle_Admin_List"; // 대회별관리자
  /* left-menu 체크 */
  ...
  function Del_Link(i1, i2) {
    if (confirm("해당 글을 삭제 하시겠습니까?") == true) {
    	post_to_url('./GameTitle_Admin_Delete_p.asp', { 'i1': i1, 'i2': i2 });
    }
    else {
    }
  }
  ...
  function CancelLink(i2) {
  	post_to_url('./GameTitle_Admin_List.asp', { 'i2': i2, 'iSubType': selSearchValue1, 'iNoticeYN': selSearchValue2, 'iDivision': selSearchValue3, 'iSearchCol1': selSearchValue4, 'iSearchCol': selSearchValue, 'iSearchText': txtSearchValue });
  }
    ...
    if (i3 == "1") {
      ...
      if (confirm("해당 글을 저장 하시겠습니까?") == true) {
        try {
          ...
          documentForm.action = "./GameTitle_Admin_Reply_p.asp";
          documentForm.submit();
        } catch (e) { }
      }
      else {
  ...
  function ReplyDel_Link(i1, i2, i3, i4) {
    if (confirm("해당 글을 삭제 하시겠습니까?") == true) {
    	post_to_url('./GameTitle_Admin_Reply_Delete_p.asp', { 'i1': i1, 'i2': i2, 'i3': i3, 'i4': i4 });
    }
    else {
    }
  }
  ...
  <div id="content" class="qna_write">
    ...
    <div class="page_title clearfix">
      <h2>대회별관리자 등록/수정</h2>
      <a href="./GameTitle_Admin_List.asp" class="btn btn-back"><span class="ic_deco"><i class="fas fa-angle-left"></i></span>뒤로가기</a>
      ...
      <div  class="navigation_box">
        ...
        <ul>
          <li>홈페이지관리</li>
          <li>커뮤니티</li>
          <li><a href="./GameTitle_Admin_List.asp">대회별관리자</a></li>
          <li><a href="./GameTitle_Admin_Write.asp">대회별관리자 등록/수정</a></li>
        </ul>
      </div>
      <!-- E: 네비게이션 -->
    ...
    <!-- S : 내용 시작 -->
      <form id="form1" name="form1" action="./GameTitle_Admin_Write_p.asp" method="post" ENCTYPE="multipart/form-data">
        <table class="left-head view-table">

          <input type="hidden" id="selSearch1" name="selSearch1" value="" />

					<input type="hidden" id="hdGTIDX" name="hdGTIDX" value="<%=iMSeq %>" />

          <tr class="half-line">
            <th>대회명</th>
            <td colspan="2">
              <span class="con">
                <%=GameTitleName %> (<%=GameS %> ~ <%=GameE %>)
              </span>
            </td>
          </tr>
            ...
            LCnt1_2 = 0
						...
						LSQL = "EXEC AdminGameTitle_R '" & iType & "','" & iSubType2 & "','" & NowPage & "','" & GameTitleIDX & "','" & AdminGameTitleIDX & "','" & iLoginID & "','','','','',''"
						'response.Write "LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL="&LSQL&"<br>"
						'response.End
            ...
            <td colspan="2">
							<span class="rights-list">
								<%
									Do Until LRs.Eof
									  LCnt1_2 = LCnt1_2 + 1
									  iRoleDetail2 = LRs("AdminMemberIDX")
										iRoleDetail2e = crypt.EncryptStringENC(iRoleDetail2)
									  iRoleDetailNm2 = LRs("AdminName")
										iRoleDetailID2 = LRs("UserID")
								%>
								<label>
									<% if LCnt1_1 > 0 then %>
										<% if Instr(iRoleDetail1_1e,iRoleDetail2e) > 0 then %>
										<input type="checkbox" name="chkiRoleDetail2" id="chkiRoleDetail2_<%=iRoleDetail2e %>" value="<%=iRoleDetail2e %>" checked><%=iRoleDetailNm2 %> (<%=iRoleDetailID2 %>)
										<% else %>
										<input type="checkbox" name="chkiRoleDetail2" id="chkiRoleDetail2_<%=iRoleDetail2e %>" value="<%=iRoleDetail2e %>"><%=iRoleDetailNm2 %> (<%=iRoleDetailID2 %>)
										<% end if %>
									<% else %>
								  <input type="checkbox" name="chkiRoleDetail2" id="chkiRoleDetail2_<%=iRoleDetail2e %>" value="<%=iRoleDetail2e %>"><%=iRoleDetailNm2 %> (<%=iRoleDetailID2 %>)
									<% end if %>
								</label>
								<% if (LCnt1_2 MOD 5) = 0 then %>
								<br />
								<% end if %>
								<%  LRs.MoveNext
								    Loop
          ...
          <tr class="file-line" style="display:none;">
            <th>파일</th>
            <td>
              <span>
          ...
          <tr style="display:none;">
            ...
            <td colspan="2">
              <span class="right_con edit_box">
                <textarea name="iContents" id="iContents" rows="20" cols="100">N</textarea>
              </span>
            </td>
          </tr>
