...
<%
	...
	IF fnd_AreaGb <> "" Then 
	 	CSearch = " AND A.Sido = '"&fnd_AreaGb&"'"
												 
		CSQL = "		SELECT A.SidoNm"
		CSQL = CSQL & " FROM [KoreaBadminton].[dbo].[tblSidoInfo] A"										 
		CSQL = CSQL & " WHERE A.DelYN = 'N'"&CSearch
		SET CRs = DBCon.Execute(CSQL)
   		IF Not(CRs.Eof Or CRs.Bof) Then 										 
			txt_FileName = CRs("SidoNm")
		End IF
			CRs.Close
		SET CRs = Nothing
	End IF 
    ...
    IF txt_FileName <> "" Then
		FileName = "등록팀현황_시도별_"&txt_FileName&"_"&fnd_Year&".xls"
	Else
		...
		numrowsGb = ubound(txtDataGb, 2)	'열
   		...
   		FOR j = 0 to numrowsGb		
			...
			CSQL = CSQL & "		AND A.RegYear = '"&fnd_Year&"'"	
   			CSQL = CSQL & "		AND A.Sido = '"&txtDataGb(0, j)&"'"
   			CSQL = CSQL & "		AND A.SEX = '"&txtDataGb(3, j)&"'"
			CSQL = CSQL & "	ORDER BY A.TeamNm, A.PteamGb"
			'response.Write CSQL
