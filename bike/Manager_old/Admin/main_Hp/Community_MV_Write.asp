...
<script type="text/javascript">
   ...
   * left-menu 체크
   */
	var locationStr = "Community_MV_List"; // 질문과 답변
  /* left-menu 체크 */
  ...
  function Del_Link(i1, i2) {
    if (confirm("해당 글을 삭제 하시겠습니까?") == true) {
    	post_to_url('./Community_MV_Delete_p.asp', { 'i1': i1, 'i2': i2 });
    }
    else {
    }
  }
    ...
    if (i3 == "1") {
      ...
      if (confirm("해당 글을 저장 하시겠습니까?") == true) {
        try {
          ...
          documentForm.action = "./Community_MV_Reply_p.asp";
          documentForm.submit();
        } catch (e) { }
      }
      else {
  ...
  function ReplyDel_Link(i1, i2, i3, i4) {
    if (confirm("해당 글을 삭제 하시겠습니까?") == true) {
    	post_to_url('./Community_MV_Reply_Delete_p.asp', { 'i1': i1, 'i2': i2, 'i3': i3, 'i4': i4 });
    }
    else {
    }
  }
  ...
  <div id="content" class="qna_write">
    ...
    <div class="page_title clearfix">
      <h2>경기동영상 등록/수정</h2>
      <a href="./Community_MV_List.asp" class="btn btn-back"><span class="ic_deco"><i class="fas fa-angle-left"></i></span>뒤로가기</a>
      ...
      <div  class="navigation_box">
        ...
        <ul>
          <li>홈페이지관리</li>
          <li>커뮤니티</li>
          <li><a href="javascript:;" onclick="./Community_MV_List.asp">경기동영상</a></li>
          <li><a href="javascript:;" onclick="./Community_MV_Write.asp">경기동영상 등록/수정</a></li>
        </ul>
      </div>
      <!-- E: 네비게이션 -->
    ...
    <!-- S : 내용 시작 -->
      <form id="form1" name="form1" action="./Community_MV_Write_p.asp" method="post" ENCTYPE="multipart/form-data">
        <table class="left-head view-table">
          ...
          <input type="hidden" id="selSearch1" name="selSearch1" value="" />
            ...
            <td colspan="2">
              <span class="con">
								<input type="radio" name="rdotype" onclick="javascript: fn_MT('youtube');" value="youtube" <% if MovieType = "youtube" then %>checked<% end if %> />유튜브&nbsp;&nbsp;&nbsp;&nbsp;
								<input type="radio" name="rdotype" onclick="javascript: fn_MT('kakao');" value="kakao" <% if MovieType = "kakao" or iType = "1" then %>checked<% end if %> />카카오
              </span>
            </td>
          </tr>
          ...
          <tr class="half-line">
            ...
            <td colspan="2">
              <span class="con">
                <!--<input type="text" id="iLink" name="iLink" value="<%'=Link %>" <%' if iType ="1" or MovieType = "kakao" then %> placeholder="전체주소 입력.. ex) http://tv.kakao.com/embed/player/cliplink/385839605" <%' else %> placeholder="개별주소만 입력.. ex) NRLhcVOSHZU" <%' end if %> />-->
								<input type="text" id="iLink" name="iLink" value="<%=Link %>" placeholder="주소 입력.. " />
								<div id="divlinkdetail" style="color:red;">전체주소 입력.. ex) http://tv.kakao.com/embed/player/cliplink/385839605</div>
            ...
            <th>링크영상<p></p>미리보기</th>
            <td colspan="2">
              <span class="con" id="divM1">
                <% if iType ="2" then %>
									<% if MovieType = "youtube" then %>
									...
									<% else %>
									<iframe width="240" height="160" src="<%=Link %>" allowfullscreen frameborder="0" scrolling="no"></iframe>
									<% end if %>
								<% else %>
          ...
          <tr class="file-line" style="display:table-row;" id="triti">
            <th>썸네일<p></p>이미지</th>
            <td>
              <span>
                <div id="iFileDiv" name="iFileDiv">
                  <span id="sFile_1">
                    ...
                    <span id="FN_iFile_1" class="added-list">
                      <a href="javascript:;" onclick="javascript:FN_iFileDivP();" class="btn" style="display:none;">
              ...
              <span class="con">
                <select id="selSearch2" name="selSearch2" class="title_select">
                  ...
                  <option value="N" selected>N</option>
							...
							<span class="existence">※공지는 최상위 <%=global_MainMVCnt %>개만 상단에 노출됩니다.</span>
            </td>
          </tr>
          <!--<tr>
            <th>공지 유/무</th>
  ...
  <script type="text/javascript">
    ...
    function AddLink() {
    	...
    	var rdovalue = $(':radio[name="rdotype"]:checked').val();
    ...
    function FileDown(i3) {
      ...
      //window.open("<%=global_depth %>/dev/dist/dl.asp?FileName=" + i3);
    	location.href = "../dev/dist/dl.asp?FileName=" + i3 + "&FileDivision=MVTHUM";
    ...
    function fn_MT(mtv) {
    	...
    	if (mtv == "youtube") {
    		...
    		//$('#iLink').attr("placeholder", "개별주소만 입력.. ex) NRLhcVOSHZU");
    		$('#divlinkdetail').html('개별주소만 입력.. ex) NRLhcVOSHZU');

    		$('#triti').css('display', 'none');
    		...
    		//$('#iLink').attr("placeholder", "전체주소 입력.. ex) http://tv.kakao.com/embed/player/cliplink/385839605");
    		$('#divlinkdetail').html('전체주소 입력.. ex) http://tv.kakao.com/embed/player/cliplink/385839605');

    		$('#triti').css('display', 'table-row');
			
    	}

    }

    var rdovalue1 = $(':radio[name="rdotype"]:checked').val();
