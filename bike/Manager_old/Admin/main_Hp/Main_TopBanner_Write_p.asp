...
<%
  ...
  If Err.Number = 0 Then
    ...
    Upload.Save global_filepath_TB, False
    ...
    LSQL = "EXEC Community_Board_M1 '" & iType & "','" & ColumnistIDX & "','" & iDivision & "','" & Name & "','" & Subject & "','" & iContents & "','" & iLink & "','" & SubType & "','" & NoticeYN & "','" & N_Year & "','" & Temp & "','" & ID & "','" & MSeq & "','" & TeamGb & "','" & FileExt & "','" & StartDate & "','" & EndDate & "','" & iLH & "','" & iDB & "','" & iBC & "','','','',''"
    ...
    response.Write "<script type='text/javascript'>alert('글 등록이 잘 되었습니다.');location.href='./Main_TopBanner_List.asp';</script>"
    response.End
  ...
  Else
    ...
    response.Write "<script type='text/javascript'>alert('글 등록에 오류가 있습니다.');location.href='./Main_TopBanner_List.asp';</script>"
    response.End
