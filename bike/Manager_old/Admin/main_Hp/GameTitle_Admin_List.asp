...
<%
  ...
  LSQL = "EXEC tblGameTitle_Searched_STR '" & NowPage & "','" & PagePerData & "','" & BlockPage & "','" & iType & "','" & iDivision & "','" & iSearchCol & "','" & iSearchText & "','','','','','" & iLoginID & "'"
...
%>
  ...
  <script type="text/javascript">
    ...
    * left-menu 체크
    */
    var locationStr = "GameTitle_Admin_List";  // 선수
    /* left-menu 체크 */
    ...
    function WriteLink(i2) {
    	post_to_url('./GameTitle_Admin_Write.asp', { 'i2': i2, 'iType': '1', 'iSubType': selSearchValue1, 'iNoticeYN': selSearchValue2, 'iDivision': selSearchValue3, 'iSearchCol1': selSearchValue4, 'iSearchCol': selSearchValue, 'iSearchText': txtSearchValue });
    }

    function ReadLink(i1, i2) {
    	post_to_url('./GameTitle_Admin_Write.asp', { 'i1': i1, 'i2': i2, 'iType': '2', 'iSubType': selSearchValue1, 'iNoticeYN': selSearchValue2, 'iDivision': selSearchValue3, 'iSearchCol1': selSearchValue4, 'iSearchCol': selSearchValue, 'iSearchText': txtSearchValue });
    }
    ...
    function fn_selSearch() {
      ...
      post_to_url('./GameTitle_Admin_List.asp', { 'i2': 1, 'iSubType': selSearchValue1, 'iNoticeYN': selSearchValue2, 'iDivision': selSearchValue3, 'iSearchCol1': selSearchValue4, 'iSearchCol': selSearchValue, 'iSearchText': txtSearchValue });
    }
  ...
  <!-- S: content qna_list -->
      ...
      <div class="page_title clearfix">
        ...
        <div  class="navigation_box">
          ...
          <ul>
            <li>대회정보</li>
            <li>대회운영</li>
            <li><a href="./GameTitle_Admin_List.asp">대회별 관리자</a></li>
          </ul>
        </div>
        <!-- E: 네비게이션 -->
    ...
    <div class="search_top community">
      ...
      <!--<a href="javascript:;" onClick="javascript:WriteLink('<%'=NowPage %>');" class="btn btn-add">대회별관리자 등록</a>-->
      </div>
    ...
    <table class="table-list">
      ...
      <thead>
        <tr>
          <th scope="col" width="150">순번</th>
          <th scope="col">대회명</th>
          <th scope="col" width="200">구분</th>
          <th scope="col" width="200">관리자유무</th>
        </tr>
      </thead>
          ...
          <%
            ...
            If Not (LRs.Eof Or LRs.Bof) Then
              Do Until LRs.Eof
                  LCnt = LCnt + 1
        %>
        <tr  style="cursor:pointer" onClick="javascript:ReadLink('<%=crypt.EncryptStringENC(LRs("GameTitleIDX")) %>','<%=NowPage %>');">
          <td><%=LRs("Num") %></td>
          <td class="name"><%=LRs("GameTitleName") %> (<%=LRs("GameS") %> ~ <%=LRs("GameE") %>)</td>
          <td><%=LRs("EnterType") %></td>
					<% if LRs("AdminYN") = "Y" then %>
					<td style="color:red">관리자 있음</td>
					<% else %>
					<td style="color:gainsboro">관리자 없음</td>
					<% end if %>
        </tr>
        <%
              LRs.MoveNext
