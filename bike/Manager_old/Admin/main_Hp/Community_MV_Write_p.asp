...
<%
  ...
  SubType = fInject(Upload.Form("selSearch1"))  
	...
	RdoType = fInject(Upload.Form("rdotype")) 
  ...
  If Err.Number = 0 Then
    ...
    Upload.Save global_filepath_MVTHUM, False
    ...
    LSQL = "EXEC Community_Board_M '" & iType & "','" & ColumnistIDX & "','" & iDivision & "','" & Name & "','" & Subject & "','" & iContents & "','" & iLink & "','" & SubType & "','" & NoticeYN & "','" & N_Year & "','" & Temp & "','" & ID & "','" & MSeq & "','" & TeamGb & "','" & FileExt & "','','','','','" & RdoType & "'"
    ...
    response.Write "<script type='text/javascript'>alert('글 등록이 잘 돼었습니다.');location.href='./Community_MV_List.asp';</script>"
    response.End
  ...
  Else
    ...
    response.Write "<script type='text/javascript'>alert('글 등록에 오류가 있습니다.');location.href='./Community_MV_List.asp';</script>"
    response.End
