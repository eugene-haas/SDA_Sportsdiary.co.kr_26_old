...
<script type="text/javascript">
   ...
   * left-menu 체크
   */
  var locationStr = "Community_Photo_List";
  /* left-menu 체크 */  
  ...
  function FN_iFileDivP() {
    ...
    if (iFileNum < iFileLimitNum) {
      ...
      iHtmlPC = iHtmlPC + '<span id="sFile_' + iFileNum + '"><input type="file" id="iFile_' + iFileNum + '" name="iFile" class="csfile" onchange="javascript:Checkfiles_Img2(&#39;iFile_' + iFileNum + '&#39;);" /><span id="FN_iFile_' + iFileNum + '" class="btn_icon">&nbsp;<a href="javascript:;" onclick="javascript:FN_iFileDivP();">+</a>&nbsp;<a href="javascript:;" onclick="javascript:FN_iFileDivM(' + iFileNum + ');">-</a></span></span>';
  ...
  function Del_Link(i1, i2) {
    if (confirm("해당 글을 삭제 하시겠습니까?") == true) {
      post_to_url('./Community_Photo_Delete_p.asp', { 'i1': i1, 'i2': i2 });
    }
    else {
    }
  }
  ...
  function CancelLink(i2) {
    post_to_url('./Community_Photo_List.asp', { 'i2': i2, 'iSubType': selSearchValue1, 'iNoticeYN': selSearchValue2, 'iDivision': selSearchValue3, 'iSearchCol1': selSearchValue4, 'iSearchCol': selSearchValue, 'iSearchText': txtSearchValue });
  }
  ...
  function Reply_Link(i1, i2, i3, i4) {
    ...
    if (i3 == "2") {
      ...
      var iHtmlcnt = '<textarea name="txtContents" id="txtContents" rows="3" cols="30">' + iContents + '</textarea>';
  ...
  function CommentSave_Link(i1, i2, i3, i4) {
    ...
    var iContentsNo = $('#txtContents').html();
    ...
    if (i3 == "1") {
      ...
      if (confirm("해당 글을 저장 하시겠습니까?") == true) {
        try {
          ...
          documentForm.action = "./Community_Photo_Reply_p.asp";
          documentForm.submit();
        } catch (e) { }
      }
      else {
      ...
      if (documentForm.txtContents.value == "") {
        alert('<%=global_Contents_Val %>');
        return documentForm.txtContents.focus();
      }
      try {
        documentForm.method = "post";
        documentForm.target = "_self";
  ...
  function ReplyDel_Link(i1, i2, i3, i4) {
    if (confirm("해당 글을 삭제 하시겠습니까?") == true) {
      post_to_url('./Community_Photo_Reply_Delete_p.asp', { 'i1': i1, 'i2': i2, 'i3': i3, 'i4': i4 });
    }
    else {
    }
  }
...
<!-- S: content photo_write -->
  <div id="content" class="photo_write">
    ...
    <div class="contents">
      ...
      <div class="page_title clearfix">
        <h2>갤러리-사진 등록/수정</h2>
        <a href="./Community_Photo_List.asp" class="btn btn-back"><span class="ic_deco"><i class="fas fa-angle-left"></i></span>뒤로가기</a>
        ...
        <div  class="navigation_box">
          ...
          <ul>
            <li>홈페이지관리</li>
            <li>커뮤니티</li>
            <li><a href="./Community_Photo_List.asp">갤러리-사진</a></li>
            <li><a href="./Community_Photo_Write.asp">갤러리-사진 등록/수정</a></li>
          </ul>
        </div>
        <!-- E: 네비게이션 -->
      </div>
      ...
      <form id="form1" name="form1" action="./Community_Photo_Write_p.asp" method="post" ENCTYPE="multipart/form-data">
        <table class="left-head view-table">
          ...
          <tr>
            <th>등록일</th>
            <td>
                <span class="regist_date"><%=InsDateCv %></span>
            </td>
          </tr>
          <% end if %>
              ...
              <span class="con">
                <div id="iFileDiv" name="iFileDiv">
                  <span id="sFile_1">
                    ...
                    <input type="file" id="iFile_1" name="iFile" onchange="javascript:CheckFiles_SizeTest1();">
          ...
          <tr>
            ...
            <td colspan="2">
              <span class="edit_box">
                <textarea name="iContents" id="iContents" rows="20" cols="100"><%=Contents %></textarea>
              </span>
            </td>
          </tr>
          ...
          <tr class="tiny-line">
            ...
            <td colspan="2">
              ...
              <span class="guide-txt">※공지는 최상위 <%=global_MainTPTCnt %>개만 상단에 노출됩니다.</span>
            </td>
          </tr>
      ...
      <!-- S: reply_wrap 리플영역 -->
      <div class="reply_wrap" id="divRP" style="display:none;">
        <form id="commentForm" name="commentForm">
          <p class="reply_number" id="reply_number" name="reply_number">답변 0개</p>
          <ul class="reply_list">
            <%
              Dim LCnt2        ' 데이터 행 개수
            ...
            %>
              <li>
                ...
                <p class="txt"><span id="cnt_<%=eRSeq2 %>"><%=Contents2 %></span></p>
                <% If LoginIDYN2 = "Y" Then %>
                <p  id="sp1_<%=eRSeq2 %>" class="btn_reply"> <a href="javascript:;" class="btn btn-confirm" onClick="javascript:Reply_Link('<%=iMSeq %>','<%=NowPage %>','2','<%=eRSeq2 %>');">수정</a> <a href="javascript:;" class="btn btn-red" onClick="javascript:ReplyDel_Link('<%=iMSeq %>','<%=NowPage %>','2','<%=eRSeq2 %>');">삭제</a> </p>
                <p  id="sp2_<%=eRSeq2 %>" class="btn_reply" style="display:none;"> <a href="javascript:;" class="btn btn-add" onClick="javascript:CommentSave_Link('<%=iMSeq %>','<%=NowPage %>','2','<%=eRSeq2 %>');">저장</a> <a href="javascript:;" class="btn btn-cancel" onClick="javascript:No_Link('<%=eRSeq2 %>');">취소</a> </p>
                <% End IF %>
              </li>
            <%
                  LRs.MoveNext
            ...
            %>
            <li class="last"> <span id="spiContentsMain">
              <p class="tit"><strong><%=iName %></strong></p>
              <textarea id="iContentsMain" name="iContentsMain"></textarea>
              <a href="javascript:;" class="btn_basic" onClick="javascript:CommentSave_Link('<%=iMSeq %>','<%=NowPage %>','1','<%=eRSeq2 %>');">입력하기</a> </span>
  ...
  <script type="text/javascript">
    ...
    function FileUp() {
      ...
      if (ImgChkExt == "N" || ImgChkLength == "N") {
      	document.getElementById('iFile_1').value = "";
      }
      else {
    ...
    function CheckFiles_SizeTest1() {

    	var file = document.getElementById('iFile_1').files[0];
    	...
    	if ( file.type == "image/jpeg") {
    		...
    		img.onload = function () {
    			...
    			cfstw = Number(img.width);
    			cfsth = Number(img.height);
    			//alert(cfstw + " , " + cfsth);

    			if (cfstw == cfstw && cfsth == cfsth) {
    				FileUp();
    			}
    			else {

    				alert(cfstw + 'px X ' + cfsth + 'px 사이즈만 업로드 가능 합니다.');
