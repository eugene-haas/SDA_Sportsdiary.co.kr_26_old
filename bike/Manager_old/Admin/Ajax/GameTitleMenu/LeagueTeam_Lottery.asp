    ...
    <tbody>
      <tr id="DP_Tr">
          ...
          <%
            ...
            Redim Array_GameNum(Cint(TeamCnt)-1)

            For i = 0 To Cint(TeamCnt) - 1
              Array_GameNum(i) = ""
            Next
              ...
              For j = 0 To  Cint(TeamCnt) - 1
                ...
                If i < j AND i <> j Then
                  ...
                  If Array_GameNum(i) = "" Then
                    ...
                    Array_GameNum(i) = Array_DBLeagueGameNum(2,leagueGameNum - 1)
                  Else
                    'Array_GameNum(i) = Array_GameNum(i) & "," & leagueGameNum
                    Array_GameNum(i) = Array_GameNum(i) & "," & Array_DBLeagueGameNum(2,leagueGameNum - 1)
                  End If

                  If Array_GameNum(j) = "" Then
                    'Array_GameNum(j) = leagueGameNum
                    Array_GameNum(j) = Array_DBLeagueGameNum(2,leagueGameNum - 1)
                  Else
                    'Array_GameNum(j) = Array_GameNum(j) & "," & leagueGameNum
                    Array_GameNum(j) = Array_GameNum(j) & "," & Array_DBLeagueGameNum(2,leagueGameNum - 1)
                  End If     
          ...
          %>
          <input type="hidden" name="LeagueGameNum" id="LeagueGameNum" value="<%=strGameNum%>">
          </table>
				</td>
      </tr>				
    </tbody>			
...
<script>
	...
	var $rightTable = $(".content-wrap.operate .lottery-50");
