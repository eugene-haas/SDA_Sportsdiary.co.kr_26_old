...
LSQL = " EXEC tblGameTourney_Searched_STR '" & DEC_GameTitleIDX & "', '" & GameDay & "', '" & DEC_StadiumIDX &"' ,'"  & StadiumNumber &"','" & "GameEmpty" & "','" & PlayLevelType & "' ,'" & DEC_TempNum & "'  ,'" & DEC_Searchkey & "'  ,'"  & DEC_Searchkeyword & "','" & GroupGameGb & "'"
'Response.WRite lsql
        ...
        <td>
          <%If GameEndGubun = "GameIng" Then%>
            <%If LRs("GroupGameGb") = "B0030001" Then%>
              <a href="#" onclick="OnResultBtnClick('<%=crypt.EncryptStringENC(LRs("GameLevelDtlIDX"))%>','<%=crypt.EncryptStringENC(LRs("TeamGameNum"))%>','<%=crypt.EncryptStringENC(LRs("GameNum"))%>','<%=crypt.EncryptStringENC("1")%>','<%=crypt.EncryptStringENC(LRs("GroupGameGb"))%>','')" class="<%=btncolor_win%>" data-toggle="modal" data-target=".play_detail_modal">경기중</a>
            <%Else%>
              <a href="#" onclick="OnGroupResultBtnClick('<%=crypt.EncryptStringENC(LRs("GameLevelDtlIDX"))%>','<%=crypt.EncryptStringENC(LRs("TeamGameNum"))%>','<%=crypt.EncryptStringENC(LRs("GameNum"))%>','<%=crypt.EncryptStringENC(LRs("GroupGameGb"))%>')" class="<%=btncolor_win%>" data-toggle="modal" data-target=".winner-sign">경기중</a>
            <%End If%>
          <%Else%>
            <%If LRs("GroupGameGb") = "B0030001" Then%>
            ...
            <%Else%>
              <a href="#" onclick="OnGroupResultBtnClick('<%=crypt.EncryptStringENC(LRs("GameLevelDtlIDX"))%>','<%=crypt.EncryptStringENC(LRs("TeamGameNum"))%>','<%=crypt.EncryptStringENC(LRs("GameNum"))%>','<%=crypt.EncryptStringENC(LRs("GroupGameGb"))%>')" class="<%=btncolor_win%>" data-toggle="modal" data-target=".winner-sign">선택</a>
            <%End If%>
          <%End If%>
        </td>
        <td>
