...
<%
  ...
  REQ = Request("REQ")
  'REQ = "{""CMD"":2,""tIdx"":""A0B63180CC3215B403232E31C8E393B4"",""ROWKEY"":0,""tGameTitleName"":""2018 전국 여성배드민턴대회 및 전국시도대항리그전"",""GameType"":""F9A43D4DE4191C125B08095CC465CD4B"",""tTeamGb"":""2F9A5AB5A680D3EDDEE944350E247FCB"",""fileName"":""2018 전국여성배드민턴대회 참가현황_종합수정본20180615-개인전.xlsx"",""tTotalCnt"":""480""}"
  ...
  Dim strPath : strPath = "D:\badminton.sportsdiary.co.kr\badmintonAdmin\FileDown\xls\PersonGame\"  &fileName
  ...
  player1_sidoCode = FN_GetSidoCode(player1_sido)
  ...
  ELSE
    if(ISNULL(isSidoTeam1) = False) Then
  ...
  player2_sidoCode = FN_GetSidoCode(player2_sido)
  ...
  ELSE
    if(ISNULL(isSidoTeam2) = False) Then
  ...
  'Response.WRite "player1_team : " & player1_sidoCode & "<br>"
  'Response.WRite "player1_teamNm : " & player1_sido & "<br>"
  'Response.WRite "player2_team : " & player2_sidoCode & "<br>"
  'Response.WRite "player2_teamNm : " & player2_sido & "<br>"
  ...
  if cdbl(tGameLevelIdx) > 0 Then
    IF constSidoType = selType Then
        ...
        LSQL = LSQL & " values ('"&tIdx & "','" &  tGameLevelIdx & "','" & tGameTitleName & "','" & tEnterType & "','" & tGroupGameGb &"','"& player1_sidoCode &"')" 
        LSQL = LSQL & " SELECT @@IDENTITY as IDX "
        ...
        LSQL = LSQL & " values ('"&tIdx & "','" &  tGameLevelIdx & "' ,'"  &  GroupRequestIdx & "' ,'" & player1_MemberIDX & "','" & player1_name & "','" & player1_sidoCode & "', '" & player1_sido &"')" 
        LSQL = LSQL & " SELECT @@IDENTITY as IDX "
        ...
        IF tPlayType = "B0020002" Then
          LSQL = " SET NOCOUNT ON insert into tblGameRequestPlayer " 
          LSQL = LSQL & " ( GameTitleIDX, GameLevelIDX, GameRequestGroupIDX,MemberIDX, MemberName, Team, TeamName) "
          LSQL = LSQL & " values ('"&tIdx & "','" &  tGameLevelIdx & "' ,'" & GroupRequestIdx & "' ,'" & player2_MemberIDX & "','" & player2_name & "','" & player2_sidoCode & "', '" & player2_sido &"')" 
          LSQL = LSQL & " SELECT @@IDENTITY as IDX "
          ...
          End IF
        End IF
    ELSEIF constTeamType = selType Then
       '개인전 그룹
        LSQL = " SET NOCOUNT ON insert into tblGameRequestGroup " 
        LSQL = LSQL & " ( GameTitleIDX, GameLevelIDX, GameTitleName, EnterType, GroupGameGb, Team) "
