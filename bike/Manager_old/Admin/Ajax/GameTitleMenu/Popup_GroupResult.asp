          ...
          <!-- E: 테이블 리스트 -->	
            ...
            <table cellspacing="0" cellpadding="0">
              ...
              <tr <%If i mod 2 = 1 Then%>style="background:#f7f9fb;"<%End If%>>
                ...
                <td rowspan="2"><a href="#" class="modified_btn btn-blue" onclick="OnGroupResultDtlBtnExcelClick('<%=crypt.EncryptStringENC(LRs("GameLevelDtlIDX"))%>','<%=crypt.EncryptStringENC(LRs("TeamGameNum"))%>','<%=crypt.EncryptStringENC(LRs("GameNum"))%>','<%=crypt.EncryptStringENC("1")%>','')">출력</a></td>
                <td rowspan="2"><a href="#" class="modified_btn" onclick="OnGroupResultDtlBtnClick('<%=crypt.EncryptStringENC(LRs("GameLevelDtlIDX"))%>','<%=crypt.EncryptStringENC(LRs("TeamGameNum"))%>','<%=crypt.EncryptStringENC(LRs("GameNum"))%>','<%=crypt.EncryptStringENC("1")%>','')">수정</a></td>
              </tr>                
              <tr <%If i mod 2 = 1 Then%>style="background:#f7f9fb;"<%End If%>>
                <td><%=LRs("RPlayer1")%></td>
                <td><%=LRs("RPlayer2")%></td>                
