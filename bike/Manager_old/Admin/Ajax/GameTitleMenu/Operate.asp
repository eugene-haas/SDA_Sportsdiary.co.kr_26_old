...
REQ = Request("Req")
'REQ = "{""CMD"":2,""GameDay"":"""",""StadiumIDX"":""7E2B6A8B1DAF27BFC004DF097801F4CE"",""StadiumNumber"":null,""PlayType"":""D096EC5BB8B02F96118B096392C18758"",""IngType"":""45E500538E0FD452A763FA5041E332C2"",""SchUserName"":""""}"
        ...
        <%End If%>
          ...
          <li>
            <%If LRs("LTourneyGroupIDX") <> "" AND LRs("RTourneyGroupIDX") <> "" Then %>
                <%If GameEndGubun = "GameEnd" Then%>
                <a onclick="sch_GameResult('<%=crypt.EncryptStringENC(LRs("GameLevelDtlIDX"))%>','<%=crypt.EncryptStringENC(LRs("TeamGameNum"))%>','<%=crypt.EncryptStringENC(LRs("GameNum"))%>','<%If LRs("LTourneyGroupIDX") = "" Then Response.Write "" ELSE Response.Write crypt.EncryptStringENC(LRs("LTourneyGroupIDX")) End If%>','<%If LRs("RTourneyGroupIDX") = "" Then Response.Write "" ELSE Response.Write crypt.EncryptStringENC(LRs("RTourneyGroupIDX")) End If%>')" class="btn btn_result" data-toggle="modal" data-target=".detail_result">결과</a>
                ...
                <a onclick="cli_enterscore('<%=crypt.EncryptStringENC(LRs("GameTitleIDX"))%>','<%=crypt.EncryptStringENC(LRs("GameLevelDtlIDX"))%>','<%=crypt.EncryptStringENC(LRs("TeamGameNum"))%>','<%=crypt.EncryptStringENC(LRs("GameNum"))%>')" class="btn btn_gray">경기중</a>
                <%Else%>
                <a onclick="cli_enterscore('<%=crypt.EncryptStringENC(LRs("GameTitleIDX"))%>','<%=crypt.EncryptStringENC(LRs("GameLevelDtlIDX"))%>','<%=crypt.EncryptStringENC(LRs("TeamGameNum"))%>','<%=crypt.EncryptStringENC(LRs("GameNum"))%>')" class="btn btn_gray">입력</a>
                <%End If%>
            <%End If%>
          </li>
        </ul>
    ...
    %>
        ...
        <%End If%>
          <li>
            ...
            <div class="up_floor">
              <span class="num"><%=LRs("TempNum")%></span>
              <span class="cont"><!--1조 / --><%=LRs("TeamGameNum")%>경기 [<%=LRs("Sex") & " " & LRs("TeamGbNM") & " " & LRs("LevelNM") & " " & LRs("GameTypeNM")%>]</span>
            </div>
            <!-- E: up_floor -->
