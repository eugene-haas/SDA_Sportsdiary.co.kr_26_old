...
%>
      <tr>
        <td>
          <span>
          <%
            If LRs("StadiumNum") = "" Or IsNull(LRs("StadiumNum")) Then
              Response.Write "-"
            Else
              Response.Write LRs("StadiumNum")
            End If
          %>코트
          </span>
        </td>      
        ...
        <td>
          ...
          <%If GameEndGubun = "GameIng" Then%>
            ...
            <%Else%>
              <a href="#" onclick="OnGroupResultBtnClick('<%=crypt.EncryptStringENC(LRs("GameLevelDtlIDX"))%>','<%=crypt.EncryptStringENC(LRs("TeamGameNum"))%>','<%=crypt.EncryptStringENC(LRs("GameNum"))%>','<%=crypt.EncryptStringENC("1")%>','')" class="<%=btncolor_win%>" data-toggle="modal" data-target=".group_modal">경기중</a>
            <%End If%>
          <%Else%>
            <%If LRs("GroupGameGb") = "B0030001" Then%>
            ...
            <%Else%>
              <a href="#" onclick="OnGroupResultBtnClick('<%=crypt.EncryptStringENC(LRs("GameLevelDtlIDX"))%>','<%=crypt.EncryptStringENC(LRs("TeamGameNum"))%>','<%=crypt.EncryptStringENC(LRs("GameNum"))%>','<%=crypt.EncryptStringENC("1")%>','')" class="<%=btncolor_win%>" data-toggle="modal" data-target=".group_modal">선택</a>
            <%End If%>
          <%End If%>
        </td>
        <td>
          <!--
          <%If GameEndGubun = "GameIng" Then%>
            <a href="#" onclick="OnAnotherResultBtnClick('<%=crypt.EncryptStringENC(LRs("GameLevelDtlIDX"))%>','<%=crypt.EncryptStringENC(LRs("TeamGameNum"))%>','<%=crypt.EncryptStringENC(LRs("GameNum"))%>')" class="<%=btncolor_anowin%>">경기중</a>
          <%Else%>
            <a href="#" onclick="OnAnotherResultBtnClick('<%=crypt.EncryptStringENC(LRs("GameLevelDtlIDX"))%>','<%=crypt.EncryptStringENC(LRs("TeamGameNum"))%>','<%=crypt.EncryptStringENC(LRs("GameNum"))%>')" class="<%=btncolor_anowin%>">선택</a>
          <%End If%>
          -->
          <%
