    ...
    <div class="sign-list ">
      <table cellspacing="0" cellpadding="0">
        ...
        <tr>
          ...
          <td>
          <%
            '경기결과페이지는 점수가 찍히는 출력페이지
            If DEC_PageGubun = "ResultPage" Then%>
            <a href="scoreboard.asp?GameLevelDtlidx=<%=LRs("GameLevelDtlIDX")%>&TeamGameNum=<%=LRs("TeamGameNum")%>&GameNum=<%=LRs("GameNum")%>&IsPrint=1&StadiumNum=<%=StadiumNum%>" class="modefid-btn">출력</a>
          <%Else%>
            <a href="scoreboard.asp?GameLevelDtlidx=<%=LRs("GameLevelDtlIDX")%>&TeamGameNum=<%=LRs("TeamGameNum")%>&GameNum=<%=LRs("GameNum")%>&IsPrint=0&StadiumNum=<%=StadiumNum%>" class="modefid-btn">출력</a>
          <%End If%>
          </td>
        </tr>
        <%
  ...
  <div class="order-footer">
    <ul class="btn-list clearfix">
      ...
      <%
        Else
      %>
      <li><a href="#" class="btn btn-blue" onclick="OnGroupResultPrintClick('<%=GameLevelDtlIDX%>', '<%=TeamGameNum%>', '<%=GameNum%>', '0', '<%=GroupGameGb%>', '<%=TempNum%>');">단체전기록지 출력</a></li>
      <%End If%>
    </ul>
  </div>
  <!-- E: order-footer -->
