...
<%
	...
	LSQL = LSQL & "   	,A.txtMemo"	
	LSQL = LSQL & "   	,CASE WHEN A.LeaderType = '2' THEN '감독' ELSE '코치' END LeaderTypeNm"
	LSQL = LSQL & "   	,A.LeaderType"
	LSQL = LSQL & "   	,A.LeaderTypeSub"
	LSQL = LSQL & "   	,E.PubName LeaderTypeSubNm"
	LSQL = LSQL & "   	,B.UserName"
	LSQL = LSQL & "   	,B.UserEnName"
	LSQL = LSQL & "   	,CASE WHEN B.Birthday<>'' THEN SUBSTRING(replace(B.Birthday,'-',''), 1, 4)+'.'+SUBSTRING(replace(B.Birthday,'-',''), 5, 2)+'.'+SUBSTRING(replace(B.Birthday,'-',''), 7, 2) END UserBirth"
	...
	FndData = FndData & " <tbody>"
  	...
  	IF Not(LRs.Eof Or LRs.Bof) Then 
    	...
    	Do Until LRs.eof
			...
			FndData = FndData & " <td>"&LRs("LeaderTypeNm")
		
			IF LRs("LeaderType") = "3" AND LRs("LeaderTypeSub") <> "" Then 
				FndData = FndData & "("&LRs("LeaderTypeSubNm")&")"				
			End IF
   	 	...
   	 	Loop  
    ELSE
      	FndData = FndData & "<tr><td colspan=13>일치하는 정보가 없습니다.</td></tr>"
    End IF  
      	LRs.Close
    SET LRs = Nothing
