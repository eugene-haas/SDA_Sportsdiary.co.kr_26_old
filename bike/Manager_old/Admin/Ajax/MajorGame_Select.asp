...
<%
	...
	dim CSQL, CRs
	dim RE_DATA
	dim strMajor, strDivision, strMajorGame
	 
	
	IF len(code) > 1  Then
	 	strMajor = Split(code, ",")	 
		strDivision = strMajor(0)									  
		strMajorGame = strMajor(1)									  						  
	End IF
	
	RE_DATA = "<select name='"&attname&"' id='"&attname&"' class=""title_select"">"
	RE_DATA = RE_DATA & "<option value=''>대회구분Ⅱ</option>"		 
	...
	CSQL = CSQL & "		AND PubType = '"&strDivision&"' "													  
	...
	If Not (CRs.Eof Or CRs.Bof) Then 
		Do Until CRs.Eof 
			If strMajorGame <> "" AND strMajorGame = CRs("PubCode") Then    		
