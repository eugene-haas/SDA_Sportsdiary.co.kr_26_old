      ...
      <table class="table table-arr table-fix-body" id="tableGameLevelList">
        ...
        <%
          ...
          If(cdbl(tIdx) > 0) Then
            ...
            End IF
            iType = "1"                      ' 1:조회, 2:총갯수
            LSQL = "EXEC tblGameNumberLevel_Searched_STR '" & tIdx & "','" & tStadiumIdx & "'"
        ...
        Dim WhereLevelIDX : WhereLevelIDX =""
          ...
          If(cdbl(tIdx) > 0) Then
            ...
            IF tStadiumIdx = "" Then
              tStadiumIdx = 0
            End IF
            LSQL = "EXEC tblGameNumberTourney_Searched_STR '" & NowPage & "','" & PagePerData & "','" & BlockPage & "','" & iType & "','" & iDivision & "','" & iSearchCol & "','" & iSearchText & "','" & iSubType & "','" & iTemp & "','" & tIDX & "','" & WhereLevelIDX  &"','" & tStadiumIdx & "','" & iLoginID & "'"        
            response.Write "LSQL=LSQL=LSQL=LSQL=LSQL=LSQL=LSQL="&LSQL&"<br>"
            Set LRs = DBCon.Execute(LSQL)
              If Not (LRs.Eof Or LRs.Bof) Then
                Do Until LRs.Eof
