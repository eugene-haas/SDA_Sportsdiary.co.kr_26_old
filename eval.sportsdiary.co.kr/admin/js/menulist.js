...
mn.CMD_CHANGEORDER = 10;



mn.ajaxurl = "/api/req/reqMenu.asp";
mn.ajaxtype = "POST";
mn.dataType = "text";
...
mn.SendEx = function (sender, packet, urlAjax) {
	...
	$.ajax({
		url: urlAjax, type: mn.ajaxtype, data: strdata, dataType: mn.dataType,
...
///////////////////////////
//순서변경
mn.changeOrder = function (idx, orderno, changeorderno) {
	var obj = {};
	obj.CMD = mn.CMD_CHANGEORDER;
	obj.IDX = idx;
	obj.ORDERNO = orderno;
	obj.CHANGEORDERNO = changeorderno;
...
mn.adminWriteOK = function (cmd) {
	...
	obj.ABIRTH = $("#ad_birth").val();
	if (obj.ABIRTH == '') {
		alert("생년월일 8자리로 입력해 주십시오.");
		$("#ad_birth").focus();
		return;
	}
	obj.APHONE = $("#ad_phone").val();
	if (obj.APHONE == '') {
...
mn.OnwritePop = function (cmd, packet, html, sender) {
	if ($('#' + sender).length == 0) {
		$('body').append("<div id='" + sender + "' class='modal fade' data-backdrop='static' role='dialog' aria-labelledby='myModalLabel'></div>");
	}
	$('#' + sender).html(html);
	$('#' + sender).modal('show');
};
...
mn.OnToggleBtn = function (cmd, packet, html, sender) {
	...
	if (packet.BTNTYPENO == 20) {
		if ($('#' + sender).hasClass('btn-warning')) {

			$('#' + sender)[0].className = 'btn btn-fix-sm btn-success';
			$('#' + sender).html('아이디');
