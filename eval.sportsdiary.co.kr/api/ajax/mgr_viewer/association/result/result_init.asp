...
<%
' http://eval_test.sportsdiary.co.kr/api/ajax/mgr_viewer/association/result/result_init.asp
%>
...
<%
   ...
   '=================================================================================
	'  평가 List 가져오기 
	'=================================================================================  
	Function getSqlEvalList(association_idx)
		Dim strSql, err_no
		
      If(association_idx = "") Then err_no = 1 End If 
		
		If( err_no <> 1 ) Then 	        
         ' 특정 협회를 평가했던 평가 Table List를 구한다 .
         strSql = strSql & " ;with cte_association As ( "
         strSql = strSql & "    Select EvalTableIdx From tblAssociation_sub "
         strSql = strSql & sprintf("       Where DelKey = 0 And AssociationIDX = {0} ", Array(association_idx))
         ...
         strSql = strSql & " Where T.DelKey = 0 And T.EvalTableIDX In (Select EvalTableIdx From cte_association) "
...
DBOpen()
   ...
   association_idx		   = InjectionChk(JsonObj.get("association_idx"))
	...
	If( err_no <> 1 ) Then 	
		'  ----------------------------------------------------
      '  평가 List 가져오기 
      strSql = getSqlEvalList(association_idx)
