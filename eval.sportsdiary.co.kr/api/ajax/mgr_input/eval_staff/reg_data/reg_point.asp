...
<%
' http://eval_test.sportsdiary.co.kr/api/ajax/mgr_input/eval_staff/reg_data/reg_point.asp
%>
...
<%
	...
	Function getSqlCheckValid(eval_table_idx, association_idx, eval_member_idx, eval_points, item_idxs)
      Dim strSql, err_no
		...
		If( err_no <> 1 ) Then 	        
         ...
         strSql = strSql & sprintf("       Select Cast(value As decimal(5,2)) As point From string_split('{0}', ',') " ,Array(eval_points))
         strSql = strSql & "    ) As C "
         strSql = strSql & " ) "
         ...
         strSql = strSql & "    Select I.EvalItemTypeIDX, (I.StandardPoint* 100) As StandardPoint, (I.StandardPoint * (C.point/5) * 100) As point "
         strSql = strSql & "       From cte_item As I  "
         strSql = strSql & "       Inner Join cte_client_item As C On C.item_type_idx = I.EvalItemTypeIDX "
         strSql = strSql & "       And (I.StandardPoint * ( Cast(C.point As Float) / 5)) > I.StandardPoint	 "
         strSql = strSql & " ) "
   ...
   Function getSqlInsertPoint(eval_table_idx, association_idx, eval_member_idx, eval_points, item_idxs)
      ...
      If( err_no <> 1 ) Then 	
         ...
         strSql = strSql & sprintf(" T.EvalItemTypeIDX, {0} As AdminMemberIDX, T.EvalTypeCD, T.EvalTypeNm, StandardPoint, I.Point* 100,  ", Array(eval_member_idx))
         strSql = strSql & " Case When EvalTypeCD = 1 Then Round(StandardPoint * (Cast(Point As Float) / 5), 2) * 100 Else I.Point* 100 End As PointCalc "
   ...
   '=================================================================================
	...
	Function getSqlModPoint(eval_table_idx, association_idx, eval_member_idx, eval_points, item_idxs)
      ...
      strSql = strSql & " Declare @group_cd int          "
         ...
         strSql = strSql & " Update tblEvalValue Set  Point = I.point*100, PointCalc = Case When EvalTypeCD = 1 Then Round(StandardPoint * (Cast(I.Point As Float) / 5), 2) * 100 Else I.Point* 100 End "
