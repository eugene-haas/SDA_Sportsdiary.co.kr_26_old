...
function fldEnc(fldvalue)
	fldEnc = " EncryptByKey(Key_GUID('eval_ability_key'), '"&fldvalue&"') "
end function

function fldDec(encfld)
	fldDec = "  Cast(DECRYPTBYKEY("&encfld&") As varChar(max)) as " & encfld
end function

'키오픈, 클로우즈 붙임
function QueryKeyOpenClose(query)
  ...
  QueryKeyOpenClose = C_OPENKEY & query & C_CLOSEKEY
end function
