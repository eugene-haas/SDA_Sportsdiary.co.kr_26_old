      ...
      <div class="l_main">
		...
		<section class="l_main__contents">
          <div class="list-pro"style=" padding-top:20px;">
					...
					<%
							If IsArray(arrMn) Then
								For ari = LBound(arrMn, 2) To UBound(arrMn, 2)
									...
									m_CDANM = arrMn(1,ari)
									%>
									<option value="D2" <%If CDA = m_CDA then%>selected<%End if%>><%=m_CDANM%></option>
									<%
								Next
							End if
					%>
					  ...
					  <span  id="s_cdbc">
								...
								<select id="cdbc" class="list-pro__search__selc-box__selc" onchange="mx.getMachTable('<%=tidx%>',$(this).val())" >
			...
			<div class="list-pro__tbl" id="sw_gametable"></div>
          </div>
