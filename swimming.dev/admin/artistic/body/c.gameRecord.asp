...
<%
 'Controller ################################################################################################
	...
	'search
	If chkBlank(F2) Then
		strWhere = " DelYN = 'N' and ( gameS >=  '"& year(date) & "-01-01" &"' and  gameS < '"& CDbl(year(date))+1 & "-01-01" &"' )  and GameTitleIDX = (select top 1 GameTitleIDX from tblRgameLevel where GameTitleIDX = a.GameTitleIDX and DelYN='N' and cda = 'F2')  "
	Else
		strWhere = " DelYN = 'N' and ( gameS >=  '"& F2 & "-01-01" &"' and  gameS < '"& CDbl(F2)+1 & "-01-01" &"' )  and GameTitleIDX = (select top 1 GameTitleIDX from tblRgameLevel where GameTitleIDX = a.GameTitleIDX and DelYN='N' and cda = 'F2') "
	End if
...
%>
		...
		<tr id="titlelist_<%=l_idx%>" style="text-align:center;">
			...
			<td><span>
			<a href="javascript:px.goSubmit({'TIDX':<%=l_idx%>,'DIDX':'','KYN':'<%=l_kgame%>'},'inputRecord3.asp')" class="btn btn-primary">경기운영</a>
			</span></td>
