...
<%
 'Controller ################################################################################################
  ...
  'request 처리##############
	...
	If tidx = "" Then
	'마지막 대회 번호를 가져오자.(다이빙)
	SQL = "Select top 1 " & fld & " from tblRGameLevel as a where delyn = 'N' and CDA='E2' and CDC<>'31' order by gametitleidx desc, gbidx asc "
	else
...
End if
  ...
  '++++++++++++++++++++++++
	...
	SQL = "select "&fld&" from tblRGameLevel  as a where delyn = 'N' and gametitleidx =  " & tidx  & "  and CDA='E2' and CDC<>'31'  " & sexstr & cdcstr
	Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
...
<%'View ####################################################################################################%>
      <div class="box box-primary">
		...
		<div class="box-body" id="gameinput_area">
			...
			<!-- s: 등록화면 -->
			  <!-- #include virtual = "/pub/html/swimming/inputRecordForm2.asp" -->
			<!-- e: 등록 화면 -->
			<%'End if%>
        </div>
      </div>
