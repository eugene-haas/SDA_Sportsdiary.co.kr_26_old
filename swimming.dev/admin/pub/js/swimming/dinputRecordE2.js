...
mx.CMD_SETROUNDDEDUCTION = 80; //감점상태변경
mx.CMD_SETROUNDEND = 85; //진행완료만들기
...
mx.CMD_ORDERLIST = 30010;
mx.CMD_FINDMEMBER = 30020;
mx.CMD_CHANGEMEMBER = 710;
////////////////////////////////////////

mx.ajaxurl = "/pub/ajax/swimming/reqInputRecordE2.asp";
mx.ajaxtype = "POST";
mx.dataType = "text";
...
mx.ReceivePacket = function (reqcmd, data, sender) {// data는 response string
		...
		switch (Number(jsondata.result)) {
			...
			case 112:
				if (confirm(jsondata.servermsg)) {
					this.setreferee(jsondata.TIDX, jsondata.LIDX, "confirmpass");
				}
				return; break;
	...
	switch (Number(reqcmd)) {
		...
		case mx.CMD_SETROUNDEND: //진행완료만들기
		case mx.CMD_SETTOTALOUT: //실격(전체)
		case mx.CMD_SETROUNDDEDUCTION: //감점상태변경
...
//요청##################################################################
mx.changeMember = function (midx, pidx, newpidx) {
	var obj = {};
	obj.CMD = mx.CMD_CHANGEMEMBER;
	obj.MIDX = midx;
	obj.PIDX = pidx;
	obj.NEWPIDX = newpidx;
...
//mx.getGameList('game_13120',13120,'오후','')


mx.findMember = function (unm, midx, pidx) {
	var obj = {};
	obj.CMD = mx.CMD_FINDMEMBER;
	obj.UNM = unm;
	obj.MIDX = midx;
	obj.PIDX = pidx;
	mx.SendPacket('findmemberarea', obj);
}

mx.orderList = function (midx, pidx) {
	var obj = {};
...
mx.setAppShow = function (btnnm, lidx) {
	...
	switch ($('#' + btnnm).attr('class')) {
		case "btn btn-primary": $('#' + btnnm).attr('class', 'btn bg-green'); $('#' + btnnm).text('경기종료'); fld = "DAY1"; break;
...
mx.setRoundValue = function (tidx, lidx, midx, roundno, judgeno, setvalue) { //심판이 주는 각점수
	//그냥 보내 안에서 체크하고 말자.
	if (setvalue > 10) {
...
mx.setRoundDeduction = function (tidx, lidx, midx, roundno, btnstate) {//감점
	var msg = "감점 처리하시겠습니까?";
	if (btnstate == "danger") {
		msg = "감점 처리를 해제하시겠습니까?";
	}
	...
	var obj = {};
	obj.CMD = mx.CMD_SETROUNDDEDUCTION;
	obj.TIDX = tidx;
	obj.LIDX = lidx;
	obj.MIDX = midx;
	obj.RNO = roundno;
...
mx.setRoundEnd = function (tidx, lidx, midx, roundno, btnstate) {//진행완료만들기
	var msg = "라운드를 진행 처리하시겠습니까?";
	if (btnstate == "danger") {
		msg = "진행상태를 해제하시겠습니까?";
	}
