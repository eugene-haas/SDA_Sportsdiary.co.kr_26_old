...
'request
  ...
  SQL = SQL & " SELECT (case when CHARINDEX('팀',cdcnm,1) >0 and cnt <4 then 'fail' when CHARINDEX('듀엣',cdcnm,1) >0 and cnt < 2 then 'fail' else 'success' end) as state from chktbl "
  SQL = SQL & " ) as a "
