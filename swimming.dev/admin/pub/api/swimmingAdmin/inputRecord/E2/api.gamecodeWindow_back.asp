															...
															For ari = LBound(arrR, 2) To UBound(arrR, 2)
																	...
																	<tr <%If j_gamecodeseq <> "0" then%>style="background:#A6B3C1"<%End if%>>
																		...
																		<td  style="text-align: center;">
																			...
																			<select id="cd_<%=r_idx%>" class="form-control" style="width:100%" onchange="mx.setGamePer(<%=j_idx%>, $(this).val()   ,<%=tidx%>,<%=lidx%>,<%=midx%>)">
																					<%If j_gamecodeseq = "0" then%>
																					<option value="" >난이율선택</option><!-- 난의도 등의 설정 가져오기 -->
																					<%End if%>
																					<%
																						If IsArray(arrC) Then 
																							For c = LBound(arrC, 2) To UBound(arrC, 2)
																								...
																								r_codename = arrC(8,c)
																								%><option value="<%=r_idx%>" <%If Cstr(j_gamecodeseq) = Cstr(r_idx) then%>selected<%End if%> ><%'=r_title%>[ 번호:<%=r_code1%> ]&nbsp;자세:<%=r_code3%>&nbsp;높이 &nbsp;<%=r_code2%>(<%=r_codename%>)&nbsp;난이율:<%=r_code4%></option><%
																							Next
																						End If
																					%>
																			</select>																		
