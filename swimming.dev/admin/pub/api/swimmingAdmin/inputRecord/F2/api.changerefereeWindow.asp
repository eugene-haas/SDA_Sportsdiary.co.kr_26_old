...
'#############################################
	...
	Set db = new clsDBHelper
		...
		jidxarr = Split(getBooJudge( tidx, lidxs , judgecnt, gameround, db,  ConStr),",")
														...
														lastno = UBound(arrR, 2)
															For ari = LBound(arrR, 2) To UBound(arrR, 2)
																	...
																	<tr id="setjftr_<%=j_seq%>" style="background:<%=trbg%>">
																		...
																		<td width="20%" style="text-align:right"><a href="javascript:mx.changeJudgeOK(<%=tidx%>,<%=lidx%>,<%=jidx%>,<%=j_jseq%>  ,<%=myfldno%> ,'<%=targetfldno%>','<%=cdc%>', <%=gameround%>)" class="btn btn-danger">변경</a></td>
																	</tr>
																	<%
																	End If
