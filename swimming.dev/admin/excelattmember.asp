...
'	SQL = "Select "&fld&" from tblGameRequest_imsi as a  where a.team in ("&teamwhere&")  and a.tidx = '"&F2&"' and delyn = 'N'  order by a.team"
	...
	fld = fld & " ,c.userphone as '선수번호'  ,c.email as '이메일' ,c.usernameen as '영문명' ,c.usernamecn as '한자' "
				...
				<tr class="gametitle">
					<%
						For i = 0 To Rs.Fields.Count - 1
								...
								If i  = 4 Then
									If InStr(rsdata(i),"초등") > 0 Then
										If isNumeric(rsdata(5)) then

											If CDbl(rsdata(5)) < 5 Then
												rsdata(i) = Replace(rsdata(i) , "초등", "유년")
											End If
								...
								Response.write "<td style=""mso-number-format:'\@'"">" & rsdata(i)   & "</td>"
						Next
					%>
				</tr>
			<%
