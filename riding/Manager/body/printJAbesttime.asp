	<%For i = 1  To CDbl(pagecnt)%>
		<div class="page">
			...
			<table class="obstacles s_TableA">
				<thead>
					...
					<th class="obstacle"><%=hurdleArr(x)%></th>
					<%next%>

					<th class="etc">비고</th>

					<th class="duration_c">소요<br>시간</th>
					<th class="timePenalty_c">벌초</th>
					<th class="totalDuration_c">총소요<br>시간</th>
		...
		%>
					<tr>
						...
						<td class="partName"><%=r_a10%></td> <!-- 통합부명 -->
						<td class="playerName"><%=r_a4%></td> <!-- 선수명 -->
						<td class="horseName"><%=r_b2%></td> <!-- 마명 -->
						<td class="belongName"><%=r_a9%></td> <!-- 소속 -->

						<%For x = 0 To ubound(inputdata)%>
						<td class="obstacle"><%=inputdata(x)%></td> <!-- 장애물 -->
						<%next%>
						...
						<td class="timePenalty"><%=r_s2%></td> <!-- 벌초 -->
						<td class="totalDuration">
						<%If r_a8 = "0" then%>
