...
'Call rsdrow(rss)
	...
	If tidx = "" Then
		%>
		<script>px.goSubmit( {'F1':[0,1,2],'F2':['<%=year(date)-1%>','',''],'F3':[]} , 'judge.asp');</script>
		<%
			...
			<div class="row">
					...
					<%Case else%>
						...
						<div class="col-sm-7 text-center bg-primary">
							...
							&nbsp;&nbsp;<a href="javascript:$('#printdiv').printThis({importCSS: false,loadCSS: 'http://ridingadmin.sportsdiary.co.kr/pub/js/print/print_table.css',header: '<h1><%=select_f_title%></h1>'});" class="btn btn-default">인쇄</a></span>
			...
			<thead>
				...
				<%Else '마장마술아닌것들 (장애물)%>
					...
					<tr>
						...
						<a href="javascript:$('#printdiv').printThis({importCSS: false,loadCSS: '/pub/js/print/print_table.css',header: '<h1><%=select_f_title%></h1>'});" class="btn btn-default">인쇄</a>&nbsp;&nbsp;</span>
