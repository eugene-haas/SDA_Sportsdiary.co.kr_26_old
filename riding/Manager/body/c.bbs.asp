...
<%
 'Controller ################################################################################################
	...
	strFieldName = " SEQ,CATE,TITLE,CONTENTS,REGDATE,MODDATE,REGID,REGNAME,VISIT,viewyn,(select top 1 FILENAME from tblTotalBoard_File b where a.SEQ = b.TotalBoard_SEQ and b.delyn = 'N' order by seq desc) as photo  "

	strSort = "  order by seq desc"
	strSortR = "  order by seq"

	F0 = oJSONoutput.Get("F0")
	If F0 = "" Then
		F0 = year(date)
	End If
	
	sdate = F0 & "-01-01"
	edate = CDbl(F0) + 1 & "-01-01"

	'search
	If chkBlank(F2) Then
		strWhere = " a.DelYN = 'N' and  a.cate = 0 and regdate >= '"&sdate&"' and regdate < '"&edate&"' "
	Else
		strWhere = " a.DelYN = 'N' and  a.cate = 0 and  a.title Like '%"&F2&"%' and regdate >= '"&sdate&"' and regdate < '"&edate&"'  "
	End if
...
<%'View ####################################################################################################%>
		...
		<div class="admin_content" >
			<div class="page_title"><h1>포토갤러리</h1></div>
				...
				<!-- s -->
					<div class="form-horizontal">
						...
						<div class="form-group">
							...
							<div class="col-sm-2">
								<select id="F0" class="form-control"  onchange="px.goSubmit( {'F0':$('#F0').val() ,'F1':$('#F1').val(),'F2':$('#F2').val()} , '<%=pagename%>')">
								  <%For yy = year(date) To 2020 Step - 1%>
								  <option value="<%=yy%>" <%If CStr(F0) = CStr(yy) then%>selected<%End if%>><%=yy%></option>
								  <%next%>
							  </select>
							</div>
							...
							<div class="col-sm-2">
								...
								<input type="text" id="F2" class="form-control"  onkeydown="if(event.keyCode == 13){px.goSubmit( {'F0':$('#F0').val()    ,'F1':$('#F1').val(),'F2':$('#F2').val()} , '<%=pagename%>')}" value="<%=F1_2%>">
							</div>
		  ...
		  <div class="btn-toolbar" role="toolbar" aria-label="btns"></div>
			...
			If IsArray(arrR) Then
				...
				<tr id="titlelist_<%=l_seq%>">
					...
					<%else%>
					<a href="javascript:mx.fileuploadPop(2,<%=l_seq%>,'<%=SENDPRE%>')" class="btn btn-primary"><%=LCase(Mid(l_FILENAME, InStrRev(l_FILENAME, "/") + 1))%></a>
					<%End if%>
					</span>
					</td>
					<!-- <td>
					<a href="javascript:mx.editor(<%=l_seq%>,'<%=l_UserName%>','<%=SENDPRE%>')" class="btn btn-primary">내용등록</a>
					</td> -->
