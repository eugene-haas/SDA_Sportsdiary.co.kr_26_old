...
<%
 'Controller ################################################################################################
	...
	'search
	If chkBlank(F2) Then
		strWhere = " DelYN = 'N' and CATE in ('1','2','3','4')   "
	Else
		If InStr(F1, ",") > 0  Then
			F1 = Split(F1, ",")
			F2 = Split(F2, ",")
		...
		If IsArray(F1) Then
			...
			If F1_2 = "" then
				...
				strWhere = " DelYN = 'N'  and CATE = '"& F1_0 &"'   "
				End if
			Else

				strWhere = " DelYN = 'N' and CATE = '"& F1_0 &"'  and TITLE Like '%"&F1_2&"%' "
			End if
...
<%'View ####################################################################################################%>
		...
		<div class="admin_content" >
			<div class="page_title"><h1><%''=strWhere%>관련규정</h1></div>
						...
						<div class="form-group">
							...
							<div class="col-sm-2">
								<select id="mk_g0" class="form-control">
								  <option value="1" <%If CStr(e_CATE) = "1" then%>selected<%End if%>>정관</option>
								  <option value="2" <%If CStr(e_CATE) = "2" then%>selected<%End if%>>각 위원회 규정</option>
								  <option value="3" <%If CStr(e_CATE) = "3" then%>selected<%End if%>>기타규정</option>
								  <option value="4" <%If CStr(e_CATE) = "4" then%>selected<%End if%>>경기규정</option>
							  </select>
							</div>
						...
						<div class="form-group">
							...
							<div class="col-sm-2">
								...
								<select id="F1_0" class="form-control">
								  ...
								  <option value="1" <%If CStr(F1_0) = "1" then%>selected<%End if%>>정관</option>
								  <option value="2" <%If CStr(F1_0) = "2" then%>selected<%End if%>>각 위원회 규정</option>
								  <option value="3" <%If CStr(F1_0) = "3" then%>selected<%End if%>>기타규정</option>
								  <option value="4" <%If CStr(F1_0) = "4" then%>selected<%End if%>>경기규정</option>
							  </select>
							  </div>
							</div>
		  ...
		  <div class="btn-toolbar" role="toolbar" aria-label="btns"></div>
			...
			If IsArray(arrR) Then
				...
				<tr id="titlelist_<%=l_seq%>">
					...
					<td onclick="mx.input_edit(<%=l_seq%>,'<%=SENDPRE%>')"><span><%=lcate%></span></td>
