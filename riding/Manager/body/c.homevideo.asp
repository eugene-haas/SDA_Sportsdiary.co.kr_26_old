...
<%
 'Controller ################################################################################################
	...
	'search
	If chkBlank(F2) Then
		strWhere = " a.DelYN = 'N' and  a.cate = 1 and regdate >= '"&sdate&"' and regdate < '"&edate&"' "
	Else
		strWhere = " a.DelYN = 'N' and  a.cate = 1 and  a.title Like '%"&F2&"%' and regdate >= '"&sdate&"' and regdate < '"&edate&"'  "
	End If
		  ...
		  <div class="btn-toolbar" role="toolbar" aria-label="btns"></div>
			...
			If IsArray(arrR) Then
				...
				<tr id="titlelist_<%=l_seq%>">
					...
					<td>
					<input type="text" id="url_<%=l_seq%>" value= "<%=l_FILENAME%>" onblur="mx.videoURL(<%=l_seq%>,'<%=SENDPRE%>',$(this).val())">
					</td>
					<td>
