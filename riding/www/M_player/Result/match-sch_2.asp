...
<div id="app" class="l m_bg_f2f2f2 matchSch" v-cloak>
  ...
  <div class="l_content m_scroll [ _content _scroll ]">
    ...
    <!--  -->
    <div>
      <!-- <a href="javascript:mx.showTourn(41,2019,191)">대진표확인</a> -->
      ...
      <div class="m_roundLayer__content [ _overLayer__wrap ]">
        <div class="gameHeader">
          ...
          <div class="gameHeader__btnwrap">
            <button class="matchbtn" v-bind:class="{on: orderlist=='match'}" @click="refreshPlayerList('match')">출전순서<span class="m_img"><img src="http://img.sportsdiary.co.kr/images/SD/icon/settings_backup_restore_white_@3x.png" alt=""></span></button>
            <button class="matchbtn" v-bind:class="{on: orderlist=='rank'}" @click="refreshPlayerList('rank')">순위<span class="m_img"><img src="http://img.sportsdiary.co.kr/images/SD/icon/settings_backup_restore_white_@3x.png" alt=""></span></button>
          </div>
        </div>
        ...
        <div class="gameBoard__wrap">
          ...
          <div v-else>
            ...
            <ul v-if="!loading_pop" class="gameBoard" v-bind:class="{s_refresh: refresh_player_list}">
              <li class="gameBoard__item" v-for="(player,key) in player_list.list" v-bind:class="{s_notice: player.name=='공지사항', s_gubun:player.gubun=='1', rank:orderlist=='rank'}">
                <p class="gameBoard__header">
                  <span class="gameBoard__index" v-if="player.name!='공지사항'">{{orderlist=="match"?player.no:player.sno}}</span>
                  <span class="gameBoard__txt"><span class="gameBoard__time" v-if="player_list.title.match('마장마술')">{{player.time}}</span> <span v-bind:class="highlightTxt(player.name)">{{player.name=="공지사항"? player.txt : player.name}}</span></span>
                ...
                <div class="recordTable__wrap" v-bind:class="{on: key==showtableno}" v-if="player.tabletype!=''">
                  <div class="recordTable">
                    <table>
                      <thead>
                        <th v-if="!sumriding" colspan="3">{{playerGameRestart(player.name)}} 심사</th>
                        <th v-else colspan="4">마장마술 심사</th>
                      </thead>
                      <tbody>
                        ...
                        <tr v-if="player.tabletype=='A' && !sumriding">
                        ...
                        <tr v-if="player.tabletype=='MA' || player.tabletype=='MB' || player.tabletype=='SUM' || (player.status=='E'&&sumriding)">
                          <td colspan="2" v-bind:class="{s_gray2: sumriding, s_gray3: !sumriding}">총비율</td>
                          <td>{{tofix(player.tableinfo[0].judgeall)}}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                  <div class="recordTable" v-if="sumriding">
                    <table>
                      <colgroup>
                        <col style="width:25%;"><col style="width:25%;"><col style="width:25%;"><col style="width:25%;">
                      </colgroup>
                      <thead>
                        <th colspan="4">장애물 심사</th>
                      </thead>
                      <tbody>
                        <tr class="td4">
                          <td class="s_gray2 s_border">소요시간</td>
                          <td>{{player.tableinfo[0].timeall}}</td>
                          <td class="s_gray2 s_border">시간감점</td>
                          <td>{{player.tableinfo[0].timeminus}}</td>
                        </tr>
                        <tr class="td4">
                          <td class="s_gray2">장애감점</td>
                          <td>{{player.tableinfo[0].disminus}}</td>
                          <td class="s_gray2">감점합계</td>
                          <td>{{player.tableinfo[0].minusall}}</td>
                        </tr>
                      </tbody>
                    </table>
                  ...
                  <div class="recordTable">
                    <table>
                      <colgroup v-if="sumriding">
                        <col style="width:25%;"><col style="width:25%;"><col style="width:25%;"><col style="width:25%;">
                      </colgroup>
                      <thead>
                        <th colspan="3" v-if="!sumriding">{{playerGameRestart(player.name)}} 실시간</th>
                        <th colspan="4" v-else>복합마술 심사</th>
                      </thead>
                      <tbody>
                        <tr v-if="sumriding" v-bind:class="{td4: sumriding}">
                          <td colspan="2" class="s_gray1 s_border">총점</td>
                          <td colspan="2">{{player.tableinfo[0].totalscore}}</td>
                        </tr>
                        <tr v-bind:class="{td4: sumriding}">
                          <td rowspan="2" class="s_gray1">순위</td>
                          <td class="s_gray2 s_border">부별</td>
                          <td v-if="!sumriding">{{Number(player.tableinfo[0].rankingpart)<0?'-':player.tableinfo[0].rankingpart}}</td>
                          <td v-else colspan="2">{{Number(player.tableinfo[0].rankingpart)<0?'-':player.tableinfo[0].rankingpart}}</td>
                        </tr>
                        <tr v-bind:class="{td4: sumriding}">
                          <td class="s_gray2">전체</td>
                          <td v-if="!sumriding">{{Number(player.tableinfo[0].rankingall)<0?'-':player.tableinfo[0].rankingall}}</td>
                          <td v-else colspan="2">{{Number(player.tableinfo[0].rankingall)<0?'-':player.tableinfo[0].rankingall}}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
...
<script>
  ...
  let app=new Vue({
    ...
    methods:{
      tofix:function(val){
        if(val=="W" || val=="R" || val=="E" || val=="D" || val=="-"){
          return val;
        }else{
          if(this.sumriding){
            return val
      ...
      gameSchedule:function(day,activeno, popsearch,gnos){
        ...
        if(popsearch=="popsearch"){
          // url='http://riding.sportsdiary.co.kr/pub/ajax/riding/mobile/reqRiding.asp?req={"CMD":"63100","tidx":"'+this.tidx+'","gno":"'+gnos+'"}';//20190613
          url="testMatch_11.html";
        ...
        // 오늘 날짜가 아니면 처음거
          ...
          if(this.game_day_cur!=day){
            ...
            // }else{
              // url='http://riding.sportsdiary.co.kr/pub/ajax/riding/mobile/reqRiding.asp?req={"CMD":"63100","tidx":"'+this.tidx+'","getDate":"'+(this.yy+'-'+day.split('/')[0]+'-'+day.split('/')[1])+'","gno":"'+this.gnos+'"}';//20190613
          url="testMatch_11.html";
            // }
      ...
      openSearching:function(popdiv,etc){
        ...
        if(etc!=undefined){
          if(etc[0]=="playerlist"){
            ...
            this.gnoval=etc[1];
            // this.loadPlayerList('http://riding.sportsdiary.co.kr/pub/ajax/riding/mobile/reqRiding.asp?req={"CMD":"63400","tidx":"'+this.tidx+'","gno":"'+etc[1]+'"}');//20190613
            this.loadPlayerList('testMatchPlayerlist_3.html');//20190613
          }
        }
      ...
      loadPlayerList:function(url){
        ...
        axios.get(url).then(response=>{
          ...
          // 복합마술 인지 확인
          if(this.player_list.title.match("복합마술")!=null){
            this.sumriding=true;
          }else{
            this.sumriding=false;
          }
          
          // 순위로 볼 때 재경기 있으면 없애고, 맨위에 있던 순위 자리에 재경기 순위를 옮김
          if(this.orderlist=="rank"){
            let regame_index, regame_len, regame_list=this.player_list.list, new_list;
            regame_list.forEach((obj,index,ary)=>{
              if(obj.txt=="재경기"){
                regame_index=index+1;// index는 gubun 재경기의 index
                regame_len=ary.length;
              }
            });
            new_list=regame_list.splice(regame_index, regame_len).reverse();// 재경기 이후 배열 삭제(잘라내기)해서 반대로 정렬
            for(let i=0;i<Math.abs(regame_index-regame_len);i++){
              regame_list.shift();//재경기 수 만큼 앞순위 삭제
            }
            for(let i=0;i<Math.abs(regame_index-regame_len);i++){
              regame_list.unshift(new_list[i]);// 재경기 등수대로 앞에 넣기
            }
          }
          //
      ...
      refreshPlayerList:function(orderlist){
        ...
        setTimeout(()=>{
          ...
          }else if(this.orderlist=="rank"){
            this.loadPlayerList('http://riding.sportsdiary.co.kr/pub/ajax/riding/mobile/reqRiding.asp?req={"CMD":"63401","tidx":"'+this.tidx+'","gno":"'+this.gnoval+'"}');
          }
        },350);
      },
      ...
      tournamentDa:function(){
        ...
        //   console.log("line : 910 : "+Url);
        //   console.log("line : 911 : "+packet);
        //   if(Url==""){
        //     alert("Ajax Url 없음");
        //     return;
        //   }
        //   let strAjaxUrl=Url,
        //       strdata="REQ="+encodeURIComponent(JSON.stringify(packet));
        //   $.ajax({
        //     url:strAjaxUrl,
        //     // type:"POST",
        //     // dataType:"html",
        ...
        //           dataType="html";
        //           resdata=data.split("`##`");
        //           try{jsondata=JSON.parse(resdata[0]);}
        //           catch(ex){}
        //           htmldata=resdata[1];
        //         }else{
        //           dataType="json";
        ...
        //         // 위 함수에서 CMD를 switch로 분리. cmd 필요없을 듯
        //         document.getElementById('DP_TourneyChart').innerHTML = '<div id="tournament" class="tournament"></div>';
        //         tournament.draw({
        //           data:jsondata,
        //           roundOf:jsondata.TotalRound,
        //           el:document.getElementById('tournament')
        //         });
        //         // //
        //       }else{
        //         alert("데이터 비었다 : "+data);
        ...
        // };
        // // ############
        // function toruneyGangsu(gameleveldtlidx){
        //   let packet={};
        // }
        // // ############

        // // ############
        // CMD_TOURNEYCATEGORY = 1;
        // CMD_TOURNEYCHART = 2;
        // CMD_REQPLAYERTEAMNAME = 3;
        // CMD_GAMEORDERLIST = 4;
        // CMD_GAMEORDERPERSONPOPUP = 5;
        // CMD_GAMEORDERTEAMPOPUP = 6;
        // CMD_PERSONPOINTLIST = 7;
        // CMD_SIGN = 8;
        // CMD_FINALTOURNEYDRAW = 9;
        // CMD_FINALTOURNEYGANGSU = 10;
        // // ############
        ...
        //   let l_player1="", l_belong1="", l_score="", l_result=false,
        //       r_player1="", r_belong1="", r_score="", r_result=false;
        //   let l_player2="", l_belong2="",
        //       r_player2="", r_belong2="";
        //   let isDouble=false, isLive=false, onclick="";

        //   if(data){
        //     let l_players=data.Lplayer.split(","),
        //         l_belongs=data.Lteam.split(","),
        //         r_players=data.Rplayer.split(","),
        //         r_belongs=data.Rteam.split(",");
        //     l_player1=l_players[0], l_belong1=l_belongs[0], l_score=data.Ljumsu, l_result=data.Lresult,
        //     r_player1=r_players[0], r_belong1=r_belongs[0], r_score=data.Rjumsu, r_result=data.Rresult;
        //     l_player2=l_players[1], l_belong2=l_belongs[1],
        //     r_player2=r_players[1], r_belong2=r_belongs[1];
        //   }
        //   let html=[
        //     (isLive) ? '<div class="liveTxt">LIVE</div>' : '',
        //     '<p class="'+ ((isLive) ? 's_live' : '') +' ttMatch ttMatch_first" onclick="'+ onclick +'">',
        //       '<span class="ttMatch__playerWrap">',
        //         '<span class="ttMatch__playerInner">',
        //           '<span class="ttMatch__player">' + l_player1 + '</span>',
        //           '<span class="ttMatch__belong">'+ l_belong1 + '</span>',
        //           (l_player2) ? '<br />' : '',
        //           (l_player2) ? '<span class="ttMatch__player">' + l_player2 + '</span>' : '',
        //           (l_player2) ? '<span class="ttMatch__belong">'+ l_belong2 + '</span>' : '',
        //         '</span>',
        //       '</span>',

        //       '<span class="'+ ((isLive) ? 's_live' : '') +' ttMatch__score ' + (/^true$/i.test(l_result) ? 's_winner' : '') + '">' + l_score + '</span>',
        //     '</p>',
        //     '<p class="'+ ((isLive) ? 's_live' : '') +' ttMatch ttMatch_second" onclick="'+ onclick +'">',
        //       '<span class="ttMatch__playerWrap">',
        //         '<span class="ttMatch__playerInner">',
        //           '<span class="ttMatch__player">' + r_player1 + '</span>',
        //           '<span class="ttMatch__belong">'+ r_belong1 + '</span>',
        //           (r_player2) ? '<br />' : '',
        //           (r_player2) ? '<span class="ttMatch__player">' + r_player2 + '</span>' : '',
        //           (r_player2) ? '<span class="ttMatch__belong">'+ r_belong2 + '</span>' : '',
        //         '</span>',
        //       '</span>',

        //       '<span class="'+ ((isLive) ? 's_live' : '') +' ttMatch__score ' + (/^true$/i.test(r_result) ? 's_winner' : '') + '">' + r_score + '</span>',
        //     '</p>',
        //   ].join('');
        ...
        // function toruneyDrawSE(gameleveldtlidx){
        //   let packet={};
        //   Url="testtourne.html";
        //   packet.CMD = CMD_FINALTOURNEYDRAW;
        ...
        // // ############ start
        // // sch_TourneyJoName('3A8A2A6E2413DF697DAA7E24AF58F91B','DA45C492C26E35E67660267B05F37D42');
        // toruneyDrawSE('DA45C492C26E35E67660267B05F37D42');
      },
      //#####################################
      //#####################################
      //#####################################
    ...
    mounted:function(){
      ...
      //###########/////////
      this.tournamentDa();
      // axios.get("testtourne.html").then(response=>{
      // }).catch(error=>{
...
$(document).ready(function(){
  console.log("mx.showTourn");
  // mx.ajaxurl="testtourne.asp";
  $.getJSON("testtourne.asp",function(response){
    mx.tablejsondata=response;
    console.log(mx.tablejsondata);
  });
  // mx.drowLimitTourn(0,0);
});
</script>
</body>
</html>
