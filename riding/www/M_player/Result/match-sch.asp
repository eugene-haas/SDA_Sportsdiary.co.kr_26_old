...
<head>
  ...
  <script type="text/javascript" src="/pub/js/riding/match.js?v=190820"></script>
...
<%End if%>
  ...
  <div class="l_content m_scroll [ _content _scroll ]">
    ...
    <div class="m_roundBoard__wrap" v-if="game_schedule!='nodata'">
	  ...
	  <!-- 제ㅇ경기, 제ㅁ경기.. 등은 버튼클릭되고, 그 외 안내문구(준비운동, 경로조정 등)는 클릭안되게 -->
      <div class="m_roundBoard" v-for="(sdata,key) in game_schedule">
        ...
        <button class="scheduleItem" v-if="sdata.playerlist!=undefined" @click="openSearching('.gameDetailPopup',['playerlist',sdata.playerlist,sdata.title,sdata.gb,sdata.gidx,sdata.time,sdata.cmt])">
          <p class="scheduleItem__index">
            <span class="scheduleItem__indexTit">{{sdata.title}}</span>
            <span class="scheduleItem__indexTime">{{sdata.time}}</span>
          </p>
  ...
  <!-- gameDetail popup. 제ㅇ경기...등 눌렀을 때의 팝업 -->
        ...
        <div class="gameBoard__wrap">
          ...
          <div v-else>
            ...
            <ul v-if="!loading_pop" class="gameBoard" v-bind:class="{s_refresh: refresh_player_list}">
              <!-- '공지사항'인지 아닌지 구분, 출전순서/순위 일 때 구분선 유무, 체전 일 때 구분선 유무  -->
              <li class="gameBoard__item" v-for="(player,key) in player_list.list" v-bind:class="{s_notice: player.name=='공지사항', s_gubun:player.gubun=='1', rank:orderlist=='rank', all:game_title.match('체전')||game_title.match('전국체육대회'), allregame:game_title.match('체전')&&player.txt=='재경기'||game_title.match('전국체육대회')&&player.txt=='재경기'||game_title.match('체전')&&player.txt=='재경기1'||game_title.match('전국체육대회')&&player.txt=='재경기1'}">
                  ...
                  <div class="recordTable">
                    <table>
                      ...
                      <tbody>
						...
						<!-- 지구력 -->
                        <tr v-if="player.tabletype=='G' ">
                          <td colspan="2" class="s_gray2 s_border">최종기록</td>
                          <td>{{player.tableinfo[0].record}}</td>
                        </tr>
                        <tr v-if="player.tabletype=='G' ">
                          <td colspan="2" class="s_gray2 s_border">평균시속</td>
                          <td>{{player.tableinfo[0].perspeed}}</td>
                        </tr>
                        ...
                        <!-- 마장마술(MA, MB), 복합마술(SUM), 실권(E)이면서 복합마술 일 때 -->
                        <tr v-if="player.tabletype=='MA' || player.tabletype=='MB' || player.tabletype=='SUM' || (player.status=='E'&&sumriding)">
                          ...
                          <td>{{tofix(player.tableinfo[0].judgeall)}}</td><!-- tofix는 소수점 3번째 자리까지 보이려고. 총비율에만 적용. 다른데는 json데이타가 .000까지 오는듯 -->
                        </tr>
                      </tbody>
                    </table>
                  </div>
                    ...
                    <table>
                      ...
                      <tbody>
                        ...
                        <tr v-bind:class="{td4: sumriding}">
						  ...
						  <td class="s_gray2 s_border" v-if="player.tabletype=='G' ">결과</td>
						  <td class="s_gray2 s_border" v-if="player.tabletype!='G' && gb.substring(0,3) == '202' ">개인</td>
						  <td class="s_gray2 s_border" v-if="player.tabletype!='G' && gb.substring(0,3) != '202' ">부별</td>
                        ...
                        <tr v-bind:class="{td4: sumriding}">
                          <td class="s_gray2" v-if="gb.substring(0,3) == '202' ">단체</td>
                          <td class="s_gray2" v-else>전체</td>
                ...
                <!-- 기록 열고 닫기 -->
                <button class="gameBoard__btn" v-bind:class="{on: key==showtableno}" v-if="player.status=='경기종료' || player.status=='E' || player.status=='R' || (player.status=='W' && orderlist=='rank' && player.name.match('재경기') || player.status=='D' && orderlist=='rank' && player.name.match('재경기'))" @click="showRecord(key)">
  ...
  <!-- 릴레이 팝업     -->
  <div class="l_upLayer relayPop [ _overLayer _searchLayer ]">
    <div class="l_upLayer__backdrop [ _overLayer__backdrop ]"></div>
	...
	<div class="l_upLayer__contBox [ _overLayer__box ]">
      ...
      <div class="m_roundLayer__header">
		...
		<div class="m_roundLayer__content [ _overLayer__wrap ]" id="relaypop"><!-- 내용붙여넣을곳 --></div>
  ...
  <!--  릴레이팝업 -->
	...
	<div class="inner-popup__layer" id="realyplayerpop" >
      
	  <div class="inner-popup" id="relayattmember">
...
<script>
  ...
  let app=new Vue({
    ...
    methods:{
      ...
      openSearching:function(popdiv,etc){
        ...
        if(etc!=undefined){
          if(etc[0]=="playerlist"){
			...
			this.gb = etc[3];
			if (this.gb == "20208"){ //릴레이
				mx.getRelayInfo(this, this.tidx, etc[4], etc[2], etc[5],etc[6]);
				return;
			}
			else{
      ...
      loadPlayerList:function(url){
        ...
        axios.get(url).then(response=>{
		  ...
		  this.gb = response.data.jlist[0].gb;
          ...
          if(this.orderlist=="rank"){
            let regame_index, regame_len, regame_list=this.player_list.list, new_list,
                allgame_index;// 체전일 때 '경기결과' 위치
            regame_list.forEach((obj,index,ary)=>{
              if(obj.txt=="재경기" || obj.txt=="재경기1"){
                regame_index=index+1;// index는 gubun 재경기의 index
                regame_len=ary.length;
              }
              if(this.game_title.match("체전")!=null && obj.txt=="경기결과" || this.game_title.match("전국체육대회")!=null && obj.txt=="경기결과"){
                allgame_index=index+1;
              }
            });
            new_list=regame_list.splice(regame_index, regame_len).reverse();// 재경기 이후 배열 삭제(잘라내기)해서 반대로 정렬

            if(this.game_title.match("체전")!=null && this.player_list.title.match("전국체전")!=null){// 체전이면서 재경기
              regame_list.splice(allgame_index, Math.abs(regame_index-regame_len));// 재경기 수 만큼 경기결과 위에서부터 삭제
              for(var i=0;i<Math.abs(regame_index-regame_len);i++){
                regame_list.splice(allgame_index, 0, new_list[i]);
              }
            }else{// 재경기
              for(var i=0;i<Math.abs(regame_index-regame_len);i++){
          ...
          // 다음날 2경기 할 때 2경기 위치로
          setTimeout(() => {
            if(this.game_all_title2!="" && this.game_all_title2.match("-2")!=null){
              document.querySelector(".gameBoard").scrollTop=document.querySelector(".s_gubun.all").offsetTop-Math.ceil(document.querySelector(".s_gubun.all").offsetHeight/2);
            }
          }, 100);
    ...
    created:function(){
	  ...
	  let param_tidx = urlParams.get('tidx');
	  if ( Number(param_tidx) > 0 ) {
		  this.tidx=param_tidx;
		  console.log(this.tidx);
	  }
  
	  
	/*	   request값이 더생겨서 내용변경 by baek
