...
<%
	...
	'시작후 가져올수 있다.
	seq = Upload.Form("SEQ")
	sendpre = Upload.Form("SENDPRE")
	chkno = Upload.Form("CHKNO")
	...
	Set File = Upload.Form("upfile") '여러개 파일폼중에 선택할수 있는 방법으로 만들어서
	orgfilenm = File.FileName

	'filename = seq & "." & Split(orgfilenm , "." )(1)
	filename = seq & "." & orgfilenm
	...
	'디비 업데이트
	Select Case sendpre
	Case "setgame_" '참가자정보 업로드
		SQL = "Update sd_tennisTitle Set  exlfile = '"&dbsavefilename&"'  where gametitleidx = " & seq
	Case "rule_","comlist_","datalist_"
		SQL = "Update tblSimpleBoard Set  FILE_PATH = '"&global_filepath_DB&"', FILE_NAME = '"&filename&"'  where seq = " & seq
	Case "kor_"
		SQL = "Update tblPlayer_korea Set  ProfileIMG = '"&dbsavefilename&"'  where seq = " & seq

	Case "notice_","news_","notice2_","shortcourse_","photo_","video_"
		SQL = "insert into tblTotalBoard_file (TotalBoard_SEQ,filename) values ("&seq&", '"&dbsavefilename&"')  "
	End Select
