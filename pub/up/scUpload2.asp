...
'	End if
	...
	tidx = Upload.Form("f_tidx2")
	teamcd = Upload.Form("f_teamcd2")
	teamnm = Upload.Form("f_teamnm2")
	leaderidx = Upload.Form("f_leaderidx2")

	Set File = Upload.Form("inpFileApply2") '여러개 파일폼중에 선택할수 있는 방법으로 만들어서
	
	orgfilenm = File.FileName
	
	filename = tidx & "_" & teamnm & "_" & leaderidx &  "_B." & right(orgfilenm , 3)
	...
	'디비 업데이트 
	SQL = "Select idx,team from sd_schoolConfirm where team = '"&teamcd&"' and gametitleidx = '"&tidx&"' and leaderIDX = '"&leaderidx&"'  and gubun = 2 "
	Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
	If rs.eof then
		SQL = "insert into  sd_schoolConfirm (gubun,gametitleidx,team,teamnm,fileUrl ,leaderIDX) values (2,"&tidx&",'"&teamcd&"','"&teamnm&"','"&dbsavefilename&"' , "&leaderidx&")  "
