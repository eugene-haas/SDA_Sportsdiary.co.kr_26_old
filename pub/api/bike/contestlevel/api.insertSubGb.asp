...
'######################
	...
	If CStr(CMD) = CStr(CMD_INSERTSUBGB) then
		'중복값확인
		SQL = "select top 1 levelno,title from sd_titleList where title = '"&stitle&"' and subtitle = '"&ssubtitle&"' and detailtitle is null order by levelno"
		...
		'생성
		SQL = " insert into sd_titleList (title,subtitle) values ('"&stitle&"', '"&ssubtitle&"')"
