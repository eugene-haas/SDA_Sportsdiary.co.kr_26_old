...
'######################
	...
	If hasown(oJSONoutput, "GH") = "ok" then
		g_h = oJSONoutput.GH
	End if	
	If hasown(oJSONoutput, "GO") = "ok" then
		g_o = oJSONoutput.GO
	End if	
	...
	'중복값확인
	SQL = "select idx from K_openList where name =  '"&newname&"'  and gubun = " & gubun
	...
	'생성
	SQL = " insert into K_openList (gubun , name) values ("&gubun&",'"&newname&"')" 
