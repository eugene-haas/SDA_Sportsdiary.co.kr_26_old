<%
	...
	Function ran_name_func()
		Dim HTable_ChoSung,HTable_JungSung,HTable_LastD,choarr,jungarr,lastgarr,rnd_value,cho,jung,las,player
		'성 
		HTable_ChoSung = "서울,부산,대구,인천,광주,대전,울산,경기,강원,충북,충남,전북,전남,경북,경남,제주,세종"
		'이름
		...
		'끝 리스트
        HTable_LastD = "김,이,박,최,정,강,조,윤,장,임,한,오,서,신,권,황,안,송,전,홍,유,고,문,양,손,배,조,백,허,유,남,심,노,정,하,곽,성,차,주,우,구,신,임,전,민,유,류,나,진,지,엄,채,원,천,방,공,강,현,함,변,염,양,변,여,추,노,도,소,신,석,선,설,마,길,주,연,방,위,표,명,기,반,라,왕,금,옥,육,인,맹,제,모,장,남,탁,국,여,진,어,은,편"

		choarr = Split(HTable_ChoSung,",")
		jungarr = Split(HTable_JungSung,",")
		lastgarr = Split(HTable_LastD,",")
		...
		Randomize
		rnd_value = clng( ((UBound(lastgarr)-1) * Rnd) + 0 )
		las = lastgarr(rnd_value)
	...
	Function make_player_func()
            ...
            Dim TeamIDX,Team,TeamNm,sidosq,sido,sidoNm,TeamTel,ZipCode,Address,AddrDtl
            Dim sidoStr1,sidoStr2
			...
			i = 0
			Do Until rs.eof 
                  teamLast = rs("teamLast")
			      TeamLen = rs("TeamLen")
			i = i + 1
			rs.movenext
			loop

            FOR c=(LEN(teamFirst) + LEN(teamLast)) TO (TeamLen-1)
                teamFirst=teamFirst&"0"
            NEXT

            '팀명
            Team   = teamFirst & teamLast
			TeamNm = ran_name_func()

            '지역 
            Randomize
			sidosq = clng((16 * Rnd) + 0 )
			sidoStr1 = array("01","02","03","04","05","06","07","08","09","10","11","12","13","14","15","16","50")
            sidoStr2 = array("서울","부산","대구","인천","광주","대전","울산","경기","강원","충북","충남","전북","전남","경북","경남","제주","세종")
            sidoNm= sidoStr2(sidosq)
			sido = sidoStr1(sidosq)
            TeamNm = sidoNm &  TeamNm

            '주소
            ZipCode =  make_random_func(1, 5 , 0, 9 ,"")
            ...
            insertvalue = " 'tennis','','"&Team&"','"&TeamNm&"','','','3','"& sido &"','"&ZipCode&"','"&sidoNm&"','','"&uphone&"', convert(nvarchar,GETDATE(),112) ,'','A','"&Team&"','','',GETDATE(),GETDATE(),'N','Y' "

			SQL = "SET NOCOUNT ON INSERT INTO tblTeamInfo ( "&insertfield&" ) VALUES " 
			SQL = SQL & "( "&insertvalue&" ) SELECT @@IDENTITY"

			Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
			pidx = rs(0)
			make_player_func = array(pidx,Team,TeamNm,sidoNm,uphone,ZipCode,sidoNm,"")
