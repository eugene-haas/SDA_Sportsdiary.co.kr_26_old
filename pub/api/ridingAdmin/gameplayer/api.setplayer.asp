	...
	End Select
		...
		If CDbl(acctotal) > 0 Then '금액이 0이상인 경우만
			...
			'==========================================
				...
				SQL = "Update SD_RookieTennis.dbo.TB_RVAS_MAST Set STAT_CD = '1' ,IN_GB = '2', PAY_AMT = " & acctotal & ", CUST_CD = '" & Left(sitecode,2) & idx & "' ,CUST_NM = '"&CONST_PAYCOM&"' , USER_NM = '" & p1nm & "',ENTRY_DATE= '" & ConvertDateFormat(Now()) & "' where " & vcwhere
