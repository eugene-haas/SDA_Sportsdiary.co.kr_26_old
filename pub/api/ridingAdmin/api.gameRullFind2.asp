...
'#############################################
    ...
    '생성 여부 확인하고 찍어준다.
    SQL = "select * from sd_TennisKATARullMake where mxjoono = '" & CStr(tidx) & CStr(fstr2_str) & "' order by sortno"
...
%> 
<table class='table-list' border='1' id="gametablelist">
<thead id="headtest">
<th>본선대진 ID</th>
<th>순번</th>
<th>예선순위</th>
...
<tr class="gametitle" id="tr_<%=p_idx %>"
		...
		<%end if %> >
			...
			<td><%=p_mxjoono%> <%if i=1 then  %> <input type=hidden value="<%=p_mxjoono %>" id="mxjoono" /> <%end if  %></td>
			<td> 
                <input type="number" style="width:70px; height:80%;  margin:5px;" id="s_<%=p_idx %>_<%=p_sortno %>" value="<%=p_sortno%>"  onfocus="this.select();" onchange="mx_gameRull.trjoonoUpdate(this);"/>
            </td>
            <td 
			<%if  p_orderno = "1" then %> style="color:red;width:30px;">
			<%elseif  p_orderno = "0" then %> style="color:blue;width:30px;">
			<%else %> style="width:30px;">
			<%end if %> 
                <input type="number" style="width:50px;height:80%; margin:5px;"id="o_<%=p_idx %>_<%=p_orderno %>" value="<%=p_orderno%>"  onfocus="this.select();" onchange="mx_gameRull.trjoonoUpdate(this);"/>
            </td>
			<td>


                <%'if  p_orderno = "0" then  %>
                    <!-- <span style="color:Blue;width:30px;">Bye</span> -->
                <%'elseif cdbl(p_seed) >0 and p_orderno = "1"  then %>
                     <!-- <span style="color:red;width:30px;"><%'=p_joono %></span> -->
                <%'else %>
                <input type="number" style="width:50px;height:80%; margin:5px;"id="j_<%=p_idx %>_<%=p_joono %>" value="<%=p_joono%>"  onfocus="this.select();" onchange="mx_gameRull.trjoonoUpdate(this);"/>
                <%'end if %>


            </td>
			<%if cdbl(p_seed) >0 and p_orderno = "1" then %>
				<td style="background-color:#454;color:#fff;width:30px;"><%=p_seed%></td>
			<%else %>
				<td >
                    <%if  p_orderno = "0" then  %>
                        <span style="color:blue;width:30px;">Bye</span>
           ...
           <!-- <td style="width:30px;">
				<%if p_joono <> 0 then %> <%=p_joono%>
				<input type="number" id="b_<%=p_orderno%>_<%=p_joono%>" value="<%=p_joono%>"  onfocus="this.select();" onchange="mx_gameRull.trjoonoSave(this);"/>
...
<%
		...
		if i mod 8 =0 then 
			%>
				<tr style="height:5px;"> 
					<td colspan="9" style="background-color:black;height:20px;"> </td>
				</tr>
			<%
		    ...
		    if i mod 4 =0 then 
			    %>
				    <tr style="height:5px;"> 
					    <td colspan="9" style="background-color:black;height:10px;"> </td>
				    </tr>
			    <%
		        ...
		        if i mod 2 <> 1 then 
			        %>
				        <tr style="height:5px;"> 
					        <td colspan="9" style="background-color:black;height:3px;"> </td>
				        </tr>
			        <%
		        end if
