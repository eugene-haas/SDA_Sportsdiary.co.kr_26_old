...
& " ,max(AA.gamenostr) "_
  ...
  sql = "select "_
      ...
      &" where AA.DelYN = 'N' and BB.DelYN = 'N' and DD.DelYN = 'N' and AA.GameTitleIDX = '"& tidx &"' and AA.gameno = '"& gameno &"' and BB.gubun < 100  "_
	  ...
	  &" order by BB.[Round],BB.gamest desc, BB.total_order,tryoutsortno " 'tryoutgroupno 공지순서번호
...
'쿼리 결과값으로 다시 가져오기
  ...
  sql = "select "_
      ...
      &" ,DD.ridingclass,DD.ridingclasshelp,BB.[Round],BB.tryoutsortNo,BB.tryoutgroupno,BB.gubun,BB.noticetitle "_
      &" ,AA.judgeB,AA.judgeE,AA.judgeM,AA.judgeC,judgeH,BB.per_1,BB.per_2,BB.per_3,BB.per_4,BB.per_5,tryoutresult,gamenostr "_
	  &" order by BB.tryoutresult, BB.total_order "
'	  &" order by BB.gamest desc, BB.total_order,tryoutsortno " 'tryoutgroupno 공지순서번호
		...
		If IsArray(arr) Then
			...
			if TeamGb = "20103" And BMresult <> "OK" Then '복합마술
				If  InStr(ridingclass,"마장마술") > 0  Then	'Else '장애물
					...
					SQL = "Select "&fldnm&" from SD_tennisMember where "&strwhere&" order by [round],gamest desc, total_order,tryoutsortno  "
					Set rs = db.ExecSQLReturnRS(SQL , null, ConStr) 
					if rs.eof Then
						arrRS = ""
					Else
...
'지구력
'#############################################################
Function setJsonTorsGG(arr, tidx)
	Dim rsObj,subObj, i
	Dim ar
...
If chkteamgb = "20105" Or chkteamgb = "20205" then
jsondata =  setJsonTorsGG(arrRS, tidx)
else
jsondata =  setJsonTors(arrRS, tidx)
End if
