...
'#############################################
	...
	If idx = "0" Then
		...
		strField = " gameMemberIDX1, gameMemberIDX2,  gubun,gamekey3,gamekeyname,GameTitleIDX, level,tryoutgroupno,m1set1,m1set2,m1set3,m2set1,m2set2,m2set3  " 'gubun 0 예선
		else
		strField = " gameMemberIDX1, gameMemberIDX2,  gubun,gamekey3,gamekeyname,GameTitleIDX, level,tryoutgroupno  " 'gubun 0 예선
		End if
		...
		SQL = SQL & "," & player2IDX
		If CDbl(startgamescore) > 0 Then
		SQL = SQL & "," &gubun&", "&gamekey3&", '"&gamekeyname&"',"&gametitleidx&", '"&levelkey&"',"&jono&","&startgamescore&","&startgamescore&","&startgamescore&","&startgamescore&","&startgamescore&","&startgamescore&")" 'stateno 0 진행전, 2진행중, 1 완료
		Else
		SQL = SQL & "," &gubun&", "&gamekey3&", '"&gamekeyname&"',"&gametitleidx&", '"&levelkey&"',"&jono&")" 'stateno 0 진행전, 2진행중, 1 완료
		End if
		Call db.execSQLRs(SQL , null, ConStr)
	...
	Else
		...
		strTableName = " sd_TennisResult "
		strField = " a.gameMemberIDX1, a.gameMemberIDX2, a.stateno, a.gubun,gamekey3,gamekeyname,GameTitleIDX "',startrecive,startrecive1,startrecive2,secondrecive,secondrecive1,secondrecive2 " 
		'gubun 0 예선
	
		strField = strField & " ,a.courtno,a.courtkind,a.recorderName,a.startserve,a.secondserve,a.courtmode,a.preresult,      b.setno,b.gameno,b.playsortno,b.gameend    ,startrecive,secondrecive "
		...
		reciveno = rs("startrecive")
		reciveno2 = rs("secondrecive")
	...
	Call oJSONoutput.Set("SERVE", serveno ) '서브위치 번호 1.2 위 3, 4 아래  (idx)
	Call oJSONoutput.Set("SERVE2", serveno2 ) '서브위치 번호 1.2 위 3, 4 아래
	Call oJSONoutput.Set("RECIVE", reciveno ) '서브위치 번호 1.2 위 3, 4 아래
	Call oJSONoutput.Set("RECIVE2", reciveno2 ) '서브위치 번호 1.2 위 3, 4 아래
	...
	'################################
	'Call oJSONoutput.Set("debug", strSQL )
	'################################
