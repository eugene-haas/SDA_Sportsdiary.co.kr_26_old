...
'	End if
    ...
    if idx <> "" then
		...
		'SQL = SQL &"  and CONVERT(varchar(10),a.GameTitleIDX) = '"& idx & "'"
    Else
		strWhere = "  and CONVERT(varchar(10),a.writedate,121) >= '"&   dateAdd("m",-3, Date())  &"' "
		'SQL = SQL&"  and CONVERT(varchar(10),a.writedate,121) >= '"&   dateAdd("m",-3, Date())  &"' "
	end if
         ...
         %>
			...
			<li class="entryList__item">
                <p class="entryList__row s_header">
					...
					<span class="entryList__cell s_team1">선수</span>
			...
			<li class="entryList__item">
	            <a href="javascript:chk_frm('FndPwd','<%=strSt %>');" class="entryList__row">
					...
					<span class="entryList__cell s_gameTitle"><%=rs("GameTitleName") %></span>
					<span class="entryList__cell s_level"><%=TeamGbNm %>(<%=LevelNm %>)
						...
						<span class="entryList__status s_nonDeposit"> 미입금 </span>

								<%else%>

						<span class="entryList__status s_deposit"> 입금완료 </span>
						...
						<span class="entryList__status s_depositStandby"> 대기 </span>
