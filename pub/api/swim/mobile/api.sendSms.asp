...
SMS_Msg = SMS_Msg & "- 참가자 : "&ph2nm&"("&ph2tnm&")\n "
...
Else '가상계좌
	...
	If CDbl(acctotal) > 0 Then '금액이 0이상인 경우만
		...
		If Not rs.eof Then
			...
			SMS_Msg = SMS_Msg & "- 은행명 : "&CONST_BANKNM&"\n"
...
contents = SMS_Msg
indexCode = Replace(Date,"-","") & Replace(time,":","") ' now()
...
Set db = Nothing
%>
<%If aaaa ="가상계좌 안보냄 문자 안보냄" then%>
<form id="form<%=phI %><%=toNumber %>" action='http://biz.moashot.com/EXT/URLASP/mssendutf.asp' method='post' target="hiddenFrame">
