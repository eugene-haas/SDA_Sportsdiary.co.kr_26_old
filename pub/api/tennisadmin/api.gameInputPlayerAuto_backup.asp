...
'#############################################
	...
	Set db = new clsDBHelper
		...
		subSql = " and UserName not in (Select P1_UserName from tblGameRequest Where  GameTitleIDX = " & titleIDX  & ")"
		subSql = subSql & " and UserName not in (Select P2_UserName from tblGameRequest Where  GameTitleIDX = " & titleIDX & ")"

		strSql = "SELECT top 2 PlayerIDX,UserName,UserPhone,Birthday,Sex,TeamNm,Team2Nm  from tblPlayer where SportsGb = 'tennis' and DelYN = 'N' " & subSql & " ORDER BY NEWID() "
		...
		gnoquery = " ( select max(requestGroupNum) + 1 from tblGameRequest ) "
		insertfield = " SportsGb, GameTitleIDX,Level,EnterType,UserPass,UserName,UserPhone,txtMemo, requestGroupNum " '여러팀을 등록했을시 ...
		insertfield = insertfield & " ,P1_PlayerIDX,P1_UserName,P1_UserLevel,P1_TeamNm,P1_Team2Nm,P1_UserPhone,P1_Birthday,P1_SEX "
		insertfield = insertfield & " ,P2_PlayerIDX,P2_UserName,P2_UserLevel,P2_TeamNm,P2_Team2Nm,P2_UserPhone,P2_Birthday,P2_SEX "
		insertvalue = " 'tennis',"&titleidx&",'"&levelno&"','A','null','운영자','01000000000','자동생성', " & gnoquery		
		...
		If IsArray(arrRS) Then
			...
			Select Case ar 
				Case 0
					...
					p1birth =	 arrRS(6, ar) 
					p1sex =	 arrRS(7, ar) 
					insertvalue = p1idx & " , "&p1nm&",'"&p1level&"','"&p1t1&"','"&p1t2&"','"&p1phone&"','"&p1birth&"','"&p1sex&"' "
				Case 1
					p2idx = arrRS(0, ar) 
					p2nm = arrRS(1, ar) 
					p2level =	 arrRS(2, ar) 
					...
					p2birth =	 arrRS(6, ar) 
					p2sex =	 arrRS(7, ar) 
					insertvalue = p2idx & " , "&p2nm&",'"&p2level&"','"&p2t1&"','"&p2t2&"','"&p2phone&"','"&p2birth&"','"&p2sex&"' "
				End Select
			Next
...
'			Dim i, uphone,ubirth,sex,unm,insertfield,insertvalue,teamno,teamnm,pidx
'			Dim uinfo
'	
'				uphone = "01000000000"
'				ubirth = "00000000"
...
'				'Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
'
'				insertfield = " SportsGb,PlayerGb,UserName,UserPhone,Birthday,Sex,Level,EnterType,Team,TeamNm "
'				insertvalue = " 'tennis','te045001','"&unm&"','"&uphone&"','"&ubirth&"','"&sex&"','null','A','"&teamno&"','"&teamnm&"' "
'
'				SQL = "SET NOCOUNT ON INSERT INTO tblPlayer ( "&insertfield&" ) VALUES " 
'				SQL = SQL & "( "&insertvalue&" ) SELECT @@IDENTITY"
'				Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
'				pidx = rs(0)
'				make_player_func = array(pidx,unm,teamnm,uphone,ubirth,sex)
'		End Function
'
'		userinfo = make_player_func()
'		userinfo2 = make_player_func()

'		gnoquery = " ( select max(requestGroupNum) + 1 from tblGameRequest ) "
'		insertfield = " SportsGb, GameTitleIDX,Level,EnterType,UserPass,UserName,UserPhone,txtMemo, requestGroupNum " '여러팀을 등록했을시 ...
'		insertfield = insertfield & " ,P1_PlayerIDX,P1_UserName,P1_UserLevel,P1_TeamNm,P1_UserPhone,P1_Birthday,P1_SEX "
'		insertfield = insertfield & " ,P2_PlayerIDX,P2_UserName,P2_UserLevel,P2_TeamNm,P2_UserPhone,P2_Birthday,P2_SEX "
'		insertvalue = " 'tennis',"&titleidx&",'"&levelno&"','A','null','운영자','01000000000','자동생성', " & gnoquery
'		insertvalue = insertvalue & " , "&userinfo(0)&",'"&userinfo(1)&"','A','"&userinfo(2)&"','"&userinfo(3)&"','"&userinfo(4)&"','"&userinfo(5)&"' "
'		insertvalue = insertvalue & " , "&userinfo2(0)&",'"&userinfo2(1)&"','A','"&userinfo2(2)&"','"&userinfo2(3)&"','"&userinfo2(4)&"','"&userinfo2(5)&"' "
'		SQL = "SET NOCOUNT ON  INSERT INTO tblGameRequest ( "&insertfield&" ) VALUES ("&insertvalue&")  SELECT @@IDENTITY" 
'		Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
'
'		idx = rs(0)
...
'		End Select
'
'		p1 = "<span style='color:blue'>" & userinfo(1) & "</span>"
'		p2 = "<span style='color:red'>" & userinfo2(1) & "</span>"
'
'		player = p1 & "&nbsp;&nbsp;&nbsp;" & p2
		...
		'대진표등록
		'################################################
'		insertfield = " gubun, GameTitleIDX,PlayerIDX,userName,gamekey1,gamekey2,gamekey3,TeamGb,key3name,TeamANa,TeamBNa "
'		insertvalue = " 0, "&titleidx&", "&userinfo(0)&", '"&userinfo(1)&"', '"&gamekey1&"',"&Left(levelno,3)&","&levelno&","&Left(levelno,5)&",'"&Split(teamgbnm,"(")(0)&"','"&userinfo(2)&"',null   "
...
'		insertvalue = " "&gamemidx&", "&userinfo2(0)&", '"&userinfo2(1)&"','"&userinfo(2)&"',null   "
'		SQL = "Insert into sd_TennisMember_partner ("&insertfield&") values ("&insertvalue&")"
'		rs = db.ExecSQLReturnRS(SQL , null, ConStr)		
