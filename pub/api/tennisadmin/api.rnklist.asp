...
If CDbl(CMD) = CMD_SETRNKINFO And teamgb <> "" Then '사용함 안함으로 설정 변경
	...
	SQL = "update sd_TennisRPoint_log Set ptuse  = '"&yn&"' where titleCode = "&titlecode&" and Teamgb = "&teamgb&" and titleIDX = "&tidx&" "
	Call db.execSQLRs(SQL , null, ConStr)
End if


fname = " max(titleName) as '대회명', teamGbName as '부명칭' ,count(*) as '명수',max(ptuse) as '사용여부' ,  "
fname = fname & " '<a href=""javascript:mx.setRnkInfo('+CAST(Max(titleCode) AS varchar(12))+' , '+CAST(titleIDX AS varchar(12))+' , '+CAST(teamgb AS varchar(12))+', "&gyear&", '''+CAST(max(ptuse) AS varchar(2))+''')"" class=""btn"">변경</a>'"
