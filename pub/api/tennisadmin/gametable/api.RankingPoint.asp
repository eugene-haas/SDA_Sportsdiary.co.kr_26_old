...
End If
	...
	If dblrnk = "Y" Then '	'만약 우승자라면
		...
		Case "베테랑부" 
			myrnkboo = "베테랑부"
			boopttitle = "<th>신인부</th><th>베테랑부</th>"
			xmax = 2
		Case else
			...
			If isnull(chklevel) = True Or chklevel = "" then
				...
				SQL = "select top 1 teamGbName from sd_TennisRPoint_log where  PlayerIDX = "&idx&"  order by idx desc"
				Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
...
'Call rsdrow(rs)
	...
	strfield = strfield & " ,ROW_NUMBER() OVER(PARTITION BY PlayerIDX, titleName ORDER BY CASE WHEN teamGb = '20108' THEN 1 ELSE 2 END DESC, getpoint DESC, CASE WHEN '20104' = teamGb THEN 1 ELSE 2 END) AS JNUM "
...
'#############################################
	...
	'20220608 합산 포인트에 모든 참가대회에 득점한 포인트 출력 요청
	SQL = "SELECT ISNULL(SUM(getpoint), 0) AS '합산포인트' FROM sd_TennisRPoint_log  WHERE ptuse= 'Y' AND PlayerIDX="&idx&" "
	Set totalRs = db.ExecSQLReturnRS(SQL , null, ConStr)
	If Not totalRs.EOF Then
		arrTRS = totalRs.GetRows()
	End If
%>
...
<%
		...
		Do Until rs.eof
			...
			%>
				<tr class="gametitle" data-ex="<%=ex%>">
					<%
						overcolor = ""
						okpt = True

						'같은 대회 중복 출전인 경우 포인트가 높으면 green, 낮으면 orange으로 표시
						For i = 0 To rscnt - 1
							If i = 0 then
									...
									For ar = LBound(arrRS, 2) To UBound(arrRS, 2)
										...
										If rsdata(i) = overrap Then
												...
												If overrapcnt = 0  then
													okpt = true
													overcolor = "style = 'background:green;color:yellow;' " '중복출전
												...
												Else
													okpt = false
													overcolor = "style = 'background:orange;color:yellow;' " '중복출전 중 포인트 낮은것
													If  pre = "" Or overrap = pre Then
														overrapcnt = 1
													Else
														overrapcnt = 0
										...
										End If
									Next
									'동대회 포인트 기준 순위 컬럼의 값이 2이면, 순위가 낮은 것이므로 컬럼의 색상을 바꿔준다.
									If cdbl(rsdata(6)) > 1 Then
								...
								For x = 1 To xmax
									...
									If dblrnk = "Y" then
										...
										Case "개나리부"
											...
											Case 1
												If orderA <= 15 then
													If CDbl(xorder) < 512 And xboonm = "개나리부" Then
									...
									Else '============================================================================================================================================
										...
										Case Else
											If myrnkboo = xboonm then										
												...
												End If
											Else
												Response.write "<td>" &xboonm & ":" &  xpoint  & "</td>"
												'어떤 제약이나 조건업이 포인트 합산되도록 요청(20220608)
												totalA = totalA + xpoint																						
											End if
										End Select 
									End if
		...
		Response.write "<tr data-x='"&xmax&"'>"
		For i = 1 To rscnt
			If i = 1 Then
			'Response.write "<td>랭킹포인트(상위15)</td>"
			Response.write "<td>랭킹포인트</td>"
			elseIf i < 7 Then
			Response.write "<td>&nbsp;</td>"
			End if
		Next
...
'			End select
'		Next
		Response.write "<td>"&arrTRS(0,0)&"</td>"
