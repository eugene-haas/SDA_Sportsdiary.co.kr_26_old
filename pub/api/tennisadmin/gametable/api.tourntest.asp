...
'#############################
  ...
  Sub tournInfo(ByVal arrRS , ByVal rd , ByVal sortno, ByVal arrRT, ByVal endround)
    ...
    If IsArray(arrRS) Then
	  ...
	  <li>  
        <%If mgubun = "3" Or CDbl(rd) > 1 Then '고정%>
          ...
          <%ElseIf m1name = "--" then%>
          <%else%>
          <span class="player" style="font-size:15px;"><%=mtryoutgroupno%>:<%=Left(m1name,4)%>&nbsp;<%=Left(m2name,4)%></span>
          <%End if%>
        <%else%>
	...
	Dim chks1,chks2
              If IsArray(arrL) Then
				...
				For ar = LBound(arrL, 2) To UBound(arrL, 2)  '소트는 그룹 , 랭킹으로 되어있슴
					  ...
					  If CDbl(g_no) = CDbl(groupno) And CDbl(rankno) < 3 Then '같은 그룹 찍기
							If CDbl(rankno) = 1 Then '1등
								chks1 = true
							%>
								<td><%=g_no%></td>
								<td style="color:red;"><%=rndno1%></td>
								<td><a href="#<%=aname%>_1" class="btn_a btn_updateMember" style="width:100%;"> <%=Left(aname,5)%><br><%=Left(bname,5)%></a></td>
							<%
							End if
              ...
              If IsArray(arrL) Then
                ...
                For ar = LBound(arrL, 2) To UBound(arrL, 2) 
                  ...
                  If CDbl(joo) = CDbl(groupno) Then
                    ...
                    End If
                    %>
                    <td><%If CDbl(rankno) = 1 then%><span style="color:red;"><%=rndno1%></span><%else%><%=rndno2%><%End if%></td>
                    <td><a href="#<%=aname%>_1" class="btn_a btn_updateMember" style="width:100%;"> <%=Left(aname,5)%><br><%=Left(bname,5)%></a></td>
                    <%            
                    r = r + 1
                   End If
...
<%End if%>
  ...
  <%'본선#################%>    
    <td class="tourney_container" id="realTimeContents">
          <table class="tourney_admin <%=drowCnt%>" id ="tourney_admin" border="0" style="width:100%;height:<%=Fix(drowCnt * 74)%>px;" >
      <thead>
              <tr>
                    <%
                    For i = 1 To depthCnt
						  ...
						  <td id="<%=i%>_row" style="padding:0px;">
							...
							For n = 1 To roundcnt
								...
								End if
								  ...
								  <%Call tournInfo(arrT, i, n, arrRS, depthCnt)%>
								  </div>
							<%
							Next
							%>
