...
If tn_last_rd = False Then '최종라운드 토너먼트가 아니라면
  ...
  '#############
    ...
    Sub updateSortNo(ByVal arrRS, ByVal rank, ByVal rndno1, ByVal rndno2, ByVal joono, ByVal midx ,arrBYE) '추첨룰, 1,2위, 랜덤번호1, 랜덤번호 2, 예선조번호,  대진인덱스 , 바이자리정보
	  ...
	  Dim n, bye   ,b_sortno 'b_orderno , b_sortno, b_joono
	  ...
	  '자기 자리에 있는 다른아이를 0으로 이아이는 다시 찾아져야해
    ...
    If Not rs.eof Then
      ...
      Do Until rs.EOF 
        ...
        If CDbl(tidx) <> 32 Then
			...
			Call updateSortNo(arrRND,  rank, rndno1,rndno2,joono, midx, arrBYE) '예선에서 넘어온 사람들 자리배정
'test__str = midx
		End if
      rs.movenext
      n = n + 1
...
'부전 -- 두개의 중복 소팅번호가 있다면 --를 지워라@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	  '중복부전제거
	 'select sortno, count(*) as cnt from sd_tennisMember where GameTitleIDX = " & tidx & " and  gamekey3 = " & levelno  & " and round = 1 cnt >1  DelYN='N' group by sortno 
	...
	'올라온사람이 겹쳐서 생성도었을수 있으니 사람은 다시 sortno를 0으로 만들고 이름이 같다면 지우자
	 ...
	 If overlapdel = True Then '살릴지 다시검토좀 해보자
		'1라운드 총갯수가 넘는다면 
...
'###################################################################################################
			...
			If USER_IP = "118.33.86.240" Then
				If nextmember =  False Then '다음라운드 빈자리
					If rd = "1" Then '1라운드이고
						If CDbl(snotno) > 0 And CDbl(sortno) Mod 2 = 0 Then '짝수 소팅번호
								
								nextSortNo = Fix(CDbl(sortno) /2)	'짝수

								If (CDbl(arrRS(11, ar -1)) > 1 And  CDbl(m1pidx) = 0)  Then '홀수승
...
'									Call db.execSQLRs(SQL , null, ConStr)
'
'									'선수자리 소팅번호에 있는 0,1 인 아이지우기
'					                SQL = "delete from sd_TennisMember where GameTitleIDX = "&tidx&" And gamekey3 = "&levelno&"  And Round = 1 and playerIDX  in (0,1)  and sortNo = " & CDbl(sortno) - 1     '바이나, 빈값인경우 지우고
'					                Call db.execSQLRs(SQL , null, ConStr)
'									
'									'//기존거 삭제 --
'									SQL = "Delete From sd_TennisMember Where GameTitleIDX = "&tidx&" And gamekey3 = "&levelno&"  And Round = 2 And sortno = '" & nextSortNo & "'"
...
'
'									'선수자리 소팅번호에 있는 0,1 인 아이지우기
'					                SQL = "delete from sd_TennisMember where GameTitleIDX = "&tidx&" And gamekey3 = "&levelno&"  And Round = 1 and playerIDX  in (0,1)  and sortNo = " & CDbl(sortno)  '바이나, 빈값인경우 지우고
'					                Call db.execSQLRs(SQL , null, ConStr)
'
'
'									'//기존거 삭제 --
