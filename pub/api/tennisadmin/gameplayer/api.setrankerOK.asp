...
'    End If
    ...
    If rnkyn = "Y" then
		...
		If rnkend = "" Then
			...
			'20104 신인부 20105 오픈부
			Select Case BOONO
			Case "20101" : BOONO = "20102"
			Case "20104" : BOONO = "20105"
			End Select
			updateField = " dblrnk = '"&rnkyn&"',  gameday='"&Left(rnkstart,10)&"',   chklevel= '"&BOONO&"'  ,openrnkboo = null " 
		else
			If Cdate(rnkend) <= Date  Then
			updateField = " dblrnk = '"&rnkyn&"',  gameday='"&Left(rnkstart,10)&"',   chklevel= '"&BOONO&"' ,openrnkboo = null  " 
			else
			updateField = " dblrnk = '"&rnkyn&"',  gameday='"&Left(rnkstart,10)&"',  endRnkdate = '"&rnkend&"', chklevel= '"&BOONO&"' ,openrnkboo = null  " 
			End if
		End if
