...
'#############################################
	...
	Sub attreg()'대진등록 삭제
		Dim strWhere , SQL
		SQLsub = " ( select gamememberidx from sd_tennismember where GameTitleIDX = " & tidx & " and  gamekey3 = " & levelno & " and isnull(round,0) = 0 and (playerIDX = " & pidx & "  or requestIDX='"&ridx&"') )"
		'Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)

		'strWhere = " a.GameTitleIDX = " & tidx & " and  a.gamekey3 = " & levelno & " and isnull(a.round,0) = 0 and a.playerIDX = "&pidx&"  "
		...
		SQL = "DELETE From sd_TennisMember From sd_TennisMember As a Left Join sd_TennisMember_partner As b On a.gameMemberIDX = b.gameMemberIDX Where a.gamememberidx  in " & SQLsub
		Call db.execSQLRs(SQL , null, ConStr)
		'Response.write "대진등록삭제"
	End sub
		...
		If CDbl(myno) <= CDbl(attmembercnt) Then '참가 인원 안쪽이라면
			...
			'################################################
				...
				If rs.eof then
					...
					insertvalue = " 0, "&tidx&", "&p1idx&", '"&p1name&"', '"&key1&"',"&Left(levelno,3)&","&levelno&","&Left(levelno,5)&",'"&teamgbNm&"','"&p1team1txt&"','"&p1team2txt&"'  "
