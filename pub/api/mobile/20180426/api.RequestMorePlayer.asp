<%
    ...
    if idx <> "" and  levelno <> ""  then 
      ...
      ' SQL = SQL&"  , ROW_NUMBER() over(order by RequestIDX Asc ) pagenum" 
       SQL = SQL&"  , RequestIDX "
       SQL = SQL&"  , a.GameTitleIDX,d.GameTitleName, b.TeamGb,case left(b.cfg,1) when 'Y' then b.TeamGbNm +'(B)' else b.TeamGbNm end  TeamGbNm,a.Level,c.LevelNm  " 
       ...
       SQL = SQL&"  ,''ch_m"  'case when ISNULL(s.gameMemberIDX,'') ='' then 'N' else 'Y' end ch_m "
       SQL = SQL&"  from tblGameRequest as a " 
       SQL = SQL&"  inner join tblLevelInfo c on  a.SportsGb = c.SportsGb and a.Level = c.Level and c.DelYN='N' and c.LevelNm not like '%최종라운드%' and  a.DelYN = 'N' " 
       ...
       SQL = SQL&"  inner join sd_TennisTitle d on a.GameTitleIDX = d.GameTitleIDX and a.SportsGb = d.SportsGb and d.DelYN='N' and d.ViewYN='Y' and d.GameYear ='"&Years&"' " 
    ...
    strFieldName="  row_num	,GameTitleIDX	,GameTitleName	,TeamGb	,TeamGbNm	,Level	,LevelNm	,RequestIDX	,UserName	,PaymentNm	,PaymentDt	,P1_PlayerIDX	,P1_UserName	,P1_TeamNm	,P1_TeamNm2	,P2_PlayerIDX	,P2_UserName	,P2_TeamNm	,P2_TeamNm2	,DelYN	,titleCode	,titleGrade	,EntryCntGame	,cfg	,Ch_i	,Ch_u	,Ch_d	,ch_m "
    strTableName=" ("&SQL&") a  "
    strWhere = "    a.DelYN = 'N'  " 
        ...
        %>
           <!-- <dd>
                <div class="info_title">
                     <span class="name">[<%=titleGradeNm  %>]<%=rs("GameTitleName") %>  </span><br />
                     <span class="name"><%=rs("TeamGbNm") %>  - <%=replace(rs("LevelNm") ,"부","")%>  ( 마감:<%=Rs("EntryCntGame") %>팀 ) </span>
                </div> 
            </dd>-->
        <%
												...
												<!--<span class="c_999">
													(<%if Cdbl(rs("EntryCntGame")) >= Cdbl(rs("row_num")) then %>
															 <%if Cstr(rs("ch_m")) ="Y" then %>  
																	<span class="Rnumber"><%=rs("row_num") %> / <%=Rs("EntryCntGame") %></span>
															 <%else %>
																	<span class="btn_org"> 대기 <i></i></span>
															 <%end if  %>
													<%else %>
