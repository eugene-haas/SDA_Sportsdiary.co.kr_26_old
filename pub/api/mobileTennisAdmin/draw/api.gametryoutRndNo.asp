...
'#############################################
	...
	orderno = oJSONoutput.STR
	RND_NO = oJSONoutput.Get("RNDNO")
...
'Response.end
	...
	If str_rndno1 = "" Then
		If orderno = "1" and Cdbl(str_rndno1) <> Cdbl(RND_NO) Then '1등자리 들어가있늘랜덤번호 <> 보낸랜덤번호가 다르다면  -> 1라운드의 소팅번호를 0으로 초기화?
	...
	If str_rndno2= "" Then
		If orderno = "2"  and CDBL(str_rndno2) <> CDBL(RND_NO)Then
		strSql = " Update sd_TennisMember SET  SortNo = 0 WHERE gubun in (2, 3) and " & strWhere & " AND Round= 1 "
		Call db.execSQLRs(strSql , null, ConStr)
		End If
	End If
	...
	If orderno = "1" Then
		strSql = strSql & " AND rndno1 = '" & RND_NO & "' AND rndno1 > 0"
	Else
		strSql = strSql & " AND rndno2 = '" & RND_NO & "' AND rndno2 > 0"
	End If
	...
	If orderno = "1" Then
		updatevalue = " rndno1 = '" & RND_NO & "'"
		updatevalue_0 = " rndno1 = '0'"
	Else
		updatevalue = " rndno2 = '" & RND_NO & "'"
