...
t1Score = oJSONoutput.T1_SCORE
t2Score = oJSONoutput.T2_SCORE
...
If CDbl(ridx) = 0 Then '결과값확인 (코트값이 없을때 넘어옴 		'?? 코트 반환X)
		...
		Preresult = "ADMIN"
		insertfield = " gameMemberIDX1,gameMemberIDX2,winIDX,stateno,gubun,set1end,recorderName,preresult,GameTitleIDX,gamekey3,gamekeyname,Level,tryoutgroupno, m1set1, m2set1 " 'gubun 0 예선 1 본선
		insertvalue = " "&midx1&","&midx2&","&winidx&", "&StateNo&",0,getdate(),'운영자','"&Preresult&"','"&tidx&"',"&Left(levelno,5)&",'"&teamnm&"',"&levelno& "," & jono & "," & t1Score & "," & t2Score
		...
		'	End if
			...
			'stateno = 1 경기종료
			SQL = "Update sd_TennisResult Set winIDX = "&winidx&",stateno = 1,preresult='ADMIN',m1set1='"&t1Score&"', m2set1='"&t2Score&"' where resultIDX = " & ridx
			Call db.execSQLRs(SQL , null, ConStr)
