...
'#############################
  ...
  Sub tournInfo(ByVal arrRS , ByVal rd , ByVal sortno, ByVal arrRT, ByVal endround, ByVal arrJ)
    ...
    If IsArray(arrRS) Then
      ...
      <%If rd <> "1" then%>
      background-image:url('http://tennisadmin2.sportsdiary.co.kr/images/tournerment/line.png');
...
<link rel="stylesheet" href="/pub/css/tennisAdmin.mobile.css?ver=8.1.5">
...
<header class="header clear">
  ...
  <div class="header__side-con">
    <button class="header__side-con__btn header__btn-reset t_ico" id="reloadbtn"
      onclick="mx.tournament_table(<%=idx%>,'<%=teamnm%>','<%=areanm%>','roundsel')">
...
<div class="l_modal__con tryout-match">
  <div class="tryout-match__seach">
    <div class="tryout-match__seach__selc-box selc-box">
      ...
      <select id="roundsel" onchange="mx.tournament_table(<%=idx%>,'<%=teamnm%>','<%=areanm%>','roundsel')">
    ...
    <button type="button" name="button" onclick="mx.tournament_ing(<%=idx%>,'<%=teamnm%>','<%=areanm%>', 'roundsel')">경기진행</button>
  </div>
</div>
