...
If tn_last_rd = False Then
	SQL = "Select top 1 JooCnt from tblRGameLevel where gametitleidx ='"&gameTitleIDX&"' and level = '"&gamekey3&"' "
	Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
	If Not rs.eof Then
		maxgno = rs("joocnt") '예선 조수
	...
	SQL = "select  COUNT(*)  from "&strtable&" where GameTitleIDX = "&gameTitleIDX&" and gamekey3 = "& gamekey3 & " and gubun in (2, 3)  and DelYN = 'N' and round = 1"
	Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
	If Not rs.eof Then
		tcnt  = rs(0)
	End if
End If
	...
	'현재 편성정보 min 2가 하나라도 있다면으로 바꾸자
	SQL = "SELECT min(gubun)  FROM sd_TennisMember where GameTitleIDX = " & gameTitleIDX & " and gamekey3 = " & gamekey3 & " and round = " & nowRound & " and DelYN = 'N' "
	Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
	tourngubun = rs(0)
