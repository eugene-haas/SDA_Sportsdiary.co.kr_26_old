...
''입금상태 정보가져오기
''#############################
SQL =  "SELECT PaymentType,P1_PlayerIDX,P2_PlayerIDX FROM tblGameRequest WHERE  DelYN = 'N' and GameTitleIDX = "&tidx&" and SportsGb = 'tennis' and Level =" & levelno 
Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
...
End If
''#############################
Function findpaytype(ByVal pidx1, ByVal pidx2, ByVal arrat)
  Dim ct,pidxA,pidxB,payflag
  ...
  For ct = LBound(arrat, 2) To UBound(arrat, 2) 
    ...
    If CDbl(pidxA) = CDbl(pidx1) And CDbl(pidxB) = CDbl(pidx2) Then
      findpaytype = payflag
      Exit for
    End if
  Next
...
End Function 
  ...
  '그룹번호, 소팅번호 생성 ################s
    'SQL = "SELECT top 1 gameMemberIDX FROM " & strtable & " where GameTitleIDX = " & tidx & " and  gamekey3 = " & levelno & " and  gubun in ( 0, 1) and tryoutgroupno > 0"
    SQL = "SELECT max(tryoutgroupno),count(*) FROM " & strtable & " where GameTitleIDX = " & tidx & " and  gamekey3 = " & levelno & " and  gubun in ( 0, 1) and tryoutgroupno >= 0"
    Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
...
'    If CDbl(attcnt) > CDbl(joocnt) * 3 Then '참가 인원이 더많을경우
'      '타입 석어서 보내기
'      Call oJSONoutput.Set("result", "500" )
'      strjson = JSON.stringify(oJSONoutput)
...
'    End if
    ...
    If isNull(maxgno) = False then
      ...
      If CDbl(joocnt) < CDbl(rs(0)) Or CDbl(rs(0)) = 0 Then '편성다시 
      'If rs.eof Then '1보다 큰게 하나라도 없다면 업데이트 (이후엔 예선 자동 편성을 이용한다. 선수가 추가되었다면 랭킹 적용을 다시한다.) 
  ...
  '참가자 목록 =============================================s
	  ...
	  placestate = True '장소가 모두 입력되었음
		...
		ReDim jooEndarr(joocnt) '조 1등 2등 완료 여부 확인
		ReDim rndChkarr(joocnt) '랜덤번호 완료 되면 
		...
		If IsArray(arrRS) Then
		For ar = LBound(arrRS, 2) To UBound(arrRS, 2) 
			chk_gn = arrRS(18,ar) '편성여부
			chk_place = arrRS(24,ar)
			chk_rnd1 = arrRs(22,ar)
			chk_rnd2 = arrRs(23,ar)

			chk_rank = arrRS(19,ar) '랭킹
			chk_jono = arrRS(0,ar)

			'Response.write chk_rank & "<br>"

			'조 결과 완료 찾기
			If CDbl(chk_jono) =  CDbl(pr_joo) Then
				If chk_rank = "1" Or chk_rank = "2" Then
					joend = joend + 1
				End if
			Else

				If CDbl(joend) > 1 then
					jooEndarr(jn)  =  1 '종료
				Else
					jooEndarr(jn)  =  0 '진행
				End if
...
For i = 1 To ubound(jooEndarr)
Response.write jooEndarr(i) & "<br>"
next
  ...
  <div class="top_control">
    ...
    <div class="league_tab clearfix">
      <div class="pull-left">
		...
		<a href="./gamerull.asp" class="btn_a btn btn_tab">본선시드배정</a> 
		<%End if%>
...
>
  ...
  <th>[출석/입금/사은품] 1팀</th><th>[출석/사은품/입금] 2팀</th><th>[출석/사은품/입금] 3팀</th><th>경기입력</th>
  </thead>

<%
	function setTeamLink(ByVal t1, ByVal t2,ByVal t31,ByVal t4)
		Dim tn
		If t1 <> "" Then
			tn = t1
		End If
  ...
  For i = 1 To endgroup 
	...
	If IsChangeJoo = True Then
		findrt = jooEndarr(jorder(i))
	Else
		findrt = jooEndarr(i)
	End if
	
	If CDbl(findrt) = 1 Then
	...
	'########################조완료여부	
%>
  <tr id="JoonoBox_<%If IsChangeJoo = True then%><%=jorder(i)%><%else%><%=i%><%End if%>"  <%=ss%>>
...
<%
  ...
  For j = 1 To 3
    ...
    For ar = LBound(arrRS, 2) To UBound(arrRS, 2) 
     Do 
      ...
      if ISNULL(rRndNo1) = True Or rRndNo1 = "0"  Then
        rRndNo1 = ""
      Else
		rndckcnt = rndckcnt + 1
	  End If

      if isnull(rRndNo2)  = True Or rRndNo1 = "0"   Then
        rRndNo2 = ""
	  Else
		rndckcnt = rndckcnt + 1
      End If
  ...
  <%If showordersell = True Then '모든조가 편성되었다면%>          
	...
	<%If j = 1 and CDbl(tdcnt) = 1 Then%>
		...
		<td>
          ...
          <%End If%>
		  class="tabable"
		   tabindex = "<%=tabno%>"  onkeydown="if(event.keyCode == 13){mx.OnFocusOut(<%=joocnt%>, this,'tabable')}">
        </td>
        <td>
          ...
          <%End If%>
	     class="tabable2"
		  tabindex = "<%=tabno%>" onkeydown="if(event.keyCode == 13){mx.OnFocusOut(<%=joocnt%>, this,'tabable2')}">
        </td>
		  <%tabno = CDbl(tabno) + 1%>
  ...
  <%If j = 1 and CDbl(tdcnt) = 1 Then%>
          <td>
          <input id="place_<%=chkjoono%>" type="text" style="width:80px"  value="<%=rPlace%>" tabindex = "<%=tabno2%>" onkeydown="if(event.keyCode == 13){mx.OnFocusOut(<%=joocnt%>, this,'tabarea',this.value)}" class="tabarea"
		  >
  ...
  <%End if%>
      ...
      If IsArray(dupchk) Then
        For z = 0 To ubound(dupchk) '0부터 들어가있다 값이
          ...
          If dupchk(z) <> "" then
            ...
            Response.write "<div class=""dupstr"">" &samjooteam&dupchk(z)& "</div>"
            End If
    ...
    <%if chkgubun = "0" then%>          
      <div class="chk_state">
      <%rPaymentFlag = findpaytype(rPlayerIDX,rPlayerIDXSUB,arrAT)%> 
		...
		<span >입금</span><label class="switch"  title="입금">
    ...
    <%If CDbl(chkgubun) = 1 then%>
      ...
      %>
	      ...
	      <%If showordersell = True And placestate = True Then '모든조가 편성되었다면%>
			  ...
			  %>style="width:70px;font-size:13px;border-color:red;margin-top:-10px;" <%
			  Else
			  %>style="width:70px;font-size:13px;margin-top:-10px;"<%      
			  END IF 
			  %>>
    ...
    role="button" 
    <%If CDbl(chkgubun) = 1 then%>style="height:60%;width:100%;font-size:20px;border:1px solid #DEBA29;"<%else%>class="btn_a btn_updateMember"  <%End if%>
    ><%=p1name%>&nbsp;<%=p2name%></a>

    <%If CDbl(ADGRADE) > 500 then%>
    <input type="text" style="width:50px;margin-top:-10px;" onblur='mx.update_rpoint(<%=strjson%>,<%=p1idx%>,this.value);' value="<%=rpointsum%>" title="포인트합: 수정후 포커스가 이동되면 수정됩니다."> 
	<%End if%>
    ...
    %>
	  ...
	  <td>
		<input id="place_<%=chkjoono%>"type="text" style="width:80px"  value="<%=rPlace%>"   tabindex = "<%=tabno2%>" onkeydown="if(event.keyCode == 13){mx.OnFocusOut(<%=joocnt%>, this,'tabarea')}" class="tabarea">
	    <%tabno2 = CDbl(tabno2) + 1%>
      </td>
	<%End If%>
    ...
    <%If CDbl(chkgubun) = 0 then%>
		<td class="making_group chk_col">
			...
			oJSONoutput.GAMEMEMBERIDX = 0
			  ...
			  strjson = JSON.stringify(oJSONoutput)
			%>
			<a href='javascript:mx.updateMember(<%=strjson%>)' role="button" class="btn_a btn_updateMember" style="height:50%;width:80%;font-size:20px;padding: 30px 0px;">팀등록</a>
			</div>
		</td>
   	<%ELSE%>
    ...
    %>
      <td>
      <div style='width:100%; height: 100%;padding: 2px 0px;' >
    <%If CDbl(chkgubun) = 0 then%>
        <a href='javascript:mx.SetJoo(<%=strjson%>)' class="btn_a btn_toggle_show">경기대기</a>&nbsp;
        <!-- <a href='javascript:alert("<%=chkjoono%>조의 편성을 완료 후 실행해 주십시오.")' class="btn_a">입력</a> -->
      <%Else%>
        <a href='javascript:if (confirm("경기가 진행 중 입니다. 정말 해제 하시겠습니까?") == true) {mx.SetJoo(<%=strjson%>)}' class="btn_a btn_toggle_show" title="홈페이지노출 본선경기체크">경기진행</a>&nbsp;
