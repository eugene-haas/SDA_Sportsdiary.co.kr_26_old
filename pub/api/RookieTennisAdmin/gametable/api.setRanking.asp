	...
	'##############################
		...
		rs.close
		  ...
		  '강수 계산##############
		   ...
		   '###################################
'Response.write drowcnt
		' SQL = "Select count(*) from sd_TennisMember where GameTitleIDX = '"&tidx&"' and gamekey3 = '"& levelno & "' and gubun in (2,3)  and DelYN = 'N' and Round = '1' and sortno > 0 "
		' Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
