  ...
  Response.write "`##`"
	...
	Else
		...
		If changetype ="member" then
			...
			SQL = SQL & " ,p.gamestartyymm as p1info1 ,p.userLevel  as p1info2 ,p.birthday as p1info3   ,p2.gamestartyymm  as p2info1,p2.userLevel  as p2info2,p2.birthday as p2info3 "
			...
			SQL = SQL & " INNER JOIN tblPlayer as p ON a.playerIDX = p.playerIDX  "
			SQL = SQL & " LEFT JOIN sd_TennisMember_partner as b ON a.gameMemberIDX = b.gameMemberIDX  "
			SQL = SQL & " LEFT JOIN tblPlayer as p2 ON b.playerIDX = p2.playerIDX  "
			...
			p1A = isNullDefault(rs("p1info1"),"")
				If P1A <> "" Then '구력
					p1yy = Left(P1A,4) &"-"& Right(P1A,2) & "-01"
					P1A = cint(datediff("m", p1yy , date())) & "개월"
				End if
			p1B = isNullDefault(rs("p1info2"),"") 'NTRP
			p1C = isNullDefault(rs("p1info3"),"")
				If P1C <> "" Then '나이
					P1C = korAge(Left(P1C,4) &"-"& mid(P1C,5,2) &"-"& Right(P1C,2))
				End if

			p2A = isNullDefault(rs("p2info1"),"")
				If P2A <> "" Then
					p2yy = Left(P2A,4) &"-"& Right(P2A,2) & "-01"
					P2A = cint(datediff("m", p2yy , date())) & "개월"
				End if

			p2B = isNullDefault(rs("p2info2"),"")
			p2C = isNullDefault(rs("p2info3"),"")
				If P2C <> "" Then
					P2C = korAge(Left(P2C,4) &"-"& mid(P2C,5,2) &"-"& Right(P2C,2))
				End if
...
<%If CDbl(GAMEMEMBERIDX) = 0 Then '신규팀생성 또는 선수 생성화면%>
			...
			<table class="sch-table">
			  ...
			  <colgroup>
				...
				<td id = "player1">
				  ...
				  <input type="text" id="nname" value="<%=p1_UserName%>" width="100px;" class="ui-autocomplete-input" autocomplete="off"><a href="javascript:mx.chkPlayer()" class="btn_a">중복체크</a>
				  </br><br>
	...
	<%else%>
	  ...
	  <div class="modal-footer">
		...
		<button class="btn" data-dismiss="modal" aria-hidden="true">닫기</button>
		<!-- <button class="btn btn-primary" onclick="mx.setPlayer()">선수등록</button> -->
		<button class="btn btn-primary" onclick="mx.setTeam(<%=idx%>)">신규팀등록</button>
		<!-- </div> -->
	  </div>
	<%End if%>
		...
		<table class="sch-table">
		  ...
		  <colgroup>
			...
			<td id = "player1">
			  ...
			  <span> 구력 : </span><span><%=p1A%></span><br><br>
			  <span> NTRP : </span><span><%=p1B%></span><br><br>
			  <span> 나이 : </span><span><%=p1C%></span><br><br>
			...
			<td id="player2">
			  ...
			  <span> 구력 : </span><span><%=p2A%></span><br><br>
			  <span> NTRP : </span><span><%=p2B%></span><br><br>
			  <span> 나이 : </span><span><%=p2C%></span><br><br>
