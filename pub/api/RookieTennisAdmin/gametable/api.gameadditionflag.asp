	...
	If Not rs.eof then
		...
		idx = rs(0)
			...
			If CDbl(acctotal) > 0 Then
				...
				SQL = "select VACCT_NO from SD_RookieTennis.dbo.TB_RVAS_LIST where CUST_CD = '" & Left(sitecode,2) & idx & "' "
				Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
				...
				If rs.eof Then
					If flagCheck = "Y" Then '현장입금 플레그 
						...
						TRD = Replace(Date(),"-","")
						TR_SEQ = " (select max(TR_SEQ)+1  from SD_RookieTennis.dbo.TB_RVAS_LIST where TR_DATE = '"&TRD&"') "
						BANK_CD = "000"
						ORG_CD = ""
						VACCT_NO = "1"
						CUST_CD = idx
