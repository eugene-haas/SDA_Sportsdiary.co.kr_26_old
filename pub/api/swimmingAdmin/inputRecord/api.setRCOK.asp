...
'Response.write sinstr
		...
		Case "한국신기록"  '1. 한국신기록이라면 내용을 찾아서(tblrecord)에서  titlecode와 rctype을 변경해서 insert한다. 
			If onoff = "on" Then 'on insert (기존자료는 대회신기록으로 변경)
				...
				If fldno = "" Then
				SQL = "select top 1 CDBNM,CDCNM from tblRecord where delyn='N' and  gametitleidx = '"&tidx&"' and rgameLevelIDX = '"&lidx&"' and gameResult = '"&rc&"' and playeridx = '"&pidx&"' "
				else
				SQL = "select top 1 CDBNM,CDCNM from tblRecord where delyn='N' and  gametitleidx = '"&tidx&"' and rgameLevelIDX = '"&lidx&"' and RCNO = '"&fldno&"' and gameResult = '"&rc&"' and playeridx = '"&pidx&"' "
				End if
...
'Response.end
				...
				'이전한국신기록은 delyn = 'Y'
				SQL = "update tblRecord set delyn = 'Y' where CDBNM = '"&cdbnm&"' and CDCNM = '"&CDCNM&"' and rctype = 'R07' " '오픈바나 이런것도 있어서...어떻하지...
				...
				'한국신기록으로 복사하고
				If fldno = "" Then
				selectSQL = "select top 1 "&fld&" from tblRecord where delyn='N' and gametitleidx = '"&tidx&"' and rgameLevelIDX = '"&lidx&"' and gameResult = '"&rc&"' and playeridx = '"&pidx&"' "
				Else
				selectSQL = "select top 1 "&fld&" from tblRecord where  delyn='N' and gametitleidx = '"&tidx&"' and rgameLevelIDX = '"&lidx&"' and RCNO = '"&fldno&"' and gameResult = '"&rc&"' and playeridx = '"&pidx&"' "
				End if
				...
				If fldno = "" Then
				SQL  = "update tblRecord Set  rcType = '"&gamegubun&"' ,midx = '"&midx&"'  where  delyn='N' and gametitleidx = '"&tidx&"' and rgameLevelIDX = '"&lidx&"' and gameResult = '"&rc&"' and playeridx = '"&pidx&"'  and rctype <> 'R07' "
				else
				SQL  = "update tblRecord Set  rcType = '"&gamegubun&"' ,midx = '"&midx&"'  where  delyn='N' and gametitleidx = '"&tidx&"' and rgameLevelIDX = '"&lidx&"' and RCNO = '"&fldno&"' and gameResult = '"&rc&"' and playeridx = '"&pidx&"'  and rctype <> 'R07' "
				End if
				Call db.execSQLRs(SQL , null, ConStr)
