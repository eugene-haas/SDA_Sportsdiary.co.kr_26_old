...
'request
	...
	SQL = "Select "&fld&" from tblPlayer as a inner join (select playeridx from tblGameRequest_imsi where delyn='N' and team = '"&team&"' and tidx = '"&tidx&"'  and leaderidx <> '"&leaderidx&"' and leaderidx <> '' group by tidx,playeridx) as b "
	...
	fld = "a.username,a.birthday,a.sex,a.CDB,a.CDBNM,a.userclass,a.playeridx,b.playeridx "
	SQL = "Select "&fld&" from tblPlayer as a left join (select playeridx from tblGameRequest_imsi where delyn = 'N' and team = '"&team&"' and tidx = '"&tidx&"'  and leaderidx = '"&leaderidx&"' group by tidx,playeridx) as b "
	'SQL = SQL & " on a.PlayerIDX = b.playeridx where a.Team = '"&team&"' and a.delyn = 'N' "
	SQL = SQL & " on a.PlayerIDX = b.playeridx where a.Team = '"&team&"' and a.delyn = 'N'  and a.nowyear = '"&year(now)&"' "
...
%>
			  ...
			  <table class="m_modal-player-list__con__tbl">
                ...
                <thead class="m_modal-player-list__con__tbl__thead">
                  <tr>
                    <th scope="col">
                       <input class="m_modal__chek" id="chekBoxModalPlayer00" type="checkbox" id="checkAll"  disabled>
