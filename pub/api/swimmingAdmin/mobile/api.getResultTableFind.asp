...
'#############################################
	...
	am = "  (select top 1 gubunam from tblRgamelevel where delyn='N' and gametitleidx = a.gametitleidx and gbidx = a.gbidx ) as  gubunam " '1,3 오전 3은 결승으로 바로시작
	pm = "  (select top 1 gubunpm from tblRgamelevel where delyn='N' and gametitleidx = a.gametitleidx and gbidx = a.gbidx ) as  gubunpm " '1.3 오후
	...
	fld = fld & " a.tryoutgroupno,a.tryoutsortno, a.tryouttotalorder, a.tryoutresult, a.roundno, a.sortno, a.gameresult,a.gameorder, a.tryoutOrder, "& pmo & ",a.gameMemberIDX , " & am & " , " & pm  
					...
					If a_tryoutsortno > 0 Then
						...
						%>
						  <tr>
							...
							<td><button class="drow-con__list__box-group__btns__btn" type="button" name="button" onclick="mx.getSectionInfo(<%=a_midx%>,'<%=ampm%>','<%=a_tryoutresult%>','<%=a_tryoutOrder%>','<%=a_tryoutsortno%>')" >조회</button></td>
						  </tr>
						<%						
					End if

					If CDbl(a_sortno) > 0 Then  '2단계진행인경우
						%>
						  <tr>
							<td><%=a_CDCNM%></td>
							...
							<td>
							<%
							Call SetRC(a_gameresult)
							%>								
							</td>
							<td><button class="drow-con__list__box-group__btns__btn" type="button" name="button" onclick="mx.getSectionInfo(<%=a_midx%>,'<%=ampm%>','<%=a_gameresult%>','<%=a_gameOrder%>','<%=a_sortno%>')" >조회</button></td>
							</tr>
						<%
					End if
