...
'#############################################
	...
	Set db = new clsDBHelper 
		...
		'개인경기 참가자 넣기
		insertFLD = " GameTitleIDX,gbIDX,levelno,CDA,CDANM,CDB,CDBNM,CDC,CDCNM,P1_ksportsno,P1_PlayerIDX,P1_UserName,P1_UserClass,P1_Team,P1_TeamNm,P1_UserPhone,P1_Birthday,P1_SEX,sido,sidonm,bestRC,ITgubun		,joo,rane ,starttype "

		fld = " GameTitleIDX,gbIDX,levelno,CDA,CDANM,CDB,CDBNM,CDC,CDCNM,kskey,pidx,username,userclass,team,teamnm,userphone,birthday,sex,sido,sidonm,bestRC,ITgubun		,joo,rane,starttype "
		...
		'SQL = "update tblGameRequest_TEMP set delyn = 'Y' where delyn = 'N' and ITgubun= 'I' and GameTitleIDX = " & tidx 
		'Call db.execSQLRs(SQL , null, ConStr)


		'tblGameRequest select 해서 넣자.  sd_gameMember 에 넣기
		'fld = " GameTitleIDX,ITgubun,P1_ksportsno,P1_PlayerIDX,P1_UserName,gbIDX,levelno,CDA,CDANM,CDB,CDBNM,CDC,CDCNM,P1_Team,P1_TeamNm,P1_UserClass,P1_SEX,sidonm,RequestIDX		,joo,rane,'1',starttype "
		'varchar형 데이터가 int로 변환되지 못해 수정
		fld = " GameTitleIDX,ITgubun,P1_ksportsno,P1_PlayerIDX,P1_UserName,gbIDX,levelno,CDA,CDANM,CDB,CDBNM,CDC,CDCNM,P1_Team,P1_TeamNm,P1_UserClass,P1_SEX,sidonm,RequestIDX,joo,rane,1,starttype "
...
'#######	
		...
		'계영 팀 넣기 (팀단위로 넣어야함)
		fld = " "&tidx&",max(gbIDX),levelno,max(CDA),max(CDANM),max(CDB),max(CDBNM),max(CDC),max(CDCNM),'','0','단체','0',team,teamnm,Null,'0',max(sex),max(sido),max(sidonm),Null,'T' ,max(joo),max(rane), '3' "
