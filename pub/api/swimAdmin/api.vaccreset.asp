...
'#############################################
	...
	If titleIDX <> "" then
		...
		If endYN = "Y" AND rpointChk = "Y" Then
			...
			strtable = "  SD_RookieTennis.dbo.TB_RVAS_MAST"
			strtablesub = " dbo.tblGameRequest "
			strwhere = " CUST_CD IN ( select '"&Left(sitecode,2)&"' +  Cast(requestIDX as varchar) from " & strtablesub & " where GameTitleIDX= " & titleIDX & " ) "
