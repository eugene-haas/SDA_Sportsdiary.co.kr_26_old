...
'실행시 중복된 값들을 찾아서 지우자.
'SQL = "select  max(idx),titleidx,teamgb,playeridx,max(userName),count(*) as c,getpoint from sd_TennisRPoint_log where ptuse = 'Y' and titleidx > 0 group by titleidx,teamgb,playeridx,getpoint order by 6 desc"
...
'	SQL = "delete from sd_TennisRPoint_log where titleidx = " & titleidx & " and teamgb = '"&teamgb&"' and playeridx =  " & playeridx & " and getpoint = "&getpoint&" and idx <> " & mxidx 
...
'기본정보#####################################
	...
	If Not rs.eof then
		...
		'당해년도 비교값은 타이
		nowyearchk = Left(gametitle,4)
		If Isnumeric(nowyearchk) = false Then
			nowyearchk = year(date)
		End if
	End if

	SQL = " Select level,endRound,lastroundmethod,LastRchk,cfg,gameday from  tblRGameLevel  where DelYN = 'N' and  RGameLevelidx = " & idx
	Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
	...
	If Not rs.eof then
		...
		gameday = rs("gameday")
		If gameday = "" Then
			gameday = games '개나리 , 신인부 우승자 우승 날짜로 사용
		End if

		cfg = rs("cfg")
		chk1 = Left(cfg,1) 'Y변형요강, N 일반요강
	End if
'기본정보#####################################
...
'#############################
	Sub tournInfo(ByVal arrRS , ByVal rd, ByVal i ,ByVal tidx,ByVal levelno,ByVal title,ByVal teamnm, ByVal ranklist ,ByVal lastroundmethod)
		...
		Dim SQL, insertvalue1,insertvalue2,rksplit ,         firstgbno,fgetpt,f_teamnm, boo1, boo2 ,insertok ,upgradedate
						...
						If ranklist = False then	'인서트자료가 없다면
							If CDbl(getpt) > 0 Then
								...
								If (Left(levelno,5) = "20101" Or Left(levelno,5) = "20104") And CDbl(rankno) = 1 Then
									SQL = "update "&ptbl&" Set dblrnk = 'N' where (levelup < " & nowyearchk& " and titlecode = "&titlecode&") or endRnkdate < GETDATE() " '종료날짜가 있다면?
									Call db.execSQLRs(SQL , null, ConStr)

									SQL = "update "&ptbl&" Set levelup = " & nowyearchk& ",gameday='"&gameday&"',titlecode = "&titlecode&",dblrnk='Y',chkTIDX = "&tidx&",chklevel='"&left(levelno,5)&"',openrnkboo=case when openrnkboo = '신인부' then null else openrnkboo end where playerIDX in ('"&m1pidx&"','"&m2pidx&"') " '플레이어 update.
									Call db.execSQLRs(SQL , null, ConStr)
								End If

								'(오픈부 경기에서 1등인데 부서가 신인부라면 )
								If (Left(levelno,5) = "20105") And CDbl(rankno) = 1 Then
									If boo1  = "신인부" Then
										SQL = "update "&ptbl&" Set levelup = " & nowyearchk& ",gameday='"&gameday&"',titlecode = "&titlecode&",dblrnk='Y',chkTIDX = "&tidx&",chklevel='"&left(levelno,5)&"',openrnkboo=case when openrnkboo = '신인부' then null else openrnkboo end where playerIDX ='"&m1pidx&"' "
										Call db.execSQLRs(SQL , null, ConStr)
									End If
									If boo2  = "신인부" Then
										SQL = "update "&ptbl&" Set levelup = " & nowyearchk& ",gameday='"&gameday&"',titlecode = "&titlecode&",dblrnk='Y',chkTIDX = "&tidx&",chklevel='"&left(levelno,5)&"',openrnkboo=case when openrnkboo = '신인부' then null else openrnkboo end where playerIDX ='"&m2pidx&"' "
										Call db.execSQLRs(SQL , null, ConStr)
									End if									
								End If

								insertvalue1 = " "&m1pidx&",'"&m1name&"',"&getpt&","&rankno&","&titlegrade&","&titlecode&","&tidx&",'"&title&"',"&Left(levelno,5)&",'"&teamnm&"','"&gameday&"',''  "
								insertvalue2 = " "&m2pidx&",'"&m2name&"',"&getpt&","&rankno&","&titlegrade&","&titlecode&","&tidx&",'"&title&"',"&Left(levelno,5)&",'"&teamnm&"','"&gameday&"','' "								

								SQL = "insert into "&logtbl&" (PlayerIDX,userName,getpoint,rankno, titleGrade,titleCode,titleIDX,titleName,teamGb,teamGbName,Gamedate,orgboo ) values ("&insertvalue1&"), ("&insertvalue2&")"
								Call db.execSQLRs(SQL , null, ConStr)
								...
								'#올해 1등한 개나리 신인부 선수처리#####################################################
									...
									If Left(levelno,5) = "20102" Or Left(levelno,5) = "20105" Or Left(levelno,5) = "20103" Then '국화, 베테랑 , 오픈부경기에서
										...
										'자기 우승한(gameday) 현재 gameday 보다 이전인 경우만 반영한다.
										SQL = "select playerIDX,chkTIDX,gameday from "&ptbl&" where levelup >= " & nowyearchk& " and levelup <  " & CDbl(nowyearchk) + 2 & " and titlecode <> "&titlecode&" and dblrnk = 'Y' and playerIDX in ( '"&m1pidx&"' , '"&m2pidx&"' ) " 
										Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)

										Do Until rs.eof
											'인서트 중복값 확인후 넣어주자.
											SQL = "select playeridx from "&logtbl&" where playerIDX = "&rs(0)&"   and titleIDX =  "&tidx&" and teamGbName  = '"&f_teamnm&"' and ptuse = 'Y' " 
											Set rsss = db.ExecSQLReturnRS(SQL , null, ConStr)

											If rsss.eof Then
												If isdate(rs(2)) = False Then '외부 우승자 날짜가 안들어가있다면
													upgradedate = date
												else
													upgradedate = rs(2)
												End if

												If CDate(upgradedate) < CDate(gameday) Then '현재보다 이전에 우승한경우 반영
												'If CDate(rs(2)) < CDate(gameday) Then '현재보다 이전에 우승한경우 반영
													If Cdbl(rs(0)) = CDbl(m1pidx) Then
														insertvalue1 = " "&m1pidx&",'"&m1name&"',"&getpt&","&rankno&","&titlegrade&","&titlecode&","&tidx&",'"&title&"',"&firstgbno&",'"&f_teamnm&"','"&gameday&"' ,'Y' " 'adminINdata 추가된데이터가 오픈부이후에 신인부를 눌러도 삭제되지 않도록
														SQL = "insert into "&logtbl&" (PlayerIDX,userName, getpoint,rankno, titleGrade,titleCode,titleIDX,titleName,teamGb,teamGbName,Gamedate,adminINdata) values ("&insertvalue1&")"
														'Call db.execSQLRs(SQL , null, ConStr)
													End If
													If CDbl(rs(0)) = CDbl(m2pidx) then
														insertvalue2 = " "&m2pidx&",'"&m2name&"',"&getpt&","&rankno&","&titlegrade&","&titlecode&","&tidx&",'"&title&"',"&firstgbno&",'"&f_teamnm&"','"&gameday&"', 'Y' "
														SQL = "insert into "&logtbl&" (PlayerIDX,userName, getpoint,rankno, titleGrade,titleCode,titleIDX,titleName,teamGb,teamGbName,Gamedate,adminINdata) values ("&insertvalue2&")"
														'Call db.execSQLRs(SQL , null, ConStr)
													End If
												End if											
											End if
						...
						If notprint = False then
							...
							If ranklist = False then	'인서트자료가 없다면
								...
								If CDbl(getpt) > 0 then
									...
									'20101 개나리 '20104 신인부
									If (Left(levelno,5) = "20101" Or Left(levelno,5) = "20104") And CDbl(rankno) = 1 Then
										SQL = "update "&ptbl&" Set dblrnk = 'N' where (levelup < " & nowyearchk& " and titlecode = "&titlecode&") or endRnkdate < GETDATE() " '이전에 있던 모든 승급자는 적용금지or 종료날짜가 있다면?
										Call db.execSQLRs(SQL , null, ConStr) 
									...
									'#올해 1등한 개나리 신인부 선수처리#####################################################
										...
										If Left(levelno,5) = "20102" Or Left(levelno,5) = "20105" Or Left(levelno,5) = "20103" Then '국화, 베테랑 , 오픈부경기에서
											...
											'자기 우승한(gameday) 현재 gameday 보다 이전인 경우만 반영한다.X
											SQL = "select playerIDX,chkTIDX,gameday from "&ptbl&" where levelup >= " & nowyearchk& " and levelup <  " & CDbl(nowyearchk) + 2 & " and titlecode <> "&titlecode&" and dblrnk = 'Y' and  playerIDX in ( '"&m1pidx&"' , '"&m2pidx&"' ) " 
											Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)

											Do Until rs.eof
											'인서트 중복값 확인후 넣어주자. (신인부를 참가하고 오픈부를 참가했다면? 어떻게 하지
											SQL = "select playeridx from "&logtbl&" where playerIDX = "&rs(0)&"   and titleIDX =  "&tidx&" and teamGbName  = '"&f_teamnm&"' and ptuse = 'Y' "  
											Set rsss = db.ExecSQLReturnRS(SQL , null, ConStr)
	...
	Function getPlayerlist(ByVal tidx, ByVal levelno, ByVal lastroundmethod)
		Dim strwhere, strsort,strAfield, strBfield, strfield,SQL,rs     ,boo1,boo2
		...
		boo1 = " , (select openrnkboo from "&ptbl&" where playerIDX = a.playerIDX)	as boo1 " '선수가 삭제된 경우 NULL
		boo2 = " , (select openrnkboo from "&ptbl&" where playerIDX = b.playerIDX)	as boo2 "		

		strsort = " order by a.Round asc,a.SortNo asc"
		strAfield = " a. gamememberIDX, a.userName,a.teamAna, a.teamBNa, a.Round, a.SortNo  "
		strBfield = " b.userName, b.teamAna, b.teamBNa,a.gubun,a.areaChanging,a.PlayerIDX,a.rndno1,a.rndno2,t_rank,tryoutgroupno,b.playerIDX      ,t_rank  " &  boo1 & boo2
...
<%'If aaa = "안보임" then%>
	<!-- 개별 목록 수정삭제 -->
		...
		<%If chk1 = "Y" Then %> <span style="color:red;">변형요강(70%만 포인트 지급)</span><%End if%>
		</div>
