...
<%If tn_last_rd = False Then%>
  <%'예선#################%>
    <%
      sub tryoutTN(ByVal g_no, ByVal arrRND)
	  Dim str1,str2,r1,r2    ,a_orderno,a_sortno,a_joono
	  Dim aname,sortno,groupno,bname,rankno,rndno1,rndno2
            ...
            If IsArray(arrL) Then
				...
				If str1 = "" Then
					'빈값구하기 #####
					If r1 = "" Then
					  For arn = LBound(arrRND, 2) To UBound(arrRND, 2) 
						a_orderno = arrRND(0,arn)
						a_sortno = arrRND(1,arn)
						a_joono = arrRND(2,arn)

						If a_joono = g_no Then
							If a_orderno = 1 Then
								r1 = a_sortno
							End if
						End If
      ...
      Sub emptyLgDrow(ByVal arrRND , ByVal n ) '빈조 추첨번호 출력
		  Dim a_orderno, a_sortno, a_joono
		  For arn = LBound(arrRND, 2) To UBound(arrRND, 2) 
			a_orderno = arrRND(0,arn)
			a_sortno = arrRND(1,arn)
			a_joono = arrRND(2,arn)

			If CDbl(a_joono) =  CDbl(n) then
				Select Case a_orderno
				Case "1"%>
					<tr>
					<td><%=n%></td>
					<td style="color:red;"><%=a_sortno%></td><td>-</td>				
				<%Case "2"%>
					<td><%=a_sortno%></td><td>-</td>	
					</tr>
				<%
				End Select
			End if
	...
	<%
				...
				If IsArray(arrL) Then
					  For ar = LBound(arrL, 2) To UBound(arrL, 2) 
						...
						If CDbl(jorder(n)) = CDbl(g_no) then
						  Response.write "<tr>"
						  Call tryoutTN(g_no, arrRND)
						  Response.write "</tr>"
						  nuse = true
						End if
					  Next

					  If nuse = False Then '조순위가 없는 번호
						  Call emptyLgDrow(arrRND, jorder(n))
					  End If
				Else'데이터 한개도 없을때
					  Call emptyLgDrow(arrRND, jorder(n))
				End if    
		...
		Else '조나누기아님######
			...
			'maxgno = 8
			'For arn = LBound(arrRND, 2) To UBound(arrRND, 2) 
			'Response.write arrRND(2,arn) & "<br>"
			'next
