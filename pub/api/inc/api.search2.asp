    <%If tabletype ="2"  and CMD <>"20003"  then%>
		  <!-- S: color-guide -->
     ...
     %>
	  ...
	  <table>
						  ...
						  <%Case 1,2,3%>
							...
							%>
							  ...
							  <%If CDbl(laststate)  = 1 then%>
									...
									<%else%>
									<a href="javascript:score.showScore({'SCIDX':<%=inputsc%>,'P1':'<%=mem1idx%>',P2:'<%=ojson.Get(c-1).AID%>','GN':0,'JONO':<%=groupno%>,'RD':0,'GRND':0,'SNO':0});" class="btn btn-<%If CDbl(c_leftetc) > 3 And CDbl(c_leftetc) < 7  then%>danger<%else%>result<%End if%>"  <%=showYn %> >결과보기</a>
									<%End if%>
							  <%else%>
								    <%If tabletype = "2" then%>
									<%If i >= c Then '왼쪽 오른쪽 바꿈 아래쪽%>
									...
									<%else%>
								    <a href="javascript:score.inputScore({'SCIDX':<%=inputsc%>,'P1':'<%=mem1idx%>',P2:'<%=ojson.Get(c-1).AID%>','GN':0,'JONO':<%=groupno%>})" class="btn btn-<%If laststate = "2" then%>enter-score<%else%>ready<%End if%>"  <%=showYn %> >스코어입력</a>
									<%End if%>
								   <%End if%>
							  <%End if%>
							  </td>
        ...
        <% if CMD <>"20003" then  %>
		      ...
		      <%End if%>
         <%else%>
             <a href="javascript:score.gameSearch(0)" class="btn btn-show-list">예선 조 목록 보기</a>
         <%End if%>
		</div>
		<!-- E: btn-list -->
