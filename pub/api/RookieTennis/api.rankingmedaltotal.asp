  ...
  Set db = new clsDBHelper
	...
	If strs2 = "tn001001" Then '개인전
		...
		If strs4 = "0" Then '전체
			...
			strFieldName = fieldstr & " ,sum(win) as ws ,sum(lose) as ls, sum(goldcnt) as gnum,sum(silvercnt) as snum,sum(bronzecnt) as bnum, (sum(goldcnt) + sum(silvercnt)  + sum(bronzecnt)) as medalcnt "

			strSort = "  Order by medalcnt Desc"
			strSortR = "  Order by  medalcnt asc "
	...
	Else '단체전############################
		...
		strFieldName = fieldstr & " ,win ,lose, goldcnt as gnum, silvercnt as snum, bronzecnt as bnum, (goldcnt + silvercnt + bronzecnt) as medalcnt "
...
<%If CMD = CMD_RANKINGMEDALTOTAL then%>
      ...
      <div class="medal ranking">
		...
		<%
			...
			snum = rs("snum")
			bnum = rs("bnum")
			totalmedal = CDbl(gnum) + CDbl(snum) +CDbl(bnum)
			%>
				<tr>
				  <td><%=i%></td>
				  <td><%=unm%></td>
