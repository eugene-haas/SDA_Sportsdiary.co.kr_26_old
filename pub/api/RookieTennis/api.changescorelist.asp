<%
	rkey = oJSONoutput.RKEY '레코드인덱스
	setno  = oJSONoutput.SETNO
	ckpos = oJSONoutput.CKPOS 'left right
	...
	chkskill = oJSONoutput.SKILL1 '체크할 스킬
	...
	'상대대표 선수
	changemidx =  oJSONoutput.MIDX
	changemname =  oJSONoutput.MNM
	...
	If chkskill = "NET" or chkskill = "OUT" Then
		If ckpos = "left" Then
			winpos = "right"	
		Else
			winpos = "left"
	...
	'수정처리 (승패변경 ####################
	If winpos  = "left"  Then '마지막 값이 왼쪽 이지만 net out 이라면 ....
		SQL = "update sd_TennisResult Set m1set"&setno&" = m1set"&setno&"-1,m2set"&setno&" = m2set"&setno&"+1 where resultIDX =  " & gameidx
		Call db.execSQLRs(SQL , null, ConStr)

		SQL = "update sd_TennisResult_record  Set leftscore = rightscore , rightscore = leftscore,score_postion = '"&revpos&"',midx="&changemidx&",name='"&changemname&"' where rcIDX = " & rkey
		Call db.execSQLRs(SQL , null, ConStr)
	Else
		SQL = "update sd_TennisResult Set m1set"&setno&" = m1set"&setno&"+1,m2set"&setno&" = m2set"&setno&"-1 where resultIDX =  " & gameidx
		Call db.execSQLRs(SQL , null, ConStr)
	...
	SQL = "select m1set"&setno&", m2set"&setno&" from sd_TennisResult where resultIDX =  " & gameidx
	Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)	

	lsc = rs(0)
	rsc = rs(1)
	Call oJSONoutput.Set("leftsc", lsc )
	Call oJSONoutput.Set("rightsc", rsc )
