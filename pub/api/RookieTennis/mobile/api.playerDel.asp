...
'#############################################
	...
	SQL = "UPDATE SD_rookieTennis.dbo.TB_RVAS_MAST Set STAT_CD = '0' where CUST_CD = '" & Left(sitecode,2) & ridx & "' and sitecode = '"&sitecode&"'"
	...
	If Not rs.eof Then
		...
		SQL = "Update SD_rookieTennis.dbo.TB_RVAS_LIST Set  recustnm = '"&inbankname&"' , rebanknm='"&inbank&"', refund = '"& inbankacc &"',tidx='"&tidx&"',levelno='"&levelno&"',refunddate=getdate() where cust_cd = '" & Left(sitecode,2) & ridx & "' and STAT_CD = '0'  "
		Call db.execSQLRs(SQL , null, ConStr)
	End If
