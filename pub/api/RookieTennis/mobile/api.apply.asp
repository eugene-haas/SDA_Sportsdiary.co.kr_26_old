...
'#############################################
	...
	'비랭킹 혼합복식
	If  gm_teamgb = "20106"   Or gm_teamgb = "20107" Or gm_teamgb = "20108" Then
		gm_teamgb = "" 
		c_TeamGbNm = ""
		teamgbNm = ""
	End if
	...
	If hasown(oJSONoutput, "p1idx") = "ok" And p1idx <> "" then
		...
		SQL = "Select teamNm,team2Nm,teamgb,belongBoo,sex from tblPlayer where playerIDX = " & p1idx
		Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
		...
		If Not rs.eof Then
			...
			teamgb_chk = rs(2) '참가신청한 사람의 코드
			boonm_chk = rs(3) '참가신청한 사람의 원래부
...
'				SQL = SQL &"  where b.DelYN='N' and b.GameTitleIDX='"&GameTitleIDX&"' and c.LevelNm not like '%최종%'  and  b.TeamGb in ('20104','20105') "
			...
			If teamgb_chk = "20101" Or teamgb_chk = "20102"  then
				teamgbupdate = "no" 
			End if
		End if

		If teamgbupdate = "no"  Then
		SQL = "update tblPlayer set sidogu = '"&addr1&"' , gamestartyymm = '"&goym1&"', userLevel = '"&p1grade&"'   "&team1Aupstr &team1Bupstr &" where playerIDX = " & p1idx
		else
		
		SQL = "update tblPlayer set sidogu = '"&addr1&"' , gamestartyymm = '"&goym1&"', userLevel = '"&p1grade&"',teamgb = '"&gm_teamgb&"',belongBoo='"&c_TeamGbNm&"'   "&team1Aupstr &team1Bupstr &" where playerIDX = " & p1idx
	...
	goym2 = goyear2 & gomonth2

	If hasown(oJSONoutput, "prize2") = "ok" then		
	...
	'중복데이터가 쌓이지 않도록 파트너가...
	If p2idx = "" Then
		SQL = "select top 1 playeridx from tblPlayer where memberidx = " & p2midx
		Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
		If Not rs.eof Then
			p2idx = rs(0)
		End if
	...
	If p2idx = "" Then
		...
		i_value = p2midx & ", '" &p2name& "', '" &rs(0)& "', '" &rs(1)& "', '" &rs(2)& "', '"&addr2&"','"&goym2&"','"&p2grade&"',        '"&p2team1&"','"&p2team1txt&"','"&p2team2&"','"&p2team2txt&"','"&gm_teamgb&"','"&c_TeamGbNm&"', '"&P2_id&"'  "
	...
	Else
		'마지막 참가 정보 업데이트 (부서가 처음 셋팅된건지 확인후 클럽도 업데이트)
		SQL = "Select teamNm,team2Nm,sex from tblPlayer where playerIDX = " & p2idx
		Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
		If Not rs.eof Then
			If rs(0) = "" Or isnull(rs(0)) Then
		...
		If teamgbupdate = "no"  then
		SQL = "update tblPlayer set sidogu = '"&addr2&"' , gamestartyymm = '"&goym2&"', userLevel = '"&p2grade&"' "&team2Aupstr &team2Bupstr &" where playerIDX = " & p2idx
		else
		SQL = "update tblPlayer set sidogu = '"&addr2&"' , gamestartyymm = '"&goym2&"', userLevel = '"&p2grade&"',teamgb = '"&gm_teamgb&"',belongBoo='"&c_TeamGbNm&"' "&team2Aupstr &team2Bupstr &" where playerIDX = " & p2idx
		End if
		Call db.execSQLRs(SQL , null, ConStr)
	End if
...
'######################
	...
	If ridx <> "" Then '수정
		...
		'파트너 아이디찾아서 수정
		SQL = "select top 1 userid  from tblPlayer where playeridx = '" & p2idx & "'"
		Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
		P2_userid = rs(0)
		'파트너 아이디찾아서 수정
		...
		setvalue = setvalue & " ,P1_PlayerIDX = "&p1idx&",P1_UserName = '"&p1name&"',P1_UserLevel = '"&p1grade&"',P1_Team = '"&p1team1&"',P1_Team2 = '"&p1team2&"',P1_TeamNm = '"&p1team1txt&"',P1_TeamNm2 = '"&p1team2txt&"',P1_UserPhone = '"&p1phone&"' ,P1_prizekey = '"&prize1&"' "
		setvalue = setvalue & " ,P2_PlayerIDX = "&p2idx&",P2_UserName = '"&p2name&"',P2_Team = '"&p2team1&"',P2_Team2 = '"&p2team2&"',P2_TeamNm = '"&p2team1txt&"',P2_TeamNm2 = '"&p2team2txt&"',P2_UserPhone = '"&p2phone&"' ,P2_prizekey = '"&prize2&"' , P2_id = '"&P2_userid&"'  "
	...
	If CDbl(attCNT) < CDbl(EntryCntGame)  Or (ridx <> "" And attmember = true) then ' 현재까지 참가자수 >= 참가 제한수   or 수정상태이고, 참가자라면
		...
		If rs.eof Then
			...
			insertvalue = " 0, "&tidx&", "&p1idx&", '"&p1name&"', '"&key1&"',"&Left(levelno,3)&","&levelno&",'"&gm_teamgb&"','"&teamgbNm&"','"&p1team1txt&"','"&p1team2txt&"' ,0,'"&ridx&"' "
		...
		'==========================================
			If CDbl(acctotal) > 0 Then '금액이 0이상인 경우만
				...
				SQL = "select top 1 CUST_CD from TB_RVAS_MAST where CUST_CD = '" & Left(sitecode,2) & ridx & "' "
				Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
