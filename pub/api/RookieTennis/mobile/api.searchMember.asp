...
'	If CDbl(Cookies_firstcnt) > 1 Then '제외조건 (1등 2회인경우 파트너 제약조건) 전체 1등횟수가 파악된다. 같은별 1등횟수로 차감되어야할꺼같다. 일딴 막아서 문제 제거
...
'	End if
	...
	SQL = "select top 30 " & mfield & pfield & " from SD_Member.dbo.tblMember as m "
	...
	SQL = SQL &" left join SD_RookieTennis.dbo.tblPlayer a On m.memberIDX = a.memberIDX and a.DelYN = 'N'   " '같은 스타
