...
'#############################################
	...
	If Not rs.eof Then
		...
		'R07	한국기록
		'R08	일반-참가기록
	//20211217- 중등부에서 일괄적으로 대회 기록 표출 안되어 R03 구분 텍스트를 '중등'에서 '중학'으로 변경
...
'		fld = fld & ",(case when (a.gameno = '"&gameno&"' or b.starttype = 3 ) then tryoutsortNo when (a.gameno2 = '"&gameno &"' ) then sortno end) as RANE, '1' as ODRNO " 'odrno 출전순서 (단체전에서만사용)
