...
'############################################
	If request("test") = "t" Then
		...
		REQ = "{""CMD"":22000,""MIDX"":19709,""HNM"":null}"
	else
		REQ = request("REQ")
	End if
	...
	CMD_GETRELAYINFOORDER = 21100 '릴레이
	CMD_GETRELAYINFO = 21000 '릴레이 화면 불러오기
	CMD_PLAYERPOP = 22000 '부의선수창오픈

	DEBUGIP = "112.187.195.132"
	DEBUGTIDX = "45"
	...
	Select Case CDbl(CMD)
		...
		Case CMD_GETRELAYINFOORDER
			%><!-- #include virtual = "/pub/api/Riding/mobile/api.getrelayOrder.asp" --><%  
			Response.end
		
		
		Case CMD_PLAYERPOP
			%><!-- #include virtual = "/pub/api/Riding/mobile/api.relayplayer.asp" --><%
			Response.end

		Case CMD_GETRELAYINFO
			%><!-- #include virtual = "/pub/api/Riding/mobile/api.getrelay.asp" --><%
			Response.end
		...
		Case CMD_GAMEGETMATCHPLAYERLIST
			%><!-- #include virtual = "/pub/api/Riding/mobile/api.gameGetMatch_playerListV2.asp" --><% 
			Response.end
