...
'Call oJSONoutput.Set("심판수", judgeCnt )
		...
		'%%%%%%%%%%%%%%%%%
			...
			Function getBooInfo(lidx, ByRef db, ByRef  ConStr ,CDA)
				...
				SQL = "Select top 1 isNull(grouplevelidx, 0) , isnull(RoundCnt,0), isnull(judgeCnt,0) , "&lidxsquery&",cdc,gametitleidx  from tblRGameLevel as L where RgameLevelidx = " & lidx
				Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
				If Not rs.EOF Then
					arrI = rs.GetRows()
				End If
