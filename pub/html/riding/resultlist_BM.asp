...
'Response.write "" &select_f_teamgb & "---"
	...
	SQL = "Select "&fldnm&" from "&tblnm&" where a.gametitleidx = " & tidx & " and a.delYN = 'N' and a.teamgb = '"&select_f_teamgb&"'  and a.round = 2 and a.gubun < 100 order by a.tryoutsortno,a.tryoutgroupno, a.pubcode, a.orgpubcode asc"
	Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)

' and a.levelno like '"&select_f_teamgb&"%'  and a.gbidx  <> '"&find_gbidx&"'
...
'Call getrowsdrow(arrz)
	...
	If IsArray(arrZ) Then 
		For ari = LBound(arrZ, 2) To UBound(arrZ, 2)
			...
			r_pt = isNullDefault(arrZ(40, ari),0) '획득포인트
