...
'	'독립적으로 지점수, 최고점수, 심판장서명완료 가 되었는지 확인할수 있게 되어야한다. (gbidx 에서 한개라도 누락이라면 안된거임) 다시적용하록 메시지
	...
	attcnt = " ( select count(*) from SD_tennisMember where  gametitleidx = " & tidx & " and delYN = 'N' and gubun < 100 and gamekey3 = a.gamekey3 and playeridx > 0) as attcnt "
	...
	SQL = "Select "&fldnm&" from "&tblnm&" where a.gametitleidx = " & tidx & " and a.delYN = 'N' and a.gamekey3 = '"&find_gbidx&"' and  a.gubun < 100 order by a.tryoutsortno,a.tryoutgroupno, a.pubcode, a.orgpubcode asc" 'round = 1 and
	Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
	...
	If IsArray(arrZ) Then 
		For ari = LBound(arrZ, 2) To UBound(arrZ, 2)
		  ...
		  <tr class="gametitle_<%=r_a2%>"  id="titlelist_<%=idx%>" >
				...
				<td><%=r_a4%></td>
				<td><%=r_b2%></td>
				<td><%If r_a2 = "2" then%>리그<%else%>토너먼트<%End if%><%'=r_a9%></td><!-- 팀명 -->
				<td><%=r_a3%></td><!-- pidx -->
...
<%End if%>
				...
				<td><%=r_pt%></td>
				<td>Y</td>
				<td><input id="price_<%=idx%>" type="number" value="<%=r_pricemoney%>" style="width:70px;"   onblur="if(this.value !=''){mx.setPriceMoney(this,this.value,<%=idx%>,<%=r_a3%>,<%=r_b1%>)}"/></td>
		  </tr>
