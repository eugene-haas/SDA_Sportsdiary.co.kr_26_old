...
'	'독립적으로 지점수, 최고점수, 심판장서명완료 가 되었는지 확인할수 있게 되어야한다. (gbidx 에서 한개라도 누락이라면 안된거임) 다시적용하록 메시지
	...
	SQL = "Select "&fldnm&" from "&tblnm&" where a.gametitleidx = " & tidx & " and a.delYN = 'N' and a.gamekey3 = '"&find_gbidx&"' and "&roundstr&" and a.gubun < 100 and a.boo_orderno < 4  order by a.boo_orderno asc "
...
'Response.end
	...
	If IsArray(arrZ) Then 
		For ari = LBound(arrZ, 2) To UBound(arrZ, 2)
			...
			%>
		  <%If r_round = "1" Then '1라운드만 그린다. 결과반영시 순위를 1라운드에 함수에서 반영해준다. %>
