  ...
  <div class="table-responsive"  id="printdivC">
		<table cellspacing="0" cellpadding="0" class="table table-hover">
			<thead>
				...
				<tr><td colspan="17" style="text-align:right;">
						...
						<%'체전의 3라운드는 제외 하고..%>
						<%If kgame = "Y" then%>
						<%If select_f_classhelp = "FEI 238.2.2" then%><a href="javascript:mx.setHurdle(<%=tidx%>,<%=find_gbidx%>,'<%=select_f_classhelp%>',4)" class="btn btn-primary">장애물 기준 및 배치정보(재경기)</a>&nbsp;&nbsp;&nbsp;<%End if%> 
						<%End if%> 

						<%If kgame = "Y" then%> 
						<a href="javascript:$('#printdivC').printThis({importCSS: false,loadCSS: '/pub/js/print/print_table.css',header: '<h1><%=select_f_title%> : 재경기(1)</h1>'});" class="btn btn-default">화면인쇄</a>&nbsp;&nbsp;</span>
						&nbsp;&nbsp;<a href='javascript:px.goPrint(<%=req%>,4)' class="btn btn-default">결과인쇄</a></span>
						<%End if%>

						<%If CDbl(maxrndno) = 4 then%>
...
<%
	...
	SQL = "Select "&fldnm&" from "&tblnm&" where a.gametitleidx = " & tidx & " and a.delYN = 'N' and a.gamekey3 = '"&find_gbidx&"' and round = 4 and gubun < 100 order by a.tryoutsortno,a.tryoutgroupno, a.pubcode, a.orgpubcode asc"
