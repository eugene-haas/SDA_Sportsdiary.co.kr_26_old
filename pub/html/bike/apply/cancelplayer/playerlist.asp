<tr id="paystate_<%=idx%>" >
    ...
    <td class="department_name cursor-btn" onclick="mx.viewLevelInfo(<%=groupno%>, <%=idx%>)"><%=levelTitle%><% If requestCount > 1 Then %> 외 <%=requestCount -1 %>개<% End If %></td>
    ...
    <td class="deposit_condition">
        <div class="switch_box">
            <ul>
                <li>
                    <div>
                        <p <% If payText = "입금" Then %> class="red-font" <%End If%>><%=payText%><p>
                    </div>
                </li>
            </ul>
        </div>
    ...
    <% If refundno <> "" Then %>
    <a class="white-btn cursor-btn" onclick="mx.viewRefundInfo(<%=idx%>);" >정보확인</a>
    <% Else %>
    <a class="white-btn" onclick="" disabled>정보확인</a>
    <% End If %>
    </td>
