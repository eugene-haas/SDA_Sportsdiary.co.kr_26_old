	<tr class="gametitle"  style="cursor:pointer" id="titlelist_<%=idx%>" title="메모 : <%=replaceContTag(txtMemo)%>  , 비밀번호 : <%=replaceContTag(p_UserPass)%> : 문자발송 : <%=lastupdate%> ">
		...
		<td>
		<a href="javascript:mx.RankingPoint(<%=pidx1%>,'<%=p1nm%>', <%=idx%>, <%=levelno%>, 1)" class="btn_a btn_func" style="height:100%;width:95%;font-size:15px;"><%=p1nm%></a>
		</td>
		...
		<td>
		<a href="javascript:mx.RankingPoint(<%=pidx2%>,'<%=p2nm%>',<%=idx%>, <%=levelno%>, 2)" class="btn_a btn_func"  style="height:100%;width:95%;font-size:15px;"><%=p2nm%></a>
		</td>
