				...
				<ul>
					...
					<li>
						<%If ridx = 0 then%>
						<a href="./cacluExcel.asp?tid=<%=tidx%>" class="search_btn"><img src="/images/favicon/excel.png" style="margin-bottom:3px;"> 대회정산</a>
						<%End if%>
					
						<%'If USER_IP = "118.33.86.240" Then%>
						<%If ridx > 0 Then
							SQLT = "select COUNT(*) from tblGameRequest as a INNER JOIN TB_RVAS_LIST as b ON a.RequestIDX = b.CUST_CD and b.refundok ='N'  where a.GameTitleIDX = "&tidx&" and a.level = '"&ridx&"' and a.delYN = 'Y' "
						...
						<a href="./attexcel.asp?tidx=<%=tidx%>&levelno=<%=ridx%>" class="search_btn" ><img src="/images/favicon/excel.png" style="margin-bottom:3px;"> 참가신청</a>
						<%End if%>
						<%'End if%>
