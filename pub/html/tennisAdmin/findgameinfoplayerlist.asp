	<tr id="titlelist_<%=idx%>" >
		...
		<td class="competition_name"><span><%=title%></span>
		<span style="font-size:11px;"><%=Mid(writeday,6)%></span>
		</td>
		...
		<%else%>
			...
			<%else%>
				...
				<td>&nbsp;</td>
				<%else%>
				<td><a href="javascript:mx.setBankNo(<%=idx%>,<%=tidx%>,<%=levelno%>)" class="name_btn" style="background:red">미발급상태</a></td>
