	...
	<tr class="gametitle" id="titlelist_<%=idx%>" title="메모 : <%=replaceContTag(txtMemo)%>  , 비밀번호 : <%=replaceContTag(p_UserPass)%>">
		...
		<td title = "완영 결과">
			<span>
				<label class="switch" onclick="mx.raceCheck(<%=idx%>)">
					<input type="checkbox" id="race_<%=idx%>"  value="Y" <%If raceok= "Y" then%>checked<%End if%>>
		...
		<td><span><%=p1t1%> (<%=jangname%>)</span></td>

		<td>
			<span>
				<%=readername%> Tel:<%=readerphone%>
			</span>
		</td>
