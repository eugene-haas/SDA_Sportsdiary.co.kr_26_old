	...
	<tr class="gametitle" id="titlelist_<%=l_midx%>"  style="text-align:center;">
		...
		<%End if%>
			...
			onkeyup="this.value=this.value.replace(/[^0-9]/g,'')" onblur="mx.setPE2(this, <%=l_midx%>, <%=l_lidx%>, <%=l_gno%>,'r1_<%=l_midx%>')"
			value="<%=l_result%>" maxlength="5" <%If l_result = "" then%>disabled<%End if%>  onfocus="px.chkZeroE2(this)"></td><!-- onfocus="px.chkZeroSm(this)"  -->
