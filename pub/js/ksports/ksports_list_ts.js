...
//	}  
		...
		switch (packet.BTYPE)	{
		case 'e': 
			for(var i = varr.length ; i < mx.edate.length; i++){
				mx.drowdate("mscul", mx.edate[i], packet.BTYPE);	
			}		
		break;
		case 'm': 
		...
		break;
		case 'c': 
			for(var i = varr.length ; i < mx.cdate.length; i++){
				mx.drowdate("mscul", mx.cdate[i], packet.BTYPE);	
			}		
		break;
		case 'd': 
			for(var i = varr.length ; i < mx.ddate.length; i++){
				mx.drowdate("mscul", mx.ddate[i], packet.BTYPE);	
			}
		break;
		case 'x': 
			for(var i = varr.length ; i < mx.xdate.length; i++){
				mx.drowdate("mscul", mx.xdate[i], packet.BTYPE);	
			}			
		break;
		}  
...
//		}
		...
		switch (packet.BTYPE)	{
		case 'e': 
			for(var i = 0 ; i < mx.edate.length; i++){
				mx.drowdate("mscul", mx.edate[i], packet.BTYPE);	
			}
		break;
		case 'm': 
		...
		break;
		case 'c': 
			for(var i = 0 ; i < mx.cdate.length; i++){
				mx.drowdate("mscul", mx.cdate[i], packet.BTYPE);	
			}
		break;
		case 'd': 
			for(var i = 0 ; i < mx.ddate.length; i++){
				mx.drowdate("mscul", mx.ddate[i], packet.BTYPE);	
			}
		break;
		case 'x': 
			for(var i = 0 ; i < mx.xdate.length; i++){
				mx.drowdate("mscul", mx.xdate[i], packet.BTYPE);	
			}
		break;
		}  
...
mx.SetLoadVodDate = function(sender, inputvalue,btype,i){ //날짜 로드할때 
	...
	switch (btype)	{
	case 'e': mx.edate[i] = inputvalue; break;
	case 'm': mx.mdate[i] = inputvalue; break;
	case 'h': mx.hdate[i] = inputvalue; break;
	case 'c': mx.cdate[i] = inputvalue; break;
	case 'd': mx.ddate[i] = inputvalue; break;
	case 'x': mx.xdate[i] = inputvalue; break;
	}  
...
mx.SetVodDate = function(sender, inputvalue,btype){
	switch (btype)	{
	case 'e': mx.edate.push(inputvalue); break;
	case 'm': mx.mdate.push(inputvalue); break;
	case 'h': mx.hdate.push(inputvalue); break;
	case 'c': mx.cdate.push(inputvalue); break;
	case 'd': mx.ddate.push(inputvalue); break;
	case 'x':  mx.xdate.push(inputvalue); break;
	}  
//  if (btype == 'm'){
...
mx.delVodDate = function(delval , btype){
	switch (btype)	{
	case 'e': mx.edate.push(inputvalue); break;
		var idx = mx.edate.indexOf(delval);
		if (idx > -1) mx.edate.splice(idx, 1);
		$('#'+btype+delval).remove();
	case 'm': 
	...
	break;
	case 'c': 
		var idx = mx.cdate.indexOf(delval);
		if (idx > -1) mx.cdate.splice(idx, 1);
		$('#'+btype+delval).remove();
	break;
	case 'd': 
		var idx = mx.ddate.indexOf(delval);
		if (idx > -1) mx.ddate.splice(idx, 1);
		$('#'+btype+delval).remove();		
	break;
	case 'x':  
		var idx = mx.xdate.indexOf(delval);
		if (idx > -1) mx.xdate.splice(idx, 1);
		$('#'+btype+delval).remove();		
	break;
	}  
...
mx.input_frm = function(){
	...
	if ( $("#on-off-sc4").is(":checked") == true ){
		obj.CDATE = mx.cdate;
	}
	if ( $("#on-off-sc5").is(":checked") == true ){
		obj.DDATE = mx.ddate;
	}
	if ( $("#on-off-sc6").is(":checked") == true ){
		obj.XDATE = mx.xdate;
	}
