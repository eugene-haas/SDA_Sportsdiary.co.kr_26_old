...
////////////////////////////////////////
	...
	mx.CMD_D = 40004; //삭제
	mx.CMD_EDITOR = 40005 //대회요강
	mx.CMD_EDITOROK = 40006 //대회요강 수정확인
...
mx.ajaxurl = "/pub/ajax/bike/reqContest.asp";
...
mx.SelectGb = function() {
  ...
  if(GbVal == "insert" ) {
    var GbPrompt = prompt("그룹/종목을 입력해주세요",'');
...
mx.Select = function(id,cmd) {
  ...
  var games = $("#GameS").val();
  var gamee = $("#GameE").val();
  var gameas = $("#GameAS").val();
  var gameae = $("#GameAE").val();
  var gamehost = $("#gamehost").val();

  var sval = $("#"+id).val();
  if(sval == "insert" ) {
    var GbPrompt = prompt("주최를 입력해주세요",'');
    if(GbPrompt != null && GbPrompt != "") {
      var obj = {};
      obj.CMD = cmd;
	  obj.NAME = GbPrompt;
...
mx.OndrowHTML =  function(cmd, packet, html, sender){
	...
	if( cmd == mx.CMD_E || cmd == mx.CMD_GAMEHOST ){
		mx.init();
	}
};
...
mx.input_frm = function(){
	...
	obj.GameS              	=	$("#GameS").val();
	obj.GameE              		=	$("#GameE").val();
	obj.GameAS            		=	$("#GameAS").val();
	obj.GameAE            		=	$("#GameAE").val();
