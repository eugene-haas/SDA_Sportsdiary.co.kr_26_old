...
////////////////////////////////////////
	...
	mx.CMD_SELECTSUBGB = 30025;//종목검색
	mx.CMD_INSERTDETAILGB = 30026; //경기종류생성
...
mx.ajaxurl = "/pub/ajax/bike/reqContestlevel.asp"; 
...
mx.ReceivePacket = function( reqcmd, data, sender ){// data는 response string
	...
	case mx.CMD_E: this.OndrowHTML( reqcmd, jsondata, htmldata, sender );		break;
	case mx.CMD_EOK: alert("데이터가 수정되었습니다."); this.OndrowHTML( reqcmd, jsondata, htmldata, sender );		break;
	case mx.CMD_GAMEHOST:
	case mx.CMD_GAMEORGN:
	case mx.CMD_SELECTGB:
	case mx.CMD_SELECTSUBGB:
	case mx.CMD_INSERTSUBGB:
	case mx.CMD_INSERTDETAILGB:
	...
	case mx.CMD_D:	alert("데이터가 삭제되었습니다."); this.OndelHTML( reqcmd, jsondata, htmldata, sender );		break;
	}
};
...
mx.SelectSubGb = function() {
  ...
  if(GbSubVal == "insert" ) {
    var GbPrompt = prompt("중분류를 입력해주세요",'');
    if(GbPrompt != null && GbPrompt != "") {
      var obj = {};
...
mx.SelectDetailGb = function() {
  ...
  var GbSubVal = $("#sportssubgb").val();
  if (GbVal == ''){
	 alert('중분류를 먼저 선택해 주십시오.');
	 return;
  }
   var GbDetailVal = $("#sportsdetailgb").val();

  if(GbDetailVal == "insert" ) {
    var GbPrompt = prompt("경기종류을 입력해주세요",'');
    if(GbPrompt != null && GbPrompt != "") {
      var obj = {};
      obj.CMD = mx.CMD_INSERTDETAILGB;
	  obj.GB = GbVal;
	  obj.GBSUB = GbSubVal;
	  obj.GBDETAIL = GbPrompt
	  if (idx != ''){
		  obj.IDX = idx;
	  }
      mx.SendPacket('ul_1', obj);
...
mx.input_frm = function(){
	...
	obj.SGBDETAIL				=	$("#sportsdetailgb").val();
	obj.GameS              		=	$("#GameS").val();
	obj.SOLOCNT		      	=	$("#gamemcnt").val();
	obj.GAMECNT      			=	$("#game_cnt").val();
...
mx.input_edit = function(idx){
	$("td").css("color", "#49586a");
	$("#titlelist_" + idx + " td").css("color", "red");
