...
* 추가 : 2017-11-06 김주영
*/
mx_player.ajaxURL = "/pub/ajax/RookieTennis/mobile/reqTennisatt.asp";
...
mx_player.ReceivePacket = function (reqcmd, data, sender, packet) {// data는 response string
    ...
    switch (Number(reqcmd)) {
        ...
        case mx_player.CMD_LMS_SEND:
			this.OnAppendHTML(reqcmd, jsondata, htmldata, sender);
		break;
...
mx_player.OnAppendHTML = function (cmd, packet, html, sender) {
    $('#' + sender).append(html);
	if ( cmd == mx_player.CMD_LMS_SEND ){
		document.forms[packet.fromid].submit();
	}
};
...
mx_player.init = function () {
    ...
    $("#p2name").autocomplete({
        ...
        source: function (request, response) {
            ...
            $("#sidogb2").attr("disabled", false).val("");
            $("#googun2").attr("disabled", false).val("");
            $("#goyear2").attr("disabled", false).val("");
            $("#gomonth2").attr("disabled", false).val("");
			$("#lvlm2").show();
            ...
            $.ajax({
                ...
                url: mx_player.ajaxURL,
                dataType: "json",
                data: { "REQ": JSON.stringify({ "CMD": mx_player.CMD_MEMBERSEARCH, "SVAL": request.term, "tidx": $("#GameTitleIDX").val(), "levelno": $("#levelno").val() }) },

				success: function (data) {
                    response($.map(data, function (item) {
                        if (typeof item.name == "undefined") { }
                        else {
                            if (Number(item.idx) == 0 && Number(item.midx) == 0 ) {
                                //labelstr = "비회원";
                                //item.name = request.term;
                            }
                            else {
                            ...
                            return {
								...
								grade:item.grade,
								sidogu:item.sidogu,
								gamestartyymm:item.gamestartyymm
        ...
        select: function (event, ui) {
            if (Number(ui.item.uidx) == 0  && Number(ui.item.midx) == 0 ) {
				alert('파트너의 회원가입이 되어있어야 신청가능합니다.');
				$("#p2idx").val("");
				$("#p2name").val("");
                return;
            }
...
//                $("#p2phone3").attr("disabled", false).val("");
                ...
                /*참가자*/
                $("#p2midx").val(ui.item.midx);
				$("#p2idx").val(ui.item.uidx); //플레이어로 등록이 안되어있음....어떻게 할까
                $("#p2name").val(ui.item.name);
				$("#p2b").html(ui.item.Birthday + ' ['+ui.item.Sex+']');

                if (Number(ui.item.tm1c) != 0){ //일딴 비어있으면 오픈
				...
				if(ui.item.sidogu != '' && ui.item.sidogu != null){
					var sidogu = ui.item.sidogu.split("_");
					$("#sidogb2span").html("<select  id='sidogb2'   disabled><option value='"+sidogu[0]+"' selected>"+sidogu[1]+"</option></select>&nbsp;<select  id='googun2' disabled><option value='"+sidogu[2]+"' selected>"+sidogu[2]+"</option></select>");
				}

				if(ui.item.gamestartyymm != '' && ui.item.gamestartyymm != null){
					$("#gym2").html("<select  id='goyear2' class='m_applicationForm__select s_date' disabled><option value='"+ui.item.gamestartyymm.substr(0,4)+"'>"+ui.item.gamestartyymm.substr(0,4)+"</option></select><select id='gomonth2' class='m_applicationForm__select s_date' disabled><option value='"+ui.item.gamestartyymm.substr(4,2)+"'>"+ui.item.gamestartyymm.substr(4,2)+"</option></select><br><p class='applicationForm__txt'> ※ 구력은 자동으로 갱신되며 수정은 불가합니다. 테니스 시작한 정보를 정확하게 입력해주세요. </p>");
				}



                if (ui.item.grade != ''  && ui.item.grade != null){
					$("#lvlm2").hide();
					$("#lvl2").attr("disabled", true).val(ui.item.grade);
                }
