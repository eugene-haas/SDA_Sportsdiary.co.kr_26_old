...
////////////////////////////////////////
	...
	mx.CMD_ACCTMSG = 30025; //계좌문자보내기(목록)
	mx.CMD_ACCTMSGSEND = 10;			//계좌문자보내기
...
mx.ajaxurl = "/pub/ajax/RookieTennis/reqTennisContestPlayer.asp";
...
mx.ReceivePacket = function( reqcmd, data, sender ){// data는 response string
	...
	if( jsondata !='' && jsondata != null){
		...
		case 501:	alert('변경되었습니다.');return; 	break;
		case 502:	alert('본선 진출 정보가 있어 변경 할 수 없습니다.');return; 	break;
    ...
    $( "#p1name" ).autocomplete({
		source : function( request, response ) {
			...
			$.ajax(
				{
						type: 'post',
						url: "/pub/ajax/RookieTennis/reqTennisContestPlayerFind.asp",
						dataType: "json",
...
mx.payCheck = function(idx, onoff){
	var obj ={};
	obj.IDX = idx;
...
/////////////////////////////////////////////////
mx.RankingPoint = function(idx, name,tidx,levelno,title, ptype){
	mx.ajaxurl = "/pub/ajax/RookieTennis/reqTennisMakePlayer.asp";
	var obj = {};
	obj.CMD = mx.CMD_RANKPOINT;
	obj.IDX = idx;

	obj.LEVELNO = levelno;//mx.gameinfo.LEVELNO;
	obj.TIDX = tidx;//mx.gameinfo.IDX;
	obj.TITLE = title;//mx.gameinfo.TITLE;
...
mx.searchPlayer = function(pageno){
	...
	if ( $('#tidx').length > 0 ) {
		obj.tidx = $('#tidx').val();
		if (obj.tidx == '' || obj.tidx == undefined){
			obj.tidx = 0;
		}
	}
	if ( $('#ridx').length > 0 ) {
		obj.ridx = $('#ridx').val();
		if (obj.ridx == '' || obj.ridx == undefined){
			obj.ridx = 0;
		}
	}
...
mx.changeBoo = function(idx,levelno){
	var obj ={};
	obj.IDX = idx;
	obj.LEVELNO = $('#LevelGb_' + idx).val();
	obj.BOOSTR = $('#LevelGb_' + idx + ' option:selected').text();
	obj.CMD = mx.CMD_CHANGEBOO;

  if (levelno == obj.LEVELNO){
	  alert("변경할 부서를 선택후 변경 버튼을 눌러주세요.");
	  return;
  }
  if (confirm("참여 부서를 변경 하시겠습니까?")) {
	//console.log(obj);
	mx.SendPacket(null, obj);
  }
  else{
...
mx.levelno = 0;
//########
mx.updateMember = function (idx,tidx,levelno){
  mx.ajaxurl = "/pub/ajax/RookieTennis/reqTennisContestlevel.asp";
  mx.tidx = tidx;
  mx.levelno = levelno;
  var obj ={};
  obj.IDX = idx;
  obj.CMD = mx.CMD_UPDATEMEMBER;
  mx.SendPacket("Modaltest", obj);
  mx.ajaxurl = "/pub/ajax/RookieTennis/reqTennisContestPlayer.asp";
};


mx.sendAcctMsg = function (idx,tidx,levelno){
  mx.ajaxurl = "/pub/ajax/RookieTennis/reqTennisAcct.asp";
  mx.tidx = tidx;
  mx.levelno = levelno;
  var obj ={};
  obj.IDX = idx;
  obj.CMD = mx.CMD_ACCTMSG;
  mx.SendPacket("Modaltest", obj);
  mx.ajaxurl = "/pub/ajax/RookieTennis/reqTennisContestPlayer.asp";
};
...
//<input type="hidden" id = "uname"  value="<%=p1name%>">
//<input type="hidden" id = "uphone" value="<%=p1phone%>">
//<input type="hidden" id = "uname"  value="<%=p2name%>">
//<input type="hidden" id = "uphone" value="<%=p2phone%>">

mx.vacctsend = function (no){
  mx.ajaxurl = "/pub/ajax/RookieTennis/reqTennisAcct.asp";
  var obj ={};
  obj.title = $("#gtitle").val();;
  obj.vacct = $("#accno").val();
  obj.money = $("#payamt").val();
  switch (Number(no))
  {
  case 1:
		obj.name = 	$("#uname").val();
  ...
  case 2:
		obj.name = 	$("#p1name").val();
		obj.phone =	$("#p1phone").val();
	  break;
  case 3:
		obj.name = 	$("#p2name").val();
		obj.phone =	$("#p2phone").val();
	  break;
  }

  obj.CMD = mx.CMD_ACCTMSGSEND;
  mx.SendPacket("Modaltest", obj);
  mx.ajaxurl = "/pub/ajax/RookieTennis/reqTennisContestPlayer.asp";
};
			...
			$.ajax(
				{
						...
						dataType: "json",
						//request.term = $("#autocomplete").val()
						data: { "REQ" : JSON.stringify({"CMD":mx.CMD_AUTOCOMPLETEALL, "SVAL":request.term, "TIDX":mx.tidx,"LIDX":mx.levelno}) },
						success: function(data) {
								//서버에서 json 데이터 response 후 목록에 뿌려주기 위함
                                //console.log(data);
								response(
