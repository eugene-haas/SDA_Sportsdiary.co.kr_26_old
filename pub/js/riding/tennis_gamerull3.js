...
mx_gameRull.SendPacket = function( sender, packet){
  ...
  var url = "/pub/ajax/" +location.hostname.split(".")[0].toLowerCase().replace('admin','') + "/reqTennisGameRull3.asp";
...
mx_gameRull.ReceivePacket = function (reqcmd, data, sender) {// data는 response string
    ...
    if (jsondata != '' && jsondata != null) {
        switch (Number(jsondata.result)) {
            ...
            case 1212: alert('바이갯수 초과 (시드수를 줄여 주십시오.)'); return; break; 
...
//	var abcsgf = '';
	...
	for (var i = 1; i <= Number(packet.ATTCNT);i++){ //드로수
		boxodrno = $('#ik_'+i).val();
		boxrndno = $('#iv_'+i).val();
		abc = $('#abc_'+i).val();
		...
		if(i == 1){
		  //tsgf  = boxodrno+":"+ boxrndno + ";";
		  tsgf  = boxodrno+":"+ boxrndno + ":" + abc + ";";
		  //abcsgf  = abc + ";";
		}
		else{
		  //tsgf  = tsgf + boxodrno+":"+ boxrndno + ";";
		  tsgf  = tsgf + boxodrno+":"+ boxrndno +":" + abc + ";";		  
		  //abcsgf  = abcsgf + abc + ";";
		}
	}
...
mx_gameRull.jooABC = function(orderX, joovalue){
	for (var i = 1 ;i < Number(orderX) ;i++ ){
		if ( $('#abc_' + i).val() == '' )
		{
			$('#abc_' + i).val(joovalue);
		}
