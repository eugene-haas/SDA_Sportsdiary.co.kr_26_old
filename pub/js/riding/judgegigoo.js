...
////////////////////////////////////////
  ...
  mx.CMD_SEARCHPLAYERGROUP = 30031; //그룹선수검색
  mx.CMD_SEARCHJUDGE =  30050; //심판검색

  mx.CMD_CHANGEPLAYER = 700;//선수변경
  mx.CMD_CHANGEGROUPPLAYER = 778;
 ...
 mx.CMD_LGMAKE = 70000;
 mx.CMD_TNMAKE = 777;
 mx.CMD_GAMEINRC = 13010; //게임결과 입력
 mx.CMD_SETSAVETN = 815;
 mx.CMD_SETORDERLEG = 112; //리그 순위 변경
 mx.CMD_SETGIGOLEG = 113;

 mx.CMD_SETGIGOO = 114;

 mx.CMD_SETVALGEEGOO = 779; //경기결과값 입력
...
mx.ajaxurl = "/pub/ajax/" +location.hostname.split(".")[0].toLowerCase().replace('admin','') + "/reqJudgeGiGoo.asp";
mx.ajaxtype = "POST";
mx.dataType = "text";
...
mx.ReceivePacket = function( reqcmd, data, sender ){// data는 response string
  ...
  if( mx.IsJsonString(data) ){
    ...
    case 88: $('#'+jsondata.ID).val(''); return;    break; 
  ...
  case mx.CMD_SETORDERLEG:
  case  mx.CMD_SETGIGOLEG :
  ...
  case mx.CMD_SETVALGEEGOO :   this.OnDrowNextFocus( reqcmd, jsondata, htmldata, sender );    break;
  ...
  case mx.CMD_SEARCHPLAYERGROUP : this.ONdrowPlayer(reqcmd, jsondata, htmldata, sender );    break;

  case mx.CMD_JRC:    this.OnJRC( reqcmd, jsondata, htmldata, sender );    break;//장애물기준및 배치정보 내용 입력

  case mx.CMD_CHANGEGROUPPLAYER : this.changeWindow(jsondata.PIDX,100,jsondata.REQIDX); break;//릴레이 선수 변경
...
//요청##################################################################
	mx.setGiGoo = function(lidx, inobj){
		var obj = {};
		obj.CMD = mx.CMD_SETGIGOO;	
		obj.LIDX = lidx;
		obj.FID = inobj[0].id.split('_')[1];
		obj.VAL = px.strReplaceAll(inobj.val(),":","");

		var inputvalue = obj.VAL.substr(0,2) + ":" + obj.VAL.substr(2,2);

		if (obj.FID == 'stm'){
			if(obj.VAL.length == 4){
				inobj.val(inputvalue);
				obj.VAL = inputvalue;
			}
			else{
	...
	mx.pad = function(n, width) {
	  n = n + '';
	  return n.length >= width ? n : new Array(width - n.length + 1).join('0') + n;
	};

	//시간차
	mx.getTimeGap = function(stm,etm){
		var tm1= new Date( '2020-05-05 ' + stm);
		var tm2 = new Date('2020-05-05 ' + etm)
		var gap = tm2.getTime() - tm1.getTime();
	
		var tt = Math.floor((gap % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
		var mm = Math.floor((gap % (1000 * 60 * 60)) / (1000 * 60));
		var ss = Math.floor((gap % (1000 * 60)) / 1000);
		
		//시/분/초
		return mx.pad(tt,2) +":"+mx.pad(mm,2) +":"+mx.pad(ss,2);
	};	

	//시간더하기
	mx.getTimeSum = function(tm,plusmm){
		var tm1= new Date( '2020-05-05 ' + tm);
		tm1.setMinutes(tm1.getMinutes()+Number(plusmm))
		var gap = tm1.getTime();
		
		var tt = tm1.getHours();
		var mm = tm1.getMinutes();
		var ss = tm1.getSeconds();
	...
	//초로변환 (00:00:00 > 초로)
	mx.getSeconds = function(tm){
		var tmarr = tm.split(':');   
		var ts = Math.floor( Number(tmarr[0]) * 60 * 60 );
		var ms = Math.floor( Number(tmarr[1]) * 60 );
		var ss = Number(tmarr[2]);

		//시/분/초
		return Math.floor(ts+ms+ss);
	};	


	mx.isTime = function(tmstr){
	//HH:MM:SS"
		var time_pattern = /^(0[0-9]|1[0-9]|2[0-3]):([0-5][0-9]):([0-5][0-9])$/;
		if( !time_pattern.test(tmstr) ){
			return false;
		}
		else{
			return true;
	...
	//초를 시분초로
	mx.setTMS = function(seconds) {
		var hour = parseInt(seconds/3600);
		var min = parseInt((seconds%3600)/60);
		var sec = seconds%60;
		return mx.pad(hour,2) +":"+mx.pad(min,2) +":"+mx.pad(sec,2);
	};

	//두개 시분초 더하기 
	mx.sumRecord = function(tm1,tm2){
		var tm1ss = mx.getSeconds(tm1);
		var tm2ss = mx.getSeconds(tm2);
		var tmss = Number(tm1ss) + Number(tm2ss);
		return mx.setTMS(tmss);
	};



	mx.setValGeeGoo = function(lidx, idx, inobj){ //loopno, posno
		var obj = {};

		var stm = "00:00:00";
		var etm = "00:00:00";
		var gap = "00:00:00";
		var starttm = "00:00:00";
		var geegookm = "10000"; //거리
		var nextstarttm = "00:00:00"; //다음 출발예정시간
		var totalroopcnt = 0;

		obj.CMD = mx.CMD_SETVALGEEGOO;	
		obj.IDX = idx;
		obj.LIDX = lidx;
		obj.ID = inobj[0].id;
		obj.INVAL = px.strReplaceAll(inobj.val(),":","");
		obj.LOOPNO = inobj[0].id.split('_')[1];

		var inputvalue = obj.INVAL.substr(0,2) + ":" + obj.INVAL.substr(2,2)+ ":" + obj.INVAL.substr(4,2);

		if (inobj[0].id.split('_')[0] == 'arrive' || inobj[0].id.split('_')[0] == 'vetgate'){
			if(obj.INVAL.length == 6){

				if ( mx.isTime(inputvalue) == false ){
					alert("시분초를 정확히 입력해 주십시오.");
					return;
				}

				inobj.val(inputvalue);
				obj.INVAL = inputvalue;
				...
				totalroopcnt = $('#geegooloopcnt').val(); //루프수
				starttm = $("#starttm_" + inobj[0].id.split('_')[1] +"_"+ inobj[0].id.split('_')[2]).val(); //출발시간
				geegookm = $("#geegookm").val(); //거리
				obj.STARTTM = starttm;

				if (inobj[0].id.split('_')[0] == 'arrive'){
					stm = inputvalue;
					etm = $("#vetgate_" + inobj[0].id.split('_')[1] +"_"+ inobj[0].id.split('_')[2]).val();
				}
				else{
					stm = $("#arrive_" + inobj[0].id.split('_')[1] +"_"+ inobj[0].id.split('_')[2]).val();
					etm = inputvalue;
				}
				//시간차
				gap = mx.getTimeGap(stm,etm);
				obj.STAYTIME = gap;
				$("#staytime_" + inobj[0].id.split('_')[1] +"_"+ inobj[0].id.split('_')[2]).val(gap);

				//시간내에 완료 되었는지 비교
				if (  (  Number(px.strReplaceAll(gap,":","")) - Number($('#g_staytime' + obj.LOOPNO ).val() + '00') ) > 0 ){//제한시간초과 잖아....
					obj.RESULT = 200; //실권 E
					$("#staytime_" + inobj[0].id.split('_')[1] +"_"+ inobj[0].id.split('_')[2]).parent().css("background-color","gray");
				}
				else{
					$("#staytime_" + inobj[0].id.split('_')[1] +"_"+ inobj[0].id.split('_')[2]).parent().css("background-color","#ffffff");
					obj.RESULT = 0; //실권 E
				}


				//소요시간
				gap = mx.getTimeGap(starttm,etm); //vet gate - 출발시간
				obj.TIMETAKEN = gap;
				$("#timetaken_" + inobj[0].id.split('_')[1] +"_"+ inobj[0].id.split('_')[2]).val(gap);

				//평균속력
				var gapss = mx.getSeconds(obj.TIMETAKEN);
				var perkm = Math.floor(geegookm/gapss* 3600 / 1000  * 1000)/1000;
				obj.AVGSP = perkm;
				$("#perspeed_" + inobj[0].id.split('_')[1] +"_"+ inobj[0].id.split('_')[2]).val(perkm);
				
				//다음 출발 예정시간 1루프 2루프만  루프수에 따라서
				var nextloopno = Number(obj.LOOPNO) + 1;
				if ($("#starttm_" + nextloopno +"_"+ inobj[0].id.split('_')[2]).length > 0 ){
					//vet gate + 의무휴식시간
					nextstarttm = mx.getTimeSum(etm,$('#geegooresttm').val());
					$("#starttm_" + nextloopno +"_"+ inobj[0].id.split('_')[2]).val(nextstarttm);
					obj.NEXTTM = nextstarttm;
					
				}
				//최종기록 평균시속
				var total_record = $("#timetaken_1_"+ inobj[0].id.split('_')[2]).val();
				var total_avghp = $("#perspeed_1_"+ inobj[0].id.split('_')[2]).val();
				for (i = 2 ; i <= totalroopcnt ;i++ ){
					if ($("#timetaken_" + i +"_"+ inobj[0].id.split('_')[2]).val().indexOf(':') > 0 ){
						total_record =  mx.sumRecord( total_record ,$("#timetaken_" + i +"_"+ inobj[0].id.split('_')[2]).val() );
					}
					if ($("#perspeed_" + i +"_"+ inobj[0].id.split('_')[2]).val() != '' ){
						total_avghp =  parseFloat(total_avghp) + parseFloat($("#perspeed_" + i +"_"+ inobj[0].id.split('_')[2]).val() );
					}
				}
				
				$('#total_record_'+ inobj[0].id.split('_')[2]).val(total_record); 
				obj.TRC = total_record;

				total_avghp = Math.floor(total_avghp /totalroopcnt * 1000)/1000;				
				$('#total_perspeed_'+ inobj[0].id.split('_')[2]).val(total_avghp); 
				obj.TAVG = total_avghp;
		...
		//###################################################################
		//라인에 하나라도 있으면 실권이다...체크방법은....(라인마다 실권인지 아닌지 판단될수 있는 input을 만들어둔다.
		if (inobj[0].id.split('_')[0] == 'bpm'){
			if(obj.INVAL.length == 2){
				//시간내에 완료 되었는지 비교
				if (  (  Number(obj.INVAL) - Number($('#g_bpm' + obj.LOOPNO ).val() ) ) > 0 ){//제한시간초과 잖아....
					obj.RESULT = 200; //실권 E
					$("#bpm_" + inobj[0].id.split('_')[1] +"_"+ inobj[0].id.split('_')[2]).parent().css("background-color","gray");
				}
				else{
					$("#bpm_" + inobj[0].id.split('_')[1] +"_"+ inobj[0].id.split('_')[2]).parent().css("background-color","#ffffff");
					obj.RESULT = 0; //실권 E
				}
			}
		}

		mx.SendPacket(obj.ID, obj);			
	};
	
	
	
	mx.showLoop = function(lidx,inobj){
		$('#loop1').hide();$('#loop2').hide();$('#loop3').hide();//$('#gg_gtn').hide();

		switch ($('#g_loopcnt').val())
		{
		case "1": $('#loop1').show(); break;
		case "2": $('#loop1').show();$('#loop2').show(); break;
		case "3": $('#loop1').show();$('#loop2').show();$('#loop3').show(); break;
		default: break;
		}
	...
	//기권/실격사유
	mx.setGiveUpGeeGoo =function(tidx,gbidx,idx,ridx, chktype){
		...
		case "W": msg ="정말 기권(W) 시작전 처리 하시겠습니까?"; break;
		case "D": msg ="정말 기권( D) 처리 하시겠습니까?"; break;
		default : msg ="선택한 내용을 처리 하시겠습니까?"; break;
		}
		...
		var obj = {};
		obj.CMD = mx.CMD_SETGIVEUPGEEGOO;	
		obj.TIDX = tidx;
		obj.GBIDX = gbidx;
		obj.IDX = idx;
		obj.RIDX = ridx;
...
//mx.vio = $("#vio").val(); //감점 점수 가져오기
	...
	//변경창오픈
	mx.changeWindow = function(midx,typeno, requestidx){
		var obj = {};
		obj.CMD = mx.CMD_CHANGEWINDOW;	
		obj.MIDX = midx; 
		obj.TYPENO = typeno;
		obj.REQIDX = requestidx;
		mx.SendPacket('rcmodalcontents', obj);			
	};
	...
	//선수검색(릴레이신청그룹)
	mx.searchPlayerGroup = function(midx,player){
		var obj = {};
		obj.CMD = mx.CMD_SEARCHPLAYERGROUP;	
		obj.MIDX = midx; 
		obj.PNM = player;
	...
	//릴레이선수변경
	mx.changeGroupPlayer = function(rseq,gidx){ //request_r idx group idx
		var obj = {};
		obj.CMD = mx.CMD_CHANGEGROUPPLAYER;	
		obj.MIDX = rseq; 
		obj.PIDX = gidx;
		mx.SendPacket('rcmodalcontents', obj);			
	};
	...
	//릴레이 리그 순위 변경
	mx.changeRank = function(midx, orderno){
		var obj = {};
		obj.CMD = mx.CMD_SETORDERLEG;	
		obj.MIDX = midx; 
		obj.ORDERNO = orderno;
		mx.SendPacket('rcmodalcontents', obj);				
	};
	...
	//릴레이 비고 입력
	mx.changeBigo = function(midx, bigo){
		var obj = {};
		obj.CMD = mx.CMD_SETGIGOLEG;	
		obj.MIDX = midx; 
		obj.BIGO = bigo;
		mx.SendPacket('rcmodalcontents', obj);				
	};
...
// 결과 입력창
mx.setResultWindow = function(idx){
	var obj = {};
...
//승패 저장 하기
mx.setSaveTn = function(idx, midxL, midxR, LR){
	var obj = {};
	obj.CMD = mx.CMD_SETSAVETN;
	obj.IDX = idx;
	obj.LR = LR;
	obj.LIDX = midxL;
	obj.RIDX = midxR;
...
mx.OnDrowNextFocus=  function(cmd, packet, html, sender){
	$('#'+sender).css("borderColor","green");

	if (packet.RT == 200){
		$("#total_result_" + packet.IDX).val("실권");
	}
	else{
		$("#total_result_" + packet.IDX).val("완주");
	}
};
