...
mx.SendPacket = function( sender, packet){
	...
	var url = "/pub/ajax/" +location.hostname.split(".")[0].toLowerCase().replace('admin','') + "/admenu1/reqtitleCode.asp";
...
mx.ReceivePacket = function( reqcmd, data, sender ){// data는 response string
	...
	case mx.CMD_USESTATE : location.reload(); break;
	}
};
...
mx.input_frm = function(){
	...
	obj.codeTitle = $("#hosttitle").val();
	obj.grade = $("#gamegrade").val();

	if (obj.codeTitle == ""){
		mx.splashmsg('대회그룹명을 입력해 주세요.',1500);
		return;				
	}
...
mx.update_frm = function(){
	...
	if (mx.gameinfo	 == '' || mx.gameinfo	 == null || $("#hosttitle").val() == '' ){
		mx.splashmsg('대상을 선택해 주십시오.',1500);
		return;
	}
	obj.idx = mx.gameinfo.IDX;
	obj.titleCode = mx.gameinfo.titleCode;
	obj.codeTitle = $("#hosttitle").val();
	obj.grade = $("#gamegrade").val();
...
mx.rnkOK = function(titlecode, useflag){
  if (!confirm("사용상태를 변경하시겠습니까?")) {
		return;
  }

	//var obj = {};
	//obj.CMD = mx.CMD_USESTATE;
