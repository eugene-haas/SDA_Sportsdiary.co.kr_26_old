...
////////////////////////////////////////
  ...
  mx.CMD_COPYTEAM = 400;
  mx.CMD_FINDLEADER = 401;
  mx.CMD_SETYN  = 500;

  mx.CMD_GETWIN = 40000;

  mx.CMD_RESETTEAM = 600;
////////////////////////////////////////



mx.ajaxurl = "/pub/ajax/riding/reqteam.asp";
mx.ajaxtype = "POST";
mx.dataType = "text";
...
mx.ReceivePacket = function( reqcmd, data, sender ){// data는 response string
  ...
  if( jsondata !='' && jsondata != null){
    ...
    case 2: alert('이름이 동일한 데이터가 존재합니다.');return;  break;
...
//요청##################################################################
	mx.resetTeam = function(leaderid){
		if(!confirm("설정이 모두 초기화 됩니다.")){return;}	
		var obj = {};
		obj.CMD = mx.CMD_RESETTEAM;
		obj.LID = leaderid;
		mx.SendPacket(null, obj);
	};

	mx.getWindow = function(team, applyyn){
		//if(!confirm("처리하시겠습니까?")){return;}
		var obj = {};
		obj.CMD = mx.CMD_GETWIN;
		obj.TEAM = team;
		obj.APPLYYN = applyyn;
		mx.SendPacket('modalM', obj);
	};
...
mx.OndrowHTML =  function(cmd, packet, html, sender){
	...
	if(cmd == mx.CMD_GETWIN){
		if( $('#modalM').length == 0 ){
			$('body').append("<div id='modalM' class='modal fade basic-modal' data-backdrop='static' role='dialog' aria-labelledby='myModalLabel'></div>");
		}
		document.getElementById("modalM").innerHTML = html;
		$('#modalM').modal('show');
	}
	else{
