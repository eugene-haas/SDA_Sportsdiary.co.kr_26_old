...
mx.ajaxurl = "/pub/ajax/" +location.hostname.split(".")[0].toLowerCase().replace('admin','ming') + "/reqJudgeDetail.asp";
mx.ajaxtype = "POST";
mx.dataType = "text";
		...
		if( jtype != "tableC"){
			...
			if (makev.chk1 == 'Y'){//소정시간 초과 벌초
				if (Number(playsec) >= Number(makev.t1)){
					...
					dsec += Math.floor( (playsec- Number(makev.t1)).toFixed(2) / Number(makev.sec5) * Number(makev.d5) ); //벌초
				}
			}
