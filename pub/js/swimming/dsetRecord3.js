...
////////////////////////////////////////
  ...
  mx.CMD_GAMECODEWINDOW = 20012; //난이율코드등록창
	...
	mx.CMD_SEACHGAMECODE = 20030; //코드

	////////////////////////////////////////



mx.ajaxurl = "/pub/ajax/swimming/reqdsetRecord_E2.asp";
mx.ajaxtype = "POST";
mx.dataType = "text";
...
mx.ReceivePacket = function( reqcmd, data, sender ){// data는 response string
  ...
  switch (Number(reqcmd)) {
	...
	case mx.CMD_DELJUDGE:
	case mx.CMD_SETAMPM:
	case mx.CMD_SETGAMEDATE2 : return; break;
...
//요청##################################################################
//다이빙 난이율찾기
mx.searchGameCode = function(midx,cdc, rndno, findcode){
	var obj = {};
	obj.CMD = mx.CMD_SEACHGAMECODE;
	obj.MIDX = midx;
	obj.CDC = cdc;
	obj.GAMERNDNO = rndno;
	obj.FINDSTR = findcode; //다이브번호로 검색
	mx.SendPacket('searchcodelist', obj);
};
...
//선수경기순서변경
mx.changeMemberOrder = function(midx, cngval , lidx , listlidx){ //listlidx 통합본 인덱스 
	if (cngval == ''){
		return;
	}
	var obj = {};
...
mx.setreferee = function(tidx, lidx, chkpass){ //배정심판 확인및 변경
    var obj = {};
...
mx.setJudgeCnt = function(tidx, lidx, gbidx, jcnt,chkjidx){
	if (chkjidx !="" && chkjidx != "0"){
		if (!confirm("심판이 배정되어있습니다. 변경시 초기화 됩니다. 진행하시겠습니까?")) {
			window.location.reload();
			return;
		}
	}
...
//난이율 등록 /수정
mx.setGamePer = function(ridx, gamecodeseq   ,tidx,lidx,midx){
	var obj = {};
