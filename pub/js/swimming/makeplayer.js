...
mx.ajaxurl = "/pub/ajax/" +location.hostname.split(".")[0].toLowerCase().replace('admin','ming') + "/reqTennisMakePlayer.asp";
...
mx.init = function(){
	...
	$( "#p1team" ).autocomplete({
		source : function( request, response ) {
			...
			$.ajax({
						type: 'post',
						url: "/pub/ajax/" +location.hostname.split(".")[0].toLowerCase().replace('admin','ming') + "/reqTennisContestTeamFind.asp",
						dataType: "json",
