...
SQL = " ;With overcnt as ( SELECT no2 FROM rtest group by no2 having count(*) > 1 ) "
SQL = SQL & "	select idx,no1,no2 from rtest where no2 not in (select * from overcnt)  Order By idx  asc"
...
SQL = "SELECT idx,no1 ,no2  FROM rtest order by no1 desc  "
Set rs = db.ExecSQLReturnRS(SQL , null, ConStr)
