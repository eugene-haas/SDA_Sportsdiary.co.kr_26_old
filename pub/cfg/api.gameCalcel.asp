...
'$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
	...
	'#######################################
		...
		Sub setNextRound(ByVal idx , ByVal rd, ByVal winpos, ByVal sortno) '대진맴버인덱스, 라운드번호, 승자위치, 홀수소팅번호
			...
			selectfield = nextgubun & " ,GameTitleIDX,PlayerIDX,userName,gamekey1,gamekey2,gamekey3,TeamGb,TeamANa,TeamBNa,tryoutgroupno,key3name,"&nextRound&","&nextSortNo&"  "
...
'Call rsDrow(rs)
	...
	Else
		...
		If rcname = "" And stateno <> "1" Then '심판명이없어
			SQL = "update sd_TennisResult Set stateno = 1, winIDX = "&midx&", winResult = '"&winner&"', recorderName = '운영자', preresult = 'ADMIN'  where resultIDX = " & scidx
			Call db.execSQLRs(SQL , null, ConStr)
